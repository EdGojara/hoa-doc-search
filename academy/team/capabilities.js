...
const CLAIM_PATTERNS = [
  ['physical_site_action', /\b(go|head|come|swing|run) (out )?(and )?(inspect|check|look at|see|walk|take a look)\b|\binspect(ing)? (the )?(pool|site|property|gate|latch|fence|damage|drain|entrance|clubhouse|area|tree|lot|house|pond|fountain|amenity)\b|\b(go|head|drive|walk|swing|stop|come|run) (over |out |down |by )?(to|by|past)(the )?(pool|site|property|gate|entrance|clubhouse|pond|fountain|drain|fence|house|lot|amenity|community)\b|\b(go|head|drive|walk) out\b|\b(inspect|walk|visit|look at|check on|check) (it|the (site|property|pool|gate|latch|fence|drain|entrance|clubhouse|area|damage|tree)) (myself|in person|personally|on site)\b|\bin person\b|\bon[- ]site\b|\b(meet|walk) (you|with you)\b|\b(been|went|go) (out )?to the (pool|site|property|gate|clubhouse)\b|\bcheck the (latch|gate|pool|site) myself\b/i],
