  ...
  try {
    ...
    try {
      ...
      const { data: openPriors } = await supabase.from('violations')
        .select('id, current_stage, continuation_count, cure_period_ends_at, current_stage_started_at')
      ...
      if (openPriors && openPriors.length) {
        ...
        const now = new Date();
        // Internal escalation cadence (Ed 2026-09-15, ALL communities): on a
        // re-observation, escalate the COURTESY steps once 10 days have passed
        // since the current stage started, even though the courtesy letter states
        // a 20-day cure to the owner. The 10-day figure is an INTERNAL cadence for
        // courtesy (pre-certified) notices, decoupled from cure_period_ends_at.
        // HARD LINES that stay: the certified step is never auto-advanced here
        // (courtesy_2 only flags ELIGIBLE for staff), the certified notice keeps
        // its statutory 30-day cure when a human issues it, and anything already
        // at certified_209 or fine_assessed just gets a continuation, never an
        // escalation (it never matches the courtesy branches below).
        const ESCALATION_MIN_DAYS = 10;
        const stageStarted = target.current_stage_started_at ? new Date(target.current_stage_started_at) : null;
        const daysAtStage = stageStarted ? (now.getTime() - stageStarted.getTime()) / 86400000 : Infinity;
        const readyToEscalate = daysAtStage >= ESCALATION_MIN_DAYS;
        let advancedTo = null, eligibleForCertified = false;
        if (readyToEscalate && target.current_stage === 'courtesy_1') {
        ...
        } else if (readyToEscalate && target.current_stage === 'courtesy_2') {
          eligibleForCertified = true;
        }

        const outcomeNote = advancedTo
          ? 'Re-observed past the 10-day internal escalation window, advanced to Courtesy 2 and a new cure clock started.'
          : eligibleForCertified
            ? 'Re-observed past the 10-day internal window at Courtesy 2, ELIGIBLE for the certified notice (staff review before sending; the certified notice keeps its 30-day cure).'
            : 'Re-observed within the 10-day internal window, or already certified, continuation logged; stage and cure clock unchanged.';
  ...
  try {
    ...
    for (const L of letters) {
      try {
        ...
        // authority, a 10-day cure, and a notice of intent to enter, NOT the §209
        // 30-day cure + hearing letter. They're persisted as type 'letter_209'
        // (because 'letter_force_mow_10day' isn't a valid interactions.type), so
        // this batch path can't tell them apart by type — it must key on the
        ...
        if (!_br && isSelfHelp10Day) {
          ...
          if (!authorizingSection || !community.declaration_doc_number || !community.declaration_county) {
            skipped.push({ id: L.id, reason: `self-help ${remedyMode} config missing (declaration_doc_number / county / section) — set in Community Profile before mailing` });
            continue;
          }
          const shortAddr = `${pRow.street_address}${pRow.unit ? ' #' + pRow.unit : ''}`;
          const cityStateZip = `${pRow.city || ''}, ${pRow.state || 'TX'} ${pRow.zip || ''}`.replace(/^,\s*/, '').replace(/\s+/g, ' ').trim();
          const adminFeeCents = community.force_mow_admin_fee_cents != null ? community.force_mow_admin_fee_cents : 2500;
          // Hearing rights on the first certified notice for this case only.
          const priorCertified = priorNoticeRows.filter((p) => p.current_stage === 'certified_209').length;
          pdfBuffer = await renderForceMowLetterPdf({
            ...
            letter_date:             postmarkIso,                 // anchor the 10-day clock to the postmark, not the draft date
            certified_mail_number:   null,
            homeowner_names_block:   [pRow.owner_name || '[Owner Name]', shortAddr, cityStateZip].filter(Boolean).join('\n'),
            property_address_full:   [shortAddr, cityStateZip].filter(Boolean).join(', '),
            property_address_short:  shortAddr,
  ...
  try {
    ...
    for (let from = 0; from < 100000; from += 1000) {
      const { data, error } = await supabase.from('violations')
        .select('id, property_id, primary_category_id, current_stage, source, opened_at, cure_period_ends_at, current_stage_started_at, enforcement_categories(slug, label)')
        .eq('community_id', communityId)
    ...
    for (const v of open) {
      ...
      // Internal 10-day escalation cadence (Ed 2026-09-15, all communities):
      // ready to escalate once 10 days have passed at the current stage, even
      // though the courtesy letter states a 20-day cure to the owner. reObserved
      // (a later drive, 2-day gap) still guards against same-drive escalation,
      // and protected stages (certified_209 / fine_assessed / 10-day self-help)
      // were already excluded above, so those only ever get a continuation.
      const stageStartedMs = v.current_stage_started_at ? new Date(v.current_stage_started_at).getTime() : new Date(v.opened_at).getTime();
      const tenDaysAtStage = (now - stageStartedMs) >= (10 * 24 * 60 * 60 * 1000);
      if (reObserved && tenDaysAtStage) {
