...
DECLARE
  spec jsonb := $spec${
  "community_name": "Lakes of Pine Forest",
  "statement": "income_statement",
  "approved": "Ed 2026-09-25 (Phase 1 reporting categories)",
  "categories": [
    { "section": "revenue", "name": "Revenue", "subcategories": [
      { "name": "Assessments", "accounts": ["4000"] },
      { "name": "Reserve Contribution", "report_label": "Less: Reserve Contribution", "accounts": ["4010"] },
      { "name": "Administrative, Late & Interest Fees", "accounts": ["4030"] },
      { "name": "Fines", "accounts": ["4035"] },
      { "name": "Other Operating Revenue", "accounts": ["4080"] },
      { "name": "Interest Income", "accounts": ["4100", "4110", "4120"] }
    ]},
    { "section": "expense", "name": "Landscaping", "subcategories": [
      { "name": "Landscape Operating & Management", "accounts": ["5200"] },
      { "name": "MUD / Lakes Landscaping", "accounts": ["5205", "5210"] },
      { "name": "Landscape Repair & Maintenance", "accounts": ["5220"] },
      { "name": "Irrigation Maintenance", "accounts": ["5126"] }
    ]},
    { "section": "expense", "name": "Pool", "subcategories": [
      { "name": "Pool Management", "accounts": ["5300"] },
      { "name": "Pool Repair & Maintenance", "accounts": ["5310"] },
      { "name": "Pool Supplies", "accounts": ["5320"] },
      { "name": "Pool Furniture & Equipment", "accounts": ["5330"] },
      { "name": "Pool Tags", "accounts": ["5340"] },
      { "name": "Pool Access System", "accounts": ["5360"] },
      { "name": "Pool Permit", "accounts": ["5380"] }
    ]},
    { "section": "expense", "name": "Utilities", "subcategories": [
      { "name": "Streetlight Electricity", "accounts": ["5105"] },
      { "name": "Common Area Electricity", "accounts": ["5110"] },
      { "name": "Water", "accounts": ["5120"] },
      { "name": "Telephone & Internet", "accounts": ["5730"] }
    ]},
    { "section": "expense", "name": "Lakes & Fountains", "subcategories": [
      { "name": "Fountain/Lake Operating & Management", "accounts": ["5130"] },
      { "name": "Fountain/Lake Service & Repairs", "accounts": ["5140"] },
      { "name": "Irrigation & Repair", "accounts": ["5127"] }
    ]},
    { "section": "expense", "name": "Common Area Maintenance", "subcategories": [
      { "name": "Electrical Repair & Maintenance", "accounts": ["5100"] },
      { "name": "Fences/Walls/Gates", "accounts": ["5450"] },
      { "name": "Common Area Repair & Maintenance", "accounts": ["5740"] },
      { "name": "Playground", "accounts": ["5400"] },
      { "name": "Power Washing", "accounts": ["5410"] },
      { "name": "Signage", "accounts": ["5420"] },
      { "name": "Flags", "accounts": ["5415"] },
      { "name": "Pest Control", "accounts": ["5795"] }
    ]},
    { "section": "expense", "name": "Security", "subcategories": [
      { "name": "Security Services", "accounts": ["5770"] }
    ]},
    { "section": "expense", "name": "Insurance & Taxes", "subcategories": [
      { "name": "Insurance", "accounts": ["5600"] },
      { "name": "Property Taxes", "accounts": ["5885"] },
      { "name": "Other Taxes & Fees", "accounts": ["5890"] }
    ]},
    { "section": "expense", "name": "Professional & Administrative", "subcategories": [
      { "name": "Bank Charges", "accounts": ["5010"] },
      { "name": "Website Maintenance", "accounts": ["5030"] },
      { "name": "Office Supplies", "accounts": ["5440"] },
      { "name": "Bad Debt", "accounts": ["5695"] },
      { "name": "Postage", "accounts": ["5800"] },
      { "name": "Administrative Expenses", "accounts": ["5805"] },
      { "name": "Management Fees", "accounts": ["5810"] },
      { "name": "Administrative Notices", "accounts": ["5820"] },
      { "name": "ACC Review Fee", "accounts": ["5850"] },
      { "name": "Legal - Corporate", "accounts": ["5860"] },
      { "name": "Legal - Collections / DRV", "accounts": ["5870"] },
      { "name": "Accounting Fees", "accounts": ["5880"] }
    ]},
    { "section": "expense", "name": "Community Events / Meetings", "subcategories": [
      { "name": "Community Events", "accounts": ["5900"] },
      { "name": "Holiday Decorations", "accounts": ["5905"] },
      { "name": "Meeting Expense", "accounts": ["5825"] }
    ]}
  ]
}$spec$::jsonb;
...
DO $t$
DECLARE
  cid uuid; other_cid uuid; a5300 uuid; a4000 uuid; a_other uuid; c_pool_mgmt uuid; c_pool_sup uuid; c_pool uuid; c_inactive uuid;
  ok boolean; msg text; n0 int; n1 int; r jsonb; res text := '';
BEGIN
  SELECT id INTO cid FROM communities WHERE name = 'Lakes of Pine Forest';
  SELECT id INTO a5300 FROM chart_of_accounts WHERE community_id = cid AND account_number = '5300';
  SELECT id INTO a4000 FROM chart_of_accounts WHERE community_id = cid AND account_number = '4000';
  SELECT id INTO c_pool FROM report_categories WHERE community_id = cid AND name = 'Pool' AND parent_category_id IS NULL;
  SELECT id INTO c_pool_mgmt FROM report_categories WHERE community_id = cid AND name = 'Pool Management';
  SELECT id INTO c_pool_sup FROM report_categories WHERE community_id = cid AND name = 'Pool Supplies';
  SELECT a.community_id, a.id INTO other_cid, a_other FROM chart_of_accounts a WHERE a.community_id <> cid AND a.account_type = 'expense' LIMIT 1;
  res := res || format('seeded: %s categories, %s mapped; ', (SELECT count(*) FROM report_categories WHERE community_id = cid), (SELECT count(*) FROM account_report_map WHERE community_id = cid));
  res := res || '4010 label=' || (SELECT c.report_label FROM account_report_map m JOIN report_categories c ON c.id = m.category_id JOIN chart_of_accounts a ON a.id = m.account_id WHERE a.community_id = cid AND a.account_number = '4010') || '; ';

  ok := false; BEGIN INSERT INTO account_report_map (community_id, account_id, category_id, updated_by) VALUES (cid, a5300, c_pool_sup, 't'); EXCEPTION WHEN unique_violation THEN ok := true; END;
  IF NOT ok THEN RAISE EXCEPTION 'double mapping allowed'; END IF; res := res || 'double map REFUSED; ';
  ok := false; BEGIN UPDATE account_report_map SET category_id = c_pool_sup WHERE account_id = a4000; EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%cannot present under%'; msg := SQLERRM; END;
  IF NOT ok THEN RAISE EXCEPTION 'revenue into expense allowed'; END IF; res := res || 'revenue->expense REFUSED ("' || msg || '"); ';
  ok := false; BEGIN INSERT INTO account_report_map (community_id, account_id, category_id, updated_by) VALUES (other_cid, a_other, c_pool_sup, 't'); EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%same community%'; END;
  IF NOT ok THEN RAISE EXCEPTION 'cross-community mapping allowed'; END IF; res := res || 'cross-community REFUSED; ';
  ok := false; BEGIN INSERT INTO report_categories (community_id, section, name, parent_category_id) VALUES (cid, 'expense', 'Too Deep', c_pool_mgmt); EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%one level deep%'; END;
  IF NOT ok THEN RAISE EXCEPTION 'two-level subcategory allowed'; END IF; res := res || '2-deep REFUSED; ';
  ok := false; BEGIN INSERT INTO report_categories (community_id, section, name, parent_category_id) VALUES (cid, 'revenue', 'Wrong section', c_pool); EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%share its parent%'; END;
  IF NOT ok THEN RAISE EXCEPTION 'cross-section subcategory allowed'; END IF; res := res || 'cross-section sub REFUSED; ';
  ok := false; BEGIN INSERT INTO report_categories (community_id, section, name) VALUES (cid, 'expense', 'pool'); EXCEPTION WHEN unique_violation THEN ok := true; END;
  IF NOT ok THEN RAISE EXCEPTION 'duplicate category name allowed'; END IF; res := res || 'dup name REFUSED; ';
  ok := false; BEGIN UPDATE report_categories SET is_active = false WHERE id = c_pool_mgmt; EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%still has accounts%'; END;
  IF NOT ok THEN RAISE EXCEPTION 'deactivating a used category allowed'; END IF; res := res || 'deactivate used REFUSED; ';
  ok := false; BEGIN DELETE FROM report_categories WHERE id = c_pool_sup; EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%not deleted%'; END;
  IF NOT ok THEN RAISE EXCEPTION 'category delete allowed'; END IF; res := res || 'category delete REFUSED; ';
  INSERT INTO report_categories (community_id, section, name, is_active, updated_by) VALUES (cid, 'expense', 'Retired', false, 't') RETURNING id INTO c_inactive;
  ok := false; BEGIN UPDATE account_report_map SET category_id = c_inactive WHERE account_id = a5300; EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%inactive%'; END;
  IF NOT ok THEN RAISE EXCEPTION 'mapping to inactive allowed'; END IF; res := res || 'map to inactive REFUSED; ';
  ok := false; BEGIN PERFORM set_account_report_category(cid, ARRAY[a5300], c_pool_sup, '  '); EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%who is making it%'; END;
  IF NOT ok THEN RAISE EXCEPTION 'rpc without actor allowed'; END IF; res := res || 'rpc w/o actor REFUSED; ';
  ok := false; BEGIN PERFORM set_account_report_category(cid, ARRAY[a_other], c_pool_sup, 'x'); EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%belong to this community%'; END;
  IF NOT ok THEN RAISE EXCEPTION 'rpc cross-community allowed'; END IF; res := res || 'rpc cross-community REFUSED; ';

  SELECT count(*) INTO n0 FROM report_mapping_events WHERE account_id = a5300;
  r := set_account_report_category(cid, ARRAY[a5300], c_pool_sup, 'rehearsal@test');
  IF (r->>'changed')::int <> 1 THEN RAISE EXCEPTION 'remap did not change'; END IF;
  r := set_account_report_category(cid, ARRAY[a5300], c_pool_sup, 'rehearsal@test');
  IF (r->>'changed')::int <> 0 THEN RAISE EXCEPTION 'no-op remap reported a change'; END IF;
  r := set_account_report_category(cid, ARRAY[a5300], NULL, 'rehearsal@test');
  IF (r->>'unmapped')::int <> 1 THEN RAISE EXCEPTION 'unmap failed'; END IF;
  r := set_account_report_category(cid, ARRAY[a5300], c_pool_mgmt, 'rehearsal@test');
  SELECT count(*) INTO n1 FROM report_mapping_events WHERE account_id = a5300;
  IF n1 - n0 <> 3 THEN RAISE EXCEPTION 'expected 3 new events (update, delete, insert), got %', n1 - n0; END IF;
  IF NOT EXISTS (SELECT 1 FROM report_mapping_events WHERE account_id = a5300 AND action = 'delete' AND actor = 'rehearsal@test' AND old_row->>'category_id' = c_pool_sup::text) THEN RAISE EXCEPTION 'unmap not logged with actor'; END IF;
  IF EXISTS (SELECT 1 FROM report_mapping_events WHERE account_id = a5300 AND action IN ('update', 'delete') AND coalesce(actor, '') <> 'rehearsal@test') THEN RAISE EXCEPTION 'change logged without actor'; END IF;
  res := res || 'remap/unmap/remap logged with actor (3 events; no-op not logged); ';
  ok := false; BEGIN UPDATE report_mapping_events SET actor = 'x' WHERE account_id = a5300; EXCEPTION WHEN OTHERS THEN ok := SQLERRM LIKE '%permanent%'; END;
  IF NOT ok THEN RAISE EXCEPTION 'events not append-only'; END IF; res := res || 'events append-only';
