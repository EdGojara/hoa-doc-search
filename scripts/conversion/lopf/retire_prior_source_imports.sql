-- ============================================================================
-- LOPF: retire PRIOR_SOURCE_IMPORT records from authoritative balances
-- PREPARED, NOT RUN. Generated from prior_source_imports.manifest.json
-- (2026-09-24T13:21:30.526Z, ids sha256 b989af4b7e94c591).
-- ============================================================================
--
-- Retires (never deletes):
--   146 vantaca_import JEs 2026-01-01..2026-07-17 + JE-2026-OPEN
--     -> status 'superseded' (needs migration 454; drops out of every balance)
--   AR batch c732b43c-57e7-4aba-b6f8-90076036515a (738 rows / 579 accounts, as of 2026-07-17)
--     -> status 'reverted', replaced_by_batch_id = the conversion AR batch
--
-- MUST run in the SAME transaction as the conversion posting (opening entries
-- per fund + conversion AR batch), never alone: on its own it would leave LOPF
-- with no GL history and no homeowner balances. Replace :conversion_ar_batch_id
-- before running. Any mismatch raises and rolls back everything.
-- Undo: restore_prior_source_imports.sql.
DO $retire$
DECLARE
  v_ids uuid[] := ARRAY[
  ...
  v_conv_batch uuid := ':conversion_ar_batch_id'::uuid;
  n bigint; dr bigint; cr bigint;
BEGIN
  PERFORM 1 FROM information_schema.columns WHERE table_name = 'journal_entries' AND column_name = 'superseded_at';
  IF NOT FOUND THEN RAISE EXCEPTION 'migration 454 not applied'; END IF;
  ...
  SELECT count(*) INTO n FROM journal_entries WHERE id = ANY(v_ids) AND status <> 'posted';
  IF n <> 0 THEN RAISE EXCEPTION '% entries are not posted', n; END IF;
  PERFORM 1 FROM transaction_upload_batches WHERE id = 'c732b43c-57e7-4aba-b6f8-90076036515a' AND status = 'committed' AND row_count = 738 AND account_count = 579 FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'AR batch c732b43c-57e7-4aba-b6f8-90076036515a is not the committed 738-row batch'; END IF;
  SELECT count(*) INTO n FROM homeowner_transactions WHERE source_batch_id = 'c732b43c-57e7-4aba-b6f8-90076036515a';
  IF n <> 738 THEN RAISE EXCEPTION 'AR batch rows % <> 738', n; END IF;
  PERFORM 1 FROM transaction_upload_batches WHERE id = v_conv_batch AND community_id = 'a0000000-0000-4000-8000-000000000002' AND status = 'committed';
  IF NOT FOUND THEN RAISE EXCEPTION 'conversion AR batch % not committed', v_conv_batch; END IF;

  UPDATE journal_entries
     SET status = 'superseded', superseded_at = now(),
         superseded_reason = 'PRIOR_SOURCE_IMPORT retired at 7/31/2026 cutover; replaced by per-fund conversion opening entries',
         superseded_by_conversion = 'CONV-LPF-20260731'
   WHERE id = ANY(v_ids) AND status = 'posted';
  GET DIAGNOSTICS n = ROW_COUNT;
  IF n <> 147 THEN RAISE EXCEPTION 'superseded % entries, expected 147', n; END IF;

  UPDATE transaction_upload_batches
     SET status = 'reverted', reverted_at = now(), replaced_by_batch_id = v_conv_batch,
         reverted_reason = 'PRIOR_SOURCE_IMPORT retired at 7/31/2026 cutover (CONV-LPF-20260731); reference only'
   WHERE id = 'c732b43c-57e7-4aba-b6f8-90076036515a' AND status = 'committed';
  GET DIAGNOSTICS n = ROW_COUNT;
  IF n <> 1 THEN RAISE EXCEPTION 'AR batch not retired'; END IF;
END
$retire$;
