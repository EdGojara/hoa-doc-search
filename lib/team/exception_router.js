...
const HARM_DRAFT_INSTRUCTION = `IMMINENT-HARM SIGNAL DETECTED — handle with urgency and OWNERSHIP, not process. Lead with genuine empathy. If a person or an animal may be in real danger, clearly point the sender to the authority that can actually act — for an animal, the county Animal Control (and note law enforcement for emergencies); for a person, 911 or local police — with specific next steps and what documentation to provide. Then state plainly what WE are doing on our end and that we are treating this as urgent and escalating it internally. Do NOT give a bureaucratic brush-off, do NOT say only that it is "not our jurisdiction," and do NOT close with a cheerful sign-off like "have a great weekend." Take ownership of driving it.`;
