...
const { classifyDisposition, dispositionForCareful, detectLegalThreat, LEGAL_THREAT_REASON, detectHarmEmergency, HARM_REASON, HARM_DRAFT_INSTRUCTION } = require('./exception_router');

// Greet the person who WROTE, not the account. A resident's email often sits on
// a household/account contact with a different name (Lorena wrote in, but her
// email is on the "Maria & William De Slavick" account) — greeting the resolved
// account name addresses the wrong human. So prefer the incoming message's
// sender display name; fall back to the resolved contact, then a neutral "there".
// Reject anything that isn't a plausible personal first name (an email address,
// a functional-mailbox word).
// A hard rule for every lane: don't recreate the gatekeeper, and don't invent
// contact details. Ed has corrected the "route it to Martha" reflex more than
// once (it just props up the human choke point), and a fabricated phone/email —
// worse, a wrong authority number in a welfare emergency — is a real hazard.
const CONTACT_ROUTING_RULE = `NEVER name a specific staff member, offer to route to a named person, or give any individual staffer's direct email or phone — a named handoff just recreates the gatekeeper. Route to the team or function instead (for example "our compliance team" or "our office"). NEVER invent or guess a phone number, email, or website; give a specific contact only if it appears in the CONTEXT provided. Directing someone to 911 in a genuine emergency is always appropriate.`;

// The failure mode Ed sees in human staff: promise big, then never follow up.
// The team must be the opposite — under-promise, and commit only to what will
// actually happen. Encoded because he has corrected it repeatedly.
const NO_OVERPROMISE_RULE = `DO NOT OVER-PROMISE. Never promise an outcome you cannot guarantee — that a problem will be fixed, that another owner will comply or be penalized, that a request will be approved, or that "we will take care of it." Never invent a deadline or timeline the team has not actually committed to (no "within a few business days", no "by Friday", no "you'll hear back from someone soon") unless that timeline is a real stated policy or SLA in the CONTEXT. Commit ONLY to concrete steps you are genuinely taking right now (logging the report, routing it to the right team, reviewing the file). Be honest about what is outside the association's control. Under-promising and following through beats over-promising and going quiet — the second is exactly how residents and boards stop trusting a manager.`;

// Read the whole conversation, not one message in a vacuum. A reply that gives
// first-contact advice on the fifth message ("please file a complaint with
// Animal Control") when the resident already did that twice reads as generic and
// dismissive. This pulls the prior messages in the thread so the drafter answers
// the ACTUAL state — what's been tried, what was already said. Best-effort: ''
// when there's no history or the read fails.
async function threadHistory(supabase, sourceMessageId) {
  if (!supabase || !sourceMessageId) return '';
  try {
    const cur = await supabase.from('email_messages').select('conversation_id, sender_email').eq('id', sourceMessageId).limit(1);
    const c = cur.data && cur.data[0];
    if (!c) return '';
    const conv = c.conversation_id;
    const sender = c.sender_email;
    // The saga often spans MORE than one thread — the resident writes again with
    // a new subject. So pull BOTH this conversation AND this person's other
    // recent correspondence (same sender_email), so the reply reflects the whole
    // relationship, not one thread in isolation.
    const byConv = conv ? await supabase.from('email_messages')
      .select('id, direction, sender_name, received_at, sent_at, body_full, body_preview')
      .eq('conversation_id', conv).order('received_at', { ascending: true }).limit(12) : { data: [] };
    const bySender = sender ? await supabase.from('email_messages')
      .select('id, direction, sender_name, received_at, sent_at, body_full, body_preview')
      .eq('sender_email', sender).order('received_at', { ascending: false }).limit(12) : { data: [] };
    const merged = new Map();
    for (const m of [...(byConv.data || []), ...(bySender.data || [])]) merged.set(m.id, m);
    const data = [...merged.values()].sort((a, b) => String(a.received_at || a.sent_at || '').localeCompare(String(b.received_at || b.sent_at || '')));
    const prior = data.filter((m) => m.id !== sourceMessageId).slice(-12);
    if (!prior.length) return '';
    const fmt = (d) => { try { return new Date(d).toLocaleString('en-US', { timeZone: 'America/Chicago', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }); } catch (_) { return ''; } };
    const lines = prior.map((m) => {
      const who = m.direction === 'outbound' ? 'Bedrock (us)' : `Resident (${m.sender_name || 'resident'})`;
      const when = fmt(m.received_at || m.sent_at);
      const body = String(m.body_full || m.body_preview || '').replace(/\s+/g, ' ').trim().slice(0, 600);
...
function greetingName(email, contactName) {
  const clean = (s) => {
    s = String(s || '').trim();
    if (!s || s.includes('@')) return null;
    if (/^(info|admin|support|billing|accounting|sales|no.?reply|noreply|bedrock|mailer|postmaster|team|office|do.?not.?reply)\b/i.test(s)) return null;
    const first = s.split(/[\s,&/]+/).filter(Boolean)[0] || '';
    return /^[A-Za-z][A-Za-z'’-]{1,}$/.test(first) ? first : null;
  };
  return clean(email && email.sender_name) || clean(contactName) || 'there';
}
...
async function assembleGrounding({ email, communityName }) {
  ...
  try {
    ...
    // 24000, not 9000: the retriever self-limits to its top chunks (~20k), and a
    // 9k slice silently dropped provisions ranked past it — a corner-lot tree rule
    // sat at char 10,487 and never reached the model (the Waterview miss, 2026-09-08).
    docContext = (await getRelevantChunks(question, communityName) || '').slice(0, 24000);
...
module.exports = {
  ...
  classifyDisposition, dispositionForCareful, detectLegalThreat, LEGAL_THREAT_REASON,
  detectHarmEmergency, HARM_REASON, HARM_DRAFT_INSTRUCTION, greetingName, CONTACT_ROUTING_RULE, NO_OVERPROMISE_RULE, threadHistory,
};
