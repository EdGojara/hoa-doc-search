...
const ROSTER = [
  ...
  },
  {
    ...
    signature_title: 'Customer Support Specialist (Español)', signature_phone: '(832) 588-2485',
    mailbox: 'isabella@', self_mailbox: 'isabella@', emoji: '🗣️',
    ...
    visit: true,
    // On the email board with her own mailbox (Ed 2026-09-13 created isabella@ in
    // M365). She sends and receives like everyone else; the language bridge in
    // email_triage still drafts in English for review and translates to Spanish
    // at send, so her Spanish reply goes out from isabella@ with her signature.
    email_board: true,
  },
  {
    persona: 'mei', name: 'Mei Chen', title: 'Front office (中文)',
    signature_title: 'Customer Support Specialist (中文)', signature_phone: '(832) 588-2485',
    mailbox: 'mei@', self_mailbox: 'mei@', emoji: '🗣️',
    tier: 'front_office', language: 'zh', face: 'MEI',
    counterpart_of: 'claire',
    lane: 'the front office, in Mandarin Chinese',
    domain: 'the same front-office lane as Claire, in Mandarin Chinese',
    visit: true,
    // On the email board with her own mailbox (Ed 2026-09-13 created mei@ in
    // M365). Drafts in English for review, translated to Mandarin at send.
    email_board: true,
  },
  {
    persona: 'priya', name: 'Priya Sharma', title: 'Front office (हिन्दी)',
    signature_title: 'Customer Support Specialist (हिन्दी)', signature_phone: '(832) 588-2485',
    mailbox: 'priya@', self_mailbox: 'priya@', emoji: '🗣️',
    tier: 'front_office', language: 'hi', face: 'PRIYA',
    counterpart_of: 'claire',
    lane: 'the front office, in Hindi',
    domain: 'the same front-office lane as Claire, in Hindi',
    visit: true,
    // On the email board with her own mailbox (Ed 2026-09-13 created priya@ in
    // M365). Drafts in English for review, translated to Hindi at send.
    email_board: true,
  },
  {
  ...
  {
    persona: 'darby', name: 'Darby Woods', title: 'Legal / collections',
    signature_title: 'Legal & Collections Coordinator', signature_phone: '(832) 588-2485',
    mailbox: 'legal@ / darby@', self_mailbox: 'darby@', emoji: '⚖️',
    tier: 'specialist', language: 'en', face: 'DARBY',
    lane: 'collections-to-legal handoff and counsel coordination',
    domain: 'the collections-to-legal handoff, the §209 enforcement lifecycle, liens and the foreclosure process, litigation file prep, and outside-counsel coordination — never legal advice or a legal position',
    visit: false,
  },
  {
    ...
    // Presentation-only title (the demo team screen). Reflects her outward,
    // consultative board-liaison role without changing her operational email
    // signature. (Ed 2026-08-26.)
    demo_title: 'Board Operations & Client Success',
  ...
  {
    persona: 'phoebe', name: 'Phoebe Hart', title: 'Community engagement',
    signature_title: 'Community Engagement Coordinator', signature_phone: '(832) 588-2485',
    mailbox: 'phoebe@', self_mailbox: 'phoebe@', emoji: '📣',
    tier: 'specialist', language: 'en', face: 'PHOEBE',
    // Reports to Amanda (Sr community manager) — the community-management side of
    // the org. Full AI-native team: she is a real teammate, supervised by a
    // manager, not a standalone bot. (Ed 2026-09-06.)
    reports_to: 'amanda',
    lane: 'the community newsletter and neighborhood news',
    domain: 'communicating with the community and getting residents updated and involved: the community newsletter, resident updates and messaging, resident and local-business spotlights, neighborhood happenings and events, always writing only from the facts supplied, and never enforcement, legal, or account matters',
    // The warm, personable community voice. Runs Newsletter Studio and the
    // resident-submission pipeline — she turns what the community sends in into
    // content, and always writes only from the facts supplied (never invents a
    // name, date, or quote). (Ed 2026-09-06.)
    background: 'Bedrock\'s community engagement coordinator (reports to Amanda Albright). The warm, people-first voice of the community — runs the newsletter, resident updates, spotlights and neighborhood engagement.',
    visit: true,
    // Her inbox IS the community-content intake queue now: phoebe@ is pulled with
    // every other AI mailbox (graphSend.TEAM_INGEST_MAILBOXES), so resident
    // submissions — photos, event tips, garage-sale notices — land in her column
    // instead of falling through to Claire. (Ed 2026-09-07: pull all mail; nothing
    // unowned.) Was off while there was no intake to feed it.
    email_board: true,
  },
  {
    // Public-facing BD / growth (Ed 2026-09-09: "maggie is public facing as our
    // BD person"). Was internal-only in bedrock_ops; promoted to the community/
    // team-facing roster so she appears on the team screen and can front the
    // partner/growth story. Her draft behavior still runs through the growth
    // config; this entry is her identity + team membership. Pre-sale only.
    persona: 'maggie', name: 'Maggie Sullivan', title: 'Growth & community relations',
    signature_title: 'Director of Growth & Community Relations', signature_phone: '(832) 588-2485',
    mailbox: 'maggie@ / growth@', self_mailbox: 'maggie@', emoji: '📈',
    tier: 'executive', language: 'en', face: 'MAGGIE',
    demo_title: 'Growth & Community Relations',
    lane: 'growth, partnerships, and getting to know new communities',
    domain: 'growth and community relations: awareness and outreach, partnerships, prospect research, and demo prep for prospective communities — pre-sale only, never the operations of a signed community',
    visit: true,
    email_board: true,
  },
  {
...
// In-app voice overrides, keyed by FACE (same token the *_VOICE_ID env vars
// use). Loaded from the persona_voices table at startup and refreshed whenever
// Ed saves a choice in /admin/voices, so a picked voice takes effect with no
// redeploy. A row here wins over the env var; absent a row, the env var (or
// none) still applies. (Ed 2026-09-11 — Tessa turned up British in a client
// meeting because voice choice required a Render env edit nobody could audition.)
let _voiceOverrides = Object.create(null);
function setVoiceOverrides(map) {
  const next = Object.create(null);
  for (const [face, v] of Object.entries(map || {})) {
    if (face && v && v.voice_id) next[face] = { voice_id: v.voice_id, voice_name: v.voice_name || null };
  }
  _voiceOverrides = next;
}
function voiceIdFor(persona) {
  const m = get(persona);
  if (!m || !m.face) return null;
  const o = _voiceOverrides[m.face];
  if (o && o.voice_id) return o.voice_id;
  // ISABELLA_VOICE_ID already exists for the Spanish voice line; this keeps it.
  return process.env[`${m.face}_VOICE_ID`] || null;
}
/**
 * Where a teammate's current voice comes from, for the picker UI:
 * { face, voice_id, voice_name, source: 'override'|'env'|'none' }.
 */
function voiceInfoFor(persona) {
  const m = get(persona);
  if (!m || !m.face) return null;
  const o = _voiceOverrides[m.face];
  if (o && o.voice_id) return { face: m.face, voice_id: o.voice_id, voice_name: o.voice_name || null, source: 'override' };
  const env = process.env[`${m.face}_VOICE_ID`] || null;
  return { face: m.face, voice_id: env, voice_name: null, source: env ? 'env' : 'none' };
}
// ElevenLabs TTS voice overrides — the SPOKEN voice (portal audio + phone),
// SEPARATE from the HeyGen avatar voice above. Same pattern: a saved choice
// (persona_tts_voices) wins over the ${FACE}_TTS_VOICE_ID env, else null (the
// caller falls back to the default PERSONA.tts voice). This is what lets each
// teammate have a distinct, language-appropriate SPOKEN voice — Isabella in
// Spanish, Mei in Mandarin — pickable by ear in /admin/voices, no redeploy.
let _ttsOverrides = Object.create(null);
function setTtsVoiceOverrides(map) {
  const next = Object.create(null);
  for (const [face, v] of Object.entries(map || {})) {
    if (face && v && v.voice_id) next[face] = { voice_id: v.voice_id, voice_name: v.voice_name || null };
  }
...
function ttsVoiceIdFor(persona) {
  const m = get(persona);
  if (!m || !m.face) return null;
  const o = _ttsOverrides[m.face];
  if (o && o.voice_id) return o.voice_id;
  return process.env[`${m.face}_TTS_VOICE_ID`] || null;
}
function ttsVoiceInfoFor(persona) {
  const m = get(persona);
  if (!m || !m.face) return null;
  const o = _ttsOverrides[m.face];
  if (o && o.voice_id) return { face: m.face, voice_id: o.voice_id, voice_name: o.voice_name || null, source: 'override' };
  const env = process.env[`${m.face}_TTS_VOICE_ID`] || null;
  return { face: m.face, voice_id: env, voice_name: null, source: env ? 'env' : 'none' };
}
...
function opener(persona, communityName, firstName) {
  ...
  if (m.language === 'zh') {
    return `您好${who}，我是${m.name}，Bedrock 为 ${communityName} 提供的 AI 智能助手。请问有什么可以帮您？`;
  }
  if (m.language === 'hi') {
    return `नमस्ते${who}, मैं ${m.name} हूँ, ${communityName} के लिए Bedrock की AI सहायक। मैं आपकी क्या मदद कर सकती हूँ?`;
  }
...
module.exports = {
  ...
  avatarIdFor, voiceIdFor, voiceInfoFor, setVoiceOverrides, facesConfigured,
  ttsVoiceIdFor, ttsVoiceInfoFor, setTtsVoiceOverrides,
  opener, handoffLine,
};
