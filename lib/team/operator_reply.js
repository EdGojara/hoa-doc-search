...
async function draftOperatorReply(config, { email, supabase, propertyId, communityId, contactName, communityName, sourceMessageId, mailbox, graphId }) {
  ...
  try {
    ...
    // Read the whole thread so the reply answers the ACTUAL state, not a generic
    // first contact (the resident may have already tried what a naive draft suggests).
    let history = '';
    try {
      if (sourceMessageId) {
        const h = await core.threadHistory(supabase, sourceMessageId);
    ...
    // Reattach: is this part of an objective the team is already driving? If so,
    // the reply reflects the CASE (goal, status, planned next step), not a fresh
    // start. This is the operator spine — see lib/team/objectives.
    let objectiveBlock = '';
    try {
      const obj = require('./objectives');
