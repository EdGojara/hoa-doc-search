...
async function draftOperatorReply(config, { email, supabase, propertyId, communityId, contactName, communityName }) {
  ...
  // Shared, persona-independent: a litigation threat must escalate loudly no
  // matter which teammate caught it (see exception_router). Merge it into the
  // reasons so it drives both the disposition tier and the review hint.
  const threat = core.detectLegalThreat(`${email.subject || ''}\n${email.body || email.body_preview || ''}`);
