...
// How well the HUMAN office can serve each language on a follow-up. English is
// implicit/full. This gates whether a teammate points a caller to the office,
// offers a callback, or stays the bridge. Levels:
//   'full'    — staff can handle it directly (Ed 2026-09-13: Spanish).
//   'limited' — a speaker exists but PART-TIME, so prefer a callback / message
//               the speaker handles when in; don't promise someone's always
//               there (Ed 2026-09-13: Hindi — one part-time speaker).
//   (absent)  — no human coverage; the AI teammate is the bridge.
const OFFICE_LANGUAGES = { es: 'full', hi: 'limited' };
...
const LANGUAGES = {
  ...
  hi: {
    name: 'Hindi',
    native: 'सहज, बातचीत वाली हिन्दी',
    bridges: ['एक पल।', 'ज़रा देखती हूँ।', 'मुझे देखने दें।', 'एक सेकंड।'],
    banned: [],
  },
};
    ...
    if (L) {
      return {
        ...
        buildSystemPromptParts(...args) {
          ...
          // Handoff is language-aware by how well the office covers the language.
          const coverage = OFFICE_LANGUAGES[m.language];
          let handoff;
          if (coverage === 'full') {
            ...
            handoff = `\n\nHUMAN HANDOFF — ${L.name.toUpperCase()} HAS ONLY PART-TIME HUMAN COVERAGE: there is ONE ${L.name}-speaking teammate and she is PART-TIME, so she is often not in. Handle everything you can yourself in ${L.name} first (you are the same brain as Claire). When something genuinely needs a person, LEAD WITH A CALLBACK: capture the full issue in ${L.name} and the best number, and tell them our ${L.name}-speaking teammate will call them back — because she is part-time, that is more reliable than calling in and hoping she's there. You may ALSO give the office number as an option, but only with the honest caveat that the ${L.name}-speaking teammate is part-time, so if she isn't in when they call, the team will arrange a callback. NEVER imply a ${L.name} speaker is always available on the line.`;
          } else {
...
// LANGUAGES is the canonical registry of the NON-English languages the AI team
// can operate in (English is the base). Exported so a single source defines what
// a teammate's `language` may be, instead of a hardcoded list drifting elsewhere.
module.exports = { packFor, LANGUAGES };
