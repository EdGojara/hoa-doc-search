...
function packFor(persona) {
  ...
  // all. Assemble it in the shape streamTurn expects ({ buildSystemPromptParts,
  // bannedPatterns }), exactly like the phone path (api/voice.js) does. The old
  // code returned the raw module and looked for a `.buildSystemPromptParts` key
  // that doesn't exist (it's `buildIsabellaSystemPromptParts`), so streamTurn
  // silently fell back to Claire and answered in ENGLISH on the portal.
  if (persona === 'isabella') {
    try {
      const r = require('../voice/reason_isabella');
      const p = require('../voice/persona_isabella');
      if (typeof r.buildIsabellaSystemPromptParts !== 'function') throw new Error('buildIsabellaSystemPromptParts missing');
      return {
        persona: 'isabella',
        buildSystemPromptParts: r.buildIsabellaSystemPromptParts,
        bannedPatterns: p.BANNED_PATTERNS,
        bridgePhrases: p.BRIDGE_PHRASES,   // Spanish "give me a beat" tokens, not English
      };
    } catch (e) {
