...
function bodyToHtml(text) {
  ...
  // Ed 2026-06-11 bug fix: when the model generates each numbered (or
  // bulleted) item as its own paragraph separated by blank lines —
  // common output shape — the prior code rendered each as a SEPARATE
  // <ol>, so every item displayed as "1.", "1.", "1." instead of
  // incrementing 1, 2, 3. The fix is to merge consecutive list-shaped
  // blocks into a single list element. Same logic for ordered and
  // unordered.
  //
  // Real example that triggered this: Canyon Gate ARC approval letter
  // had 5 numbered conditions, all displayed as "1." to the homeowner.
  // Client-facing render bug, sat in production until 2026-06-11.

  const NUMBERED_RE = /^\s*\d+\.\s+/;
  const BULLETED_RE = /^\s*[-*]\s+/;
  const splitItems = (block, stripRe) => block
    .split(/\n+/)
    .map((line) => line.replace(stripRe, '').trim())
    .filter(Boolean);
  ...
  while (i < blocks.length) {
    ...
    if (NUMBERED_RE.test(block)) {
      // Greedily consume consecutive numbered blocks into one <ol> so
      // numbering increments naturally (auto-numbered by the browser /
      // PDF renderer from the <ol>'s start=1).
      const items = [];
      while (i < blocks.length && NUMBERED_RE.test(blocks[i])) {
        items.push(...splitItems(blocks[i], NUMBERED_RE));
        i++;
      }
      out.push(`<ol>${items.map((it) => `<li>${escapeHtml(it)}</li>`).join('')}</ol>`);
      continue;
    }

    if (BULLETED_RE.test(block)) {
      const items = [];
      while (i < blocks.length && BULLETED_RE.test(blocks[i])) {
        items.push(...splitItems(blocks[i], BULLETED_RE));
        i++;
      }
