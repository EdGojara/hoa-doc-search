...
needs_human_review_before_send=true when: legal exposure mentioned, attorney involvement, fee waiver / policy discretion call, board governance topic, fair housing / ADA, anything where a wrong answer is hard to retract.`;
  ...
  // IP-leak guard: this content is going to a HOMEOWNER. Run through the
  // strict customer-audience filter. Auto-rewrite the soft phrases
  // ("defensible" → "supported by the documents"), block the hard ones
  // ("processing strategy", verbatim CC&R Article references). If hard-
  // blocked, force needs_human_review_before_send=true so the manager
  // has to hand-rewrite before it goes out.
  try {
    const { screenForLeaks } = require('../voice/leak_filter');
    const screen = screenForLeaks(parsed.content, { audience: 'customer', autoRewrite: true });
    if (screen.blocks.length > 0) {
      logger.warn(`[draft_response] content BLOCKED by leak filter — flagging for human review: ${screen.blocks.map((b) => b.reason).join('; ')}`);
      parsed.needs_human_review_before_send = true;
      parsed.needs_review_reason = (parsed.needs_review_reason ? parsed.needs_review_reason + ' · ' : '')
        + 'leak filter blocked: ' + screen.blocks.map((b) => `"${b.matches.slice(0, 2).join('", "')}"`).join(', ');
    } else if (screen.rewrites.length > 0) {
      parsed.content = screen.text;
    }
    const subjScreen = screenForLeaks(parsed.subject, { audience: 'customer', autoRewrite: true });
    if (subjScreen.blocks.length > 0) {
      parsed.needs_human_review_before_send = true;
      parsed.needs_review_reason = (parsed.needs_review_reason ? parsed.needs_review_reason + ' · ' : '')
        + 'subject blocked: ' + subjScreen.blocks.map((b) => `"${b.matches.slice(0, 2).join('", "')}"`).join(', ');
    } else if (subjScreen.rewrites.length > 0) {
      parsed.subject = subjScreen.text;
    }
  } catch (e) { logger.warn(`[draft_response] leak filter threw (shipping unscrubbed): ${e.message}`); }
