// Management Proposal generator (Proposal domain). A community-specific
// commercial proposal — scope, management fee, onboarding fee, term — delivered
// AFTER a prospect asks for a proposal. This is NOT the Board demo; it lived
// under lib/presentations/board.js by historical accident (Ed 2026-09-20).
// Variables come from a form filled in trustEd; the cover image is optional.

const pptxgen = require("pptxgenjs");
// Presentation rendering helpers are shared; the proposal is its own business
// object but reuses the same brand-consistent pptx primitives.
const { COLORS, LOGO_PATH, asset, addFooter, addSectionLabel, bufferToDataUri } = require("../presentations/shared");
...
function build(config = {}, ctx = {}) {
  ...
  // ---------- Slide 5: The model ----------
  // Ed 2026-08-18: "our people are behind the 24/7 AI team managing the
  // community." That inversion was only landing as a closing line on the team
  // slide, which meant a board could leave thinking "innovative firm with AI
  // employees" rather than "they changed what a management company is." It
  // needs its own beat, BEFORE the faces, so the reveal has something to mean.
  //
  // NOTE on the ordering in the right-hand column: our people set policy and
  // own consequential decisions. They are NOT a downstream inspection step
  // after the work is done — that framing is the industry's hedge ("staff
  // review everything") and it is the exact thing we removed. Authority sits
  // ahead of and above the work, not behind it.
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "04 / The model");
    s.addText("Everyone else runs people on software.", {
      x: 0.6, y: 0.9, w: 8.8, h: 0.7,
      fontFace: "Calibri", fontSize: 30, color: COLORS.NAVY, bold: true, margin: 0,
    });
    s.addText("We built it the other way around.", {
      x: 0.6, y: 1.6, w: 8.8, h: 0.7,
      fontFace: "Calibri", fontSize: 30, color: COLORS.NAVY, bold: true, italic: true, margin: 0,
    });
    const by = 2.65, bh = 1.55;
    s.addShape(pres.shapes.RECTANGLE, { x: 0.6, y: by, w: 4.3, h: bh, fill: { color: "F1F5F9" }, line: { color: COLORS.RULE, width: 0.5 } });
    s.addText("THE TRADITIONAL MODEL", { x: 0.8, y: by + 0.16, w: 4, h: 0.3, fontFace: "Calibri", fontSize: 10, color: COLORS.SLATE_MUTED, charSpacing: 3, bold: true, margin: 0 });
    s.addText("Software  ->  a manager  ->  the work", { x: 0.8, y: by + 0.5, w: 4, h: 0.35, fontFace: "Calibri", fontSize: 13, color: COLORS.NAVY, bold: true, margin: 0 });
    s.addText("Everything your association needs has to fit inside one person's day.", { x: 0.8, y: by + 0.88, w: 4, h: 0.6, fontFace: "Calibri", fontSize: 12, color: COLORS.SLATE, margin: 0 });

    s.addShape(pres.shapes.RECTANGLE, { x: 5.1, y: by, w: 4.3, h: bh, fill: { color: COLORS.NAVY }, line: { color: COLORS.NAVY, width: 0 } });
    s.addText("BEDROCK", { x: 5.3, y: by + 0.16, w: 4, h: 0.3, fontFace: "Calibri", fontSize: 10, color: COLORS.ICE, charSpacing: 3, bold: true, margin: 0 });
    s.addText("Our people set the policy  ->  the team does the work", { x: 5.3, y: by + 0.5, w: 4, h: 0.35, fontFace: "Calibri", fontSize: 13, color: COLORS.WHITE, bold: true, margin: 0 });
    s.addText("Nobody's day is the bottleneck, and our people own every decision that carries consequence.", { x: 5.3, y: by + 0.88, w: 4, h: 0.6, fontFace: "Calibri", fontSize: 12, color: COLORS.ICE, margin: 0 });

    s.addText(
      "The AI is not helping our managers run your community. The company is built so the team can run it, and so our people are accountable for it.",
      { x: 0.6, y: 4.5, w: 8.8, h: 0.6, fontFace: "Calibri", fontSize: 14, color: COLORS.NAVY, bold: true, italic: true, margin: 0 }
    );
    addFooter(s, FOOTER);
  }
  ...
  // here is what makes the rest of the deck credible.
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.NAVY_DEEP };
    addSectionLabel(s, "05 / The team", COLORS.ICE);
    s.addText("Nine teammates. None of them go home.", {
      x: 0.6, y: 0.9, w: 8.8, h: 0.7,
      fontFace: "Calibri", fontSize: 32, color: COLORS.WHITE, bold: true, margin: 0,
    });
    s.addText(
      // Ed 2026-08-19: email carries the Bedrock Intelligence mark, not an explicit
      // AI line — he does not want to put people off. So the claim is scoped to
      // where disclosure IS explicit: voice and video. Overstating it on a board
      // slide is the one thing that would make the rest of the deck untrustworthy.
      "They are Bedrock AI teammates, and they say so whenever they speak with you. Nobody is out sick, nobody is buried in another community's crisis, and nothing sits in an inbox until Monday.",
    ...
    s.addText(
      // Ed 2026-08-18: "our people are behind the 24/7 AI team." Every other firm
      // puts people out front with software behind them. Stating the reversal is
      // a stronger claim than denying the industry's usual hedge.
      "Our people stand behind the team, not in front of it. They set the policy it works to, handle the exceptions, and own the decisions that carry legal weight. Fines, liens, and denials are decided by a person, and money does not leave your account without one.",
      { x: 0.6, y: 4.45, w: 8.8, h: 0.7, fontFace: "Calibri", fontSize: 12, color: COLORS.ICE, italic: true, margin: 0 }
    );
    addFooter(s, FOOTER);
  }
  ...
  // ---------- Slide 7: Six things ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "06 / What we manage for you");
  ...
  // ---------- Slide 8: Senior judgment ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "07 / How we think");
  ...
  // ---------- Slide 9: Compounds ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "08 / It compounds for you");
    ...
    const steps = [
      ...
      // "More than any new manager could learn in five years" is unfalsifiable and
      // invites a board to argue with the number instead of hearing the point.
      // Institutional memory walking out the door is a problem every board has
      // actually lived. (Ed 2026-08-18.)
      { n: "YEAR 2+", t: "Institutional memory", b: "Decisions, vendor history, financial patterns and board direction, retained even when people change. The system does not forget." },
    ];
  ...
  // ---------- Slide 10: Transition ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "09 / What month one looks like");
  ...
  // ---------- Slide 11: Pricing (dark) ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.NAVY_DEEP };
    addSectionLabel(s, "10 / What's included", COLORS.ICE);
...
module.exports = {
  slug: "management_proposal",
  title: "Management Proposal",
  description: "Community-specific management proposal: scope, management fee, onboarding fee, and term. Delivered after a prospect asks for a proposal (not part of the demo).",
  variables: [
