...
Return ONLY the body of the reply email.`;
  ...
  try {
    ...
    const body = String(r.content?.[0]?.text || '').trim();
    if (!body) return { draftable: false, reason: 'empty_draft' };
    return {
      draftable: true,
      docType,
...
module.exports = { draftAmandaDocumentReview, classifyDocument, extractText };
