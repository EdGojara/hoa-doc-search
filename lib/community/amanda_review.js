...
${isPrincipal
  ? '- End with your recommendation, then sign off as "Amanda Albright, Senior Community Manager, Bedrock Association Management" so the board sees whose analysis this is. Never sign with the reader\'s name.'
...
- Write it so the whole board can read it: professional, substantive, no internal chatter. Sign off as "Amanda Albright, Senior Community Manager, Bedrock Association Management".`
...
- For any legal point, hedge to counsel ("treat as real risk unless counsel advises otherwise"); never state a legal conclusion as settled fact.
...
The findings array lists only what STILL needs action (open or cleanup), not what is resolved. Never write a rule id in the body.`;
  ...
  const prompt = priorRevision ? comparisonPrompt : standardPrompt;
...
3. OVERSTATEMENTS. Soften any legal conclusion stated as fact to "may be X depending on the circumstances, treat as real risk unless counsel advises otherwise."
...
The findings array must list every problem raised in the body. Never write a rule id in the body.`;
      const cr = await client.messages.create({ model: 'claude-sonnet-4-5', max_tokens: 4000, messages: [{ role: 'user', content: criticPrompt }] });
