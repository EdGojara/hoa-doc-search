...
async function loadPriorReviews({ staffEmail, documentType, limit = 3 }) {
  if (!staffEmail) return [];
  try {
    const { data, error } = await _sb().from('staff_document_reviews')
...
async function draftAmandaDocumentReview({ email, attachments = [], senderFirstName = null }) {
  ...
  const history = await loadPriorReviews({ staffEmail: email.sender_email, documentType: docType });
