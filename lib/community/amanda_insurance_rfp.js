...
async function draftAmandaInsuranceRfp({ email, supabase, communityId, communityName, _build = buildCommunityRfp }) {
  ...
  let result;
  try {
    result = await _build({ supabase, communityId: resolved.id });
  } catch (e) {
    return {
      draftable: true, careful: true, persona: 'amanda',
      subject: `Re: ${email.subject || 'Insurance RFP'}`,
  ...
  return {
    ...
    // persistDraftAttachments reads `content` (the Paige/nomination shape), NOT
    // `buffer` — using the wrong key silently drops the PDF while the body still
    // says "attached" (exactly the scar draft_attachments.js was written for).
    attachments: [{ filename: result.filename, content: result.pdfBuffer, contentType: 'application/pdf' }],
