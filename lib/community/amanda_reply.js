...
async function draftAmandaReply({ email, supabase, propertyId, communityId, contactName, communityName }) {
  ...
  try {
    ...
    const resp = await anthropic.messages.create({
      ...
      system: AMANDA_SYSTEM(communityName),
      messages: [{ role: 'user', content: userContent }],
    });
    // Strip a leading "Subject: ..." line if the model emits one — the subject
    // is carried separately and must not appear inside the sent body.
  ...
  return {
    ...
    body,
    review_hint: reviewHint(trig.reasons, ctx) + (grounded ? ' · grounded' : ' · ownership-ack (fallback)') + (gate.allow ? '' : ` · RESERVED:${gate.reason}`),
    context: ctx,
  };
}

module.exports = { draftAmandaReply, looksLikeEscalation, reservedAsk };
