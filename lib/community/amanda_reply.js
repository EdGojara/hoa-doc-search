...
// The exception router — the primitive that turns "review every draft" into
// "review the exceptions." After Amanda drafts, she classifies her own work
// from the signals already computed: did a reserved-decision gate fire, is the
// answer actually grounded, is the thread charged/hardship/legal, is the sender
// verified. 'auto_ok' means routine, grounded, in-bounds, verified sender — the
// work a supervisor would wave through. 'needs_review' means a human must look,
// and WHY. This does not change the manual-send hold: everything still queues.
// It makes the queue an EXCEPTION queue, and it is the gate that a future
// auto-send would check (auto_ok + high confidence only). This is the operator
// model — the system handles the routine, humans supervise the exceptions.
// (Ed 2026-08-28. New-category north star: the manager IS the system.)
...
async function draftAmandaReply({ email, supabase, propertyId, communityId, contactName, communityName }) {
  ...
  const disp = classifyDisposition({ gateAllowed: gate.allow, grounded, escalationReasons: trig.reasons, audience });
  ...
  return {
    ...
    review_hint: reviewHint(trig.reasons, ctx) + ` · aud:${audience}` + (grounded ? ' · grounded' : ' · ownership-ack (fallback)') + (gate.allow ? '' : ` · RESERVED:${gate.reason}`) + ` · ${disp.disposition}/${disp.confidence}`,
    context: ctx,
  };
}
