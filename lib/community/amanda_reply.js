...
async function draftAmandaReply({ email, supabase, propertyId, communityId, contactName, communityName }) {
  ...
  // Only claim the history when there IS one. propertyContext returns empty
  // whenever the message did not resolve to a property, and this line went out
  // regardless — telling people we had pulled records nobody opened. It went to
  // Martha, a community manager with no lot here, about a board thread.
  // Promising work we did not do is worse than saying less. (Ed 2026-08-20.)
  const looked = ctx.violations.length || ctx.acc.length || ctx.ar_balance != null;
  const seePlan = looked
