...
function pickAudience({ senderEmail, isBoardMember, isOwner }) {
...
async function resolveAudience({ email, supabase, communityId, propertyId }) {
  ...
  try {
    if (senderEmail && communityId) {
      const { data, error } = await supabase.from('board_members')
        .select('email').ilike('email', String(senderEmail).trim())
        .eq('community_id', communityId).eq('is_active', true).limit(1);
      if (!error) isBoardMember = !!(data && data.length);
    }
  } catch (e) { console.warn('[amanda_reply] board lookup failed:', e.message); }
  return pickAudience({ senderEmail, isBoardMember, isOwner: !!propertyId });
}
...
async function draftAmandaReply({ email, supabase, propertyId, communityId, contactName, communityName }) {
  ...
  try {
    ...
    const system = (audience === 'board' ? AMANDA_BOARD_SYSTEM : AMANDA_SYSTEM)(communityName);
