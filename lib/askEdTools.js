    ...
    if (tokens.length) {
      ...
      if (accts && accts.length) {
        ...
        const { data: glLines, error: glErr } = await supabase.from('journal_entry_lines')
          .select('vendor_id, debit_cents, credit_cents, account_id, journal_entries!inner(community_id, posting_date, status)')
        ...
        for (const l of glLines || []) {
          if (l.journal_entries && /void/i.test(l.journal_entries.status || '')) continue;
