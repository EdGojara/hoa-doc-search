...
async function lookupCommunityVendor({ community, category }) {
  ...
  // 3) GL PAYMENT HISTORY — a vendor actually PAID against an account whose name
  //    matches the category, even when there is no contract or manual fact on
  //    file. This is how Lake Pro, Inc. (Lakes of Pine Forest's lake vendor,
  //    paid monthly straight through the ledger but never entered as a contract)
  //    was invisible to askEd, which then told staff "no vendor on file" while
  //    the vendor sat right there in the GL. The ledger is a source of truth for
  //    "who do we actually pay for X," so it must be searched. (Ed 2026-09-02.)
  try {
    const tokens = [...new Set(catQ.split(/\s+/).map((t) => t.replace(/[^a-z0-9]/gi, '')).filter((t) => t.length >= 3))];
    if (tokens.length) {
      const { data: accts, error: acctErr } = await supabase.from('chart_of_accounts')
        .select('id, account_name')
        .eq('community_id', match.id)
        .or(tokens.map((t) => `account_name.ilike.%${t}%`).join(','));
      if (acctErr) throw acctErr;
      if (accts && accts.length) {
        const acctName = new Map(accts.map((a) => [a.id, a.account_name]));
        const { data: glLines, error: glErr } = await supabase.from('journal_entry_lines')
          .select('vendor_id, debit_cents, credit_cents, account_id, journal_entries!inner(community_id, posting_date, status, void_reversal_je_id)')
          .in('account_id', [...acctName.keys()])
          .eq('journal_entries.community_id', match.id)
          .not('vendor_id', 'is', null)
          .limit(2000);
        if (glErr) throw glErr;
        const byVendor = new Map(); // vendor_id -> { net, accounts:Set, last }
        for (const l of glLines || []) {
          if (!countsInGl(l.journal_entries)) continue; // void + its reversal both count (net zero)
          const e = byVendor.get(l.vendor_id) || { net: 0, accounts: new Set(), last: null };
          e.net += Number(l.debit_cents || 0) - Number(l.credit_cents || 0);
          e.accounts.add(acctName.get(l.account_id));
          const pd = l.journal_entries && l.journal_entries.posting_date;
          if (pd && (!e.last || pd > e.last)) e.last = pd;
          byVendor.set(l.vendor_id, e);
        }
        const already = new Set(matches.map((m) => normalize(m.vendor_name || '')));
        if (byVendor.size) {
          const { data: vrows, error: vErr } = await supabase.from('vendors')
            .select('id, name, contact_name, contact_email, contact_phone, phone, email')
            .in('id', [...byVendor.keys()]);
          if (vErr) throw vErr;
          const vmap = new Map((vrows || []).map((v) => [v.id, v]));
          const glMatches = [];
          for (const [vid, e] of byVendor) {
            const v = vmap.get(vid);
            if (!v || already.has(normalize(v.name || ''))) continue;
            const accts2 = [...e.accounts].filter(Boolean).join(', ');
            glMatches.push({
              source: 'gl_payment_history',
              vendor_name: v.name,
              vendor_category: accts2,
              contact_name: v.contact_name || null,
              role: null,
              phone: v.phone || v.contact_phone || null,
              email: v.email || v.contact_email || null,
              start_date: null, end_date: null, last_verified_at: null,
              last_updated_at: e.last,
              is_expired: false,
              notes: `Paid $${Math.round(e.net / 100).toLocaleString('en-US')} against ${accts2} in the general ledger (no contract on file). Last posting ${e.last || 'n/a'}.`,
            });
          }
          glMatches.sort((a, b) => (b.last_updated_at || '').localeCompare(a.last_updated_at || ''));
          matches.push(...glMatches);
        }
      }
    }
  } catch (err) {
    console.warn('[askEdTools] GL vendor lookup failed:', err.message);
  }
