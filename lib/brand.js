...
const BRAND = Object.freeze({
  ...
  /**
   * Inline SVG of the Bedrock B-mark. Drop into any HTML or PDF.
   * @param {object} [opts]
   * @param {string} [opts.fill='#D4AF37'] - mark fill (Heritage Gold by default)
   * @param {string} [opts.cutout='#0B1D34'] - inner cutout color (navy — visible where the mark sits on a navy bg)
   * @param {number} [opts.height=36]      - height in px; width auto-scales (~0.77 aspect)
  ...
  cornerstoneSvg(opts = {}) {
    ...
    // B letterform with stepped columns inside — captures the foundation/
    // architecture theme of the brand. The cutouts are filled with the
    // surrounding bg color so the columns appear as negative space in the
    // mark when rendered on that bg. Default cutout = navy for use on navy
    // surfaces; pass cutout='#FFFFFF' (or whatever bg) for light surfaces.
