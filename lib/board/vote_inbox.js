...
// ONE canonical HTML->text helper (lib/email/graph_attachments) so <br> lines
// don't collapse into a run-on. The old local copy matched </br> (never exists).
const { htmlToText } = require('../email/graph_attachments');

// Dedup WITHOUT marking the message read — that keeps the app permission to
// just Mail.Read (marking read would need Mail.ReadWrite). Mirrors tessa_inbox.
async function alreadySeen(graphId) {
  const { data } = await supabase.from('board_vote_inbox_seen').select('graph_id').eq('graph_id', graphId).maybeSingle();
  return !!data;
}
async function markSeen(graphId, fromEmail, outcome) {
  try { await supabase.from('board_vote_inbox_seen').insert({ graph_id: graphId, from_email: fromEmail || null, outcome }); } catch (_) { /* best-effort */ }
}
...
async function pollVoteInbox({ max = 25 } = {}) {
  ...
  for (const m of messages) {
    ...
    if (await alreadySeen(id)) { stats.ignored += 1; continue; }        // processed on a prior poll
    ...
    catch (e) { console.warn('[vote_inbox] resolve failed:', e.message); stats.errors += 1; continue; } // NOT marked seen → retried next poll
