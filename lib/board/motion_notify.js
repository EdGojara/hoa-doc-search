...
function buildEmail(kind, { motion, communityName, recipientName, tally, voteUrl }) {
  ...
  // For vote emails, the primary action is the no-login ballot link; the portal
  // is offered as a secondary option.
  const portalAlt = url ? `<p style="font-size:12px;color:#64748b;">Prefer the portal? <a href="${esc(url)}" style="color:#0B1D34;">Sign in to the board portal</a>.</p>` : '';
    ...
    return {
      ...
      html: shell(
        `<p>Hi ${esc(who)},</p>
         ...
         <p style="font-size:16px;font-weight:600;color:#0B1D34;">${title}</p>
         ${motion.description ? `<p style="color:#475569;">${esc(motion.description)}</p>` : ''}
         <p>Click below to cast your vote, no login needed.</p>
         ${portalAlt}`,
        'Cast your vote', voteUrl || url),
    };
  }
  if (kind === 'reminder') {
    return {
...
async function enqueueMotionNotifications(motion, kind, recipients, opts = {}) {
  ...
  try {
    ...
    for (const r of (recipients || [])) {
      ...
      const send = await sendEmail({ to: email, bcc: process.env.BOARD_RECORDS_EMAIL || undefined, subject, html, tags: [{ name: 'module', value: 'board_motion' }, { name: 'event', value: kind }] });
      if (send && send.ok) {
        await supabase.from('board_motion_notifications')
