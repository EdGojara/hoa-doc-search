...
const VOTE_INBOX = process.env.BOARD_VOTE_INBOX || 'vote@bedrocktx.com';
...
function buildEmail(kind, { motion, communityName, recipientName, tally, voteUrl }) {
  ...
  // Ref tag in the subject → deterministic reply matching. A reply keeps it.
  const ref = motion.ref_code ? ` [${motion.ref_code}]` : '';
  if (kind === 'to_move') {
    return {
  ...
  if (kind === 'opened') {
    return {
      subject: `Board vote needed${ref}: ${motion.title}`,
      html: shell(
        `<p>Hi ${esc(who)},</p>
  ...
  if (kind === 'reminder') {
    return {
      subject: `Reminder, your board vote is needed${ref}: ${motion.title}`,
      html: shell(
        `<p>Hi ${esc(who)},</p>
