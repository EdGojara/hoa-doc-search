...
async function runRequest(text, { resolveRecipient, searchMailbox, mailboxes }) {
  ...
  // A pure calendar invite ("send Dan an invite for a Teams meeting tomorrow
  // 3-5") is not an email — the Teams invitation IS the message. Don't draft a
  // separate email for it; just stage the meeting. Anything with an email intent
  // (action email / email_and_meeting) still drafts. (Ed 2026-09-10.)
  const pureInvite = parsed.action === 'meeting' && parsed.meeting && parsed.meeting.direct_invite;
  let draft = null;
