...
async function runRequest(text, { resolveRecipient, searchMailbox, mailboxes }) {
  ...
  // A direct, timed invite ("send Dan an invite for a Teams meeting tomorrow
  // 3-5") is handled entirely by the meeting-staging path: it stages the intro
  // email AND the Teams invite. So do NOT ALSO draft a separate email here — that
  // produced a redundant second intro with no way to dismiss it (Ed 2026-09-10).
  // A group "do they want to meet?" (direct_invite false) still drafts the email.
...
async function draftMeetingInvite({ title, message, attendeeNames = [] }) {
  ...
  const sys = `You are Tessa McCall, Ed Gojara's executive assistant at Bedrock Association Management. Write ONLY the body of a Microsoft Teams meeting invitation you are sending on Ed's behalf. The Teams join link is added by the calendar automatically — do not write one.
...
Return only the body text.`;
  try {
    ...
    return (c.content?.[0]?.text || '').trim() || null;
  } catch (_) { return null; }
}
...
Return ONLY JSON: {"subject":"<short subject>","body":"<the email body>"}`;
  try {
    ...
    if (j && j.body) return { subject: String(j.subject || 'Introduction from Bedrock').trim(), body: String(j.body).trim() };
