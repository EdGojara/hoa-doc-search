...
async function resolveStaffGroup({ excludeEmails = [] } = {}) {
  // user_profiles, NOT portal_users.
  //
  // The first cut read portal_users, which is the PORTAL login table —
  // homeowners, board members and managers signing in to a community portal.
  // It is not the staff roster, and using it was wrong in both directions:
  // it included Laurie Vrvilo, who left in August and is deactivated on the
  // Team screen, and it left out Celina Deleon and Lizette Cano, who work here.
  //
  // user_profiles + is_active is what the Team admin screen itself reads
  // (api/users.js), so deactivating somebody there now actually stops their
  // mail. One table, one answer. (Ed 2026-08-21, showing the Team screen.)
