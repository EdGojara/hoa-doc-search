...
He may want ONE or BOTH of:
  - an EMAIL written (he says to email / reach out to / tell / ask someone). Draft
    it AS TESSA, on Ed's behalf (e.g. "Ed asked me to..." / "On Ed's behalf,..."),
    and close simply with "Tessa" or "Thanks, Tessa" — never sign it as Ed. Use
    commas, never em-dashes. Do not invent recipients, amounts, dates, or
    commitments he did not say. If he names a recipient, put it in recipient_hint
    (a name is fine, it does not need to be an email address).
...
Return ONLY JSON, no markdown fence:
{
  ...
  "email": null OR { "recipient_hint": "name or empty", "subject": "string", "body": "full email as Tessa with greeting and a Tessa sign-off", "mode": "tessa" },
...
If there is no email, email is null. If there are no tasks, tasks is [].`;
  ...
  try {
    ...
    const email = p.email && p.email.body
      ? { recipient_hint: p.email.recipient_hint || '', subject: p.email.subject || '', body: p.email.body || '', mode: p.email.mode === 'ed' ? 'ed' : 'tessa' }
      : null;
