 ...
 */
/**
 * Render the call for nominations for a cycle as a PDF attachment.
 *
 * Pulled out of handleNominationRequest so the document-review path can build
 * the same document when it finds something wrong with a staffer's draft — one
 * renderer, so a reviewed replacement and a generated original can never drift
 * into being two different letters. (Ed 2026-08-21: "if paige can just do one
 * better have paige send it to martha.")
 *
 * Returns [] rather than throwing. The caller must word its reply from what
 * came back: an empty list means say "I can generate one", never "attached".
 */
async function buildCallForNominations({ community, cycle }) {
  if (!cycle) return [];
  try {
    const html = await renderCallForNominationsHTML(cycle, {});
    const pdf = await letterPdf(html);
    const year = String(cycle.annual_meeting_date || '').slice(0, 4) || String(new Date().getUTCFullYear());
    const name = `${(community && community.name) || cycle.community_name || 'Association'} Call for Nominations ${year}.pdf`;
    return [{ filename: name, content: pdf, contentType: 'application/pdf' }];
  } catch (e) {
    // The letter still exists in the platform; the caller says so rather than
    // pretending a file is attached.
    console.warn('[paige_nominations] could not render the call for nominations:', e.message);
    return [];
  }
}

async function handleNominationRequest({ email, supabase, community, meetingDate, senderFirstName }) {
  // Filenames included: "Call for Nominations LOPF.2026.docx" IS the request,
  // even when the message body never uses the word. (Ed 2026-08-21.)
  const text = [email.subject, email.body_full, email.body, email.body_preview]
  ...
  // No hand-typed sign-off. Every persona email is wrapped by buildPaigeEmail at
  // send (api/email_drafts.js, api/email_triage.js), which appends the ONE
  // branded signature block — Paige's name, title, headshot and the Bedrock
  // logo. A typed "Paige / Board Operations" here doubled that block when wrapped
  // and, on any path that skipped the wrapper, was the ONLY sign-off — a plain
  // line with no photo (the gap Ed caught on the letter to Martha, 2026-09-03).
  // The body signs off through the branded block, never by hand. See
  // lib/email/persona_signature.js.
  const sign = '';
  ...
  // Before asking, look at what is already on file.
  //
  // Ed's standing rule: never ask staff for something the system already holds.
  // Martha's email did not state a date, so Paige asked "what date is the annual
  // meeting?" — while an OPEN Lakes of Pine Forest cycle sat in the platform
  // with annual_meeting_date 2026-10-13, nominations 8/19–9/18, one seat. Asking
  // a community manager for a date the platform set is exactly the kind of
  // question that teaches people the assistant is not paying attention.
  // (Ed 2026-08-21.)
  let openCycle = null;
  if (!meetingDate) {
    ...
    if (cErr) console.warn('[paige_nominations] cycle lookup failed:', cErr.message);
    // Prefer one still open; otherwise the most recent, which is still a better
    // starting point than an empty question.
    openCycle = (cycles || []).find((c) => c.status === 'open') || (cycles || [])[0] || null;
    if (openCycle && openCycle.annual_meeting_date) meetingDate = openCycle.annual_meeting_date;
  }
  ...
  const attachments = await buildCallForNominations({ community, cycle });
...
module.exports = { handleNominationRequest, buildCallForNominations, detectNominationIntent, extractStatedSeats, extractStatedTime };
