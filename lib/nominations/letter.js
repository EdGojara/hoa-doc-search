...
// Contact is ALWAYS ${BRAND.service.email} (role address — survives staff turnover).
...
// Meeting times arrive from the cycle row however staff typed them: "0600pm",
// "6:00pm", "6 PM", "18:00". The live LOPF letter printed "at 0600pm" to
// members. Normalise to "6:00 PM", and if it cannot be parsed pass it through
// untouched rather than inventing a time. (Ed 2026-08-19.)
function fmtTime(t) {
  if (!t) return '';
  ...
  if (ap) { if (h === 12) h = ap === 'a' ? 0 : 12; else if (ap === 'p') h += 12; }
  if (h < 0 || h > 23 || Number(mins) > 59) return raw;
...
const DEFAULT_EXPECTATIONS = `Board members serve a three-year term. The role is volunteer and typically takes ten to twenty hours a month. The board approves the annual budget, oversees the management agent, and represents the homeowners. Meeting cadence varies by community, from monthly to quarterly to only as needed. If you would like to serve, please submit your nomination.`;
...
async function renderCallForNominationsHTML(cycle, opts = {}) {
  ...
  const base = opts.base_url || BRAND.tech.appUrl;
  const slug = public_slug || id;
  const publicUrl = `${base}/nominate/${slug}`;

  let qrDataUri = '';
  try {
    qrDataUri = await QRCode.toDataURL(publicUrl, { width: 360, margin: 1, color: { dark: BRAND.colors.navy, light: '#FFFFFF' } });
  } catch (_) { qrDataUri = ''; }
...
<style>
  ...
  .head .community { font-size: 17pt; font-weight: 700; color: ${BRAND.colors.navy}; letter-spacing: 0.2px; }
  ...
  .return-block .b { color: ${BRAND.colors.navy}; font-weight: 700; }
  ...
  .hero-card {
    background: #f8fafc; border: 1px solid #e2e8f0; border-left: 3px solid ${BRAND.colors.navy};
  ...
  .hero-card .val { font-size: 11.5pt; font-weight: 700; color: ${BRAND.colors.navy}; margin-top: 2px; }

  /* Current board */
  .board-section { margin: 14px 0; }
  .board-h { font-family: Calibri, Arial, sans-serif; font-size: 10pt; font-weight: 700; color: ${BRAND.colors.navy}; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 5px; }
  table.board { width: 100%; border-collapse: collapse; font-size: 10.5pt; }
  table.board th { text-align: left; font-family: Calibri, Arial, sans-serif; font-size: 8.5pt; color: #475569; letter-spacing: 0.5px; text-transform: uppercase; border-bottom: 1.5px solid ${BRAND.colors.navy}; padding: 4px 6px; }
  ...
  .submit .copy .h { font-family: Calibri, Arial, sans-serif; font-size: 14pt; font-weight: 700; color: ${BRAND.colors.navy}; margin-bottom: 4px; letter-spacing: 0.3px; }
  .submit .copy .u { font-family: Consolas, "Courier New", monospace; font-size: 10.5pt; color: ${BRAND.colors.navy}; font-weight: 700; word-break: break-all; padding: 4px 8px; background: #f8fafc; border-radius: 4px; display: inline-block; margin: 4px 0 6px; }
  ...
  .other-ways .h { font-family: Calibri, Arial, sans-serif; font-size: 9pt; font-weight: 700; color: ${BRAND.colors.navy}; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 5px; }
  .other-ways ul { margin: 0; padding-left: 18px; }
  .other-ways li { margin: 2px 0; }
  .other-ways .b { font-weight: 700; color: ${BRAND.colors.navy}; }
  ...
  .deadline .lbl { font-family: Calibri, Arial, sans-serif; font-size: 8.5pt; letter-spacing: 1.5px; text-transform: uppercase; color: ${BRAND.colors.gold}; font-weight: 700; }
  ...
  .signature .line1 { font-weight: 700; color: ${BRAND.colors.navy}; }
...
<div class="return-block">
  ...
  <div>${BRAND.service.addressInline}</div>
  <div>${BRAND.service.phone} &nbsp;·&nbsp; ${BRAND.service.email}</div>
</div>

<div class="head">
  ${logo
    ? `<img src="${logo}" alt="${escapeHtml(community_name)}">`
    : `<div style="font-size:20pt; font-weight:700; color:${BRAND.colors.navy};">${escapeHtml(community_name)}</div>`}
...
<div class="doc-title">Call for Nominations to the Board of Directors</div>

<div class="salutation">Dear ${escapeHtml(community_name)} Homeowner,</div>

<p>The annual meeting of the ${escapeHtml(community_name)} Homeowners Association will be held on <strong>${escapeHtml(fmtDate(annual_meeting_date))}</strong>${annual_meeting_time ? ` at <strong>${escapeHtml(fmtTime(annual_meeting_time))}</strong>` : ''}${annual_meeting_location ? `, at <strong>${escapeHtml(annual_meeting_location)}</strong>` : ''}. The purpose of this meeting is to elect ${seatPhrase} of the Board of Directors to serve a ${termPhrase} and to discuss the affairs of the Association.</p>
...
<div class="board-section">
  <div class="board-h">${seats === 1 ? 'Open Position' : 'Open Positions'}</div>
  <div class="open-seat">
    <strong>${numberToWord(seats).replace(/^./, (ch) => ch.toUpperCase())} (${seats}) Director${seats === 1 ? '' : 's'}</strong>, ${numberToWord(termN)} (${termN}) year term
  </div>
</div>
...
<div class="other-ways">
  ...
  <ul>
    ${acceptElectronic ? `<li><span class="b">Email:</span> ${BRAND.service.email} (include nominee name, address, a brief bio, and a photo if available)</li>` : ''}
    ${acceptPhysical ? `<li><span class="b">Mail:</span> ${escapeHtml(community_name)} HOA, c/o ${BRAND.service.name}, ${BRAND.service.addressInline}</li>` : ''}
    ${showOnsite ? `<li><span class="b">Drop off:</span> ${escapeHtml(onsite.location_name || 'On-site office')}${onsite.address ? `, ${escapeHtml(onsite.address)}` : ''}</li>` : ''}
  </ul>
</div>
` : ''}
...
<div class="deadline">
  <div class="lbl">Nominations Close</div>
  ${escapeHtml(fmtDate(nominations_close_at))}${nominations_close_time ? ` at ${escapeHtml(fmtTime(nominations_close_time))}` : ''}
</div>
...
<p>If you have questions about the role or the nomination process, please email <strong>${BRAND.service.email}</strong> or call <strong>${BRAND.service.phone}</strong>. Thank you for your continued participation in the ${escapeHtml(community_name)} community.</p>
...
<div class="signature">
  ...
  <div class="footer-contact">${BRAND.service.phone} &nbsp;|&nbsp; ${BRAND.service.email} &nbsp;|&nbsp; ${BRAND.service.website}</div>
</div>
