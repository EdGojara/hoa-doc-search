...
async function renderAnnualMeetingNoticeHTML({
  ...
  const opts = {
    ...
    // 'compact' = Waterview 2025 style (single-page notice+proxy+ballot,
    // §209 inline). 'detailed' = Canyon Gate 2026 style (separate pages).
    // Migration 144 added this field; default 'compact' when not set since
    // that's the postage-saving + most-prior-Bedrock-packets shape.
    layout_mode:         options.layout_mode || cycle.layout_mode || 'compact',
  };
