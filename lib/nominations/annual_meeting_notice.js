...
async function renderAnnualMeetingNoticeHTML({
  ...
  // 2 candidates per page: pair-wrap every two cards and force a page break
  // after each pair. Reduces page count by ~50% on slates of 3+ candidates.
  // Last pair (with 1 or 2 cards) doesn't force a break — natural EOF.
  const candidatePairs = [];
  for (let i = 0; i < candidateCards.length; i += 2) {
    candidatePairs.push(candidateCards.slice(i, i + 2).join(''));
  }
  const candidateBlocks = candidatePairs.map((pair, idx) => {
    const isLast = idx === candidatePairs.length - 1;
    return `<div class="cand-pair${isLast ? '' : ' cand-pair-break'}">${pair}</div>`;
  }).join('');
...
<style>
  ...
  .deadline-line { color: ${BRAND.colors.navy}; }
  ...
  .tx209-callout .hdr { font-weight: 700; color: ${BRAND.colors.navy}; margin-bottom: 4px; }
  ...
  .cand-photo img { width: 95px; height: 95px; object-fit: cover; border-radius: 6px; }
  .cand-photo-empty {
    width: 95px; height: 95px; border-radius: 6px;
    background: #f1f5f9; color: #94A3B8;
    display: flex; align-items: center; justify-content: center;
    text-align: center; font-size: 9.5pt;
  }
  .cand-name { font-size: 12.5pt; font-weight: 700; color: ${BRAND.colors.navy}; }
  .cand-name .incumbent { color: #475569; font-weight: 500; font-style: italic; font-size: 10pt; }
  .cand-years { font-size: 10pt; color: #475569; font-style: italic; margin: 2px 0 6px; }
  .cand-bio { font-size: 10pt; line-height: 1.5; white-space: pre-line; }
...
<div class="page">
  <div class="head">
    ${logo
      ? `<img src="${logo}" alt="${escapeHtml(communityName)}">`
      : `<div style="font-size:18pt; font-weight:700; color:${BRAND.colors.navy};">${escapeHtml(communityName)}</div>`}
    <div class="community">${escapeHtml(upperName)}</div>
