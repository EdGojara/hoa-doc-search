...
async function renderPaperFormHTML(cycle, opts = {}) {
  ...
  const mailAddr = opts.mail_address || BRAND.service.addressInline;
  const emailAddr = opts.email_address || BRAND.service.email;
  const phone = opts.phone || BRAND.service.phone;
...
<style>
  ...
  /* Letterhead */
  .head { text-align: center; padding-bottom: 10px; border-bottom: 2px solid ${BRAND.colors.navy}; margin-bottom: 12px; }
  .head img { max-height: 64px; max-width: 180px; display: block; margin: 0 auto 4px; }
  .head .legal { font-size: 13pt; font-weight: 700; color: ${BRAND.colors.navy}; letter-spacing: 0.3px; }
  ...
  .deadline .lbl { font-family: Calibri, Arial, sans-serif; font-size: 8pt; letter-spacing: 1.4px; text-transform: uppercase; color: ${BRAND.colors.gold}; font-weight: 700; }
  ...
  .opt-box { font-family: Consolas, monospace; font-size: 12pt; color: ${BRAND.colors.navy}; }
  ...
  .photo-instructions strong { color: ${BRAND.colors.navy}; }
  .photo-box {
...
<div class="head">
  ${logo
    ...
    : `<div style="font-size:18pt; font-weight:700; color:${BRAND.colors.navy};">${escapeHtml(community)}</div>`}
...
<div class="self-row">
  <strong style="color:${BRAND.colors.navy};">I am submitting this nomination:</strong>
...
<div class="fld-row">
  ...
  <div class="fld">
    <label>If nominating a neighbor, your name</label>
    <div class="line"></div>
  </div>
</div>
...
<div class="photo-area">
  <div class="photo-instructions">
    <strong>Photo (optional but encouraged).</strong> A head-and-shoulders photo helps your neighbors recognize you on the ballot. Include a photo with this form when you return it, printed, attached, or as a separate digital file if emailing.
  </div>
...
<div class="form-must-sign">
  <strong>FORM MUST BE SIGNED TO BE VALID.</strong> ${BRAND.service.short} will confirm receipt of your nomination by email and phone within one business day.
</div>
...
<div class="submit-box">
  ...
  <ul>
    ...
    <li><strong>Mail:</strong> ${escapeHtml(community)} HOA, c/o ${BRAND.service.name}, ${escapeHtml(mailAddr)}</li>
    ${showOnsite ? `<li><strong>Drop-off:</strong> ${escapeHtml(onsite.location_name || 'On-site office')}${onsite.address ? ', ' + escapeHtml(onsite.address) : ''}</li>` : ''}
  </ul>
</div>

<div class="footer">
  ${escapeHtml(legalName)} &nbsp;|&nbsp; c/o ${BRAND.service.legal} &nbsp;|&nbsp; ${escapeHtml(mailAddr)} &nbsp;|&nbsp; ${escapeHtml(phone)} &nbsp;|&nbsp; ${BRAND.service.website}
</div>
