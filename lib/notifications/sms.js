...
async function sendSms(opts) {
  // Demo isolation boundary — FIRST, before any config/validation.
  const _g = await require('../demo/outbound_guard').guardOutbound({ channel: 'sms', communityId: (opts && opts.communityId) || null, to: opts && opts.to });
  if (_g.blocked) return { ok: false, vendor: 'twilio', suppressed: true, reason: _g.reason, to: opts && opts.to };
  if (!isConfigured()) {
  ...
  // Owner/internal alerts (e.g. Tessa texting Ed) skip the TCPA STOP footer —
  // that footer is for customer messaging, not a note to yourself/your team.
  const body = opts.noFooter ? String(opts.body) : ensureStopFooter(opts.body);
