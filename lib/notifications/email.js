...
async function sendEmail(opts) {
  ...
  {
    const _to = opts && (Array.isArray(opts.to) ? opts.to[0] : opts.to);
    const _g = await require('../demo/outbound_guard').guardOutbound({ channel: 'email_resend', communityId: (opts && opts.communityId) || null, to: _to, subject: opts && opts.subject });
    if (_g.blocked) return { ok: false, vendor: 'resend', suppressed: true, reason: _g.reason };
  }
  if (!isConfigured()) {
