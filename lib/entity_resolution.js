...
// Common English given-name nicknames <-> formal names, so "Jim Storm" matches
// "James Storm". Grouped; every member maps to every other member of its group.
const NICKNAME_GROUPS = [
  ['james', 'jim', 'jimmy', 'jamie'], ['robert', 'bob', 'rob', 'bobby', 'robbie'],
  ['william', 'bill', 'will', 'billy', 'willie'], ['richard', 'rick', 'rich', 'dick', 'richie', 'ricky'],
  ['michael', 'mike', 'mikey', 'mick'], ['thomas', 'tom', 'tommy'], ['charles', 'chuck', 'charlie', 'chas'],
  ['john', 'jack', 'johnny', 'jon'], ['joseph', 'joe', 'joey'], ['edward', 'ed', 'eddie', 'ted', 'teddy', 'ned'],
  ['daniel', 'dan', 'danny'], ['david', 'dave', 'davey'], ['christopher', 'chris'], ['anthony', 'tony'],
  ['matthew', 'matt'], ['andrew', 'andy', 'drew'], ['nicholas', 'nick', 'nicky'], ['benjamin', 'ben', 'benny', 'benji'],
  ['samuel', 'sam', 'sammy'], ['patrick', 'pat', 'paddy'], ['timothy', 'tim', 'timmy'], ['kenneth', 'ken', 'kenny'],
  ['ronald', 'ron', 'ronnie'], ['donald', 'don', 'donnie'], ['gerald', 'gerry', 'jerry'], ['lawrence', 'larry'],
  ['frederick', 'fred', 'freddy', 'freddie'], ['gregory', 'greg'], ['stephen', 'steven', 'steve', 'stevie'],
  ['peter', 'pete'], ['raymond', 'ray'], ['eugene', 'gene'], ['philip', 'phillip', 'phil'], ['albert', 'al', 'bert'],
  ['leonard', 'leo', 'len', 'lenny'], ['vincent', 'vince', 'vinny'], ['walter', 'walt', 'wally'],
  ['henry', 'hank', 'harry'], ['douglas', 'doug'], ['jeffrey', 'jeff'], ['joshua', 'josh'], ['zachary', 'zach', 'zack'],
  ['elizabeth', 'liz', 'beth', 'betty', 'betsy', 'eliza', 'libby'], ['margaret', 'maggie', 'peggy', 'meg', 'marge', 'margie'],
  ['katherine', 'catherine', 'kate', 'katie', 'kathy', 'cathy', 'katy'], ['jennifer', 'jen', 'jenny', 'jenn'],
  ['patricia', 'patty', 'tricia', 'trish'], ['deborah', 'deb', 'debbie', 'debra'], ['barbara', 'barb', 'babs'],
  ['susan', 'sue', 'susie', 'suzy', 'suzanne'], ['sandra', 'sandy'], ['victoria', 'vicky', 'vic', 'tori'],
  ['virginia', 'ginny', 'ginger'], ['dorothy', 'dot', 'dottie', 'dolly'], ['rebecca', 'becca', 'becky'],
  ['cynthia', 'cindy'], ['pamela', 'pam'], ['kimberly', 'kim'], ['theresa', 'teresa', 'terry', 'tess', 'tessa'],
  ['christine', 'christina', 'tina', 'christy'], ['jacqueline', 'jackie'], ['stephanie', 'steph'],
  ['samantha', 'sammy'], ['angela', 'angie'], ['judith', 'judy'], ['janet', 'jan'], ['joanne', 'joan'],
  ['frances', 'fran', 'frankie'], ['abigail', 'abby', 'abbie'],
];
const NICKNAME_MAP = (() => {
  const m = {};
  for (const g of NICKNAME_GROUPS) for (const n of g) { m[n] = m[n] || new Set(); for (const x of g) if (x !== n) m[n].add(x); }
  return m;
})();

// Two given names match if identical, one is the other's initial, or they are
// nickname-equivalent (Jim/James, Bob/Robert).
function firstNamesMatch(a, b) {
  if (!a || !b) return false;
  a = String(a).toLowerCase(); b = String(b).toLowerCase();
  if (a === b) return true;
  if ((a.length === 1 || b.length === 1) && a[0] === b[0]) return true;
  const g = NICKNAME_MAP[a];
  return !!(g && g.has(b));
}
...
function namesAreEquivalent(a, b) {
  ...
  // Same last name + first names match (exact, initial, or nickname).
  if (na.first && nb.first && na.last && nb.last
      && na.last === nb.last && firstNamesMatch(na.first, nb.first)) return true;
  return false;
}
...
async function findProperty(supabase, communityId, addressText) {
  ...
  if (/^\d/.test(canonical)) {
    const near = candidates.filter((c) => {
      const cCore = streetCore(normalizeAddress(c.street_address).canonical);
      // Compare the STORED address's suffix-stripped core against the FULL typed
      // address too — not just core-to-core. A street-NAME word that also lives
      // in the suffix list ("Meadow", "View", "Ridge", "Bend") gets wrongly
      // stripped off the typed side, so "19338 Stable Meadow" would never match
      // "19338 Stable Meadow Drive" on core-to-core alone. (Ed 2026-07-20.)
      return cCore && (cCore === canonical || cCore === inCore);
    });
    if (near.length === 1) {
      const c = near[0];
