 ...
 */
// Structured remit fields off an extraction — prefers the parsed remit_address;
// falls back to the raw remit_to blob in line1 so we at least capture something.
function _remitFields(extracted) {
  const a = extracted && extracted.remit_address;
  if (a && a.line1) {
    return {
      remit_address_line1: a.line1, remit_address_line2: a.line2 || null,
      remit_city: a.city || null, remit_state: a.state || null, remit_zip: a.zip || null,
    };
  }
  const blob = extracted && extracted.remit_to ? String(extracted.remit_to).replace(/\s+/g, ' ').trim().slice(0, 250) : null;
  return blob ? { remit_address_line1: blob } : null;
}

// Backfill a matched vendor's remit address from the invoice IF it has none yet —
// so vendors created before we captured addresses (or as name-only stubs) get a
// mailing address the first time a new bill comes in. Never overwrites an
// existing address. (Ed 2026-08-14.)
async function _backfillRemit(vendorId, extracted) {
  try {
    const rf = _remitFields(extracted);
    if (!rf) return;
    const { data: cur } = await supabase.from('vendors').select('remit_address_line1, remit_city, remit_zip').eq('id', vendorId).maybeSingle();
    if (cur && (cur.remit_address_line1 || cur.remit_city || cur.remit_zip)) return; // already has one
    await supabase.from('vendors').update(rf).eq('id', vendorId);
  } catch (e) { console.warn('[vendor_master] remit backfill skipped:', e.message); }
}
...
async function ensureVendorForInvoice({ extracted, actor = 'Emma (AP)' }) {
  ...
  if (email) {
    ...
    if (byEmail.length === 1) { await _backfillRemit(byEmail[0].id, extracted); return { vendor: byEmail[0] }; }
  }
  if (name) {
    const n = normName(name);
    const exact = vendors.filter((v) => normName(v.name) === n || (v.dba && normName(v.dba) === n));
    if (exact.length === 1) { await _backfillRemit(exact[0].id, extracted); return { vendor: exact[0] }; }
  ...
  // Capture the vendor's contact + remit address straight off the invoice the
  // way the upload path does, so a vendor born from an emailed bill isn't a
  // name-only stub. (Ed 2026-08-10.) Structured remit fields (line/city/state/
  // zip) so the check prints a clean 3-line block. (Ed 2026-08-14.)
  const row = {
    management_company_id: BEDROCK_MGMT_CO_ID,
    name,
    email: email || null,
    contact_email: email || null,
    ...(_remitFields(extracted) || {}),
    status: 'active',
    is_active: true,
