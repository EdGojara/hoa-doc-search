...
async function ensureVendorForInvoice({ extracted, actor = 'Emma (AP)' }) {
  ...
  // name-only stub. (Ed 2026-08-10.)
  const remitTo = extracted && extracted.remit_to ? String(extracted.remit_to).replace(/\s+/g, ' ').trim().slice(0, 250) : null;
  const row = {
    management_company_id: BEDROCK_MGMT_CO_ID,
    name,
    email: email || null,
    contact_email: email || null,
    remit_address_line1: remitTo,
    status: 'active',
    is_active: true,
