...
async function postAccrualForInvoice(a) {
  try {
    ...
    const post = await apPostingDateFor(a);
    const je = await postJournalEntry({
      community_id: a.communityId, posting_date: post.postingDate,
      description: `AP invoice ${a.vendorInvoiceNumber || ''} — ${a.vendorName || 'vendor'}${post.note}`.trim(),
...
// GL date for an AP accrual: the invoice date, or the community's GL cutover
// date when the invoice is dated before it (the pre-cutover period is converted
// and certified). The invoice keeps its real date; the entry says why it moved.
async function apPostingDateFor(a) {
  const { getGlCutoverDate, apInvoicePostingDate } = require('../accounting/cutover');
  const invoiceDate = String(a.invoiceDate).slice(0, 10);
  const postingDate = apInvoicePostingDate(invoiceDate, await getGlCutoverDate(supabase, a.communityId));
  return { postingDate, note: postingDate !== invoiceDate ? ` (invoice dated ${invoiceDate}; posted at GL cutover ${postingDate})` : '' };
}
