...
async function autoIntake({ buffer, filename, intakeMethod, sourceRef, communityId, vendorIdHint, achHintText, communityHint, staffNote, staffSenderEmail }) {
  ...
  if (fromPdf.community) cid = fromPdf.community.id;
  ...
  if (!cid && directives.applies && directives.community) cid = directives.community.id;
  if (!cid) cid = communityId || null;
  if (!cid && communityHint) { const c = await resolveCommunity(communityHint); cid = c.community ? c.community.id : null; }
  ...
  return commitInvoice({ extracted, vendorId: v.vendor.id, communityId: cid, sha256, storagePath, intakeMethod, sourceRef, achHintText, staffGl });
}
