...
async function commitInvoice({ extracted, vendorId, communityId, sha256, storagePath, intakeMethod, sourceRef, achHintText }) {
  ...
  // ACH auto-pay: if the vendor is flagged as auto-draft, this bill is paid by
  // ACH — Emma records it and keeps it OUT of the check run. She also reads the
  // invoice/email to CONFIRM (belt-and-suspenders vs. the vendor flag).
  let isAch = false, achConfirmed = null;
  const vres = await supabase.from('vendors').select('auto_pay_ach').eq('id', vendorId).maybeSingle();
  if (!vres.error && vres.data && vres.data.auto_pay_ach) {  // !error: safe before migration 267 adds the column
    isAch = true;
    const hay = `${extracted.terms || ''} ${(extracted.line_items || []).map((l) => l.description).join(' ')} ${achHintText || ''}`.toLowerCase();
    achConfirmed = /\bach\b|auto.?draft|auto.?pay|automatic\w*\s+(draft|debit|withdraw|payment)|e-?check|electronic\w*\s+(payment|draft)/.test(hay);
  }
  ...
  if (error && /ach_confirmed_by_invoice|is_ach_autopay|coded_gl_account_id|classification_reason|needs_review|column .* does not exist/i.test(String(error.message || ''))) {
