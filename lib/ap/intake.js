...
async function commitInvoice({ extracted, vendorId, communityId, sha256, storagePath, intakeMethod, sourceRef, achHintText, staffGl, staffGlSplit }) {
  ...
  try {
    ...
    if (splitOk) {
      codedLines = staffGlSplit.lines.map((l, i) => ({
        line_number: i + 1,
        description: `Staff-directed: ${l.account_number} ${l.account_name}`,
        amount_cents: l.amount_cents,
        gl_account_id: l.account_id,
        reason: 'Staff directive from the forwarding note',
      }));
