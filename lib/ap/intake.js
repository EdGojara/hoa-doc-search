...
// A utility "bill-to" is the account-holder association name FOLLOWED BY THE
// SERVICE ADDRESS: "WATERVIEW EST. OWNERS ASSOC. — 5100 WATERVIEW MEADOWS DR-IRR".
// Only the association name should drive community matching. The street address
// drags in tokens that belong to OTHER communities — here "MEADOWS" (from the
// service street "WATERVIEW MEADOWS DR") collided with "August Meadows", made the
// token match ambiguous, and stranded a plainly-Waterview MUD bill as
// no_community while its address-free siblings filed fine. Cut the hint at the
// service address before matching. (Ed 2026-09-03 — the recurring "Emma didn't
// process the MUD invoices" batch: the fee works; the FILING stranded here.)
function assocNameOnly(hint) {
  let s = String(hint || '');
  s = s.split(/\s+[—–-]\s+/)[0];                    // spaced dash: address trails it
  s = s.replace(/\b\d{3,6}\s+[A-Za-z].*$/, '');     // "5100 WATERVIEW MEADOWS DR ..."
  s = s.trim();
  return s || String(hint || '');                  // never empty the hint entirely
}

async function resolveCommunity(hint) {
  if (!hint) return { community: null, candidates: [] };
  hint = assocNameOnly(hint);
...
// Did staff explicitly ask, in the forwarding email, to add the ~$1 convenience
// fee? Requires an "add" instruction near a $1 / dollar / convenience-fee
// mention, so a passing reference (e.g. the vendor's own quoted email) doesn't
// trigger it. (Ed 2026-08-27)
function staffAsksForConvenienceFee(text) {
  const t = String(text || '').toLowerCase();
  if (!t) return false;
  return /\badd(?:ing|s|\s+the|\s+a|\s+in)?\b[^.\n]{0,40}(?:\$\s*1(?:\.00)?\b|\bone dollar\b|\ba dollar\b|\bthe dollar\b|convenience\s*fee)/.test(t);
}

async function commitInvoice({ extracted, vendorId, communityId, sha256, storagePath, intakeMethod, sourceRef, achHintText, staffNote, staffGl, staffGlSplit, reimbursementSource }) {
  ...
  // it. Two triggers: (1) the vendor is flagged MUD; (2) staff explicitly ask in
  // the forwarding email body to add the $1 — Emma reads it now, so a MUD bill
  // from a not-yet-flagged vendor still gets the fee when staff say to. (Ed
  // 2026-07-23, extended to read the staff email 2026-08-27.)
  let convenienceFeeCents = 0;
  let convenienceFeeSource = null;
  try {
    ...
    let cents = fee.cents, label = fee.label, source = fee.cents > 0 ? 'vendor setting' : null;
    if (cents === 0 && staffAsksForConvenienceFee(staffNote)) {
      cents = 100; label = 'MUD convenience fee'; source = 'staff instruction in the email';
    }
    if (cents > 0) {
      applyConvenienceFee(extracted, { cents, label }, 'line_items');
      convenienceFeeCents = cents;
      convenienceFeeSource = source;
    }
  ...
  if (convenienceFeeCents > 0) {
    row.notes += ` Added $${(convenienceFeeCents / 100).toFixed(2)} MUD convenience fee (${convenienceFeeSource || 'vendor setting'}) — folded into the bill total, no separate line item.`;
  }
  if (openDeposit) {
...
async function postAccrualForInvoice(a) {
  try {
    ...
    const glDate = await cutoverPostingDate(a);   // pre-cutover: review, never auto-post
    if (!glDate) return null;
    const { postJournalEntry } = require('../accounting/posting');
    const je = await postJournalEntry({
      community_id: a.communityId, posting_date: glDate,
      description: `AP invoice ${a.vendorInvoiceNumber || ''} — ${a.vendorName || 'vendor'}${cutoverNote(a, glDate)}`.trim(),
      source_module: 'ap_invoice', source_reference: a.invoiceId, ap_posting: true,
      lines: [
...
async function postSplitAccrual(a, gl) {
  ...
  const je = await postJournalEntry({
    community_id: a.communityId, posting_date: glDate,
    description: `AP invoice ${a.vendorInvoiceNumber || ''} — ${a.vendorName || 'vendor'}${cutoverNote(a, glDate)}`.trim(),
    source_module: 'ap_invoice', source_reference: a.invoiceId, lines, ap_posting: true,
  });
  try { await supabase.from('journal_entries').update({ source_document_id: a.sourceDocumentId || null, source_document_path: a.sourceDocumentPath || null, classification_reason: a.classificationReason || null }).eq('id', je.entry.id); } catch (_) { /* Phase 1 doc-link cols */ }
  await supabase.from('ap_invoices').update({ posting_journal_entry_id: je.entry.id }).eq('id', a.invoiceId);
  return je.entry.id;
...
// Pre-cutover AP invoice (dated before the community's gl_cutover_date): never
// auto-posted, not even at cutover. It may already be in the converted books;
// it waits in cutover_review = 'PENDING' for a person (lib/ap/cutover_review.js).
const cutoverNote = (a, glDate) => (glDate !== String(a.invoiceDate).slice(0, 10)
  ? ` (invoice dated ${String(a.invoiceDate).slice(0, 10)}; reviewed not in converted books; posted at GL cutover)` : '');

// Returns the GL date to post at, or null to hold. A reviewer's decision stands:
// NOT_IN_CONVERTED_BOOKS posts at the cutover date; ALREADY_IN_CONVERTED_BOOKS /
// NEEDS_REVIEW never post; an undecided invoice is parked as PENDING.
async function cutoverPostingDate(a) {
  const { preCutoverHold, markPendingReview } = require('./cutover_review');
  const invoiceDate = String(a.invoiceDate).slice(0, 10);
  const { hold, cutover } = await preCutoverHold(supabase, a.communityId, invoiceDate);
  if (!hold) return invoiceDate;
  const { data: inv, error } = await supabase.from('ap_invoices').select('cutover_review').eq('id', a.invoiceId).maybeSingle();
  if (error) throw error;
  const decision = inv && inv.cutover_review;
  if (decision === 'NOT_IN_CONVERTED_BOOKS') return cutover;
  if (!decision || decision === 'PENDING') await markPendingReview(supabase, a.invoiceId, cutover, invoiceDate);
  return null;
}
...
async function autoIntake({ buffer, filename, intakeMethod, sourceRef, communityId, vendorIdHint, achHintText, communityHint, staffNote, staffSenderEmail, emailSubject }) {
  ...
  // Backfill the vendor's MAILING ADDRESS from the invoice when the bill carries a
  // remit-to and the record has none — so a check never reaches the run addressless
  // and Emma isn't forced to hunt it later. Only fills an EMPTY record (never
  // overwrites), only from a real street+city/zip. When the bill has no address at
  // all (the RABKA case), nothing to fill — the check-run guard catches it instead.
  // (Ed 2026-09-08.)
  try {
    const ra = extracted.remit_address || null;
    if (v.vendor && v.vendor.id && ra && ra.line1 && (ra.city || ra.zip)) {
      const { data: cur } = await supabase.from('vendors')
        .select('remit_address_line1, remit_city, remit_zip, address').eq('id', v.vendor.id).maybeSingle();
      const hasAddr = cur && (cur.remit_address_line1 || cur.remit_city || cur.remit_zip || cur.address);
      if (!hasAddr) {
        await supabase.from('vendors').update({
          remit_address_line1: ra.line1, remit_address_line2: ra.line2 || null,
          remit_city: ra.city || null, remit_state: ra.state || null, remit_zip: ra.zip || null,
        }).eq('id', v.vendor.id);
        console.log(`[ap intake] backfilled remit address for vendor ${v.vendor.id} from invoice remit-to`);
      }
    }
  } catch (e) { console.warn('[ap intake] vendor address backfill skipped:', e.message); }
  ...
  // Read the EMAIL WITH the invoice. A Bedrock staffer who forwards a bill and
  // names the community in the SUBJECT or BODY ("August 21 Canyon Gate", "post
  // invoice for Canyon Gate") is authoritative — Emma should not hold a bill for
  // review when a colleague already said which community it is. Alias-aware, so
  // "Canyon Gate" resolves to "Canyon Gate at Cinco Ranch" (the directive matcher
  // only caught the full legal name and never read the subject). Still BELOW the
  // bill's own bill-to (the PDF wins first), so a scanned multi-bill batch where
  // each bill names its own community is unaffected. (Ed 2026-09-08.)
  if (!cid && /@bedrocktx\.com$/i.test(String(staffSenderEmail || '').trim().toLowerCase())) {
    const envelope = `${emailSubject || ''}\n${staffNote || ''}`.trim();
    if (envelope) {
      try { const sc = await resolveCommunity(envelope); if (sc.community) { cid = sc.community.id; cidSource = 'staff_named'; } } catch (_) {}
    }
  }
  ...
  const commit = await commitInvoice({ extracted, vendorId: v.vendor.id, communityId: cid, sha256, storagePath, intakeMethod, sourceRef, achHintText, staffNote, staffGl, staffGlSplit });
