...
async function autoIntake({ buffer, filename, intakeMethod, sourceRef, communityId, vendorIdHint, achHintText, communityHint }) {
  ...
  if (!cid) {
    // Prefer the community named on the invoice; fall back to the operator's
    // context (the forwarded email's subject/service address) when the PDF is
    // thin — a Fw: with an empty body still says "Canyon Gate Blvd".
    let c = await resolveCommunity(extracted.community_hint);
    if (!c.community && communityHint) c = await resolveCommunity(communityHint);
    cid = c.community ? c.community.id : null;
  }
