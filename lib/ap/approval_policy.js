...
function approvalPath(recurrence, openCredits) {
  ...
  if (credits.length) {
    ...
    return {
      path: 'manager_review',
      reason: `This vendor owes ${credits.length === 1 ? 'a credit' : `${credits.length} credits`} on this community${amt} — ${c.reason}. Confirm the credit is applied before releasing.`,
