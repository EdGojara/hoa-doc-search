...
async function _postReversal(supabase, args, postJE) {
  try {
    const je = await postJE({
      ...
      ap_posting: true,   // never into the converted pre-cutover period
    });
    return je.entry.id;
  } catch (e) {
...
async function _postReversalDirect(supabase, { community_id, posting_date, description, lines, invoiceId }) {
  // The direct fallback bypasses the poster, so it enforces the cutover itself.
  const { getGlCutoverDate, assertNotBeforeCutover } = require('../accounting/cutover');
  assertNotBeforeCutover(posting_date, await getGlCutoverDate(supabase, community_id), 'AP prepaid recognition');
...
async function payInvoiceEarlyAsPrepaid(supabase, {
  ...
  if (invoice.posting_journal_entry_id) {
    ...
    // Voiding an accrual posts its reversal on the accrual's own date; never
    // into the converted pre-cutover period.
    const { getGlCutoverDate, assertNotBeforeCutover } = require('../accounting/cutover');
    assertNotBeforeCutover((acc && acc.posting_date) || invoice_date, await getGlCutoverDate(supabase, community_id), 'AP accrual void');
