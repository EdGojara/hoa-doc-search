  ...
  if (inv.posting_journal_entry_id && inv.coded_gl_account_id) {
    try {
      ...
      await postJournalEntry({
        ...
        // part of the invoice's accrual: same cutover rule as the invoice itself
        posting_date: apInvoicePostingDate(String(inv.invoice_date || new Date().toISOString()).slice(0, 10),
          await getGlCutoverDate(supabase, inv.community_id)),
        ap_posting: true,
