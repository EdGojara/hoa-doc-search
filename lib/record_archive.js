...
async function sealFinalizedRecord(supabase, {
  ...
  try {
    ...
    if (!buf) {
      ...
      if (!blob) { console.warn('[record-archive] source missing:', source_path); return; }
    ...
    if (upErr && !/exists|already/i.test(upErr.message)) { console.warn('[record-archive] upload failed:', upErr.message); return; }
