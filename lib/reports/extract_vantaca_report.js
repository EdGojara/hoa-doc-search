...
CRITICAL DISTINCTION between the two violation-related types — the TITLE of the report can lie, so look at the CONTENT structure:

- vantaca_drv_summary: a MULTI-VIOLATION AGGREGATE summary covering a DATE RANGE (a month, quarter, etc.) across MANY properties. Indicators:
  * Has SECTION HEADINGS labeled by enforcement step ("Certified Letter Notice", "Closed", "First Notice", "Second Notice", "Monthly Fine Assessed", "Pending Hearing", etc.) with a NUMERIC TOTAL next to each.
  * Each section lists categories (Mildew, Fences, Trash Cans, Sod Yard, etc.) with COUNTS — not addresses.
  * Often has a pie chart or distribution chart labeled "Top Distribution by Type" or similar.
  * Title may include "Violation Report - Detail for [date range]" — but the word "Detail" here means detailed BREAKDOWN by category, NOT single-property detail. The date range gives it away.
  * NO single property address or owner name as the focus.

- vantaca_violation_detail: a SINGLE-PROPERTY drilldown for ONE specific violation. Indicators:
  * Has ONE property address (e.g., "4502 Quiet Loch Court") prominently displayed.
  * Has ONE owner of record (e.g., "Tania Ramirez Diego").
  * Has a chronological history of actions taken AGAINST THIS ONE VIOLATION (courtesy notice, 1st notice, fine assessed, etc.).
  * Title may include "Violation" but only ONE case is being reported.
  * NO multi-section enforcement-step aggregation.

If the report contains MULTIPLE enforcement-step section headings each with their own total → vantaca_drv_summary, even if the title says "Detail".
If the report focuses on a SINGLE property/owner with one violation's history → vantaca_violation_detail.
...
Return ONLY a JSON object with these fields. No commentary, no markdown fences.
{
  ...
  "period_label": string | null,   // e.g., "May 2026" or null
  "period_start": "YYYY-MM-DD" | null,
  "period_end": "YYYY-MM-DD" | null,
  "confidence": "high" | "medium" | "low",
  "detection_evidence": string  // 1 sentence on what tipped you off — what STRUCTURAL feature you saw
}`;
...
// layout. The Vantaca source is "Violation Report - Detail for [date range]"
// which structures data as enforcement-step sections (First Notice / Second
// Notice / Certified Letter Notice / Closed) with sub-breakdowns by
// violation category, plus a "Top Distribution by Type" pie chart with
// percentages.
const DRV_SUMMARY_PROMPT = `You are reading a Vantaca "Violation Report - Detail" PDF for an HOA. The output will feed a community newsletter summary.

The report is structured as a SUMMARY column on the left and a TOP DISTRIBUTION BY TYPE pie chart on the right.

The SUMMARY column groups violations by ENFORCEMENT STEP — section headings like:
  - "First Notice" (or "1st Notice" / "Notice 1 Sent")
  - "Second Notice" (or "2nd Notice" / "Notice 2 Sent")
  - "Certified Letter Notice" (or "Certified Letter" / "Final Notice Certified")
  - "Closed" (or "Resolved" / "Compliant")
  - There may be others (e.g., "Hearing", "Fine Assessed") — capture them too if present.

Each section has a TOTAL count to the right of its header, then sub-rows listing each violation CATEGORY with its count for that section.

The TOP DISTRIBUTION BY TYPE pie chart shows top categories with their % of total violations.

Return ONLY a JSON object with these fields. No commentary, no markdown fences.

{
  "community_name": string,                 // e.g., "Lake of Pine Forest Homeowners Association, Inc"
  "period_label": string,                    // human-readable e.g., "May 2026" (derive from date range if needed)
  "period_start": "YYYY-MM-DD" | null,      // start of the report's date range
  "period_end": "YYYY-MM-DD" | null,        // end of the report's date range
  "metrics": {
    "first_notices_issued": number,         // total from First Notice section
    "second_notices_issued": number,        // total from Second Notice section
    "violations_resolved": number,          // total from Closed/Resolved section
    "certified_letters_sent": number,       // total from Certified Letter Notice section
    "total_violations_in_period": number    // grand total across ALL sections (use for percentage denominator if pie chart unavailable)
  },
  "by_step": [                                // every enforcement step section found, with per-category breakdown
    {
      "step": string,                         // section name as it appears, e.g., "First Notice"
      "total": number,
      "categories": [
        { "category": string, "count": number }
      ]
  ...
  "top_categories": [                         // top 6-8 categories sorted by % desc
    { "category": string, "percentage": number, "count": number | null }
  ]
}

Rules:
1. If a metric section is missing from the report (e.g., no "Second Notice" section appears), set that metric to 0 — never null.
2. For top_categories, PREFER the percentages from the "Top Distribution by Type" pie chart if visible — that's the source of truth Vantaca generates. Round to nearest integer.
3. If the pie chart isn't readable, sum the per-category counts across ALL by_step sections and compute percentages from the grand total.
4. period_label format: "Month YYYY" (e.g., "May 2026"). Derive from period_start/period_end if not explicit.
5. The community name often has " Homeowners Association, Inc" or similar suffix — keep it as-is from the report.`;
