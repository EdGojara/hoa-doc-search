  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (sortedActions.length === 0) {
        ...
        // Timeline rows with a left date column + dot rail + content.
        // Layout: date 54-115 (61pt wide for "May 5, 2026"), rail at 124,
        // content from 138 to right margin.
        const dateColX = 54;
        const dateColW = 61;
        const railX = 124;
        const contentX = 138;
        ...
        for (let i = 0; i < sortedActions.length; i++) {
          ...
          try {
            ...
            // Date column — full width for "May 5, 2026" without wrapping.
            doc.fillColor('#6b7280').font('Helvetica-Bold').fontSize(9)
               .text(fmtCentralDate(a.date) || '—', dateColX, y0 + 2, { width: dateColW });

            // Action type + actor + notes (right of rail)
            const label = ACTION_LABELS[actionType] || (a.action_type || 'Action');
            doc.fillColor('#0B1D34').font('Helvetica-Bold').fontSize(10.5)
               .text(label, contentX, y0, { width: contentW });
            if (a.actor) {
              doc.fillColor('#6b7280').font('Helvetica').fontSize(9)
                 .text(`by ${s(a.actor)}`, contentX, doc.y + 1, { width: contentW });
            }
            if (a.notes) {
              doc.fillColor('#374151').font('Helvetica').fontSize(9.5)
                 .text(s(a.notes), contentX, doc.y + 2, { width: contentW, lineGap: 1 });
            }
