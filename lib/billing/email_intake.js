...
async function replyAsTessa({ to, subject, staged, communityName, clarify }) {
  ...
  // Automated outbound is held (Ed 2026-07-15). The billing item is still staged
  // and appears on the activity draft for review — only Tessa's auto-reply waits.
  if (!graphSend.autoSendEnabled()) {
    console.warn(`[billing_intake] auto-reply HELD (automated outbound is OFF) — would have replied to ${to}`);
    return null;
  }
  try { await graphSend.sendAs({ from: graphSend.TESSA_MAILBOX, to, subject: replySubject, html, attachments }); return graphSend.TESSA_MAILBOX; }
