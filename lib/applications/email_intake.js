...
async function intakeApplicationFromEmail({ email, extracted = {}, communityId = null, sendAck = true, allowBodyOnly = false }) {
  ...
  try {
    ...
    if (openApp) {
      ...
      // Body-only follow-up with an application already open for this property:
      // there's nothing to attach, and we don't want a duplicate — treat it as
      // already tracked.
      if (bodyOnly) return { status: 'exists', application_id: openApp.id };
