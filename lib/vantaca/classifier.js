...
const VALID_REPORT_TYPES = [
  'ar_aging', 'gl_export', 'trial_balance', 'ap_ledger', 'bank_reconciliation',
  'check_register', 'owner_statement', 'transaction_history', 'vendor_history', 'budget_actual', 'unknown',
];
...
const FILENAME_PATTERNS = [
  ...
  // GL Export — cash-account ledger detail (existing gl_export.js extractor)
  { re: /\bgl[\s_-]?(?:export|listing|detail)\b/i, report_type: 'gl_export', weight: 0.85 },
  { re: /\bgeneral[\s_-]?ledger\b/i, report_type: 'gl_export', weight: 0.7 },
  // Trial Balance — full GL snapshot at a point in time (new trial_balance.js extractor)
  // Higher weight + checked before generic "general ledger" so a file named
  // "GLTrialBalance.csv" routes to the trial balance extractor.
  { re: /\b(?:gl[\s_-]?)?trial[\s_-]?balance\b/i, report_type: 'trial_balance', weight: 0.95 },
  { re: /\bgltb\b/i, report_type: 'trial_balance', weight: 0.85 },
  // AP / Vendor invoices
  ...
  // Bank rec (Vantaca-generated output, not our workflow's input)
  { re: /\bbank[\s_-]?(?:rec|reconciliation)\b/i, report_type: 'bank_reconciliation', weight: 0.9 },
  // Check register (issued checks list for the bank rec workflow input)
  { re: /\bcheck[\s_-]?register\b/i, report_type: 'check_register', weight: 0.9 },
  { re: /\b(?:disbursements?|check[\s_-]?list)\b/i, report_type: 'check_register', weight: 0.6 },
  // Owner statement
  ...
  { re: /\bstatement(?:.*last|.*recent)?\b/i, report_type: 'owner_statement', weight: 0.45 },
  // Transaction history (Vantaca per-owner ledger — migration unlock)
  { re: /\btransaction[\s_-]?history/i, report_type: 'transaction_history', weight: 0.9 },
  { re: /\btransactionhistoryassoc/i, report_type: 'transaction_history', weight: 0.95 },
  { re: /\bowner[\s_-]?ledger\b/i, report_type: 'transaction_history', weight: 0.8 },
  // Vendor 1099 history
