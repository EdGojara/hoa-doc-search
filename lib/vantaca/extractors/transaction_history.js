...
const HEADER_ALIASES_CSV = {
  ...
  name:          ['name', 'homeowner', 'homeowner name', 'owner', 'owner name'],
  address:       ['address', 'property', 'property address'],
...
async function extractFromCsv(fileBuffer) {
  ...
  for (const req of ['date', 'account', 'description']) {
    if (!(req in colMap)) {
      throw new Error(`CSV is missing required column: ${req}. Detected headers: ${headerRow.join(', ')}`);
    }
  }
  ...
  rows.slice(1).forEach((r, idx) => {
    ...
    if (!date || !account || !description) {
      warnings.push(`Row ${idx + 2} skipped — missing date/account/description`);
      return;
    }
