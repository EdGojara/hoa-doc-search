...
async function extractFromCsv(fileBuffer) {
  ...
  rows.slice(1).forEach((r, idx) => {
    // GROUP-HEADER MODE: check if this row is an owner header.
    // Headers are matched on ANY cell so we tolerate SSRS-merged cells
    // landing the string in unpredictable columns.
    if (formatMode === 'group_header') {
      for (const cell of r) {
        ...
        if (m) {
          ...
          return; // header rows don't emit transactions
        }
      }
    }
