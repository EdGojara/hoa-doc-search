...
function _deriveTypeCsv(chargeCents, paymentCents, description) {
  ...
  if (desc.includes('prior balance') || desc.includes('initial balance') || desc.includes('balance brought forward')) {
...
// Owner-header pattern: "<account> - <name> - <address>"
// e.g., "10311380 - Majorie Green - 610 Shady Dale Drive"
// Hyphen can be ASCII or unicode dashes. Whitespace tolerant on either side.
const OWNER_HEADER_RE = /^\s*(\d{4,12})\s*[-–—]\s*(.+?)\s*[-–—]\s*(.+?)\s*$/;
...
async function extractFromCsv(fileBuffer) {
  ...
  // Date + description columns are required regardless of layout.
  for (const req of ['date', 'description']) {
    if (!(req in colMap)) {
  ...
  // ====================================================================
  // FORMAT DETECTION — two layouts in the wild:
  //   A. PER-ROW ACCOUNT: each transaction row has the account in its
  //      own column. Older simpler Vantaca exports.
  //   B. GROUP-HEADER: report is grouped by owner. Each group starts
  //      with a header row containing "<account> - <name> - <address>"
  //      in one of the generic textBox columns. Transaction rows that
  //      follow have NO account column; they inherit from the most
  //      recent group header. This is what Vantaca's SSRS
  //      TransactionHistoryAssoc export uses (Ed 2026-06-08).
  //
  // Detect group-header format by scanning all rows for the owner
  // header pattern. If ANY rows match, use group-header mode.
  // ====================================================================
  let formatMode = 'per_row';
  for (const r of rows.slice(1)) {
    for (const cell of r) {
      if (typeof cell !== 'string') continue;
      if (OWNER_HEADER_RE.test(cell)) { formatMode = 'group_header'; break; }
    }
    if (formatMode === 'group_header') break;
  }

  // If still per-row, try to auto-detect the account column from
  // generic textBoxN columns (the original heuristic, kept as fallback).
  if (formatMode === 'per_row' && !('account' in colMap)) {
    const generic = [];
    headerRow.forEach((h, i) => {
    ...
    if (generic.length) {
      ...
      const scoreAccount = (vals) => vals.filter(v => /^\d{4,12}$/.test(String(v || '').trim())).length;
      const scores = generic.map(i => ({ idx: i, score: scoreAccount(sample.map(r => r[i])) }));
      const winner = scores.reduce((best, s) => s.score > best.score ? s : best, { score: 0, idx: -1 });
      if (winner.idx >= 0 && winner.score >= 3) colMap.account = winner.idx;
    }
  }

  if (formatMode === 'per_row' && !('account' in colMap)) {
    throw new Error(
      `CSV is missing required column: account.\n\n` +
      `The parser couldn't find an account-number column OR group-header pattern. Either:\n` +
      `  • Open the CSV in Excel, rename the account column header to "Account Number", save, re-upload, OR\n` +
      `  • Use Vantaca's report export that groups by owner with "<account> - <name> - <address>" header rows.\n\n` +
      `Detected headers: ${headerRow.join(', ')}`
    );
  }
  ...
  // Group-header carry-forward state (only used in group_header mode)
  let groupAccount = null;
  let groupName = null;
  let groupAddress = null;

  rows.slice(1).forEach((r, idx) => {
    // GROUP-HEADER MODE: check if any cell on this row has the owner
    // identifier pattern. Vantaca's CSV export REPEATS the group value
    // on every transaction row (not just on a separate header row), so
    // when we find a match we update the carry-forward state but do
    // NOT return — the same row also carries transaction data in the
    // date/description/amount columns. The empty-row check further down
    // handles the case of a row that has ONLY the header and no data.
    if (formatMode === 'group_header') {
      for (const cell of r) {
        if (typeof cell !== 'string') continue;
        const m = OWNER_HEADER_RE.exec(cell);
        if (m) {
          groupAccount = m[1].trim();
          groupName    = m[2].trim();
          groupAddress = m[3].trim();
          break;
        }
      }
    }
    ...
    // In per-row mode, account comes from a column. In group mode, it
    // comes from the most recent group header.
    const account = (formatMode === 'group_header')
      ? (groupAccount || '')
      : String(r[colMap.account] || '').trim();

    // Silently skip SSRS caption bleed-through rows — row exists but
    // its data columns are empty.
    const rowAllEmpty = !date && !description;
    if (rowAllEmpty) return;

    if (!date || !account || !description) {
      // Only warn if it looks like a real attempted data row (has SOME
      // data) — otherwise stay silent.
      if (date || description) {
    ...
    if (!byAccount.has(account)) {
      byAccount.set(account, {
        // In group_header mode, address+name come from the carry-forward
        // group state; otherwise from the per-row columns (when present).
        property_address: (formatMode === 'group_header')
          ? (groupAddress || '')
          : (('address' in colMap) ? String(r[colMap.address] || '').trim() : ''),
        homeowner_name: (formatMode === 'group_header')
          ? (groupName || '')
          : (('name' in colMap) ? String(r[colMap.name] || '').trim() : ''),
...
async function writeDownstream({ importRow, extracted, community, supabase }) {
  ...
  // Tally totals + compute date range (used for overlap detection below
  // + persisted on the batch row for future overlap checks against THIS one).
  let totalCharges = 0;
  let totalPayments = 0;
  let rowCount = 0;
  let minDate = null;
  ...
  // ====================================================================
  // OVERLAP REPLACEMENT — Ed 2026-06-08 Option B
  // ----------------------------------------------------------------------
  // After the new batch commits, find any OTHER committed batches for
  // the same community whose transaction date range overlaps ours and
  // mark them as reverted ("replaced by"). Two ranges overlap if
  // (newMin <= priorMax) AND (newMax >= priorMin).
  //
  // The new batch is now the source of truth for that date range; the
  // portal's v_homeowner_current_balance view filters status=committed
  // so the replaced batches drop out automatically.
  //
  // Order: insert new batch FIRST (above), then revert overlapping
  // older ones. If the revert step fails for any reason, the new batch
  // is already safely committed — no data loss.
  // ====================================================================
  if (minDate && maxDate) {
    try {
      const { data: priorBatches } = await supabase
        .from('transaction_upload_batches')
        .select('id, period_label, min_transaction_date, max_transaction_date, source_storage_path')
        .eq('community_id', community.id)
        .eq('status', 'committed')
        .neq('id', batch.id)
        .not('min_transaction_date', 'is', null)
        .not('max_transaction_date', 'is', null);
      const overlapping = (priorBatches || []).filter(p =>
        minDate <= p.max_transaction_date && maxDate >= p.min_transaction_date
      );
        ...
        for (const p of overlapping) {
          await supabase
            ...
            .update({
              status: 'reverted',
              reverted_at: now,
              reverted_reason: `Auto-replaced by batch ${batch.id} (newer overlapping import covering ${minDate} to ${maxDate})`,
              replaced_by_batch_id: batch.id,
            })
            .eq('id', p.id);
        }
        console.log(`[transaction_history/writeDownstream] auto-reverted ${overlapping.length} overlapping batch(es) for community ${community.id}`);
      }
    } catch (overlapErr) {
      // Don't fail the upload over the overlap step — log and continue.
      // Worst case: operator sees double-counting on the portal and can
      // manually revert the older batch via the Transactions tab.
      console.warn('[transaction_history/writeDownstream] overlap revert failed:', overlapErr.message);
      (extracted.warnings || []).push(`Auto-overlap detection failed: ${overlapErr.message}. If you see duplicates on the portal, manually revert older batches in the Transactions tab.`);
    }
  }
  ...
  for (const o of extracted.owners || []) {
    for (const t of o.transactions || []) {
      ...
      const txnType = ['charge','payment','credit','adjustment','balance_brought_forward'].includes(t.type)
        ? t.type
        : (t.amount_cents < 0 ? 'payment' : (t.amount_cents > 0 ? 'charge' : 'adjustment'));
      // Categorize ledger line for amenity-access decisions (migration 203).
      // Drives Claire's "can this homeowner use the pool?" answer AND the
      // operator UI breakdown of what's assessment vs fine.
      const { categorizeChargeDescription } = require('../../ar/categorize');
      const chargeCategory = categorizeChargeDescription(t.description, {
        txn_type: txnType,
...
// Excel mime types — XLS (legacy binary) and XLSX (Office Open XML).
// Some Vantaca exports come from SSRS as application/octet-stream when
// downloaded via browser; we also check the extension as a fallback.
function _detectFormat(mime, filename) {
  const m = String(mime || '').toLowerCase();
  const fn = String(filename || '').toLowerCase();
  if (m === 'application/pdf' || /\.pdf$/i.test(fn)) return 'pdf';
  if (/\.xlsx$/i.test(fn) || m === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') return 'xlsx';
  if (/\.xls$/i.test(fn)  || m === 'application/vnd.ms-excel') return 'xls';
  if (m === 'text/csv' || /\.csv$/i.test(fn)) return 'csv';
  return null;
}

// Excel path — convert sheet rows to the same shape the CSV parser
// produces, then run through the same group_header detection logic.
async function extractFromExcel(fileBuffer) {
  let XLSX;
  try {
    XLSX = require('xlsx');
  } catch (e) {
    throw new Error('xlsx package not available — cannot parse Excel. Convert to CSV and re-upload.');
  }
  const wb = XLSX.read(fileBuffer, { type: 'buffer', cellDates: false, cellNF: false });
  const sheetName = wb.SheetNames[0];
  if (!sheetName) throw new Error('Excel file has no sheets.');
  const sheet = wb.Sheets[sheetName];
  // Sheet to array-of-arrays, blank cells = '' (not skipped)
  const rows = XLSX.utils.sheet_to_json(sheet, { header: 1, blankrows: false, defval: '' });
  if (!rows || rows.length < 2) {
    throw new Error(`Excel file has no data rows (parsed ${rows?.length || 0}).`);
  }
  // Convert to CSV-shaped text so we can pipe through the same parser.
  // Properly escape: wrap fields with commas/quotes/newlines in quotes,
  ...
  return await extractFromCsv(buf);
}

async function run({ importRow, fileBuffer, mime, filename, community, supabase }) {
  const fmt = _detectFormat(mime, filename);
  if (!fmt) {
    throw new Error(`Transaction history extractor expects PDF, CSV, XLS, or XLSX; got mime=${mime} filename=${filename || ''}`);
  }
  console.log(`[vantaca/transaction_history] extracting ${filename || '(unnamed)'} (${fmt.toUpperCase()}) for community=${community?.name}`);

  let extracted;
  if (fmt === 'pdf')          extracted = await extractTransactionHistory(fileBuffer);
  else if (fmt === 'csv')     extracted = await extractFromCsv(fileBuffer);
  else                        extracted = await extractFromExcel(fileBuffer); // xls or xlsx
...
module.exports = { run, extractTransactionHistory, extractFromCsv, extractFromExcel, writeDownstream };
