...
function renderCoverageCard(c, opts) {
  // Cap the limits shown. A Hartford "Premier Stretch" property schedule carries
  // 70+ enhancement sub-limits; an RFP needs the headline coverage a broker
  // quotes against, not the full schedule (the winner gets the actual policy).
  // Keep dollar-amount limits first, then any labeled non-dollar terms, cap ~14.
  const allLimits = (c.limits || []).filter((l) => l && (l.label || l.amount));
  const hasDollar = (l) => /\$\s*\d/.test(String(l.amount == null ? '' : l.amount));
  const limits = allLimits.length > 14
    ? [...allLimits.filter(hasDollar), ...allLimits.filter((l) => !hasDollar(l))].slice(0, 14)
    : allLimits;
...
function renderInsuranceRfpHTML(program, opts = {}) {
  const o = {
    includeCarrier: opts.includeCarrier === true,    // default WITHHOLD incumbent carrier (Ed 2026-09-03): an
                                                     // insurance RFP states the COVERAGE SCOPE we need, not who
                                                     // writes it now — naming the incumbent just tells competitors
                                                     // exactly whom to undercut. Same logic as withholding premium.
    ...
    contactName: opts.contactName || 'Bedrock Association Management',
    contactEmail: opts.contactEmail || 'info@bedrocktx.com',
    contactPhone: opts.contactPhone || BRAND.service.phone,
    managerName: opts.managerName || 'Bedrock Association Management',
  };
  ...
  const entRows = [
    ...
    // Year built deliberately NOT surfaced in the profile (Ed 2026-09-03): the
    // extractor sometimes mis-maps a policy effective/issue year into this field,
    // and a coverage-scope RFP doesn't need the association's founding year. A
    // structure's real construction year, when known, rides on the SOV table below
    // where it's declarations-sourced.
  ...
  * { box-sizing: border-box; }
  ...
  <p>${escapeHtml(BRAND.service.name)}, managing agent for <b>${dash(ent.named_insured)}</b>, invites your agency to submit a proposal for the association's insurance program. The specifications below define the coverage scope the association requires; please quote each line to meet or improve upon it, and note any material difference from what is shown.</p>
...
// ----------------------------------------------------------------------------
// Full HOA line set, in the order a broker expects to read a program. Lines not
// listed sort to the end (index 99). Cyber + Workers Compensation were missing
// here (Ed 2026-09-03 Waterview scar) so they rendered last / out of order.
const LINE_ORDER = ['Property', 'General Liability', 'Directors & Officers', 'Umbrella/Excess Liability', 'Crime/Fidelity', 'Cyber', 'Workers Compensation', 'Equipment Breakdown', 'Ordinance or Law', 'Flood', 'Hired/Non-Owned Auto'];
...
// A few line names that mean the same coverage, so they merge into one RFP line
// instead of rendering twice. "Employers Liability" is Part B of the Workers
// Compensation policy — it shows up separately when extracted from an umbrella's
// underlying schedule, but it is not its own line on an RFP.
const LINE_ALIASES = { 'Employers Liability': 'Workers Compensation' };

function normalizeInsuranceProgram(program) {
  const groups = new Map();
  for (const c of program.coverages || []) {
    if (!c || !c.line) continue;
    const raw = String(c.line).trim();
    const key = LINE_ALIASES[raw] || raw;
