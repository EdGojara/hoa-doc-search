...
//   { kind:'staff'|'board', email, name, scope:'all'|Set<communityId> }  or null.
async function resolveBoardViewer(req) {
  // 1) Staff/admin/manager via Supabase JWT → full portfolio.
  try {
    const { getActingUser } = require('../../api/_acting_user');
    const actor = await getActingUser(req);
    if (actor && actor.is_active !== false && STAFF_ROLES.has(actor.role)) {
      return { kind: 'staff', email: actor.email, name: actor.full_name || actor.email, scope: 'all' };
    }
  } catch (_) { /* no JWT — try the portal cookie next */ }

  // 2) Board member via the portal magic-link session.
  try {
    const { resolvePortalUser } = require('../../api/portal');
    const resolved = resolvePortalUser(req) || {};
    const portalUserId = resolved.portalUserId;
    if (portalUserId) {
      const { data: u } = await supabase.from('portal_users')
        .select('id, email, full_name, role, status').eq('id', portalUserId).maybeSingle();
      if (u && u.status !== 'revoked') {
        if (STAFF_ROLES.has(u.role)) return { kind: 'staff', email: u.email, name: u.full_name || u.email, scope: 'all' };
