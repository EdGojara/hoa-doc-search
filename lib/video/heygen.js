...
// The locked face. Claire must look the SAME every time, on every surface, in
// both languages. A presenter whose appearance drifts between videos reads as
// a stock-footage brand, which is the opposite of the point. Set once in env
// and left alone.
function claireAvatarId() { return process.env.CLAIRE_AVATAR_ID || null; }
function claireVoiceId(language = 'en') {
  return (language === 'es' ? process.env.ISABELLA_VOICE_ID : process.env.CLAIRE_VOICE_ID) || null;
}
...
/** The avatars available to this account — used once, to pick and lock Claire. */
 ...
 */
async function renderExplainer({ script, language = 'en', avatarId, voiceId, background = '#0B1D34' }) {
  const avatar_id = avatarId || claireAvatarId();
  const voice_id = voiceId || claireVoiceId(language);
  if (!avatar_id) throw new Error('No Claire avatar configured (CLAIRE_AVATAR_ID unset)');
  if (!voice_id) throw new Error(`No Claire voice configured for "${language}"`);
