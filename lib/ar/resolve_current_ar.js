...
const { currentOwnerBalance, currentOwnerActivity } = require('./current_owner_ledger');
...
async function resolveCurrentAR(supabase, opts = {}) {
  ...
  const [txnRes, snapRes, esRes] = await Promise.allSettled([
    // With a property: the CURRENT OWNER's tenure only (mig 456/457). A prior
    // owner's rows and legacy accounts never count toward this owner.
    // A lot with no Vantaca account (native, trustEd-numbered) keeps today's
    // behavior in this deployment (no ledger balance; snapshot fallback):
    // surfacing native balances to the portal / payments is a separate decision.
    (propertyId && vantacaAccountId)
      ? currentOwnerBalance(supabase, propertyId).then((b) => ({
          data: b ? { balance_cents: b.balance_cents, most_recent_txn_date: b.most_recent_txn_date } : null,
          error: null,
        }))
    ...
    : (vantacaAccountId && communityId)
      ? supabase
          ...
          // The view groups by account + property + contact, so one account can
          // have several rows (e.g. a converted opening balance and a later
          // payment carrying a different contact link). The account's balance is
          // their SUM; .maybeSingle() errored on 2+ rows and silently dropped the
          // balance to "none".
          .then((r) => {
            if (r.error || !r.data || !r.data.length) return { data: null, error: r.error || null };
            return { data: {
              balance_cents: r.data.reduce((sum, x) => sum + Number(x.balance_cents || 0), 0),
              most_recent_txn_date: r.data.map((x) => x.most_recent_txn_date).filter(Boolean).sort().pop() || null,
            }, error: null };
          })
  ...
  if (es?.state === 'in_bankruptcy' && es.bankruptcy_filing_date && propertyId && vantacaAccountId) {
    try {
      // Current owner's tenure only, split at the petition date.
      const filingDate = String(es.bankruptcy_filing_date).slice(0, 10);
      const rows = await currentOwnerActivity(supabase, propertyId, { ascending: true });
      prePetitionBalanceCents  = rows.filter((r) => String(r.transaction_date).slice(0, 10) <= filingDate).reduce((a, r) => a + Number(r.amount_cents || 0), 0);
      postPetitionBalanceCents = rows.filter((r) => String(r.transaction_date).slice(0, 10) > filingDate).reduce((a, r) => a + Number(r.amount_cents || 0), 0);
    } catch (e) {
      console.warn('[resolveCurrentAR] pre/post-petition split failed:', e.message);
    }
  } else if (es?.state === 'in_bankruptcy' && es.bankruptcy_filing_date && vantacaAccountId && communityId) {
    try {
  ...
  if (txn) {
    ...
    // Only an enforcement state is on file: the balance is unknown, not zero.
    balance_cents = null;
    as_of         = null;
    source        = 'none';
  }
