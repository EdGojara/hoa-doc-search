...
// Categories that justify restricting amenity access. DEFAULT is base
// ASSESSMENTS ONLY (Ed 2026-07-13). Late fees + interest are NOT included by
// default: restricting someone current on base assessments who owes only a
// late fee resembles the fines-only wrongful-restriction cases. Whether a
// Declaration folds late fees/interest into "assessment" is document-specific
// and an RMWBH call — to include them for a community, add 'late_fee' and/or
// 'interest' here (ideally per-community). Fines / attorney / admin fees NEVER
// justify a restriction.
const { currentOwnerComposition } = require('./current_owner_ledger');

const ASSESSMENT_DENIAL_CATEGORIES = new Set(['assessment']);
...
async function evaluateAmenityAccess(supabase, opts = {}) {
  ...
  // Step 2 — balance composition lookup. With a property: the CURRENT OWNER's
  // tenure only, so a buyer is never restricted for a seller's or a legacy
  // account's balance (mig 456/457).
  //
  // ENFORCEMENT SCOPE IS UNCHANGED (Ed 2026-09-24): a lot with no Vantaca
  // account (native, trustEd-numbered, e.g. August Meadows) was never evaluated
  // for delinquency and stays "allow" here. Whether native lots should be
  // evaluated is a separate decision; the tenure reader must not create a new
  // enforcement consequence on its own.
  let rows;
  if (propertyId && !vantacaAccountId) {
    return {
      allowed: true,
  ...
  if (propertyId) {
    try {
      rows = await currentOwnerComposition(supabase, propertyId);
    } catch (e) {
      return {
        allowed: true,
        reason: `Couldn't read balance composition (${e.message}). Defaulting to allow.`,
        basis: {},
        threshold_applied_cents: grace,
      };
    }
  } else if (!vantacaAccountId || !communityId) {
  ...
  if (!propertyId) {
    // Account-only lookup (no property given): unchanged account-keyed read.
  ...
  // Assessment-class charges that CAN justify a restriction: base assessments,
  // plus late_fee/interest ONLY if this community opted them in. Everything else
  // — fines, attorney/admin fees, AND uncategorized 'other' charges — never can.
  //
  // The old code computed this as (total − known fee buckets), which silently
  // treated any UNCATEGORIZED charge as an assessment: v_homeowner_balance_
  // composition groups NULL charge_category into 'other', so a null-category
  // certified-letter fee landed in other_cents (never subtracted) and wrongly
  // restricted amenity access. (Ed 2026-07-25, 5506 Hickory Harvest — a $95
  // balance of DRV certified-letter fees, the annual assessment already paid,
  // was restricting the pool.) Sum the assessment class DIRECTLY instead.
  const assessmentClassCents =
    basis.assessment_cents +
    (ASSESSMENT_DENIAL_CATEGORIES.has('late_fee') ? basis.late_fee_cents : 0) +
    (ASSESSMENT_DENIAL_CATEGORIES.has('interest') ? basis.interest_cents : 0);
  // Payments + credits (stored negative) apply to the assessment side FIRST —
  // conservative, because collections/plans target assessments preferentially,
  // and wrongful DENIAL is the litigated risk (not wrongful allow).
  const paymentsCreditsCents = basis.payment_cents + basis.credit_cents;
  const assessmentPastDue = Math.max(0, assessmentClassCents + paymentsCreditsCents);
  // For display: everything in the balance that is NOT restrictable assessment.
  const nonRestrictableCents = totalBalance - assessmentPastDue;
  const assessmentDelinquent = assessmentPastDue > grace;

  if (totalBalance <= grace) {
    return {
      allowed: true,
      reason: 'Balance is within the grace threshold. Amenity access allowed.',
      basis, assessment_past_due_cents: Math.max(0, assessmentPastDue),
      threshold_applied_cents: grace,
    };
  }
  ...
  if (!assessmentDelinquent) {
    return {
      allowed: true,
      reason: `Past-due balance exists but it is not in restrictable assessments — only late fees / interest / fines / attorney or admin fees. Amenity access cannot be restricted on those alone.`,
      basis, assessment_past_due_cents: 0,
      threshold_applied_cents: grace,
    };
  }

  // Assessments past-due + no plan/bankruptcy → restrict
  const dollars = (cents) => '$' + (cents / 100).toFixed(2);
  return {
    allowed: false,
    reason: `Past-due assessments of ${dollars(assessmentPastDue)} with no active payment plan. Amenity access is restricted until the assessments are brought current or a payment plan is set up. (Late fees, interest, fines, and attorney/admin fees of ${dollars(nonRestrictableCents)} do NOT factor in — assessments only.)`,
    basis, assessment_past_due_cents: assessmentPastDue,
    threshold_applied_cents: grace,
  };
}
