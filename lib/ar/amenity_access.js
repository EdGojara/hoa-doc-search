...
async function evaluateAmenityAccess(supabase, opts = {}) {
  ...
  // Everything that does NOT justify a restriction. Late fees + interest sit
  // here unless the community explicitly opts them in (ASSESSMENT_DENIAL_
  // CATEGORIES); fines / attorney / admin never justify a restriction.
  // Subtracting these from the total nets payments/credits (negative in the
  // total) against the assessment side — conservative, so we restrict less.
  const nonRestrictableCents =
    (ASSESSMENT_DENIAL_CATEGORIES.has('late_fee') ? 0 : basis.late_fee_cents) +
    (ASSESSMENT_DENIAL_CATEGORIES.has('interest') ? 0 : basis.interest_cents) +
    basis.fine_cents + basis.attorney_fee_cents + basis.admin_fee_cents;
  const assessmentPastDue = totalBalance - nonRestrictableCents;
