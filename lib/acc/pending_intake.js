...
// Format the portal's structured application_data into the same plain-text shape
// the system engine already reads from `typedDetails` (staff door). This is what
// lets the PORTAL go through the ONE brain: its typed form fields become engine
// input alongside any uploaded files, instead of a second assessment engine.
function formatPortalAppForEngine(applicationData = {}, meta = {}) {
  const d = applicationData || {};
  const line = (label, v) => (v == null || v === '' ? null : `${label}: ${typeof v === 'object' ? JSON.stringify(v) : v}`);
  const rows = [
    line('Project type', d.project_type),
    line('Description', d.project_description),
    line('Materials', d.materials),
    line('Colors', d.colors),
    line('Dimensions', d.dimensions),
    line('Location on property', d.location_on_property),
    line('Setbacks / distance from lot lines', d.setbacks),
    line('Contractor', d.contractor),
    line('Estimated cost', d.estimated_cost != null ? `$${d.estimated_cost}` : null),
    line('Expected start', d.start_date),
    line('Expected completion', d.completion_date),
    line('Homeowner', meta.submitterName),
    line('Property address', meta.propertyAddress),
  ].filter(Boolean);
  return rows.length ? `HOMEOWNER-SUBMITTED PORTAL APPLICATION (structured form):\n${rows.join('\n')}` : '';
}
...
// typedDetails: optional plain-text application facts (portal form data) fed to
//   the engine alongside files — lets a form-only portal app (no PDF) run.
// communityApplicationId: link back to the portal intake row for status sync-out.
async function createPendingAccDecision({ community, communityId, files = [], submitterEmail = null, submitterName = null, source = 'email', intakeSourceRef = null, propertyAddress = null, reference = null, typedDetails = null, communityApplicationId = null }) {
  if (!isReady()) return { status: 'skipped', reason: 'engine_not_ready' };
  if (!community) return { status: 'skipped', reason: 'no_community' };
  // A portal app can be form-only (no upload). Require files OR typed details.
  if (!files.length && !(typedDetails && typedDetails.trim())) return { status: 'skipped', reason: 'no_input' };
  ...
  // Run the shared engine (isAdmin so we get the workpaper to store). Pass the
  // portal's typed form facts as typedDetails so a form-only app still assesses.
  let out;
  try { out = await runEngine({ community, files, typedDetails: typedDetails || undefined, isAdmin: true }); }
  ...
  // Include the portal link when we have one — but degrade gracefully if
  // migration 333 (community_application_id) isn't applied yet: PostgREST fails
  // the WHOLE insert on an unknown column, which would silently drop the app from
  // the queue. On that specific error, retry WITHOUT the link so the app still
  // reaches staff (unlinked); once the migration is applied it links normally.
  let rec, error;
  {
    const withLink = communityApplicationId ? { ...baseRow, community_application_id: communityApplicationId } : baseRow;
    ({ data: rec, error } = await supabase.from('acc_decisions').insert(withLink).select('id').single());
    if (error && communityApplicationId && /community_application_id/.test(error.message || '')) {
      console.warn('[acc_pending] community_application_id column missing (apply migration 333) — inserting unlinked.');
      ({ data: rec, error } = await supabase.from('acc_decisions').insert(baseRow).select('id').single());
    }
  }
  if (error) {
  ...
  // Back-link the portal intake row to its canonical decision so the portal
  // status page can find the outcome without a scan (best-effort).
  if (communityApplicationId) {
    try { await supabase.from('community_applications').update({ acc_decision_id: id }).eq('id', communityApplicationId); } catch (_) {}
  }
...
module.exports = { createPendingAccDecision, attachDocsToApplication, formatPortalAppForEngine };
