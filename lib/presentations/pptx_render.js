...
// Optional restrained secondary logo (contract: cover/statement/closing `logo`).
// Small, bottom-right, aspect-preserved (contain) — never stretched or recolored.
// Absent, or the file missing on disk, draws nothing. Shared by the three
// builders so the mark sits consistently across slide types.
function addLogo(slide, pres, s) {
  if (!s || !s.logo) return;
  const p = localAssetPath(s.logo);
  if (!fileExists(p)) return;
  addContain(slide, pres, p, 8.45, 4.35, 0.95, 0.68);
}
...
function buildVideo(pres, s) {
  ...
  // Optional labelled step sequence (contract: video.chain). Rendered as a
  // vertical stack of connected cards in the space to the RIGHT of the poster —
  // the previously-empty region, so the video-slide design is unchanged. A
  // screen without `chain` skips this entirely (backwards compatible).
  if (Array.isArray(s.chain) && s.chain.length) {
    const steps = s.chain.map(String);
    const n = steps.length;
    const cx = vx + vw + 0.45;              // just right of the poster
    const cw = (10 - GX) - cx;              // out to the right margin
    const gap = 0.1;
    const cardH = Math.min(0.32, (vh - gap * (n - 1)) / n);
    steps.forEach((label, i) => {
      const y = vy + i * (cardH + gap);
      slide.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: cx, y, w: cw, h: cardH, rectRadius: 0.06, fill: { color: t.card }, line: { color: t.rule, width: 1 } });
      slide.addText([
        { text: String(i + 1), options: { fontSize: 9, bold: true, color: t.muted } },
        { text: '   ' + label, options: { fontSize: 11.5, bold: true, color: t.fg } },
      ], { x: cx + 0.14, y, w: cw - 0.24, h: cardH, valign: 'middle', fontFace: 'Calibri', margin: 0 });
      if (i < n - 1) slide.addText('▼', { x: cx, y: y + cardH - 0.03, w: cw, h: gap + 0.06, fontFace: 'Calibri', fontSize: 7, color: t.muted, align: 'center', margin: 0 });
    });
  }
}
