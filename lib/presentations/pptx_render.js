...
function buildCover(pres, s) {
  ...
  const slide = pres.addSlide();
  slide.background = { color: t.bg };
  if (fileExists(LOGO_PATH)) slide.addImage({ path: LOGO_PATH, x: GX, y: 0.5, w: 1.35, h: 0.4, sizing: { type: 'contain', w: 1.35, h: 0.4 } });
...
function buildStatement(pres, s) {
  ...
  if (s.image) {
    ...
    if (fileExists(p)) slide.addImage({ path: p, x: GX, y: 2.2, w: GW, h: 2.0, sizing: { type: 'contain', w: GW, h: 2.0 } });
...
function buildVideo(pres, s) {
  ...
  if (fileExists(posterPath)) slide.addImage({ path: posterPath, x: vx, y: vy, w: vw, h: vh, sizing: { type: 'cover', w: vw, h: vh } });
