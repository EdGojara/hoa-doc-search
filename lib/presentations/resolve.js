...
async function resolveStory(audience, opts = {}) {
  ...
  const applyCover = (s) => {
    if (s.type !== 'cover' || !cover) return s;
    const pf = s.prepared_for || {};
    const org = cover.org || pf.org || '';
    const attendees = (cover.attendees && cover.attendees.length) ? cover.attendees : (pf.attendees || []);
    const date = cover.date || pf.date || '';
