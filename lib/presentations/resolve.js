...
//
// resolveStory(audience, { language, cover, supabase }) ->
//   { audience, language, screens: [ ...resolved ] }
//
// Resolution steps, in order:
//   1. getStory(audience)          — the audience's ordered screen definitions
//                                    (throws on an unknown audience).
//   2. applyCover(screen)          — a typed "Prepared for" overrides the cover
//                                    screen, matching present.html precedence.
...
// Presentations carry no generic per-string variables; a management proposal is
// a separate business object (lib/proposals) with its own inputs.
// ============================================================================
...
async function resolveStory(audience, opts = {}) {
  ...
  // A typed "Prepared for" (the browser's COVER, in localStorage) overrides the
  // cover screen's seeded prepared_for, with the SAME per-field precedence
  // present.html uses, so the exported cover matches what is on screen.
  const cover = opts.cover || null;
  const applyCover = (s) => {
    if (s.type !== 'cover') return s;
    const pf = s.prepared_for || {};
    // A deck that ships its own board (seeded prepared_for) wins over a passed
    // cover, matching present.html — a stale global cover cannot hide it.
    if (pf.org || (pf.attendees && pf.attendees.length) || pf.date) return s;
    if (!cover) return s;
    const org = cover.org || '';
    const attendees = cover.attendees || [];
    const date = cover.date || '';
    if (!org && !attendees.length && !date) return s;
    return { ...s, prepared_for: { org, attendees, date } };
  };

  // Definitions -> cover-merged -> validated. Presentations carry no generic
  // per-string variables (the only presentation "variable" is the cover context
  // above); a management proposal is a separate object with its own inputs.
  const screens = story.getStory(aud).map((s, i) => {
    const merged = applyCover(s);
    contract.validateScreen(merged, i);
    return merged;
  });
