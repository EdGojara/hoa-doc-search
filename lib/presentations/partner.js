...
function build(config = {}) {
  ...
  // Ed 2026-08-18. The category argument cast for PARTNERS AND INVESTORS.
  // The B2B precedents live HERE and nowhere else: this audience thinks in
  // categories, moats and multiples, so Salesforce and AWS are load-bearing
  // for them. The board deck deliberately uses lived consumer analogies
  // instead, because five volunteer homeowners have no relationship with
  // either company and "we are the Salesforce of HOAs" invites a comparison
  // an eight-community firm loses.
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "03 / The category");
    s.addText("New categories start where an intermediary stops being necessary.", {
      x: 0.6, y: 0.9, w: 8.8, h: 1.0,
      fontFace: "Calibri", fontSize: 28, color: COLORS.NAVY, bold: true, margin: 0,
    });
    s.addText(
      "None of these companies won by building a better version of the incumbent product. Each one made the previous arrangement look like an accident of its era, and the incumbents could not follow without dismantling the revenue they were built on.",
      { x: 0.6, y: 2.0, w: 8.8, h: 0.9, fontFace: "Calibri", fontSize: 14, color: COLORS.SLATE, margin: 0 }
    );
    const priors = [
      { t: "Salesforce", b: "Sold CRM as a subscription when serious software meant a server in your building. On-premise vendors could not match it without cannibalising licence revenue." },
      { t: "AWS", b: "Turned infrastructure into a utility. The question stopped being which servers to buy and became why anyone would buy servers." },
      { t: "Bedrock", b: "Removes the management company as the thing an association waits behind. Incumbents cannot copy it without automating away the staffing model they bill for." },
    ];
    const px = 0.6, py = 3.1, pw = 2.9, ph = 1.9, pgx = 0.05;
    priors.forEach((P, i) => {
      const x = px + i * (pw + pgx);
      const mine = i === priors.length - 1;
      s.addShape(pres.shapes.RECTANGLE, { x, y: py, w: pw, h: ph, fill: { color: mine ? COLORS.NAVY : COLORS.WHITE }, line: { color: mine ? COLORS.NAVY : COLORS.RULE, width: 0.5 } });
      s.addText(P.t, { x: x + 0.16, y: py + 0.18, w: pw - 0.24, h: 0.4, fontFace: "Calibri", fontSize: 14, color: mine ? COLORS.ICE : COLORS.NAVY, bold: true, margin: 0 });
      s.addText(P.b, { x: x + 0.16, y: py + 0.62, w: pw - 0.24, h: 1.2, fontFace: "Calibri", fontSize: 10.5, color: mine ? COLORS.WHITE : COLORS.SLATE, margin: 0 });
    });
    s.addText(
      "The defensibility is the same in all three cases. It is not the technology, it is that the incumbent's own economics prevent them from following.",
      { x: 0.6, y: 5.15, w: 8.8, h: 0.5, fontFace: "Calibri", fontSize: 13.5, color: COLORS.NAVY, bold: true, italic: true, margin: 0 }
    );
    addFooter(s, FOOTER);
  }

  // ---------- Slide 5: There is no substitute (PORSCHE / dark) ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.NAVY_DEEP };
    addSectionLabel(s, "04 / Our philosophy", COLORS.ICE);
    s.addImage({
  ...
  // ---------- Slide 6: Senior judgment encoded ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "05 / What we're building");
    s.addText("Senior judgment, encoded as software.", {
      x: 0.6, y: 0.9, w: 8.8, h: 1.0,
      fontFace: "Calibri", fontSize: 32, color: COLORS.NAVY, bold: true, margin: 0,
    });
    s.addText(
      // No credential names (Ed 2026-08-18) — "acts the way a seasoned CPA and
      // fraud examiner would act" is exactly the false assurance we are removing.
      "trustEd is an operating system for HOA management. It reads the bylaws, the roster, the budget, and the board's voice, and it acts the way seven years of running these communities taught us to act. Not faster. Better. And it gets sharper every month it's in use.",
  ...
  // ---------- Slide 7: Four layer moat ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "06 / Why this can't be copied");
    ...
    const cards = [
      ...
      // Lenses named by the question each asks, not by a professional credential
      // (Ed 2026-08-18) — credential names imply accounting, fraud-examination and
      // legal services that are not part of a management engagement.
      { n: "02", title: "Encoded judgment", body: "Eight lenses on every recommendation, from do the numbers add up to will this hold up in front of the board. Not a generic model with a prompt." },
  ...
  // ---------- Slide: Why this exists (founder arc, dark) ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.NAVY_DEEP };
    addSectionLabel(s, "Why this exists", COLORS.ICE);
    s.addText("This was built out of frustration, not inspiration.", {
      x: 0.6, y: 0.85, w: 8.8, h: 0.8,
      fontFace: "Calibri", fontSize: 30, color: COLORS.WHITE, bold: true, margin: 0,
    });
    s.addText(
      "I did not set out to reinvent the industry. I set out to fix my own company’s problems. Fixing them is how I finally saw what was really wrong, not how I ran things, but the model itself.",
      { x: 0.6, y: 1.68, w: 8.8, h: 0.7, fontFace: "Calibri", fontSize: 14, color: COLORS.ICE, margin: 0 }
    );
    const steps = [
      { n: "FIRST", title: "A better management company", body: "When I got into this, I thought if I just ran it better than everyone else, that would be enough." },
      { n: "THEN", title: "Better software", body: "When I built trustEd, I thought better tools for the same model would fix it. They did not." },
      { n: "NOW", title: "A new category", body: "I stopped trying to build a better gatekeeper. A true AI management company gives homeowners and boards real power and transparency, instead of a middleman to wait behind." },
    ];
    const sx = 0.6, sy = 2.45, sw = 2.85, sgap = 0.2;
    steps.forEach((st, i) => {
      const x = sx + i * (sw + sgap);
      s.addShape(pres.shapes.RECTANGLE, { x, y: sy, w: sw, h: 0.03, fill: { color: COLORS.ICE }, line: { color: COLORS.ICE, width: 0 } });
      s.addText(st.n, { x, y: sy + 0.12, w: sw, h: 0.3, fontFace: "Calibri", fontSize: 11, color: COLORS.ICE, bold: true, charSpacing: 3, margin: 0 });
      s.addText(st.title, { x, y: sy + 0.44, w: sw, h: 0.35, fontFace: "Calibri", fontSize: 14, color: COLORS.WHITE, bold: true, margin: 0 });
      s.addText(st.body, { x, y: sy + 0.84, w: sw, h: 1.45, fontFace: "Calibri", fontSize: 10.5, color: COLORS.SLATE_MUTED, margin: 0 });
    });
    s.addText(
      "The test for everything we build is the same. If it does not make life better for the homeowner or the board, we do not ship it. This was never about monetizing them.",
      { x: 0.6, y: 4.85, w: 8.8, h: 0.45, fontFace: "Calibri", fontSize: 11, color: COLORS.ICE, italic: true, margin: 0 }
    );
    s.addText("Ed Gojara, founder of Bedrock", {
      x: 0.6, y: 5.3, w: 8.8, h: 0.25, fontFace: "Calibri", fontSize: 10.5, color: COLORS.SLATE_MUTED, bold: true, charSpacing: 1, margin: 0,
    });
  }

  // ---------- Slide: The portal (dark) ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.NAVY_DEEP };
    addSectionLabel(s, "The portal", COLORS.ICE);
    s.addText("Homeowners and boards, no longer waiting behind a desk.", {
      x: 0.6, y: 0.85, w: 8.8, h: 0.9,
      fontFace: "Calibri", fontSize: 30, color: COLORS.WHITE, bold: true, margin: 0,
    });
    s.addText(
      "This conviction is where the portal came from. It is where the community gets things done. Homeowners file, pay, and request on their own time. Boards review and decide on theirs. And everyone can see exactly where a request stands.",
      { x: 0.6, y: 1.9, w: 8.8, h: 0.9, fontFace: "Calibri", fontSize: 14, color: COLORS.ICE, margin: 0 }
    );
    const groups = [
      { label: "HOMEOWNERS", items: ["Pay, submit an architectural request, book an amenity, pull a document, in a few taps.", "Track the status of anything they filed, in plain language.", "A voice and an assistant they can actually reach."] },
      { label: "BOARDS", items: ["A live read on their community, not a quarterly binder.", "Approvals, packets, and decisions in one place.", "Every action on the record, so nothing is lost when people change."] },
    ];
    groups.forEach((g, i) => {
      const x = 0.6 + i * 4.65;
      s.addShape(pres.shapes.RECTANGLE, { x, y: 3.15, w: 4.4, h: 0.03, fill: { color: COLORS.ICE }, line: { color: COLORS.ICE, width: 0 } });
      s.addText(g.label, { x, y: 3.27, w: 4.4, h: 0.3, fontFace: "Calibri", fontSize: 11, color: COLORS.ICE, bold: true, charSpacing: 3, margin: 0 });
      s.addText(
        g.items.map((t, k) => ({ text: t, options: { bullet: { indent: 14 }, breakLine: k < g.items.length - 1, paraSpaceAfter: 8 } })),
        { x, y: 3.65, w: 4.4, h: 1.6, fontFace: "Calibri", fontSize: 12.5, color: COLORS.SLATE_MUTED, margin: 0 }
      );
    });
    addFooter(s, FOOTER);
  }

  // ---------- Slide: Meet the team (sourced from the roster) ----------
  // WHO is on the team comes from lib/team/roster.js — the same source the
  // in-platform demo uses — so the export and the demo cannot show a different
  // team. Only the compact role LABELS live here, because the roster's full
  // titles do not fit under a small headshot. Photos in
  // public/assets/presentations/team/<persona>.jpg. (Ed 2026-08-24.)
  {
    const roster = require("../team/roster");
    const SHORT_ROLE = {
      claire: "Front office", isabella: "Front office, Espanol",
      emma: "Accounts payable", kat: "Accounting mgr", annie: "Architectural review",
      miranda: "Compliance", amanda: "Sr community mgr", reese: "Resale, estoppels",
      paige: "Board operations", darby: "Legal, collections", tessa: "Executive assistant",
    };
    const team = roster.people().filter((m) => !m.not_a_person);
    const s = pres.addSlide();
    s.background = { color: COLORS.NAVY_DEEP };
    addSectionLabel(s, "The team", COLORS.ICE);
    s.addText("Meet the team that runs your community.", {
      x: 0.6, y: 0.9, w: 8.8, h: 0.8,
      fontFace: "Calibri", fontSize: 30, color: COLORS.WHITE, bold: true, margin: 0,
    });
    s.addText("One system, many hands, each with a lane and each getting sharper the longer it works your community.", {
      x: 0.6, y: 1.75, w: 8.8, h: 0.5, fontFace: "Calibri", fontSize: 13, color: COLORS.ICE, margin: 0,
    });
    const tw = 0.78, tgap = 0.12, ty = 2.7;
    const totalW = team.length * tw + (team.length - 1) * tgap;
    const tx0 = (10 - totalW) / 2;
    team.forEach((m, i) => {
      const x = tx0 + i * (tw + tgap);
      try { s.addImage({ path: asset("team/" + m.persona + ".jpg"), x, y: ty, w: tw, h: tw }); } catch (_) { /* missing photo: skip image, keep name */ }
      s.addShape(pres.shapes.RECTANGLE, { x, y: ty + tw, w: tw, h: 0.022, fill: { color: "D4AF37" }, line: { color: "D4AF37", width: 0 } });
      s.addText(m.name, { x: x - 0.12, y: ty + tw + 0.09, w: tw + 0.24, h: 0.3, fontFace: "Calibri", fontSize: 8.5, color: COLORS.WHITE, bold: true, align: "center", margin: 0 });
      s.addText(SHORT_ROLE[m.persona] || (m.signature_title || m.title || ""), { x: x - 0.12, y: ty + tw + 0.4, w: tw + 0.24, h: 0.34, fontFace: "Calibri", fontSize: 7, color: COLORS.SLATE_MUTED, align: "center", margin: 0 });
    });
    addFooter(s, FOOTER);
  }

  // ---------- Slide 8: Operators not franchisees ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "07 / Where you fit");
  ...
  // ---------- Slide 9: On the roadmap — access control (light) ----------
  // Keep in step with story.js screen 'future_access'. (Ed 2026-09-04: show
  // where we're going; access control as a future vertical.)
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.OFFWHITE };
    addSectionLabel(s, "08 / On the roadmap");
    s.addText("Access control, run from the system that already knows the community.", {
      x: 0.6, y: 0.9, w: 8.8, h: 1.0,
      fontFace: "Calibri", fontSize: 30, color: COLORS.NAVY, bold: true, margin: 0,
    });
    s.addText(
      "A next vertical turns the clubhouse door, the pool gate, and the front gate into surfaces trustEd controls — issued and revoked by the same rules that already run the community, not a separate system bolted on. Planned, not yet built.",
      {
        x: 0.6, y: 2.05, w: 8.8, h: 1.2,
        fontFace: "Calibri", fontSize: 15, color: COLORS.SLATE, margin: 0, paraSpaceAfter: 6,
      }
    ...
    const acols = [
      { label: "AMENITIES", body: "Clubhouse and pool access issued to residents — suspended the moment an account falls behind, restored when it clears." },
      { label: "BOARDS SEE IT", body: "Who entered which amenity, and when, on the same dashboard as everything else." },
      { label: "GATES", body: "License-plate entry for residents, drawn from the community's own vehicle registry." },
    ];
    acols.forEach((c, i) => {
      const x = 0.6 + i * 3.0;
      s.addShape(pres.shapes.RECTANGLE, {
      ...
      s.addText(c.body, {
        x, y: 4.05, w: 2.8, h: 1.2,
        fontFace: "Calibri", fontSize: 12.5, color: COLORS.SLATE, margin: 0,
      });
    });
    addFooter(s, FOOTER);
  }

  // ---------- Slide 10: Where this goes (dark close) ----------
  {
    const s = pres.addSlide();
    s.background = { color: COLORS.NAVY_DEEP };
    addSectionLabel(s, "09 / Where this goes", COLORS.ICE);
