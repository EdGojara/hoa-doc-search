...
async function sendDueNotices({ today = iso(new Date()), balanceFor, sendEmail, log = console } = {}) {
  let _nq = supabase.from('assessment_autopay')
  ...
  _nq = await require('../demo/demo_guard').excludeDemo(_nq, 'community_id'); // never auto-notice a demo org
  const { data: rows, error } = await _nq;
...
async function chargeDue({ today = iso(new Date()), balanceFor, log = console } = {}) {
  let _cq = supabase.from('assessment_autopay')
  ...
  _cq = await require('../demo/demo_guard').excludeDemo(_cq, 'community_id'); // never auto-charge a demo org
  const { data: rows, error } = await _cq;
