...
const dedupKey = (project) => String(project || '').toLowerCase().replace(/[^a-z0-9]+/g, '').slice(0, 80);
const toCents = (amt) => {
...
// Persist extracted decisions as PENDING proposals for human review. Idempotent:
// a re-scan refreshes a still-pending row in place, but NEVER overwrites one a
// person already confirmed or dismissed — their judgment stands. Best-effort:
// tolerant of the table not existing yet (migration 413 pending).
async function persistProposals(supabase, communityId, decisions) {
  if (!communityId || !decisions || !decisions.length) return { inserted: 0, updated: 0, skipped: 0 };
  let inserted = 0, updated = 0, skipped = 0;
  try {
    const keys = decisions.map((d) => dedupKey(d.project));
    const { data: existing, error } = await supabase.from('project_email_decisions')
      .select('id, dedup_key, review_status').eq('community_id', communityId).in('dedup_key', keys);
    if (error) { console.warn('[project_decisions] persist skipped:', error.message); return { inserted, updated, skipped, error: error.message }; }
    const byKey = new Map((existing || []).map((r) => [r.dedup_key, r]));
    for (const d of decisions) {
      const key = dedupKey(d.project);
      const row = {
        community_id: communityId, project: d.project, status: d.status,
        amount_cents: toCents(d.amount), vendor: d.vendor || null, decided_by: d.decided_by || null,
        decided_on: d.date || null, quote: d.quote || null, source: d.source || null, dedup_key: key,
      };
      const prev = byKey.get(key);
      if (!prev) { const { error: e } = await supabase.from('project_email_decisions').insert(row); if (!e) inserted++; }
      else if (prev.review_status === 'pending') { const { error: e } = await supabase.from('project_email_decisions').update(row).eq('id', prev.id); if (!e) updated++; }
      else skipped++; // confirmed/dismissed — leave the human's call alone
    }
  } catch (e) { console.warn('[project_decisions] persist threw:', e.message); }
  return { inserted, updated, skipped };
}

module.exports = { scanProjectDecisions, gatherStaffThreads, extractDecisions, persistProposals, dedupKey, DECISION_STATUSES };
