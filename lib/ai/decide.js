...
// evidenceReadinessGate(pkg) — runs BEFORE any ACC reasoning. Given a frozen
// EvidencePackage (lib/ai/shadow/acc_evidence.js) it returns a verdict when the
// package is NOT ready to reason on, or null when it is READY (proceed to reason).
//
// Semantics deliberately separate OPERATIONAL HEALTH from SUBSTANTIVE SAFETY
// (Ed/ChatGPT 2026-09-19): a document that could not be extracted is a SYSTEM
// failure -> execution ERROR (never BLOCK; BLOCK stays reserved for a known
// substantive/policy/safety condition where the action must not execute). A
// required document never provided is NEED_INFO (ask for it). A contradiction in
// the evidence (Phase 2) is NEED_INFO (one clarification), not a silent guess.
function evidenceReadinessGate(pkg, opts) {
  if (!pkg || !pkg.readiness) return null; // no package supplied -> gate not applicable
  const o = opts || {};
  switch (pkg.readiness) {
    case 'EXTRACTION_FAILED':
      // business is UNRESOLVED (we could not even establish the facts), execution ERROR.
      // Operational health, not substantive safety -> ERROR, never BLOCK.
      return verdict('UNRESOLVED', EXEC.ERROR, 'EVIDENCE_EXTRACTION_FAILED', NOTE.OPERATIONAL_EXCEPTION);
    case 'INCOMPLETE':
      // A required source was never provided -> ask for it.
      return verdict('NEED_INFO', EXEC.REVIEW, 'EVIDENCE_MISSING', NOTE.DECISION_REQUIRED, { action: 'REQUEST_DOCUMENT' });
    case 'CONFLICT': {
      // Contradictory evidence is NOT an Ed exception by default — Miranda knows
      // exactly what to do: ask the homeowner the minimum question to resolve the
      // fact. It becomes a human exception ONLY if it can't be resolved that way.
      // Execution is EXECUTE when comms are enabled AND autonomy permits (send the
      // clarification); otherwise it holds at REVIEW (shadow). Notification is
      // routine, never DECISION_REQUIRED. (Ed/ChatGPT 2026-09-19.)
      const canAct = !!o.comms_enabled && o.autonomy_state && o.autonomy_state !== 'shadow';
      return verdict('NEED_INFO', canAct ? EXEC.EXECUTE : EXEC.REVIEW, 'EVIDENCE_CONFLICT', NOTE.NONE, { action: 'REQUEST_CLARIFICATION' });
    }
    case 'READY':
    default:
      return null;
...
module.exports = { route, evidenceReadinessGate, EXEC, NOTE };
