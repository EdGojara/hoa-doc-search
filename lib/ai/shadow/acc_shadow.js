...
async function evaluateApplication(opts, deps) {
  ...
  try {
    ...
    if (pkg) {
      ...
      rec.evidence_conflicts = pkg.conflicts || [];
      const gate = evidenceReadinessGate(pkg, { comms_enabled: false, autonomy_state: policy.autonomy_state });
      if (gate) {
    ...
    // deterministic enforcement of objective curing conditions: once the model
    // established a rule applies (produced the item/scope), CODE guarantees the
    // required condition is attached — the model never has to remember it.
    const oc = enforceObjectiveConditions(pStruct, opts.community_name || (pkg && pkg.community));
    rec.objective_conditions_enforced = oc.enforced;
    ...
    if (vcfg) {
      ...
      if (v.ok) {
        ...
        if (vp.ok) {
          ...
          enforceObjectiveConditions(vStruct, opts.community_name || (pkg && pkg.community)); // parity: same deterministic conditions on both sides
