...
async function evaluateApplication(opts, deps) {
  ...
  try {
    ...
    const p = await callModel({ provider: t.provider, model: t.model, price: t, system: acc.ACC_SYSTEM, prompt, maxTokens: 4500, kind: 'shadow_primary' });
    ...
    if (vcfg) {
      const v = await callModel({ provider: vcfg.provider, model: vcfg.model, price: vcfg, system: acc.ACC_SYSTEM + '\nYou are an INDEPENDENT reviewer from a different provider; reach your own decision.', prompt, maxTokens: 4500, kind: 'shadow_verify' });
      if (v.ok) {
