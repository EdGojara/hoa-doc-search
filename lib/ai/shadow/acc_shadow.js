...
async function evaluateApplication(opts, deps) {
  ...
  try {
    ...
    const p = await callModel({ provider: t.provider, model: t.model, price: t, system: 'You are the ACC review function. Return ONLY the JSON contract.', prompt, maxTokens: 4500, kind: 'shadow_primary' });
    ...
    if (vcfg) {
      const v = await callModel({ provider: vcfg.provider, model: vcfg.model, price: vcfg, system: 'You are an independent ACC reviewer from a different provider. Return ONLY the JSON contract.', prompt, maxTokens: 4500, kind: 'shadow_verify' });
      if (v.ok) {
