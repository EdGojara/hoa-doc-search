...
(async () => {
  ...
  assert('happy path -> decision-level agreement true (both DENY)', ok.agreement === true);
  assert('happy path -> item-level structure disagreement recorded', ok.structure_agreement === false);
  ...
  assert('differ from human w/ complete evidence, models agree -> DECISION', rDiff.disagreement_type === 'DECISION');
