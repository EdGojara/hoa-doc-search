...
// A live model tends to SPLIT one real contradiction into several near-duplicate
// "conflicts" with unstable topic keys (observed on masonry: "replacement scope"
// + "treatment requirement" = the same stone-veneer fact). Left as-is that (a)
// yields more than one clarification for one fact and (b) breaks auto-resume
// (answering one leaves a phantom twin OPEN). Collapse conflicts about the same
// underlying fact into ONE canonical conflict, deterministically. Same artifact
// class as the disposition-set / item-name normalization elsewhere.
const _CSTOP = new Set(['the', 'and', 'for', 'with', 'that', 'this', 'will', 'being', 'been', 'are', 'was', 'not', 'yet', 'has', 'have', 'application', 'materials', 'stated', 'described', 'requirement', 'scope', 'treatment', 'information', 'provided', 'requested', 'whether', 'existing', 'other']);
function _sig(c) {
  const txt = [(c.assertions || []).map((a) => (a && a.claim) || '').join(' '), c.topic_label || '', c.topic || ''].join(' ');
  return new Set(String(txt).toLowerCase().replace(/[^a-z0-9 ]/g, ' ').split(/\s+/).filter((w) => w.length > 3 && !_CSTOP.has(w)));
}
function _overlap(a, b) { if (!a.size || !b.size) return 0; let i = 0; a.forEach((x) => { if (b.has(x)) i++; }); return i / Math.min(a.size, b.size); }
function canonicalizeConflicts(conflicts) {
  const kept = [];
  for (const c of conflicts) {
    const sig = _sig(c);
    if (kept.some((k) => _overlap(k._sig, sig) >= 0.4)) continue; // same underlying fact already captured
    kept.push({ ...c, _sig: sig });
  }
  return kept.map((c) => { const { _sig: _drop, ...rest } = c; return rest; });
}
...
async function detectConflicts(pkg, deps) {
  try {
    ...
    // collapse split/near-duplicate conflicts about the same fact BEFORE keying/rendering
    const raw = canonicalizeConflicts(p.value.conflicts || []);
    const conflicts = raw.map((c, i) => ({
...
module.exports = { detectConflicts, withConflicts, applyClarification, renderQuestion, canonicalizeConflicts, CONFLICT_SYSTEM, CONFLICT_SCHEMA };
