...
// 3c) genuine SUBSTANTIVE disposition variation (the masonry pattern): same
// top-line, but an item cleared outright on one run gains a condition on another.
(() => {
  const rec2 = (a, b) => ({
    shadow_status: 'ok', primary_decision: 'APPROVE_WITH_CONDITIONS', verifier_decision: 'APPROVE_WITH_CONDITIONS',
    business_decision: 'APPROVE_WITH_CONDITIONS', reason_code: 'AUTONOMY_STATE', execution: 'REVIEW', severity: null, administrative_status: 'COMPLETE',
    primary_structured: { items: [{ type: 'brick', disposition: a, requirements: [] }, { type: 'stone veneer', disposition: b, requirements: [] }] },
  });
  const runs = [rec2('APPROVE', 'APPROVE'), rec2('APPROVE', 'APPROVE_WITH_CONDITIONS'), rec2('APPROVE', 'APPROVE'), rec2('APPROVE', 'APPROVE_WITH_CONDITIONS'), rec2('APPROVE', 'APPROVE')].map(extractRun);
  const s = summarizeRuns(runs);
  assert('item cleared on some runs, conditioned on others -> SUBSTANTIVE_DISPOSITION_INSTABILITY (must_fix)', s.classification === 'SUBSTANTIVE_DISPOSITION_INSTABILITY' && s.kind === 'must_fix');
  assert('  ...top-line decision still reported stable', s.primary.stable === true);
})();

// 3d) itemization/naming varies but disposition SET is identical -> harmless
// (the oak-tree / pergola pattern: 5 buckets vs 2, all APPROVE_WITH_CONDITIONS).
(() => {
  const recN = (types) => ({
    shadow_status: 'ok', primary_decision: 'APPROVE_WITH_CONDITIONS', verifier_decision: 'APPROVE_WITH_CONDITIONS',
    business_decision: 'APPROVE_WITH_CONDITIONS', reason_code: 'AUTONOMY_STATE', execution: 'REVIEW', severity: null, administrative_status: 'COMPLETE',
    primary_structured: { items: types.map((t) => ({ type: t, disposition: 'APPROVE_WITH_CONDITIONS', requirements: [] })) },
  });
  const runs = [
    recN(['tree trimming', 'resod', 'contractor', 'timeline', 'signature']),
    recN(['tree trimming', 'resod']),
    recN(['tree trimming', 'resod']),
  ].map(extractRun);
  const s = summarizeRuns(runs);
  assert('5 buckets vs 2, all AWC -> GENERATIVE_VARIATION_ONLY (harmless, not disposition instability)', s.classification === 'GENERATIVE_VARIATION_ONLY' && s.kind === 'harmless');
})();
