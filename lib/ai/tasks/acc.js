...
const ACC_SYSTEM = [
  ...
  'Separate ADMINISTRATIVE completeness from SUBSTANTIVE compliance. Administrative items — a missing signature, missing date, owner acknowledgment, contractor license/insurance on file — are recorded in administrative_status and NEVER change the substantive architectural decision, NEVER by themselves cause a denial, and NEVER by themselves cause an escalation. Perform the full substantive review regardless of administrative gaps.',
  'If the application is unsigned or undated, set administrative_status="PENDING_SIGNATURE": you, the manager, contact the homeowner to obtain THEIR signature, record it, and the application auto-completes. You never sign or attest on the homeowner\'s behalf. A missing signature is not grounds to deny or escalate.',
  'Mark any requirement that is purely administrative (signature, date, owner acknowledgment) with "administrative": true so it is excluded from the substantive compliance gate.',
...
const ACC_SCHEMA_HINT = `Return ONLY JSON, no prose, matching exactly:
{
  ...
  "administrative_status": "COMPLETE" | "PENDING_SIGNATURE" | "PENDING_INFO",
  "administrative_deficiencies": string[],
...
administrative_status is SEPARATE from the substantive decision: use PENDING_SIGNATURE
for a missing/unsigned application, PENDING_INFO for a missing administrative form, else
COMPLETE. List administrative gaps in administrative_deficiencies and mark those
requirements "administrative": true. A missing signature NEVER changes decision and
NEVER causes DENY or ESCALATE.`;
...
function severity(struct) {
  for (const item of (struct.items || [])) {
    ...
    for (const r of (item.requirements || [])) {
      if (r.administrative) continue; // administrative gaps are not substantive violations
...
function assess(struct) {
  ...
  for (const item of (struct.items || [])) {
    if (!APPROVES(item.disposition)) continue;
    for (const r of (item.requirements || [])) {
      if (r.administrative) continue; // administrative completeness is tracked separately, not a substantive gate
...
const failCompliance = (item) => (item.requirements || []).some((r) => !r.administrative && r.complies === false);

// administrative_status is normalized + validated so a garbage value never
// silently reads as a real state. It is REPORTED alongside the substantive
// decision; it never alters it.
const ADMIN_STATES = ['COMPLETE', 'PENDING_SIGNATURE', 'PENDING_INFO'];
function administrativeStatus(struct) {
  const s = String((struct && struct.administrative_status) || 'COMPLETE').toUpperCase();
  return ADMIN_STATES.includes(s) ? s : 'COMPLETE';
}
...
module.exports = { ACC_SYSTEM, ACC_SCHEMA_HINT, buildPrompt, parse, applyDeterministicChecks, severity, assess, evidenceOk, agree, administrativeStatus };
