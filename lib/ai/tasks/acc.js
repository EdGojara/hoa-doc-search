...
// The decision contract primary AND verifier must both return. Every requirement
// carries a rule_type and the EVIDENCE that type needs to be evaluable — so the
// model can never claim complies:true while withholding the proof (ChatGPT/Ed:
// don't ask the verifier to compensate for a contract that should have required
    ...
    { "type": string,
      ...
      "requirements": [
        { "rule": string, "source_document": string, "requirement": string,
          ...
          "document_provided": boolean|null,
...
Evidence required by rule_type (compliance cannot be asserted without it):
- NUMERIC (setback/height/size/count): submitted_value_num, threshold_num, operator.
- ENUM (approved material/color/type): submitted_value AND allowed_values.
- BOOLEAN (permitted / not permitted): submitted_value AND complies.
- DOCUMENT (survey/site plan/photo required): document_provided.
- SUBJECTIVE (harmonious/consistent/appropriate): describe it in subjective_judgments;
  do NOT self-approve a subjective standard — it is decided by a human/precedent.
Never set complies for a NUMERIC rule by intuition; provide the numbers.`;
...
// Is a requirement's compliance actually EVALUABLE given its rule_type's evidence?
// A missing-evidence requirement can never count as "complies".
function evidenceOk(r) {
  switch (r.rule_type) {
    ...
    case 'SUBJECTIVE': return false; // never code-evaluable — needs human/precedent
    default: return false; // unknown/absent type is not evaluable
  }
}
...
function severity(struct) {
  for (const item of (struct.items || [])) {
    if (item.disposition !== 'APPROVE') continue;
    if ((item.requirements || []).some((r) => evidenceOk(r) && r.complies === false)) return 'catastrophic';
  }
  if (struct.decision === 'APPROVE' && (struct.items || []).some((it) => (it.requirements || []).some((r) => evidenceOk(r) && r.complies === false))) return 'catastrophic';
  return null;
}

// Fuller assessment for the router: severity (action/error) + whether any APPROVE
// rests on a requirement whose evidence is incomplete (can't be confirmed) — that
// must route to REVIEW/EVIDENCE_INCOMPLETE, never be trusted as compliant.
function assess(struct) {
  const issues = [];
  let evidence_incomplete = false;
  for (const item of (struct.items || [])) {
    for (const r of (item.requirements || [])) {
      const ok = evidenceOk(r);
      if (!ok && (item.disposition === 'APPROVE' || r.complies === true)) {
        evidence_incomplete = true;
        issues.push({ item: item.type, rule: r.rule, rule_type: r.rule_type, why: 'evidence missing for rule_type; compliance cannot be established' });
      }
    }
    // a subjective judgment that the model tried to APPROVE itself is an exception
    if (item.disposition === 'APPROVE' && (item.requirements || []).some((r) => r.rule_type === 'SUBJECTIVE')) {
      issues.push({ item: item.type, why: 'subjective standard cannot be self-approved' });
      evidence_incomplete = true;
    }
  }
...
module.exports = { ACC_SCHEMA_HINT, buildPrompt, parse, applyDeterministicChecks, severity, assess, evidenceOk, agree };
