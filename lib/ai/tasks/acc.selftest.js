...
assert('self-approved SUBJECTIVE standard (no precedent basis) -> evidence_incomplete', acc.assess(subjectiveApprove).evidence_incomplete === true);

// V2: conditional approvals + subjective-via-precedent
const awcCured = { decision: 'APPROVE_WITH_CONDITIONS', items: [{ type: 'shed', disposition: 'APPROVE_WITH_CONDITIONS', requirements: [shedReq(false, { condition: 'Relocate shed to >=5ft from rear property line before installation' })] }], variance_required: false };
const awcUncured = { decision: 'APPROVE_WITH_CONDITIONS', items: [{ type: 'shed', disposition: 'APPROVE_WITH_CONDITIONS', requirements: [shedReq(false)] }], variance_required: false };
assert('APPROVE_WITH_CONDITIONS + curing condition over a violation -> NOT catastrophic', acc.severity(awcCured) === null);
assert('APPROVE_WITH_CONDITIONS WITHOUT a curing condition over a violation -> catastrophic', acc.severity(awcUncured) === 'catastrophic');
const subjPrec = { decision: 'APPROVE_WITH_CONDITIONS', items: [{ type: 'paint', disposition: 'APPROVE_WITH_CONDITIONS', requirements: [{ rule: '8.3', rule_type: 'SUBJECTIVE', requirement: 'consistent with neighborhood', resolved_from: 'precedent', complies: true, condition: 'Match approved palette' }] }], variance_required: false };
assert('subjective resolved from precedent -> NOT evidence_incomplete (judgment allowed)', acc.assess(subjPrec).evidence_incomplete === false);

// agreement: decision-level vs structure-level (item names normalized)
assert('primary DENY vs verifier APPROVE on shed -> decision disagree', acc.agree(correctDeny, dangerousApprove).agree === false);
assert('two matching correct DENYs -> agree', acc.agree(correctDeny, JSON.parse(JSON.stringify(correctDeny))).agree === true);
assert('same decision, differently-named/split items -> decision_agree true, structure_agree false', (() => {
  const a = { decision: 'NEED_INFO', items: [{ type: 'solar panel', disposition: 'NEED_INFO', requirements: [] }, { type: 'battery storage', disposition: 'NEED_INFO', requirements: [] }] };
  const b = { decision: 'NEED_INFO', items: [{ type: 'roof-mounted solar panel and battery', disposition: 'NEED_INFO', requirements: [] }] };
  const r = acc.agree(a, b); return r.decision_agree === true && r.structure_agree === false;
})());
