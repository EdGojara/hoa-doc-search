...
(async () => {
  ...
  const p = await callModel({ provider: t.provider, model: t.model, price: t, system: 'You are the ACC review function. Return ONLY the JSON contract.', prompt, maxTokens: 2600, kind: 'primary' });
  ...
  const v = await callModel({ provider: vcfg.provider, model: vcfg.model, price: vcfg, system: 'You are an independent ACC reviewer from a different provider. Return ONLY the JSON contract.', prompt, maxTokens: 2600, kind: 'verify' });
