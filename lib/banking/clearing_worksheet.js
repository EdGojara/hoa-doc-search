...
const GROUP_DATE_WINDOW_DAYS = 21;   // batched payout/check lag (deposits)
const PAYMENT_WINDOW_DAYS = 45;      // checks float longer — written, then clears weeks later
...
function buildWorksheet({ periodEnd, glItems = [], bankItems = [], overrides = {}, bankEndingCents = 0, glBalanceCents = 0 }) {
  ...
  // Payments (checks) float longer than deposits, so use the wider window for debits.
  for (const bt of bank) {
    if (bt._matched) continue;
    const win = bt.amount_cents < 0 ? PAYMENT_WINDOW_DAYS : MATCH_DATE_WINDOW_DAYS;
    const cands = gl.filter((g) => !g._matched && Math.abs(g.amount_cents - bt.amount_cents) <= TOLERANCE_CENTS && Math.abs(days(g.date, bt.date)) <= win);
    if (cands.length === 1) { const grp = newGroup('high'); bt._matched = true; bt._group = grp; cands[0]._matched = true; cands[0]._group = grp; }
  }
  // Pass 2.5: lockbox date-batch — deposits sharing a deposit DATE were deposited
  // together and clear the bank as ONE credit a few days later. Group remaining
  // GL deposits by date and match each date's total to a single bank deposit on/
  // after that date. This is how Vantaca lockbox batching actually works and is
  // what makes the bulk of the deposits clear. HIGH.
  {
    const byDate = {};
    for (const g of gl) { if (g._matched || !(g.amount_cents > 0)) continue; (byDate[g.date] = byDate[g.date] || []).push(g); }
    for (const date of Object.keys(byDate)) {
      const group = byDate[date].filter((g) => !g._matched);
      if (!group.length) continue;
      const total = group.reduce((a, g) => a + g.amount_cents, 0);
      const bt = bank.find((b) => !b._matched && b.amount_cents > 0
        && Math.abs(b.amount_cents - total) <= TOLERANCE_CENTS
        && days(date, b.date) >= 0 && days(date, b.date) <= GROUP_DATE_WINDOW_DAYS);
      if (bt) { const grp = newGroup('high'); bt._matched = true; bt._group = grp; group.forEach((g) => { g._matched = true; g._group = grp; }); }
    }
  }
  ...
  for (const bt of bank) {
    ...
    const W = bt.amount_cents < 0 ? PAYMENT_WINDOW_DAYS : GROUP_DATE_WINDOW_DAYS;
    const pool = gl.filter((g) => !g._matched && (g.amount_cents >= 0) === sameSign
      && days(g.date, bt.date) >= -W && days(g.date, bt.date) <= W);
...
// Shape the MAIN matcher's output (lib/banking/matcher.js reconcile(), which
// provably ties Quail Ridge to $0) into the worksheet. The matcher's partition —
// matched / deposit_in_transit / outstanding_check / bank_only — IS the
// reconciliation; here we just present it as GL|Bank and apply Accept/Reopen.
// glItems carry ref = id; bankItems are in matcher index order (B0, B1, …).
function worksheetFromMatcher(result, { glItems = [], bankItems = [], overrides = {}, bankEndingCents = 0, glBalanceCents = 0, periodEnd = null }) {
  const glById = {}; glItems.forEach((g) => { glById[g.id] = g; });
  const bankByIdx = {}; bankItems.forEach((b, i) => { bankByIdx['B' + i] = b; });
  const slimGl = (g) => ({ id: g.id, date: g.date, amount_cents: g.amount_cents, description: g.description || '', check_number: g.check_number || null });
  const slimBank = (b) => ({ id: b.id, date: b.date, amount_cents: b.amount_cents, description: b.description || '', check_number: b.check_number || null });
  const isCleared = (key) => overrides[key] && overrides[key].status === 'cleared';
  const isOpen = (key) => overrides[key] && overrides[key].status === 'open';

  let seq = 0;
  const matched = [], openGl = [], openBank = [];
  const pushOpenGl = (item) => openGl.push({ ...item, kind: item.amount_cents >= 0 ? 'deposit_in_transit' : 'outstanding_check' });

  for (const it of (result.items || [])) {
    if (it.category === 'matched') {
      const bank = (it.bank_transaction_idx && bankByIdx[it.bank_transaction_idx]) ? [slimBank(bankByIdx[it.bank_transaction_idx])] : [];
      const gl = String(it.gl_ref || '').split(',').map((x) => x.trim()).filter(Boolean).map((id) => glById[id]).filter(Boolean).map(slimGl);
      if (!bank.length && !gl.length) continue;
      const keys = [...bank.map((x) => 'bank:' + x.id), ...gl.map((x) => 'gl:' + x.id)];
      if (keys.some(isOpen)) {                 // operator reopened the match
        bank.forEach((b) => openBank.push({ ...b, kind: 'unrecorded' }));
        gl.forEach((g) => pushOpenGl(g));
      } else {
        const conf = it.match_confidence || 'medium';
        matched.push({ match_group: 'm' + (++seq), bank, gl, status: 'cleared', confidence: conf, confirmed: conf === 'high' || keys.some(isCleared) });
      }
    } else if (it.category === 'deposit_in_transit' || it.category === 'outstanding_check' || it.category === 'gl_only') {
      const g = it.gl_ref ? glById[it.gl_ref] : null;
      const item = g ? slimGl(g) : { id: it.gl_ref || ('x' + (++seq)), date: it.date_ref, amount_cents: it.amount_cents, description: it.description || '', check_number: it.check_number || null };
      if (isCleared('gl:' + item.id)) matched.push({ match_group: 'm' + (++seq), bank: [], gl: [item], status: 'cleared', confidence: 'high', confirmed: true });
      else pushOpenGl(item);
    } else if (it.category === 'bank_only') {
      const b = bankByIdx[it.bank_transaction_idx]; if (!b) continue;
      const item = slimBank(b);
      if (isCleared('bank:' + item.id)) matched.push({ match_group: 'm' + (++seq), bank: [item], gl: [], status: 'cleared', confidence: 'high', confirmed: true });
      else openBank.push({ ...item, kind: 'unrecorded' });
    }
  }

  const ditCents = openGl.filter((g) => g.kind === 'deposit_in_transit').reduce((a, g) => a + g.amount_cents, 0);
  const ocCents = openGl.filter((g) => g.kind === 'outstanding_check').reduce((a, g) => a + g.amount_cents, 0);
  const reconciledBalance = bankEndingCents + ditCents + ocCents;
  const difference = reconciledBalance - glBalanceCents;
  return {
    period_end: periodEnd,
...
module.exports = { buildWorksheet, worksheetFromMatcher };
