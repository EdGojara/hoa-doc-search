...
function buildWorksheet({ periodEnd, glItems = [], bankItems = [], overrides = {}, bankEndingCents = 0, glBalanceCents = 0 }) {
  ...
  let groupSeq = 0;
  const newGroup = () => 'm' + (++groupSeq);

  // --- PRE-POPULATE -------------------------------------------------------
  // Pass 1: exact check-number match (bank check ↔ GL line[s] with that check#).
  for (const bt of bank) {
    ...
    if (hits.length) { const grp = newGroup(); bt._matched = true; bt._group = grp; hits.forEach((g) => { g._matched = true; g._group = grp; }); }
  }
  // Pass 2: 1:1 same signed amount within the window (unambiguous only).
  for (const bt of bank) {
    if (bt._matched) continue;
    ...
    if (cands.length === 1) { const grp = newGroup(); bt._matched = true; bt._group = grp; cands[0]._matched = true; cands[0]._group = grp; }
  }
  // Pass 3: one bank line ↔ several GL lines summing to it (payout / check / lockbox batch).
  for (const bt of bank) {
    if (bt._matched) continue;
    ...
    if (subset && subset.length) { const grp = newGroup(); bt._matched = true; bt._group = grp; subset.forEach((g) => { g._matched = true; g._group = grp; }); }
  }
  // Pass 4: ambiguous 1:1 — same amount repeats (e.g. many $260 dues / payouts). Take
  // the closest-date GL line of equal amount. The operator confirms via Accept,
  // so a reasonable guess beats leaving every repeated payment open.
  for (const bt of bank) {
    if (bt._matched) continue;
    ...
    const grp = newGroup(); bt._matched = true; bt._group = grp; cands[0]._matched = true; cands[0]._group = grp;
  }
  ...
  const matchedRows = Object.values(groups).map((grp) => {
    ...
    return { ...grp, status: allCleared ? 'cleared' : 'open' };
  });
