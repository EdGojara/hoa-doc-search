...
// Demo status comes from the ONE canonical predicate (is_demo OR the demo tenant),
// not a hardcoded UUID list, so any demo organization (Demo HOA, Demo LMA, and
// future ones) is watermarked automatically. Row-level sync check on the community
// object already in hand.
const { isDemoCommunityRow } = require('../demo/demo_guard');

function installDemoWatermark(doc, opts) {
  const community = (opts && opts.community) || null;
  if (!community) return null;
  if (!isDemoCommunityRow(community)) return null;
