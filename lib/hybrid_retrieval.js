  ...
  const titleMatchPromise = (async () => {
    ...
    try {
      ...
      const chunkResults = await Promise.all(scoredDocs.map(async ({ doc }) => {
        ...
        // documents (e.g. Waterview's Declaration — promoted via migration, not
        // re-ingested through the new path) were invisible to title-match.
        // The Declaration WOULD surface for "Section 3.3" queries but the 3x
        // title boost never fired -- ARC application chunks won on lexical
        // overlap because nothing else got the boost. Ed 2026-06-16.
        ...
        // Dedupe (same chunk could match both paths if both link fields are set).
        const seen = new Set();
