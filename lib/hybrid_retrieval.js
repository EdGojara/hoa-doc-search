...
const CONCEPT_EXPANSIONS = [
  ...
  // A REQUIREMENT question about trees/plants/landscaping ("how many trees am I
  // required to have," "minimum trees front yard," "do I need to replace it") points
  // at the Declaration's landscaping COVENANT, not the maintenance/nuisance clause.
  // The maintenance rule above fires on "yard" but sends retrieval toward weeds and
  // sanitary condition, so the planting requirement never surfaced and Claire punted
  // on Noreen's corner-lot tree count. (Ed 2026-09-07.)
  { on: /\btrees?\b|\bshrubs?\b|\bplant(?:ing|ed|s)?\b|\bsod\b|flower ?bed|caliper|\bhedge/i, add: 'landscaping trees planting shrubs sod front yard corner lot minimum required planted replaced street trees' },
...
async function getRelevantChunks(text, community) {
  ...
  const titleMatchPromise = (async () => {
    ...
    try {
      ...
      // A title naming a specific street address is a PER-PROPERTY RECORD (an
      // approved ACC application, an individual approval/denial letter), not the
      // community's RULE. On a "what does the rule say" question these flood the
      // title-match half — Noreen's tree-count question pulled 55 chunks from
      // "Tree Removal - 20002 Juniper Berry Dr - Approved" and buried the
      // Declaration's landscaping covenant, so Claire punted. Exclude them from
      // title-match; they can still surface via content (vector/keyword) if a
      // specific precedent is genuinely relevant. (Ed 2026-09-07.)
      const ADDRESS_IN_TITLE = /\b\d{2,6}\s+[A-Za-z][A-Za-z0-9 .'-]*?\b(?:dr|drive|ln|lane|ct|court|st|street|ave|avenue|way|blvd|rd|road|cir|circle|trl|trail|pl|place|ter|terrace|cove|cv|bnd|bend|hwy|pkwy|parkway|loop|run|path|walk|row|glen|ridge|creek|crossing|xing)\b/i;
      const eligibleDocs = (docs || []).filter((d) => {
        const cname = String(d.communities?.name || '').toLowerCase();
        if (!communityLower.has(cname)) return false;
        if (ADDRESS_IN_TITLE.test(String(d.title || ''))) return false;
        return true;
      });
  ...
  // A SUPERSEDED / non-current document must never be citable. Nothing enforced
  // this: supersession logic only handled amendments-vs-declaration sections, so
  // a retired FORM still answered as gospel.
  // Scar (Ed 2026-07-15): Waterview's clubhouse agreement in the corpus is the
  // PREVIOUS management company's (CastleCare) — status 'current', never
  // superseded — and the 2025 Bedrock form was never ingested. Claire quoted a
  // homeowner "$75 rental, $300 deposit" in writing; the real form says $150 +
  // $400 + a $70 cleaning fee. Understated by ~$145. This is worse than the
  // hedge it replaced: better retrieval over a stale corpus turns "I'll check"
  // into confidently wrong. No answer beats a wrong answer — drop these chunks.
  try {
    const allLibIds = new Set(libDocIds);
    if (allLibIds.size > 0) {
      const { data: retired } = await supabase
        .from('library_documents')
        .select('id, title, superseded_at, status')
        .in('id', [...allLibIds])
        .or('superseded_at.not.is.null,status.eq.superseded,status.eq.retired,status.eq.archived');
      if (retired && retired.length) {
        // Drop from byKey — the candidate pool. `merged` is built ~140 lines
        // below, so touching it here is a temporal-dead-zone throw that would
        // kill EVERY query (the exact bug the libDocIds comment above records).
        const dead = new Set(retired.map((r) => r.id));
        let dropped = 0;
        for (const [k, e] of byKey.entries()) {
          const lid = e.row?.metadata?.library_document_id || e.row?.migrated_to_library_id || null;
          if (lid && dead.has(lid)) { byKey.delete(k); dropped += 1; }
        }
        if (dropped) console.log(`[hybrid-retrieval] dropped ${dropped} chunk(s) from superseded doc(s): ${retired.map((r) => r.title).join(', ')}`);
      }
    }
  } catch (e) { console.warn('[hybrid-retrieval] superseded-doc filter skipped:', e.message); }
