...
// Amendment supersession factors (migration 228). When a parent doc has been
// amended and the amendment is confirmed-linked, the parent's chunks lose
// 80% of their score so the amendment's text outranks the original.
// Conversely, the amendment's own chunks get a 1.5x boost so they reliably
// surface even when the parent doc is also keyword/title-matched.
...
async function getRelevantChunks(text, community) {
  ...
  // Amendment supersession (migration 228, Ed 2026-06-18). When a chunk's
  // library_document has a confirmed amendment, we want the amendment to
  // outweigh the original. Strategy:
  //   1. Collect all library_document_ids in the candidate pool.
  //   2. Fetch supersession links (which docs amend which) in one query.
  //   3. For each chunk whose parent doc is superseded by a confirmed
  //      amendment, multiply its score by SUPERSEDED_DERANK_FACTOR (severe
  //      penalty, mirrors the 'superseded' status semantic).
  //   4. For each chunk whose own doc IS an amendment (supersedes_library_
  //      document_id IS NOT NULL with supersession_recorded_at set), boost
  //      its score by AMENDMENT_BOOST_FACTOR.
  // Section-level granularity (only deprioritize when the chunk covers an
  // amended section, not the whole parent doc) is a phase 4 refinement.
  // Today's pass is doc-level: any confirmed amendment outranks its parent.
  try {
    ...
    if (allLibIds.size > 0) {
      const { data: superRows } = await supabase
        ...
        .in('id', [...allLibIds]);
      // Map of parent_doc_id -> true (parent is superseded by a CONFIRMED amendment)
      const supersededParents = new Set();
      // Set of amendment_doc_ids (have a confirmed supersession link)
      const confirmedAmendments = new Set();
      for (const r of (superRows || [])) {
        if (r.supersedes_library_document_id && r.supersession_recorded_at) {
          supersededParents.add(r.supersedes_library_document_id);
          confirmedAmendments.add(r.id);
        }
      }
      if (supersededParents.size > 0 || confirmedAmendments.size > 0) {
        let derankedByAmend = 0, boostedAsAmend = 0;
        for (const entry of byKey.values()) {
          const libId = entry.row?.metadata?.library_document_id;
          if (!libId) continue;
          if (supersededParents.has(libId)) {
            entry.score *= SUPERSEDED_BY_AMENDMENT_DERANK;
            entry.sources.add('superseded-by-amendment');
            derankedByAmend += 1;
          } else if (confirmedAmendments.has(libId)) {
            entry.score *= AMENDMENT_BOOST_FACTOR;
            entry.sources.add('is-amendment');
            boostedAsAmend += 1;
          }
        }
        console.log(`[hybrid-retrieval] amendment adjust: deranked ${derankedByAmend} stale chunks, boosted ${boostedAsAmend} amendment chunks`);
      }
    }
  } catch (e) {
