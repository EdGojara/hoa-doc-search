...
// Owners are work-item ASSIGNEES on the Status board — a name routes to that
// person's queue; a shared inbox (info@) routes to a team queue anyone can grab.
// Nothing routes to a specific staffer's desk by default except Ed's own
// buckets (legal/government/collections he handles personally) — everything
// else lands either in its dedicated pipeline (invoices -> Payables/Emma) or on
// the shared info@ queue so the team can see it and pick up who does it. No
// individual staffer (e.g. Martha) is a default routing target. (Ed 2026-08-10.)
const ROUTES = [
  ...
  { match: /invoice|vendor|ap\b/i,                    owner: 'Payables (Emma)',   urgency: 'normal',   item_type: 'invoice' },
  { match: /insurance/i,                              owner: 'info@',             urgency: 'normal',   item_type: 'insurance' },
  { match: /homeowner|owner corresp|resident/i,       owner: 'Community Manager', urgency: 'normal',   item_type: 'owner_correspondence' },
  { match: /junk|marketing|solicit/i,                 owner: 'info@',             urgency: 'low',      item_type: 'other' },
];
...
function defaultRoute(typeStr) {
  ...
  return { owner: 'info@', urgency: 'normal', item_type: 'other' };
}
