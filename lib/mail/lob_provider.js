...
async function createCertifiedLetter({ pdfBuffer, recipient, sender, options = {} }) {
  // Demo isolation boundary FIRST: a demo-org letter is suppressed and recorded,
  // never physically mailed. The caller threads options.communityId.
  const _g = await require('../demo/outbound_guard').guardOutbound({ channel: 'certified_mail', communityId: options.communityId || null, to: (recipient && (recipient.name || recipient.address_line1)) || null, summary: options.description || null });
  if (_g.blocked) return { id: null, tracking_number: null, expected_delivery_date: null, price_cents: null, suppressed: true, reason: _g.reason, is_test_mode: true };
