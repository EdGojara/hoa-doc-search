...
const { gatherNumbers, gatherProjects } = require('./community_numbers');
const { gatherResidentQA } = require('./resident_qa');
const { listKeyEvents } = require('../events/key_events');
let QRCode = null; try { QRCode = require('qrcode'); } catch (_) {}
async function qrDataUrl(text) { if (!QRCode || !text) return null; try { return await QRCode.toDataURL(text, { margin: 1, width: 240 }); } catch (_) { return null; } }
...
async function gatherEvents(supabase, communityId, from, to) {
  ...
  // Amenity rentals are DELIBERATELY excluded: a private clubhouse/pool
  // reservation by one resident is not community news, and naming it exposes a
  // homeowner's plans. The newsletter carries only community-WIDE happenings —
  // meetings and staff/community events. (Ed 2026-09-07.)
  try {
...
async function generateNewsletterDraft({ supabase, communityId, issueMonth, formatKey }) {
  ...
  // Proof-of-work: real metrics + project status from platform data (Ed 2026-09-07).
  const numbers = await gatherNumbers(supabase, communityId, issueMonth);
  const projects = await gatherProjects(supabase, communityId);
  const residentQA = await gatherResidentQA(supabase, communityId);
  // Key events from the prior month, for "This Month in the Community" (mig 412).
  const _pm = new Date(`${issueMonth}-01T12:00:00`); const _pmEnd = _pm.toISOString().slice(0, 10);
  _pm.setMonth(_pm.getMonth() - 1); const _pmStart = _pm.toISOString().slice(0, 10);
  const keyEvents = await listKeyEvents(supabase, communityId, { from: _pmStart, to: _pmEnd });
  if (keyEvents.length) notes.push(`This Month in the Community: ${keyEvents.length} key event(s) captured.`);
  if (numbers.stats.length) notes.push(`By the Numbers (${numbers.label}): ${numbers.stats.map((x) => `${x.value} ${x.label.toLowerCase()}`).join(', ')}.`);
  if (projects.length) notes.push(`Project Watch: ${projects.length} community project(s).`);
...
Return JSON: {
  ...
  sections.push({
    ...
    body_json: { tagline: ai.intro || `Your ${label} community update`, month: label, logo_url: logoUrl, logo_wordmark: !!(community.profile && community.profile.logo_includes_name) },
  ...
  // By the Numbers — real proof-of-work, high on the page. Only when there's data.
  if (numbers.stats.length) {
    sections.push({
      section_type: 'community_numbers', title: `${community.name} By the Numbers`,
      subtitle: numbers.label,
      body_json: { stats: numbers.stats, period: numbers.label },
      ai_generated: false, source_metadata: { source: 'community_numbers', period: numbers.label },
    });
  }

  // Project Watch — what's underway / recently done, from vendor_projects.
  if (projects.length) {
    sections.push({
      section_type: 'project_watch', title: 'Project Watch',
      body_json: { projects },
      ai_generated: false, source_metadata: { source: 'vendor_projects', count: projects.length },
    });
  }

  // This Month in the Community — key events captured from the community's email
  // and platform data (migration 412). Also the source for the annual recap.
  if (keyEvents.length) {
    sections.push({
      section_type: 'key_events', title: 'This Month in the Community',
      body_json: { events: keyEvents.map((e) => ({ date: e.event_date, date_label: fmtDate(e.event_date), title: e.title, summary: e.summary, category: e.category })) },
      ai_generated: false, needs_review: true, source_metadata: { source: 'community_key_events', count: keyEvents.length },
    });
  }
  ...
  // You Asked, We Answered — the month's top general resident questions (Phoebe,
  // from the real inbox; community-level only). (Ed 2026-09-07.)
  if (residentQA.length) {
    sections.push({
      section_type: 'resident_qa', title: 'You Asked, We Answered',
      body_json: { qa: residentQA },
      ai_generated: true, needs_review: true, source_metadata: { source: 'resident_inbox' },
    });
  }
  ...
  if (nextMonthEvents.length) {
    sections.push({
      section_type: 'calendar', title: 'Coming Up',
  ...
  if (contacts.length) {
    ...
    sections.push({
      section_type: 'community_contacts', title: 'Save This Page · Quick Reference',
      subtitle: 'Everything you might need, in one place.',
  ...
  // Important links — resident portal + management, with a scannable portal QR so
  // residents can reach their portal from the printed page. (Ed 2026-09-07.)
  const base = process.env.PUBLIC_BASE_URL || 'https://www.bedrocktx.com';
  const portalUrl = `${base}/portal.html${community.slug ? `?community=${community.slug}` : ''}`;
  const links = [
    { label: 'Resident Portal', url: portalUrl },
    { label: 'Bedrock Association Management', url: 'https://www.bedrocktx.com' },
  ];
  const portalQr = await qrDataUrl(portalUrl);
  sections.push({
    section_type: 'important_links', title: 'Stay Connected',
    body_json: { links, qr: portalQr, qr_label: 'Scan for your Resident Portal', note: 'Questions? Reach us at info@bedrocktx.com or 832-588-2485.' },
