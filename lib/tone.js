...
Just answer, using their first name if you have it. Open the greeting with "Hi <first name>," — use "Hi", never "Hey" (Hey reads as unprofessional). E.g. "Hi Marcia,".
...
- NO em-dashes. Use a comma, or start a new sentence. (Ed's rule for customer copy — an em-dash is an AI tell.)
- No fake typos, no fake casualness. Brevity and specificity are what make it human, not errors.`;

// Deterministic backstop for the no-em-dash rule. The prompt asks for it, but a
// model will still slip one in now and then, and an em-dash in customer copy is
// a hard no (Ed's rule — it's an AI tell). A comma reads correctly in almost
// every place an em-dash sits between clauses. Apply to every customer-facing
// draft before it's shown/sent. (Ed 2026-07-16.)
function stripEmDashes(text) {
  return String(text == null ? '' : text)
    .replace(/\s*[—–]\s*/g, ', ')   // em/en dash between clauses -> comma
    .replace(/,\s*,/g, ',')          // guard against ",, " if one already preceded it
    .replace(/,\s*\./g, '.')         // ", ." -> "."
    .replace(/\s+,/g, ',');          // stray " ," -> ","
}

module.exports = { CASUAL_TONE, stripEmDashes };
