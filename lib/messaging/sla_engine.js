...
async function runSlaTick() {
  ...
  // The SLA sweep never mutates demo-tenant threads. Exclude the demo community
  // ids from every bulk update/scan below (item 7: jobs exclude demo).
  const _demoIds = await require('../demo/demo_guard').demoCommunityIds();
  const notDemo = (q) => (_demoIds.length ? q.not('community_id', 'in', '(' + _demoIds.join(',') + ')') : q);
  ...
  const { data: yellowRows, error: yellowErr } = await notDemo(supabase
  ...
  const { data: redRows, error: redErr } = await notDemo(supabase
    .from('homeowner_threads')
  ...
  const { data: overdueRows, error: overdueErr } = await notDemo(supabase
    .from('homeowner_threads')
  ...
  const { data: stale, error: staleErr } = await notDemo(supabase
    .from('homeowner_threads')
  ...
  // Quiet mode (Ed 2026-06-08): only log when the tick actually did
  // something. Routine no-op ticks (zero threads flipped, zero auto-closed)
  // stay silent. Set SCHEDULER_VERBOSE=true on Render to restore the chatty
  // mode for debugging.
  const didWork = stats.yellow_flipped > 0 || stats.red_flipped > 0
               || stats.overdue_flipped > 0 || stats.auto_closed > 0;
  if (didWork || process.env.SCHEDULER_VERBOSE === 'true') {
