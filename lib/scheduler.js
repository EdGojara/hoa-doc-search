...
function startScheduler({ logger = console } = {}) {
  ...
  scheduler.register({
    ...
    run: async () => {
      ...
      const mailbox = process.env.EMAIL_INGEST_MAILBOX || 'info@bedrocktx.com';
      const sinceISO = new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString();
      return graphIngest.ingestMailbox(mailbox, { sinceISO, light: false, onlyLinked: false, max: 2000 });
    },
  });
