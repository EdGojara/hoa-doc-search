...
function startScheduler({ logger = console } = {}) {
  ...
  if (process.env.OPENAI_API_KEY) {
    ...
    scheduler.register({
      ...
      // Poll mode: fire whenever 30+ minutes have passed since the last run
      // started. No targetHour — uploads at any hour should be searchable
      // within ~45 min (worst case: 15min tick lag + 30min refire window).
      // Upstream is async-upload (api/documents.js marks new docs as
      // index_status='pending'); this job reads the queue via the
      // index_status partial index added in migration 104.
      minIntervalMin: 30,
