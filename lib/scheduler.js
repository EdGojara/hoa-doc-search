...
function startScheduler({ logger = console } = {}) {
  ...
  // Incremental Outlook ingest — every 2 hours, pull recent homeowner mail from
  // the archive journal into the 360 (Ed 2026-07-05: current mail doesn't need
  // to run often). Full AI triage on current mail; keeps only homeowner-linked.
  // 3-hour window overlaps the 2-hour cadence so nothing between runs is missed
  // (idempotent by internet_message_id). No-ops until the Azure app (GRAPH_*)
  // is configured. Enable by adding 'email_ingest_incremental' to
  // SCHEDULER_ENABLED. Backfill (1 year) is a one-time POST /api/email-triage/ingest.
  scheduler.register({
    ...
    run: async () => {
      ...
      const mailbox = process.env.EMAIL_INGEST_MAILBOX || 'archive1emails@bedrocktx.com';
      const sinceISO = new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString();
      return graphIngest.ingestMailbox(mailbox, { sinceISO, light: false, onlyLinked: true, max: 2000 });
    },
  });
