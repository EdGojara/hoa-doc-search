...
const { processCureLapses, processPostcardReminders, detectCategoryAliases } = require('../api/enforcement');
...
class Scheduler {
  ...
  register({ name, targetHour, minIntervalMin, run }) {
    ...
    if (!enabled) {
      // Disabled jobs are NOT scheduled and cost nothing at runtime — no need to
      // log one line each; they're summarized once in start().
      (this._skipped = this._skipped || []).push(name);
  ...
  start() {
    ...
    const on = this.jobs.map((j) => j.name).join(', ') || 'none';
    const off = (this._skipped || []).length;
    this.logger.log(`[scheduler] started — ${this.jobs.length} enabled (${on})${off ? ` · ${off} disabled (enable via SCHEDULER_ENABLED)` : ''}, tick=${TICK_MS / 60000}min`);
  }
...
function startScheduler({ logger = console } = {}) {
  ...
  // Assessment autopay — the Reg E notice at 07:00, the charge at 09:00.
  //
  // Two hours apart on purpose. If the notice job stalls, the charge run finds
  // no valid notice and skips rather than debiting an amount nobody was told
  // about. Enabling autopay_charges without autopay_notices would be a mistake.
  try { require('./payments/autopay_jobs').registerAutopayJobs(scheduler); }
  catch (e) { logger.warn('[scheduler] autopay jobs not registered: ' + e.message); }
  ...
  // Weekly governing-doc health digest — daily fire at 08:00 Central, but the
  // run function itself no-ops unless today is Monday. Same monthly-gate
  // pattern as ar_monthly_reminder below: keeps the scheduler framework
  // simple (no weekly mode needed); the day-of-week gate lives inside the
  // run. Hits POST /api/documents/governing-health/digest internally.
  scheduler.register({
    name: 'governing_health_weekly_digest',
    targetHour: 8,
    run: async () => {
      const now = new Date();
      const cpNow = centralParts(now);
      // Monday gate: get the weekday at central-midnight of the current
      // central date. 1 = Monday.
      const anchor = new Date(centralMidnightUtc(cpNow.year, cpNow.month, cpNow.day));
      if (anchor.getUTCDay() !== 1) {
        return { fired: false, reason: 'not_monday' };
      }
      try {
        const { sendEmail } = require('./notifications/email');
        const { buildDigest } = require('./governing_doc_health_digest');
        const to = process.env.RESEND_DIGEST_TO || process.env.RESEND_FROM_EMAIL || null;
        if (!to) return { fired: false, reason: 'no_recipient' };
        const { totals, html, subject } = await buildDigest({ supabase });
        if (totals.error === 'no_communities') return { fired: false, reason: 'no_communities' };
        const send = await sendEmail({ to, subject, html });
        return { fired: true, sent: send.ok, ...totals };
      } catch (err) {
        return { fired: true, sent: false, error: err.message };
      }
    },
  });

  // Key-events capture — weekly (Monday 07:00 Central). Phoebe reads each
  // community's email and logs the notable community-wide developments to the
  // key-events ledger (community_key_events, mig 412), so the timeline stays
  // current ALL YEAR without anyone clicking "scan". Feeds the newsletter's
  // "This Month in the Community" and the annual year-in-review. The system
  // runs it; staff supervise the result. (Ed 2026-09-07.)
  scheduler.register({
    name: 'key_events_capture',
    targetHour: 7,
    run: async () => {
      const cpNow = centralParts(new Date());
      const anchor = new Date(centralMidnightUtc(cpNow.year, cpNow.month, cpNow.day));
      if (anchor.getUTCDay() !== 1) return { fired: false, reason: 'not_monday' };
      const { captureKeyEvents } = require('./events/key_events');
      const { data: comms, error } = await supabase.from('communities').select('id, name').neq('is_demo', true).limit(200);
      if (error) return { fired: true, error: error.message };
      let added = 0; const detail = [];
      for (const c of comms || []) {
        const r = await captureKeyEvents({ supabase, communityId: c.id }).catch((e) => ({ added: [], error: e.message }));
        const n = (r.added || []).length; added += n; if (n) detail.push(`${c.name}:${n}`);
      }
      return { fired: true, communities: (comms || []).length, added, detail: detail.join(', ') };
    },
  });
  ...
  // Tessa's standing instructions (Ed 2026-08-01): recurring emails Ed set up
  // once ("remind the team monthly to set OOO"). Runs once/day after 8am
  // Central; each task's own schedule (daily/weekly/monthly) is checked inside.
  scheduler.register({
    name: 'tessa_standing',
    targetHour: 8,
    run: () => require('./ea/tessa_standing').runDueStandingTasks({ supabase }),
  });

  // Tessa's own inbox (Ed 2026-08-20). Until now pollTessaInbox ran ONLY when
  // someone opened the Tessa screen and clicked Poll, so forwarding her a
  // message and expecting a draft back was a dead drop — the mail sat in
  // Outlook until Ed happened to look. Ed: "i am going to rely more on tessa
  // from now on," and an assistant you have to go and wake up is not one you
  // can rely on.
  //
  // She stays OUT of the PERSONA_MAILBOXES ingest below on purpose: that path
  // files mail into Communications where staff can read it, and Tessa's
  // correspondence is owner-only. This job reads her mailbox into her own
  // owner-gated store instead, so the privacy boundary is unchanged.
  //
  // Drafts only. Nothing is sent without Ed pressing send.
  scheduler.register({
    name: 'tessa_inbox_poll',
    minIntervalMin: 15,
    run: async () => {
      const { pollTessaInbox, isConfigured } = require('./ea/tessa_inbox');
      if (!isConfigured()) return { skipped: 'graph_not_configured' };
      return pollTessaInbox({ max: 25, mode: 'tessa' });
    },
  });
  ...
  // Incremental Outlook ingest — every 2 hours, pull NEW mail from the
  // homeowner-facing inbox (info@) into the board so Claire drafts replies
  // (Ed 2026-07-06: Claire responds to info@, NOT the archive — the archive is
  // everyone's mail and is history-only). Full AI triage + auto-draft on this
  // live mail; keeps ALL non-spam (onlyLinked:false) so nothing to answer is
  // dropped. 3-hour window overlaps the 2-hour cadence so nothing between runs
  // is missed (idempotent by internet_message_id). No-ops until the Azure app
  // (GRAPH_*) is configured. Enable via SCHEDULER_ENABLED. The one-time 1-year
  // HISTORY backfill from the archive is POST /api/email-triage/ingest (light,
  // onlyLinked, no drafting).
  scheduler.register({
    name: 'email_ingest_incremental',
    minIntervalMin: Number(process.env.EMAIL_INGEST_INTERVAL_MIN || 120),
    run: async () => {
      // Manual by default (Ed reviews everything via the Communications 'Pull
      // inbox' button). Auto-pull is opt-in via EMAIL_INGEST_AUTO=true, so
      // enabling the scheduler for other jobs doesn't silently auto-feed mail.
      if (process.env.EMAIL_INGEST_AUTO !== 'true') return { fired: false, reason: 'manual_only' };
      const graphIngest = require('./email/graph_ingest');
      if (!graphIngest.isConfigured()) return { fired: false, reason: 'graph_not_configured' };
      // Every mailbox a teammate actually works out of, not just info@ and
      // claire@ (Ed 2026-08-18). Vendors email Emma's box directly as well as
      // info@, and her invoice intake only sees what has been ingested — so a
      // bill sent straight to emma@ sat unread until someone pressed "Pull
      // inbox" by hand. Same exposure for every other specialist: mail
      // addressed to the person we told the sender to write to was the mail
      // least likely to be pulled.
      //
      // The default list is the source of truth on purpose. Relying on
      // EMAIL_INGEST_MAILBOX being set correctly on Render means a persona
      // silently stops being served the moment that variable drifts, with no
      // error anywhere.
      //
      // tessa@ is deliberately EXCLUDED. She is Ed's private EA and her
      // correspondence is owner-only; ingesting it here would file it into
      // Communications where staff can read it.
      const PERSONA_MAILBOXES = [
        'info@bedrocktx.com',     // main intake
        'claire@bedrocktx.com',   // front office
        'emma@bedrocktx.com',     // accounts payable — vendor invoices
        'annie@bedrocktx.com',    // architectural review
        'miranda@bedrocktx.com',  // compliance
        'paige@bedrocktx.com',    // board relations
        'kat@bedrocktx.com',      // accounting manager
        'reese@bedrocktx.com',    // resale and estoppels
        'amandaalbright@bedrocktx.com',   // senior community manager
      ];
      const extra = (process.env.EMAIL_INGEST_MAILBOX || '').split(',').map((m) => m.trim()).filter(Boolean);
      const mailboxes = [...new Set([...PERSONA_MAILBOXES, ...extra])];
      const sinceISO = new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString();
      const results = {};
      for (const mailbox of mailboxes) {
        try { results[mailbox] = await graphIngest.ingestMailbox(mailbox, { sinceISO, light: false, onlyLinked: false, max: 2000 }); }
        catch (e) { results[mailbox] = { error: e.message }; }
      }
      return results;
    },
  });
  ...
  // Category-alias auto-detection — fires daily at 07:00 Central. Detects
  // category equivalences (Vantaca "Fences" ≡ trustEd "fence_damage", operator-
  // added labels, etc.) and queues them as ai_suggested for one-click staff
  // confirmation — so duplicate open cases stop slipping through without anyone
  // having to NOTICE the mismatch. Cheap: skips categories that already have a
  // mapping and only calls the model when a NEW unmapped category exists, so
  // most days it's an early no-op (no API spend). Confirmation stays a human
  // step (it gates the §209 escalation math).
  if (process.env.ANTHROPIC_API_KEY) {
    scheduler.register({
      name: 'category_alias_detect',
      targetHour: 7,
      run: () => detectCategoryAliases({}),
    });
  } else {
    logger.warn('[scheduler] ANTHROPIC_API_KEY missing — category_alias_detect job not registered');
  }
