...
function startScheduler({ logger = console } = {}) {
  ...
  scheduler.register({
    ...
    run: async () => {
      ...
      // Construct a midnight-Central anchor and ask which weekday it is.
      // Date.getUTCDay() against the central-midnight UTC instant gives the
      // weekday in Central. 1 = Monday.
      const anchor = new Date(centralMidnightUtc(cpNow.year, cpNow.month, cpNow.day));
      const weekday = anchor.getUTCDay();
      if (weekday !== 1) {
        return { fired: false, reason: 'not_monday', weekday };
      }
      // Inline call to the digest endpoint logic. Easier than spinning an
      // outbound HTTP request to ourselves; same supabase client.
      try {
        ...
        // Fetch + compute (same shape as the API endpoint).
        ...
        const { data: communities } = await supabase
          ...
          .eq('active', true);
        if (!communities || communities.length === 0) return { fired: false, reason: 'no_communities' };
        ...
        const uploadedSince = docs.filter((d) => d.created_at && d.created_at >= sinceIso).length;
        const confirmedSince = docs.filter((d) =>
          d.supersedes_library_document_id && d.supersession_recorded_at && d.supersession_recorded_at >= sinceIso).length;
        ...
        const subject = missingFoundation.length > 0
          ? `[Bedrock Doc Health] ${missingFoundation.length} structural gap${missingFoundation.length === 1 ? '' : 's'} this week`
          : `[Bedrock Doc Health] All ${communities.length} communities clear · ${uploadedSince} new docs · ${confirmedSince} amendments confirmed`;
        const html = `
          <div style="font-family: -apple-system, sans-serif; max-width: 600px;">
            <h2 style="color: #1e3a8a;">⚖️ Weekly Governing-Doc Health Digest</h2>
            <div style="font-size:12px; color:#6b7280;">${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</div>
            <h3>This week</h3>
            <ul><li>${uploadedSince} new doc${uploadedSince === 1 ? '' : 's'} uploaded</li><li>${confirmedSince} amendment${confirmedSince === 1 ? '' : 's'} confirmed</li></ul>
            <h3 style="color:${missingFoundation.length > 0 ? '#dc2626' : '#1e40af'};">Structural exposures</h3>
            ${missingFoundation.length > 0
              ? `<div style="background:#fef2f2; border:2px solid #dc2626; padding:10px; border-radius:6px;"><strong>${missingFoundation.length} communities missing Declaration or Bylaws.</strong><ul>${missingFoundation.map((m) => `<li>${escape(m.name)} — missing: ${escape(m.missing.join(', '))}</li>`).join('')}</ul></div>`
              : `<div style="background:#dcfce7; padding:8px 12px; border-radius:6px; color:#166534;">All ${communities.length} communities have both Declaration and Bylaws ingested. ✓</div>`}
            <div style="margin-top:24px; font-size:11px; color:#9ca3af;">Bedrock Intelligence · Generated automatically by the in-process scheduler · governing_health_weekly_digest job</div>
          </div>`;
        const send = await sendEmail({ to, subject, html });
        return { fired: true, sent: send.ok, uploadedSince, confirmedSince, missingFoundation: missingFoundation.length };
      } catch (err) {
