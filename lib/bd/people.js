...
//   lean — same minus long free-text, for encoding directly INTO a QR code
//          when there's no usable signal in the room. QR capacity is finite
//          and a scan that fails is worse than a scan that saves less.
// -----------------------------------------------------------------------------
...
function buildVCard(p, { lean = false } = {}) {
  const lines = [];
  ...
  if (p.phoneDirect) {
    lines.push(`TEL;TYPE=CELL,VOICE:${telHref(p.phoneDirect)}`);
  }
  if (p.phoneOffice) {
