...
const PERSONA = Object.freeze({
  ...
  stt: {
    ...
    punctuate: true,
    smart_format: true,
    endpointing: 800, // ms of silence to consider an utterance complete. Bumped from 500 → 800 after Ed's 2026-05-23 test: 500 was too aggressive — declared utterances final while the caller was mid-sentence thinking. 800ms gives natural pauses room without making Claire feel slow. Tradeoff: ~300ms more latency before Claire starts responding.
...
function buildOpener(communityName, callerFirstName) {
  ...
  // 2026-05-24 Ed redesign: open with self-introduction + confirm-identity
  // question ("am I speaking with X at Y?"), then handoff to them. The old
  // form jumped straight to "How can I help?" which framed every call as
  // problem-solving from word one. The new form gives the caller space to
  // either confirm/correct who they are and what they're calling about
  // before Claire pivots to help. More natural and surfaces caller-ID
  // mismatches before they cause confusion. Also: the trailing "what
  // brings you in today" is open enough that callers self-disclose name
  // AND reason in one breath without Claire interrupting.
  if (callerFirstName && communityName) {
    return `Hi, this is Claire — an AI team member with ${brand}. Am I speaking with ${callerFirstName} from ${communityName}, and what can I help you with today?`;
  }
  if (callerFirstName) {
    return `Hi, this is Claire — an AI team member with ${brand}. Am I speaking with ${callerFirstName}, and what can I help you with today?`;
  }
  if (communityName) {
    return `Hi, this is Claire — an AI team member with ${brand}. I see you're calling from ${communityName} — who am I speaking with, and what can I help you with today?`;
  }
  return `Hi, this is Claire — an AI team member with ${brand}. Who am I speaking with, and what can I help you with today?`;
}
