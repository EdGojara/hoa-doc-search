...
//   1. Caller matched by phone AND community known   → "Hey Ed — this is Claire Bennett from Bedrock. Calling about Canyon Gate?"
//   2. Caller matched by phone, community unknown    → "Hey Ed — this is Claire Bennett from Bedrock. What can I help with?"
//   3. Community known, caller anonymous             → "Hey, this is Claire Bennett from Bedrock — AI assistant for Canyon Gate. What can I help with?"
//   4. Generic Bedrock greeting                      → "Hey, this is Claire Bennett from Bedrock — an AI assistant for the property management team. What can I help with?"
//
...
function buildOpener(communityName, callerFirstName, warmupHint) {
  ...
  if (callerFirstName && warmupHint) {
    return `${greeting}, ${callerFirstName} — Claire Bennett here from ${brand}. ${warmupHint}`;
  }
  if (callerFirstName && communityName) {
    return `${greeting}, ${callerFirstName} — Claire Bennett here from ${brand}. What's going on at ${communityName} today?`;
  }
  if (callerFirstName) {
    return `${greeting}, ${callerFirstName} — Claire Bennett here from ${brand}. What can I help with today?`;
  }
  if (communityName) {
    return `${greeting}, this is Claire Bennett — an AI team member with ${brand}. I see you're calling from ${communityName} — what's going on today?`;
  }
  return `${greeting}, this is Claire Bennett — an AI team member with ${brand}. What's your name and what can I help with today?`;
}
...
const BANNED_PATTERNS = [
  ...
  // ---- Customer-service AI-tells (added 2026-06-08 per Ed's voice-prompt audit) ----
  // These three were missing from the previous list. Each signals "AI customer
  // service rep" in the first 3 seconds and erodes the trust the rest of the
  // call is supposed to build.
  /^i (?:completely |totally |fully )?understand[.,!?\s]/i,  // bare "I completely understand."
  /^thank you for (?:bringing this|raising this) to (?:our|my) attention/i,
  /^i apologize for (?:the |any )?(?:inconvenience|trouble|confusion)/i,
];
