...
const PERSONA = Object.freeze({
  ...
  stt: {
    ...
    punctuate: true,
    smart_format: true,
    endpointing: 1500, // ms of silence to consider an utterance complete. Bumped 800 → 1500 after Ed's 2026-05-24 test: 800 was still too aggressive — Claire interrupted the caller multiple times mid-sentence ("I need to bring the RV in to..." cut off). 1500ms gives real natural pauses room. Tradeoff: ~700ms more latency before Claire starts responding — acceptable because the prior interrupts were brand-damaging. If this feels too slow, layer in CLAIRE_SEMANTIC_ENDPOINTING=true which adds a Haiku sentence-completeness check on top.
