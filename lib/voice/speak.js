...
async function speakOnce(text, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      // Output format is caller-chosen so ONE voice serves both surfaces.
      // Default stays ulaw_8000 -- what the phone bridge has always
      // received -- so nothing on the telephony path changes. The web
      // visit asks for mp3, because a browser cannot play mu-law.
      path: `/v1/text-to-speech/${voiceId}?output_format=${opts.outputFormat || 'ulaw_8000'}`,
      method: 'POST',
      headers: {
        'xi-api-key': apiKey,
        'Content-Type': 'application/json',
        'Content-Length': Buffer.byteLength(body),
        Accept: (opts.outputFormat || '').startsWith('mp3') ? 'audio/mpeg' : 'audio/pcm',
      },
    }, (res) => {
