...
async function handleClaireLiveWs(ws, req) {
  ...
  const session = new LiveClaireSession({
    ...
    // GPT-Live-1 takes ONE shared audio.format (applies both directions) plus
    // audio.output.voice. There is no session.audio.input block — sending one
    // is rejected as an unknown parameter. Shape verified from a live
    // session.started echo. For Twilio, override format with audio/pcmu @ 8k.
    audio: {
      output: { voice: process.env.CLAIRE_LIVE_VOICE || 'marin' },
  ...
  try {
    ...
    send({ type: 'error', message: 'Could not start Claire Live. ' + (e.message || '') });
