...
const BANNED_PHRASES = [
  ...
  { pattern: /\bworkpaper(s)?\b/gi, why: 'internal CPA-style label', replacement: 'review notes', severity: 'rewrite' },
  ...
  { pattern: /\bprocessing\s+strategy\b/gi, why: 'reveals decision playbook', severity: 'block' },
  { pattern: /\bthe\s+playbook\b/gi, why: 'reveals proprietary playbook', severity: 'block' },
  { pattern: /\btriangulat(e|ed|ion|ing)\b/gi, why: 'reveals multi-lens analytical framework', severity: 'block' },
  { pattern: /\bmulti-?lens\b/gi, why: 'reveals lens-firing framework', severity: 'block' },
  { pattern: /\bencode[- ]ed\b/gi, why: 'reveals encode-Ed product thesis', severity: 'block' },
  { pattern: /\bthe\s+system\s+flag(s|ged|ging)?\b/gi, why: 'reveals automated triage', severity: 'rewrite', replacement: 'we noticed' },
  { pattern: /\boperating\s+model\b/gi, why: 'reveals Bedrock operating-model framing', severity: 'rewrite', replacement: 'our process' },
  ...
  { pattern: /\btrustEd\b/g, why: 'product/platform brand name should not appear in customer-facing text', severity: 'rewrite', replacement: 'Bedrock' },
  { pattern: /\bsmell[- ]test\b/gi, why: 'internal HFT-derived term', severity: 'rewrite', replacement: 'review' },
  ...
  // Block VERBATIM Article/Section/Paragraph references. Allow conversational
  // mentions ("there's a rule about it", "your community standards say...").
  { pattern: /\bArticle\s+[IVXLCDM]+(?:,?\s+Section\s+\d+(?:\.\d+)*)?\b/g, why: 'verbatim CC&R Article reference in customer voice', severity: 'block' },
  { pattern: /\bSection\s+\d+\.\d+(?:\.\d+)*\b/g, why: 'verbatim CC&R Section reference in customer voice', severity: 'block' },
  { pattern: /\bparagraph\s+\d+(?:\.\d+)*\b/gi, why: 'verbatim paragraph reference in customer voice', severity: 'block' },
  ...
  { pattern: /§\s*\d{3}(?:\.\d+)*/g, why: 'statute section symbol in customer voice', severity: 'block' },
...
function screenForLeaks(text, opts = {}) {
  ...
  // For admin audience, only ALWAYS_BLOCKED matters
  const rules = audience === 'admin' ? ALWAYS_BLOCKED.map((p) => ({ pattern: p, why: 'never in any output', severity: 'block' })) : BANNED_PHRASES;
