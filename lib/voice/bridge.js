...
class CallBridge {
  ...
  async start() {
    ...
    // Speak the opener
    await this.speakAndPlay(buildOpener(this.callContext.community?.name));
  ...
  async onUtteranceFinal(text) {
    ...
    if (this.utteranceInProgress) {
      // Already processing a turn; this shouldn't happen but log it
      this.logger.warn(`[bridge ${this.callContext.call_sid}] utterance arrived mid-turn, ignoring: "${text}"`);
      return;
    }
    this.utteranceInProgress = true;
    this.logger.log(`[bridge ${this.callContext.call_sid}] caller: "${text}"`);
  ...
  endCall(reason) {
    ...
    // Async: extract the Stage-1 brief + persist to homeowner_calls.
    // Implemented in lib/voice/call_log.js (TODO — landing in the next
    // commit). For now we just log that we'd extract here.
    this.logger.log(`[bridge ${this.callContext.call_sid}] TODO: extract Stage-1 brief + persist transcript`);
  }
}
