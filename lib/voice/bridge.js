...
// Per-turn latency telemetry (mig 417). Fire-and-forget; never fail a turn on a
// write. Lets us tune against REAL production numbers instead of guesses.
const { createClient: _createPerfClient } = require('@supabase/supabase-js');
const _perfDb = (process.env.SUPABASE_URL && process.env.SUPABASE_KEY)
  ? _createPerfClient(process.env.SUPABASE_URL, process.env.SUPABASE_KEY)
  : null;
...
class CallBridge {
  ...
  async start() {
    ...
    // warmup_opener_hint comes from /api/voice/incoming's pre-call context
    // fetch and is a soft probe like "I see we have an open ARC submission
    // for you — calling about that?" when there's exactly one clear flag.
    // Empty when no flag or ambiguous.
    await this.speakAndPlay(buildOpener(
      this.callContext.community?.name,
      this.callContext.caller?.first_name,
      this.callContext.warmup_opener_hint || null
    ));
  ...
  async onUtteranceFinal(text) {
    ...
    try {
      ...
      // Per-turn latency capture (mig 417). t0 = we commit to answering; measure
      // first spoken sentence (brain) and first audio byte out (brain + TTS).
      this._perfT0 = Date.now();
      this._perfFirstSentenceMs = null;
      this._perfFirstAudioMs = null;
      for await (const sentence of streamTurn({
        ...
        caller_phone: this.callContext.caller_phone || this.callContext.from_phone || null,
        warmup: this.callContext.warmup || null, // pre-call AR/violations/ACC/recent calls (Ed 2026-06-08)
        empathyPromise, // reason.js awaits this in its parallel context block
      })) {
        if (this.closed) break;
        if (this._perfFirstSentenceMs === null) this._perfFirstSentenceMs = Date.now() - this._perfT0;
        assistantFull += sentence + ' ';
        await this.speakAndPlay(sentence);
      }
      this.history.push({ role: 'assistant', content: assistantFull.trim() });
      this._recordTurnPerf(text, voiceModel);
    } catch (err) {
  ...
  // Persist this turn's latency (mig 417). Fire-and-forget; a telemetry write
  // must never affect the call. total_ms is measured at record time (end of the
  // streamTurn loop = full answer generated + streamed).
  _recordTurnPerf(question, model) {
    if (!_perfDb || !this._perfT0) return;
    const row = {
      call_sid: this.callContext.call_sid || null,
      community_id: (this.callContext.community && this.callContext.community.id) || null,
      question: String(question || '').slice(0, 500),
      first_sentence_ms: this._perfFirstSentenceMs,
      first_audio_ms: this._perfFirstAudioMs,
      total_ms: Date.now() - this._perfT0,
      model: model || null,
    };
    this.logger.log(`[bridge ${this.callContext.call_sid}] turn perf`, JSON.stringify({ fs: row.first_sentence_ms, fa: row.first_audio_ms, tot: row.total_ms }));
    _perfDb.from('voice_turn_perf').insert(row).then(({ error }) => {
      if (error) this.logger.warn(`[bridge ${this.callContext.call_sid}] perf write failed: ${error.message}`);
    }).catch(() => {});
  }
  ...
  async speakAndPlay(text) {
    ...
    try {
      ...
      await speakStream(text, (mulawChunk) => {
        if (this.closed) return;
        // First audio byte of the turn — the number that ends the caller's wait.
        if (this._perfT0 && this._perfFirstAudioMs === null) this._perfFirstAudioMs = Date.now() - this._perfT0;
