...
// Map the session language to Deepgram settings. English uses the default
// phonecall model; other languages use nova-2 general with their language code.
const STT_LANG = {
  es: { language: 'es', model: 'nova-2' },
  zh: { language: 'zh-CN', model: 'nova-2' },   // Mandarin (Simplified)
  hi: { language: 'hi', model: 'nova-2' },      // Hindi
};
...
async function handleClaireSttWs(ws, req) {
  ...
  let lang = '';
  try { lang = new URL(req.url, 'http://localhost').searchParams.get('lang') || ''; } catch (_) {}
  const langCfg = STT_LANG[lang] || {};
  ...
  const stt = new DeepgramSession({
    audio: { encoding: 'linear16', sampleRate: 16000, utteranceEndMs: 1000, ...langCfg },
