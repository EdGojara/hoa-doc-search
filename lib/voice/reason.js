      ...
      if (event.type === 'content_block_start') {
        ...
        if (event.delta?.type === 'text_delta' && currentBlock?.type === 'text') {
          if (!_loggedPerf) {
            ...
            console.log(`[voice/reason] perf ms — retrieval:${_perf.retrieval_ms ?? '-'} profile:${_perf.profile_ms ?? '-'} playbook:${_perf.playbook_ms ?? '-'} ctx:${_perf.ctx_ms ?? '-'} ttft:${_perf.ttft_ms} | model:${_perf.model} fast:${_perf.fastlane} acked:${_perf.acked}`);
            _loggedPerf = true;
          }
