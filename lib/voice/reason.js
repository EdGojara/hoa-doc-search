...
const { CUSTOMER_WRITING_STANDARD } = require('../ai/customer_writing_standard');
...
const { isCommonFactQuestion } = require('./fact_fastpath');

// Bridge phrases for the document lane — an immediate, short acknowledgment
// spoken/streamed while retrieval + Sonnet run, so there is no dead air before
// the answer (most valuable on voice). Kept casual and varied. (Ed 2026-09-10.)
// Short, natural "give me a beat" tokens that cover the 2-4s document-retrieval
// gap WITHOUT sounding like a database query. Kept brief and conversational (a
// person's "let's see", not "let me look that up in the system"). Only ever
// played on a genuine question that triggers retrieval (see the bridge gate),
// and never the same one twice in a row (pickBridgePhrase). Ed 2026-09-12: the
// old set was lookup-framed and repeated robotically.
...
let _lastBridgeIdx = -1;
// Accepts a persona's own phrases (e.g. Isabella's Spanish set) so a non-English
// answer never opens with an English "One sec." Falls back to the English set.
function pickBridgePhrase(phrases) {
  const list = (Array.isArray(phrases) && phrases.length) ? phrases : BRIDGE_PHRASES;
  if (list.length <= 1) return list[0] || '';
  let i;
  do { i = Math.floor(Math.random() * list.length); } while (i === _lastBridgeIdx && list.length > 1);
  _lastBridgeIdx = i;
  return list[i];
}

// Only bridge (play a "give me a beat" token) on a GENUINE question that will
// trigger a document lookup — not on statements, fragments, or trailing
// mid-thoughts, which is where the spurious "let me pull that up" came from.
function looksLikeQuestion(text) {
  const t = String(text || '').toLowerCase().trim();
  if (!t) return false;
  if (t.includes('?')) return true;
  return /\b(how|what|when|where|why|who|which|can i|can you|do i|do you|does|is there|are there|should i|could you|need to|how do|how much|how long|what'?s)\b/.test(t);
}
...
// Conversational / non-lookup turns: greetings, smalltalk, thanks, closings,
// short confirmations, and the caller HANDING OVER contact info (email/phone).
// None of these need document retrieval, and none should trigger a "let me look
// that up" bridge — that was the #1 filler complaint: she said it while taking
// an email or saying goodbye (Ed 2026-09-12). Routed like acks (profile-only,
// no retrieval, no bridge) so she answers immediately and naturally.
function isConversationalUtterance(text) {
  if (!text) return false;
  const t = String(text).toLowerCase().trim();
  if (!t) return false;
  // The caller is giving an email address, or a phone/number (mostly digits).
  if (/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/i.test(t)) return true;
  const digits = (t.match(/\d/g) || []).length;
  if (digits >= 7 && digits >= t.replace(/\s/g, '').length * 0.5) return true;
  // Greetings / smalltalk.
  if (/\b(how are you|how are ya|how'?s it going|how'?s your day|how have you been|how you doing|good morning|good afternoon|good evening|nice to meet)\b/.test(t)) return true;
  // Thanks / closings.
  if (/\b(thank you|thanks|appreciate it|that'?s all|that'?s it|that'?s everything|nothing else|we'?re good|i'?m good|all good|take care|have a good|goodbye|good bye|bye now|see you|talk to you)\b/.test(t)) return true;
  // Short confirmations.
  if (/^(yes|yeah|yep|correct|right|exactly|sounds good|perfect|ok|okay|sure|no|nope|that'?s right|go ahead)\b/.test(t) && t.split(/\s+/).length <= 5) return true;
  // Handing over contact info conversationally ("you can send it to...", "it's
  // ..."). Strip a leading filler word first so "OK you can send it to" counts.
  const t2 = t.replace(/^(ok|okay|yeah|yep|so|um|uh|alright|well|and then|and|hey)[,\s]+/i, '').trim();
  if (/^(you can (send|email|text) it to|send it to|it'?s |here'?s )/.test(t2)) return true;
  return false;
}
...
async function* streamTurn(opts) {
  const {
    ...
    empathyPromise,
    // How the person is reaching Claire: 'phone' (voice call, no screen — may be
    // offered a texted link) or 'portal' (on a screen, already in the portal —
    // never offer to text/email a link; point to the on-screen tile). Default
    // 'phone' since the original pipeline is the phone bridge. api/claire.js
    // passes 'portal'.
    channel = 'phone',
  } = opts;
  const passthroughSet = new Set(passthroughToolNames);
  // Model per LANE (Ed 2026-09-10): fast-lane fact answers use Haiku (~1s to
  // first word, and its quality is identical to Sonnet on plain facts);
  // document/compliance answers use Sonnet (crisper and more accurate on the
  // hard, retrieval-grounded ones — e.g. the corner-lot tree count, where Haiku
  // hedged). An explicit `model` (the phone bridge, or CLAIRE_PORTAL_MODEL)
  // overrides the split. Both models are env-tunable. The actual pick happens
  // after the lane is known (below); this is the fallback.
  const FACT_MODEL = process.env.CLAIRE_FACT_MODEL || 'claude-haiku-4-5-20251001';
  const DOC_MODEL = process.env.CLAIRE_DOC_MODEL || 'claude-sonnet-4-6';
  let modelName = model || DOC_MODEL;
  const noBridge = opts.noBridge === true;
  ...
  // --- latency instrumentation (Ed 2026-09-10: Claire "must respond faster to
  // be taken seriously"). Logs the pre-answer cost breakdown + time-to-first-
  // token so we tune from real ms, not estimates. Cheap: a few Date.now() calls
  // and one log line per turn; no behavior change.
  const _t0 = Date.now();
  const _perf = { model: modelName, acked: false, fastlane: false };
  let _loggedPerf = false;
  const isAck = isAcknowledgmentUtterance(utt);
  // Fast lane: a plain operational-fact question (trash day, pool hours,
  // contact, assessment, meeting date) is answerable from the profile block
  // alone — skip the 2-4s governing-doc retrieval. Compliance/doc questions are
  // vetoed inside isCommonFactQuestion, so this never touches the "can I build"
  // / tree-count class. (Ed 2026-09-10.)
  const isFactQ = !isAck && isCommonFactQuestion(utt);
  // Conversational, non-lookup turns (greetings, thanks, closings, confirms,
  // contact-info) route like the fast lane: no doc retrieval, NO bridge phrase.
  const isConvo = !isAck && !isFactQ && isConversationalUtterance(utt);
  // Route the model by lane unless the caller pinned one explicitly. Fast lane
  // (ack + facts + conversational) -> Haiku; document/compliance lane -> Sonnet.
  if (!model) modelName = (isAck || isFactQ || isConvo) ? FACT_MODEL : DOC_MODEL;
  _perf.model = modelName;
  let docContext = '';
  let profileBlock = '';
  let playbookContext = '';
  let empathy = null;
  ...
  } else if (isFactQ || isConvo) {
    _perf.fastlane = true;
    console.log(`[voice/reason] ${isConvo ? 'conversational' : 'fact'} fast-path — profile-only, no bridge, skipping doc retrieval for: "${utt}"`);
    // Only the profile block (cached) + empathy. No getRelevantChunks, no
    // playbook. If the profile is missing this fact, the persona rules offer a
    // human rather than fabricate — the honest fallback, and a signal to fill
    // the community profile in.
    const [_profileBlock, _empathy] = await Promise.all([
      community?.name
        ? buildCommunityContextBlock(community.name)
            .then((r) => { _perf.profile_ms = Date.now() - _t0; return r; })
            .catch((e) => { _perf.profile_ms = Date.now() - _t0; console.warn(`[voice/reason] community profile failed: ${e.message}`); return ''; })
        : Promise.resolve(''),
      empathyPromise || Promise.resolve(null),
    ]);
    _perf.ctx_ms = Date.now() - _t0;
    profileBlock = _profileBlock;
    empathy = _empathy;
  } else {
    // Doc lane: retrieval + Sonnet take a few seconds. Kick the context fetch
    // off FIRST, then yield an immediate bridge phrase so there's no dead air
    // before the answer — the fetch runs while the bridge plays/streams. (Ed
    // 2026-09-10.) Skippable via opts.noBridge.
    const _ctxPromise = Promise.all([
      getRelevantChunks(utt, community?.name || '')
        .then((r) => { _perf.retrieval_ms = Date.now() - _t0; return r; })
        .catch((e) => { _perf.retrieval_ms = Date.now() - _t0; console.warn(`[voice/reason] doc retrieval failed: ${e.message}`); return ''; }),
      community?.name
        ? buildCommunityContextBlock(community.name)
            .then((r) => { _perf.profile_ms = Date.now() - _t0; return r; })
            .catch((e) => { _perf.profile_ms = Date.now() - _t0; console.warn(`[voice/reason] community profile failed: ${e.message}`); return ''; })
      ...
      getRelevantPlaybook(utt, { matchCount: 6 })
        .then((r) => { _perf.playbook_ms = Date.now() - _t0; return r; })
        .catch((e) => { _perf.playbook_ms = Date.now() - _t0; console.warn(`[voice/reason] playbook lookup failed: ${e.message}`); return []; }),
      empathyPromise || Promise.resolve(null),
    ]);
    if (!noBridge && looksLikeQuestion(utt)) { _perf.bridged = true; yield pickBridgePhrase(personaPack && personaPack.bridgePhrases); }
    const [_docContext, _profileBlock, _playbookEntries, _empathy] = await _ctxPromise;
    _perf.ctx_ms = Date.now() - _t0;
...
The principle in one line: find what they are protecting, name it, then help.`;
  ...
  // Channel awareness — does the person have a screen in front of them?
  const channelDirective = channel === 'portal'
    ? `\n\n=== CHANNEL: PORTAL (on a screen) ===
The person is using you on a SCREEN, inside the homeowner portal — not on a phone call. They can see and tap things right now.
- NEVER offer to "text you the link" or "email you the link." They are already here. Offering to send a link reads as if you don't realize where they are.
- For anything that lives in the portal (architectural/ACC requests, payments, documents, account balance), tell them it's right here and name the tile/section to tap ("the architectural request tile on your dashboard").
- A phone number is still fine for an OUTSIDE party (e.g. the pool management company); that's not a portal link.`
    : `\n\n=== CHANNEL: PHONE (voice call, no screen) ===
The person is on a phone call and has no screen in front of them. When something lives in the portal, you MAY offer to text them the portal link so they can get to it. Do not tell them to "look at the tile on your screen" — they can't see one.`;

  const systemBlocks = [
    { type: 'text', text: stable, cache_control: { type: 'ephemeral' } },
    ...(variable ? [{ type: 'text', text: variable }] : []),
    { type: 'text', text: channelDirective },
    ...
    for await (const event of streamResp) {
      ...
      if (event.type === 'content_block_start') {
        ...
        if (event.delta?.type === 'text_delta' && currentBlock?.type === 'text') {
          if (!_loggedPerf) {
            _perf.ttft_ms = Date.now() - _t0;
            console.log(`[voice/reason] perf ms — retrieval:${_perf.retrieval_ms ?? '-'} profile:${_perf.profile_ms ?? '-'} playbook:${_perf.playbook_ms ?? '-'} ctx:${_perf.ctx_ms ?? '-'} ttft:${_perf.ttft_ms} | model:${_perf.model} fast:${_perf.fastlane} bridged:${!!_perf.bridged} acked:${_perf.acked}`);
            _loggedPerf = true;
          }
...
function buildVoiceSystemPrompt(community, caller, docContextOverride, profileBlockOverride, playbookContextOverride) {
  ...
  // A caller can reach Claire two ways. On the PHONE they are matched (weakly) by
  // caller ID, which can be spoofed, so sensitive data needs an address check
  // first. In the PORTAL they are SIGNED IN — a strong identity, and the property
  // is provably theirs — so re-asking their address is exactly the friction a
  // logged-in experience is supposed to remove. `caller.verified` marks the
  // signed-in case. (Ed 2026-09-11: the portal Claire is the keystone experience;
  // she must answer for their own property straight from their login.)
  const verified = !!(caller && caller.verified);
  const callerHeader = verified
    ? "WHO YOU'RE HELPING (SIGNED IN to their portal — identity already verified, this property is THEIRS):"
    : "WHO'S CALLING (matched by phone number):";
  const sensitiveLine = verified
    ? '- Their identity is already verified by their portal login, and the property below is THEIRS. Answer their own account questions (balance, notices, violations, ARC on this property) directly for this property. Do NOT ask them to confirm their address or "which account" to prove who they are — that friction is the whole point of being signed in. The AR/account lookup for THEIR property is authorized by their login.'
...
${sensitiveLine}
- If the info here is wrong (e.g., they say "no I'm not John, I'm John's wife"), trust what they say and adjust.
...
RIGHT:
  "Anything visible from the street needs ARC approval first — front door definitely counts. It's pretty quick if the color's reasonable. The easiest way is to submit it right through your homeowner portal — the architectural request is right there, so it goes straight to the review team. Want me to point you to it?"
  (If they're already signed into the portal: "You can actually do it right here in the portal — look for the architectural or ARC request tile.")

QUESTION: "How much are my assessments?" / "What are the dues?"
The standard assessment AMOUNT is a PUBLIC community fact — it is in the COMMUNITY
PROFILE above (Assessment / dues). Lead with that number straight from the
profile. Do NOT ask for an address first — the amount is the same for everyone,
so there is nothing to look up. Only their personal BALANCE / what THEY owe is
sensitive account data that needs identity verification (see the balance rules
below); "how much are the assessments" is not that.
RIGHT (state the profile figure, then offer the balance as a separate thing):
  "They're [the amount from the profile — e.g. $615 a year]. Want me to pull your specific balance too? I'd just need the address for that one."
WRONG (treats a public figure as sensitive):
  "What's the property address so I can pull up the right account?"
...
HARD RULE #8 — APPLICATIONS ARE SUBMITTED IN THE HOMEOWNER PORTAL. Architectural / ACC / ARC requests (paint, fences, structures, modifications), and similar homeowner applications, are already built into the homeowner portal. When someone asks how to submit one, DIRECT THEM TO THE PORTAL — do NOT offer to email them a blank form or describe a paper process. If they are signed into the portal, tell them they can do it right there (the architectural / ARC request tile). On the phone, offer to text them the portal link. The portal route sends it straight to the review team and is the fastest path for them.

HARD RULE #9 — ACCESS FOBS / GATE REMOTES — two DIFFERENT cases, do not conflate them:
  (a) A fob that ISN'T WORKING (won't open the gate/pool, stopped working, "trouble getting in") — do NOT assume it just needs a replacement and do NOT send them to the portal for a new one. There is usually a REASON it stopped (an account hold, a deactivation, or a gate/system issue) that the team has to look into. Have them CONTACT THE OFFICE to speak with someone — give the office number if you have it, and/or take a message so the team follows up and figures out WHY it isn't working.
  (b) They need a NEW or REPLACEMENT fob (lost, damaged, stolen, or an additional one for the household) — THAT is a request they submit in the homeowner portal (fob / access request).
  In one line: not working → call the office to look into it; need a new one → portal.

HARD RULE #10 — THE OFFICE NUMBER IS ALWAYS THE MAIN BEDROCK LINE: (832) 588-2485. Whenever you give a number to reach "the office," or offer a callback, use THAT number. NEVER hand out a community clubhouse, amenity, pool-company, or other local number as the office line (those exist for their own narrow purpose) — the caller reaches Bedrock on the main line and the team transfers internally from there. If retrieved context shows a clubhouse or amenity phone, do not repurpose it as the office number.
...
PAYMENT TIMING (they mailed it on time but it arrived after the due date):
This comes up a lot, and the honest answer matters. Assessments are credited on
the date the payment is RECEIVED, not the postmark and not the date it was
mailed, unless this community's governing documents explicitly say the postmark
controls (read them, do not assume). So a payment received after the due date is
treated as late even when it was mailed earlier. Say that plainly and kindly.
Do NOT imply the late fee will be waived, and never say it is "worth presenting
to the board." A fee waiver is a decision, not yours to grant or to predict, and
dangling one sets a false expectation. If they want it reviewed or waived, that
is a message for "someone from accounting." You CAN offer the practical fix: pay
through the portal, or mail a few days earlier, so the received-date gap does not
catch them again.

DO NOT INVENT THE NUMBERS. The received-date principle above is safe to state.
But the SPECIFICS — the grace-period length, how many days until a late fee hits,
the late-fee dollar amount, the interest rate, when interest starts, when an
account goes to an attorney — vary by community and live in that community's
collection policy. State such a number ONLY if it is right there in the
retrieved documents/context for THIS community, and then quote it exactly. If the
exact figure is not in front of you, DO NOT guess a day count or an amount —
getting a grace period wrong (e.g., saying "15 days" when the policy says "three
business days") makes a homeowner miss a real deadline and is the worst kind of
wrong. Instead say the exact terms are in the community's collection policy and
offer to confirm them or have accounting follow up. Vague-but-honest beats
specific-but-fabricated on anything that costs the homeowner money.
...
\n`;
  ...
  const stableBase = tailIdx > 0
    ? fullWithEmptyVariable.slice(0, tailIdx).trimEnd()
    : fullWithEmptyVariable;
  // Shared customer-facing writing standard (channel-neutral single source).
  const stable = `${stableBase}\n\n${CUSTOMER_WRITING_STANDARD}`;
