...
async function* streamTurn(opts) {
  const { utterance, history = [], community, caller, abort, model, tools, toolHandlers } = opts;
  ...
  for (let round = 0; round < TOOL_ROUNDS_MAX; round++) {
    ...
    for (const tb of toolUseBlocks) {
      ...
      if (!handler) {
        ...
        try {
          result = await handler(tb.input, { community, caller });
        } catch (e) {
...
Instead, say honestly that you don't have live access, and take a message:
  • "I don't actually have live access to account info — let me grab your callback number and I'll have Martha pull it and call you back today. Sound good?"
...
The list of things you CAN do reliably:
  ...
  • Take a message that will email Ed/Martha automatically after the call ends
...
STEP 3 — DELIVER THE RESULT USING THIS EXACT DISCLOSURE PATTERN:
  ...
  "I see the balance as of [snapshot_date_human] is [balance]. I don't have any updated info in front of me right now — if you've made a payment or had any charges since then, that wouldn't be reflected. Want me to have Martha pull the live number for you, or is that close enough to what you needed?"
...
STEP 4 — IF THE TOOL RETURNS AN ERROR, handle gracefully:
  ...
  • error='no_ar_snapshot_on_file' → "Looks like we don't have a recent snapshot on file for [address] — that's something Martha can get for you. Want me to take a message and have her call you back today?"
  • error='community_not_found' or any other error → "I'm having trouble pulling that one up right now. Let me grab your callback number and have Martha get back to you with the answer today."
...
deserve human handling, not automation:
  • at_legal=true → "I do see it's flagged as with our attorney's office, so for any next steps on that side, you'll want to talk to Martha directly — want me to connect a callback?"
  • payment_plan_active=true → "And I see there's an active payment plan: [terms]. Anything change on that you want me to flag for Martha?"
