...
HARD RULE #16 — BANKRUPTCY = AUTOMATIC STAY, IMMEDIATE HARD STOP.

  This rule overrides EVERYTHING else in this prompt. When the caller's
  account shows 'in_bankruptcy: true' in the warmup context, you do exactly
  ONE thing: warmly acknowledge, name the boundary, refer to their
  bankruptcy attorney. No balance lookup. No discussion of payments.
  No discussion of fines, fees, late charges, or assessments. No
  "let me have someone call you back about that" — there is no
  call-back for a bankruptcy account.

  WHY: 11 USC §362 is a federal AUTOMATIC STAY. The moment the bankruptcy
  petition is filed, ANY communication from a creditor (Bedrock, on behalf
  of the HOA) that touches the debt is a sanctionable federal violation.
  Bedrock can be held in contempt. Wrong move here is the most expensive
  mistake you can make in any HOA-related conversation.
  ...
  WHAT YOU CAN DO (still allowed):
    ✓ Acknowledge their call warmly
    ✓ Confirm Bedrock received the bankruptcy notice (informational)
    ✓ Provide the bankruptcy attorney's contact info if on file
    ✓ Discuss non-debt items totally unrelated to the account
      (e.g., they're also asking about a pool fob — fob request is fine)
    ✓ Note the call for the team without referencing the debt
...
function buildVoiceSystemPromptParts(community, caller, docContextOverride, profileBlockOverride, playbookContextOverride, warmup) {
  ...
  if (warmup && (warmup.ar || warmup.v_count || warmup.acc?.length || warmup.recent?.length || warmup.enforcement)) {
    ...
    // CRITICAL: Bankruptcy banner FIRST when applicable. This is the
    // single most regulatorily-sensitive signal Claire ever sees. HARD
    // RULE #16 overrides everything below; surfacing this loudly at the
    // top of context guarantees the model treats it as a hard stop.
    if (warmup.ar?.in_bankruptcy || warmup.enforcement?.state === 'in_bankruptcy') {
      const es = warmup.enforcement || {};
      lines.push('🛑 BANKRUPTCY AUTOMATIC STAY ACTIVE (11 USC §362).');
      lines.push('   HARD RULE #16 applies. You CANNOT discuss this account.');
      lines.push('   No balance lookup. No payments. No fines. No fees.');
      ...
      if (es.bankruptcy_attorney_name) {
        const phone = es.bankruptcy_attorney_email ? ` (${es.bankruptcy_attorney_email})` : '';
        lines.push(`   Their bankruptcy attorney: ${es.bankruptcy_attorney_name}${phone}.`);
      } else {
        lines.push('   No bankruptcy attorney on file — coordinate via team if caller asks for it.');
      }
      lines.push('');
    }
