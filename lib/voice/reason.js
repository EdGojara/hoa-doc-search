...
const BRIDGE_PHRASES = [
  ...
  'Let me check the guidelines on that.',
  'Sure, let me find that for you.',
  'Let me grab the details on that.',
];
function pickBridgePhrase() { return BRIDGE_PHRASES[Math.floor(Math.random() * BRIDGE_PHRASES.length)]; }
...
async function* streamTurn(opts) {
  ...
  if (isAck) {
    ...
    if (!noBridge) { _perf.bridged = true; yield pickBridgePhrase(); }
