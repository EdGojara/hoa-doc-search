...
function buildVoiceSystemPromptParts(community, caller, docContextOverride, profileBlockOverride, playbookContextOverride, warmup) {
  ...
  let warmupBlock = '';
  if (warmup && (warmup.ar || warmup.v_count || warmup.acc?.length || warmup.recent?.length)) {
    const lines = ['CALLER CONTEXT (pre-call lookup — what you already know about this caller. Use it to be specific, but DON\'T recite it like a database dump. Reference naturally when relevant; don\'t lead with everything you know — that\'s creepy.):'];
    if (warmup.ar) {
      const parts = [];
      ...
      if (warmup.ar.at_legal) parts.push('AT LEGAL — handoff required if discussion goes here');
      if (warmup.ar.in_collections) parts.push('in collections');
      if (warmup.ar.payment_plan_active) parts.push('active payment plan');
      if (warmup.ar.enforcement_stage) parts.push(`stage: ${warmup.ar.enforcement_stage}`);
