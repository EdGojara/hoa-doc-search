...
async function* streamTurn(opts) {
  ...
  // (ack + facts) -> Haiku; document/compliance lane -> Sonnet.
  if (!model) modelName = (isAck || isFactQ) ? FACT_MODEL : DOC_MODEL;
  ...
  if (isAck) {
    ...
    _perf.fastlane = true;
    console.log(`[voice/reason] fact fast-path — profile-only, skipping doc retrieval for: "${utt}"`);
