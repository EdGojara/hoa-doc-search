...
function pickBridgePhrase() {
  if (BRIDGE_PHRASES.length <= 1) return BRIDGE_PHRASES[0] || '';
  let i;
  do { i = Math.floor(Math.random() * BRIDGE_PHRASES.length); } while (i === _lastBridgeIdx);
  _lastBridgeIdx = i;
  return BRIDGE_PHRASES[i];
}
...
async function* streamTurn(opts) {
  ...
  if (isAck) {
    ...
    if (!noBridge && looksLikeQuestion(utt)) { _perf.bridged = true; yield pickBridgePhrase(); }
