...
async function* streamTurn(opts) {
  ...
  // STEP 1 — per-turn hybrid retrieval. Pull document chunks relevant to
  // THIS specific utterance, scoped to the caller's community. Same
  // pipeline askEd Chat uses (vector + keyword + title-match merged via
  // Reciprocal Rank Fusion). Costs ~500-800ms of latency per turn but
  // is the difference between Claire-as-demo and Claire-actually-knowing-
  // the-CC&Rs. Best-effort: if retrieval fails or times out, we still
  // produce an answer using just the system prompt + community name.
  //
  // Community name passed to retrieval is what filters which docs are
  // eligible. Empty string = "All / Law / General" only — won't surface
  // community-specific docs.
  let docContext = '';
  if (utterance && utterance.trim().length > 0) {
    try {
      docContext = await getRelevantChunks(utterance, community?.name || '');
    } catch (err) {
      console.warn(`[voice/reason] doc retrieval failed: ${err.message}`);
    }
  }

  const systemPrompt = buildVoiceSystemPrompt(community, caller, docContext);
  const messages = [
...
function buildVoiceSystemPrompt(community, caller, docContextOverride) {
...
${callerBlock}${communityBlock}${docBlock}
