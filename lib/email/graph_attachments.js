...
const MAX_BLOCKS = 8;                    // cap how many we hand to Claude
const MAX_BYTES = 16 * 1024 * 1024;      // per-attachment raw ceiling; Claude reads PDFs far larger than the old 4.5MB
const MAX_TOTAL_BYTES = 24 * 1024 * 1024; // keep the whole request under Anthropic's ~32MB cap across all blocks
// Inline signature logos / pasted "image001.png" are noise, not documents the
// sender is asking about — same filter used by fetchAttachmentNames.
const LOGO_NAME_RE = /^(image|logo|signature|outlook-)[-_0-9]*\.(png|jpe?g|gif)$/i;

// Returns { blocks: [...Claude content blocks], summary: 'text describing what was attached' }
//
// Scar (Ed 2026-09-17): a vendor emailed three security proposals; Amanda's draft
// asked him to "send" the files he had already attached. Root cause was HERE, not
// in the drafter: this function pulled EVERY attachment's bytes in one
// /attachments call (no $select), which is slow and returns nothing on a large
// message (one bid was 6.3MB) — and it read only inline contentBytes, which Graph
// OMITS for large files, with no by-id fallback and a 4.5MB cap that dropped a
// normal-size proposal outright. So NOTHING reached the model and she drafted
// blind. Now: list metadata first (cheap), then fetch each wanted file's bytes BY
// ID (with a /$value fallback that works regardless of size), higher caps, and
// nested-item expansion — the same robust pattern fetchAllAttachmentBuffers uses.
async function fetchAttachmentBlocks(mailbox, graphId) {
  if (!isConfigured() || !mailbox || !graphId) return { blocks: [], summary: '' };
  let token, base, items = [];
  try {
    token = await getToken();
    base = `https://graph.microsoft.com/v1.0/users/${encodeURIComponent(mailbox)}/messages/${encodeURIComponent(graphId)}/attachments`;
    // Metadata ONLY — no contentBytes. A message with a big attachment used to
    // fail the whole (inline-bytes) fetch; listing first is cheap and reliable.
    const r = await fetch(`${base}?$select=id,name,contentType,size,isInline`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  // Base64 bytes for one attachment id — /$value works whether or not Graph would
  // have inlined contentBytes (the large-file omission that dropped the 6.3MB bid).
  const bytesB64 = async (id, inlineBytes) => {
    if (inlineBytes) return inlineBytes;
    try {
      const vr = await fetch(`${base}/${encodeURIComponent(id)}/$value`, auth);
      if (vr.ok) return Buffer.from(await vr.arrayBuffer()).toString('base64');
    } catch (_) {}
    try {
      const or = await fetch(`${base}/${encodeURIComponent(id)}`, auth);
      if (or.ok) { const o = await or.json(); if (o.contentBytes) return o.contentBytes; }
    } catch (_) {}
    return null;
  };

  // Flatten: top-level file attachments + files nested inside a forwarded email item.
  const flat = [];
  for (const a of items) {
    const type = a['@odata.type'] || '';
    if (/fileAttachment/i.test(type)) { flat.push({ a, id: a.id, inline: null }); continue; }
    if (/itemAttachment/i.test(type) && a.id) {
      try {
        const er = await fetch(`${base}/${encodeURIComponent(a.id)}?$expand=microsoft.graph.itemAttachment/item`, auth);
        if (er.ok) {
          const item = (await er.json()).item;
          for (const na of ((item && item.attachments) || [])) {
            if (/fileAttachment/i.test(na['@odata.type'] || '')) flat.push({ a: na, id: na.id, inline: na.contentBytes || null });
          }
        }
      } catch (_) {}
    }
    // referenceAttachment (OneDrive/SharePoint link) has no bytes — skipped below.
  }

  const blocks = []; const named = []; const skipped = []; const seen = new Set();
  let totalBytes = 0;
  for (const { a, id, inline } of flat) {
    const name = a.name || 'attachment';
    const ct = String(a.contentType || '').toLowerCase().split(';')[0].trim();
    const size = a.size || 0;
    const isImage = IMAGE_TYPES.has(ct) || /\.(png|jpe?g|gif|webp)$/i.test(name);
    const isPdf = ct === 'application/pdf' || /\.pdf$/i.test(name);
    if (isImage && LOGO_NAME_RE.test(name)) { continue; } // drop signature logos silently
    if (!isImage && !isPdf) { skipped.push(`${name} (${ct || 'unknown type'})`); continue; }
    if (size && size > MAX_BYTES) { skipped.push(`${name} (too large to display)`); continue; }
    if (blocks.length >= MAX_BLOCKS) { skipped.push(name); continue; }
    if (size && totalBytes + size > MAX_TOTAL_BYTES) { skipped.push(`${name} (over size budget)`); continue; }

    const b64 = await bytesB64(id, inline);
    if (!b64) { skipped.push(`${name} (couldn't download)`); continue; }
    // Dedupe identical bytes (a logo/screenshot repeated across a forward chain).
    const fp = `${size}:${b64.slice(0, 96)}`;
    if (seen.has(fp)) { continue; }
    seen.add(fp);
    totalBytes += Math.floor(b64.length * 0.75);

    if (isImage) {
      const media = (ct === 'image/jpg' || !IMAGE_TYPES.has(ct)) ? 'image/jpeg' : ct;
      blocks.push({ type: 'image', source: { type: 'base64', media_type: media, data: b64 } });
  ...
  let summary = '';
  if (named.length) summary += `Attachments included below for you to READ (do not ask the sender to resend them): ${named.join(', ')}.`;
  if (skipped.length) summary += `${summary ? ' ' : ''}Also attached but not shown here — acknowledge these by name and do NOT ask the sender to resend them; if you genuinely need what's inside one, say specifically which file and what you need from it: ${skipped.join(', ')}.`;
...
async function fetchAllAttachmentBuffers(mailbox, graphId, opts = {}) {
  ...
  const all = opts.all === true;   // capture EVERY file type (for the record archive)
  try {
    const token = await getToken();
    ...
    // Get the raw bytes for one fileAttachment. Graph inlines contentBytes for
    // small files but OMITS it for large ones — those must be pulled by id via
    // /$value, or the attachment silently vanishes (the dropped-contract bug).
    const bytesFor = async (a) => {
      if (a.contentBytes) return Buffer.from(a.contentBytes, 'base64');
      if (!a.id) return null;
      try {
        const vr = await fetch(`${base}/${encodeURIComponent(a.id)}/$value`, auth);
        if (vr.ok) return Buffer.from(await vr.arrayBuffer());
      } catch (_) {}
      return null;
    };
    const classify = (a) => {
      ...
      const isDoc = /wordprocessingml|spreadsheetml|presentationml|msword|ms-excel|ms-powerpoint|officedocument/.test(ct) || /\.(docx?|xlsx?|pptx?|csv)$/i.test(a.name || '');
      return { ct, isPdf, isImage, isDoc };
    };
    const pushFile = async (a) => {
      const { ct, isPdf, isImage, isDoc } = classify(a);
      // `all` keeps every type (the record archive); otherwise the historical
      // pdf/image[/doc] filter that existing callers rely on.
      if (!all && !isPdf && !isImage && !(includeDocs && isDoc)) return;
      const buffer = await bytesFor(a);
      if (!buffer || !buffer.length) return;
      out.push({
        filename: a.name || (isPdf ? 'application.pdf' : isDoc ? 'document' : isImage ? 'photo.jpg' : 'attachment'),
        buffer,
    ...
    for (const a of (j.value || [])) {
      const type = a['@odata.type'] || '';
      if (/fileAttachment/i.test(type)) {
        await pushFile(a);
      } else if (/itemAttachment/i.test(type) && a.id) {
        // A forwarded email carried as a message item — the real document lives
        // INSIDE it. Expand the nested item and pull ITS file attachments.
        try {
          const er = await fetch(`${base}/${encodeURIComponent(a.id)}?$expand=microsoft.graph.itemAttachment/item`, auth);
          if (er.ok) {
            const item = (await er.json()).item;
            const nested = (item && item.attachments) || [];
            for (const na of nested) { if (/fileAttachment/i.test(na['@odata.type'] || '')) await pushFile(na); }
          }
        } catch (_) {}
      }
      // referenceAttachment (OneDrive/SharePoint link) has no bytes to fetch — skipped.
    }
    return out;
  } catch (_) { return []; }
}
