  ...
  if (communityId) {
    try {
      ...
      const { data: contacts } = await supabase.from('community_contacts')
        ...
        .in('category', ['trash', 'utility', 'tv_internet', 'community']);
      const label = { trash: 'TRASH/RECYCLING', utility: 'Utility', tv_internet: 'TV/Internet', community: 'Community' };
      for (const c of (contacts || [])) {
        lines.push(`${label[c.category] || c.category}: ${c.name || ''}${c.phone ? ' ' + c.phone : ''}${c.url ? ' ' + c.url : ''}${c.notes ? ' — ' + String(c.notes).slice(0, 150) : ''}`);
      }
      if (lines.length) services = `COMMUNITY SERVICES & LOCAL CONTACTS for ${communityName || 'this community'} — when the homeowner asks about trash/recycling, water, utilities, TV/internet, or the clubhouse, use the EXACT provider and phone number here. Do NOT send them to "the county" for a service the community already has a specific provider for.\n- ${lines.join('\n- ')}`;
    } catch (_) {}
  }
