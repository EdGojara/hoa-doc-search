...
async function draftReply({ email, classification, contactId, propertyId, communityId, contactName, communityName, force }) {
  ...
  try { convo = await conversationHistory({ conversationId: email.conversation_id, senderEmail: email.sender_email, currentText: email.body_full || email.body_preview }); } catch (_) {}
  ...
  if (email.has_attachments && email.graph_id && email.mailbox) {
