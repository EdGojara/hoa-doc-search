...
Draft Claire's reply body now.`;
  ...
  try {
    ...
    const known = new Set((grounding.match(phoneRe) || []).map(digits10));
    const ungrounded = [...new Set((body.match(phoneRe) || []))].filter((p) => digits10(p).length === 10 && !known.has(digits10(p)));
    if (ungrounded.length) {
      warnings.push(`⚠ This draft cites ${ungrounded.length === 1 ? 'a phone number' : 'phone numbers'} not found in the community's records: ${ungrounded.join(', ')}. The platform did not supply ${ungrounded.length === 1 ? 'it' : 'them'} — verify or remove before sending.`);
    }
  } catch (_) {}
  ...
  // A fabricated-contact warning outranks the advisory hint — surface it first.
  if (warnings.length) review_hint = warnings.join(' ') + (review_hint ? ' ' + review_hint : '');
  return { draftable: true, careful: careful || warnings.length > 0, subject, body, review_hint, warnings, grounded: { account: !!acct, knowledge: !!docs } };
}
