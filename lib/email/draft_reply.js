...
Draft Claire's reply body now.`;
  ...
  // A fabricated-specific warning outranks the advisory hint — surface it first.
  if (warnings.length) review_hint = warnings.join(' ') + (review_hint ? ' ' + review_hint : '');
  return { draftable: true, careful: careful || hardFlag, subject, body, review_hint, warnings, grounded: { account: !!acct, knowledge: !!docs } };
}
