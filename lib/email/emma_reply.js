...
async function draftEmmaReply({ email, vendorId, vendorName, notes, currentDraft, examples }) {
  ...
  // Emma has to know her own colleagues. Superior LawnCare addressed invoice
  // 42778 to Martha Bravo (Bedrock staff), and Emma — who had no idea who Martha
  // is — drafted "this invoice was sent to us by mistake, we're Bedrock
  // Association Management, not Martha Bravo." The invoice was addressed
  // correctly. (Ed 2026-07-15.)
  const { teamRosterBlock } = require('./team_roster');
  const roster = await teamRosterBlock();

  // Encode-Ed: how Ed has edited Emma's past replies of this kind (single-teacher).
  let examplesBlock = '';
  ...
  const sys = `You are Emma Brooks, Bedrock Association Management's AI accounts-payable team member, drafting a reply to a VENDOR for a human to review before it sends. Warm, professional, concise — like a great AP specialist.${roster}
${examplesBlock ? '\n' + examplesBlock + '\n' : ''}
...
- TELL THE DIFFERENCE between a vendor ASKING ABOUT an invoice and a vendor SENDING one. If this email IS the invoice arriving (an invoice notification, a bill attached, "here is invoice 42778"), then it HAS reached us. Never ask them to resend it or send it somewhere else, and never call it misdirected. It just isn't logged in the ledger YET, which is our job, not theirs. Usually the right answer to a bill landing is no reply at all, so keep it to a short acknowledgement of receipt and nothing more.
...
- ATTACHMENTS: if the vendor attached files (an invoice, W-9, ACH form, statement), they are included below for you to READ. Read them and reference the actual details. NEVER ask the vendor to "send" or "resend" a document they already attached. If a file can't be shown it's named below — acknowledge it by name.
...
${payLines}
${currentDraft ? `\nCURRENT DRAFT (revise THIS — keep what works, change what the notes ask for):\n${String(currentDraft).slice(0, 4000)}\n` : ''}
${notes ? `\nTHE REVIEWER'S NOTES — direction from the human who will send this. Incorporate their thoughts and follow their instructions. Weave it in naturally in Emma's voice; do NOT quote the notes verbatim:\n${String(notes).slice(0, 3000)}\n` : ''}
Draft Emma's reply body now.`;

  // Read the vendor's attachments (invoice / W-9 / ACH form) so Emma references
  // real details instead of asking them to resend what they attached. Best-effort.
  ...
  const userText = ctx + (attachSummary ? `\n\n${attachSummary}` : '');
  const call = (blocks) => anthropic.messages.create({
    model: MODEL, max_tokens: 700, system: sys,
    messages: [{ role: 'user', content: [{ type: 'text', text: userText }, ...blocks] }],
  });
  let resp;
  try { resp = await call(attachBlocks); }
  catch (e) {
    if (attachBlocks.length) { console.warn('[emma_reply] attachment blocks rejected, retrying text-only:', e.message); resp = await call([]); }
    else throw e;
  }
  ...
  const out = { draftable: true, persona: 'emma', subject, body, grounded: { ap: status.hasData } };
  // Receipts: when Emma read attached documents (an invoice, W-9, ACH form),
  // ground + independently verify the figures she cites before a human approves.
  if (attachBlocks.length) {
    try {
      const { groundAndVerify } = require('../team/grounding');
      const gv = await groundAndVerify({ draftBody: body, sourceText: userText, sourceBlocks: attachBlocks });
