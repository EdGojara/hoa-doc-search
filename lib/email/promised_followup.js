...
async function resolvePartyEmail({ name, threadText, conversationId, propertyId }) {
  ...
  // 1) Named party in the thread body ("Carlos <carlos@…>", "From: Carlos …").
  if (!generic) {
    const want = firstTok(nm);
    const pairs = extractNameEmailPairs(threadText);
    let hit = pairs.find((p) => firstTok(p.name) === want && !isInternal(p.email))
  ...
  if (!generic) {
    try {
      const { data } = await supabase.from('contacts')
        ...
        .ilike('full_name', `%${nm}%`).not('primary_email', 'is', null).limit(3);
