...
// Internal-ops agents (HR/Growth) live in a SEPARATE registry so they never
// leak onto community surfaces, but they still send branded email — so the
// signature builder consults that registry as a fallback and pulls their
// headshot from the internal photo dir. (Ed 2026-09-07.)
const opsRoster = require('../team/bedrock_ops');
...
const PHOTO_DIR_INTERNAL = path.join(__dirname, '..', '..', 'public', 'assets', 'presentations', 'team-internal');
function photoB64(persona) {
  const key = String(persona || '').toLowerCase();
  if (_photoCache.has(key)) return _photoCache.get(key);
  const dir = opsRoster.isInternalPersona(key) ? PHOTO_DIR_INTERNAL : PHOTO_DIR;
  let b64 = null;
  try { b64 = fs.readFileSync(path.join(dir, key + '.jpg')).toString('base64'); } catch (e) { /* headshot optional */ }
...
function identity(persona) {
  const key = String(persona || '').toLowerCase();
  const m = byPersona.get(key) || opsRoster.get(key);
...
function buildPersonaEmail(persona, bodyText, communityName, quotedHtml) {
  // Last-line enforcement of Ed's no-em-dash rule: strip them here, at the ONE
  // boundary every persona's outbound body passes through to get its signature,
  // so a slipped em-dash in any drafter (or a hand-written template) can never
  // reach a customer. Only OUR copy is scrubbed; quotedHtml is the other party's
  // original thread and stays verbatim. (Ed 2026-09-11: "m dashes are the sign
  // of ai created work.")
  const cleanBody = stripEmDashes(bodyText);
