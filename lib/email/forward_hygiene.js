...
/**
 * Who a review forward is allowed to reach when the operator DELIBERATELY adds an
 * outside person (a board member, a colleague on another domain). The one hard
 * rule survives: the forward discusses the homeowner, so it must NEVER reach the
 * homeowner themselves — the original sender or any of their on-file addresses.
 * Everything else the operator typed is honored (and surfaced, never silently
 * dropped). (Ed 2026-08-01 — an external Cc the operator intended was being
 * stripped without a word.)
 *
 * @param {string[]} blockedEmails — homeowner addresses that must never receive
 *   (the original sender + the resolved contact's on-file emails), lowercased or not.
 * @returns {{ to:string[], cc:string[], blocked:string[] }}
 *   blocked — requested addresses removed because they're the homeowner, for a
 *             LOUD warning to the operator (not a silent drop).
 */
function forwardRecipients({ toEmail, ccEmail, blockedEmails = [] }) {
  const block = new Set((blockedEmails || []).map((a) => String(a || '').trim().toLowerCase()).filter(Boolean));
  const allow = (a) => a && !block.has(a.toLowerCase());
  const rawTo = parseList(toEmail);
  const rawCc = parseList(ccEmail);
  const to = [...new Set(rawTo.filter(allow))];
  const toSet = new Set(to.map((a) => a.toLowerCase()));
  const cc = [...new Set(rawCc.filter(allow))].filter((a) => !toSet.has(a.toLowerCase()));
  const blocked = [...new Set([...rawTo, ...rawCc].filter((a) => !allow(a)))];
  return { to, cc, blocked };
}
...
function stripQuoted(text) {
  ...
  const markers = [
    ...
    // Apple Mail: "On Jul 26, 2026, at 8:11 AM, <name> <email> wrote:" — month
    // after "On" (no weekday), a year, then ", at <time>", then "wrote:".
    /\bOn\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{1,2},?\s+(?:19|20)\d{2},?\s+at\s.{0,140}?\bwrote:/is,
...
module.exports = { internalRecipients, forwardRecipients, stripQuoted, parseList };
