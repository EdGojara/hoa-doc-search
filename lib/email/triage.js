...
- violation_report: someone reporting a problem, nuisance, or DISPUTE — uncut yard, parked vehicle, trash, OR a complaint about a neighbor's fence/wall/structure, an encroachment, or a property-line disagreement. The sender is reporting/complaining, usually about SOMEONE ELSE's property or a shared boundary. Photos of a problem or a boundary belong here, NOT acc_request.
- acc_request: the SENDER is requesting approval to make an exterior change to THEIR OWN property — an architectural/ACC application, a contractor notice, or "I want to build/install/paint X on my house." There must be an intent by the sender to modify their own property. A complaint ABOUT a fence, wall, or structure (a dispute, a neighbor, an encroachment) is NOT an acc_request — it is a violation_report. When unsure between the two, ask: is the sender asking PERMISSION to change their own property (acc_request), or reporting a PROBLEM (violation_report)?
...
async function communityMap(sb) {
  const { data } = await sb.from('communities').select('id, name, management_status');
  return (data || []);
}
// Score every community and take the strongest match, instead of the first
// name whose leading token appears in the hint. That first-token-wins rule let
// a PROSPECT hijack a live thread: "Cinco Landscape Maintenance Association"
// (first token "cinco") matched an email plainly about "Canyon Gate at Cinco
// Ranch", so Emma signed off as the wrong association (Ed 2026-09-16). Now a
// full-name containment always beats a single shared token, and an ACTIVE
// community edges out a prospect on a tie so a not-yet-signed prospect can
// never outrank a community we actually manage.
function matchCommunity(hint, comms) {
  if (!hint) return null;
  const h = String(hint).toLowerCase().trim();
  if (!h) return null;
  const hTokens = new Set(h.split(/[^a-z0-9]+/).filter(Boolean)); // whole-word set of the hint
  let best = null, bestScore = 0;
  for (const c of comms) {
    const n = String(c.name || '').toLowerCase().trim();
    if (!n) continue;
    let score = 0;
    if (n === h) score = 100;                 // exact
    else if (h.includes(n)) score = 90;       // hint contains the FULL community name (strongest partial)
    else if (n.includes(h)) score = 80;       // community name contains the whole hint
    else {
      // Weakest: the hint contains the community's leading token as a WHOLE
      // word. Only a DISTINCTIVE token (>= 4 chars) counts, and it scores far
      // below any full-name containment, so a shared token like "cinco" can
      // never beat a fuller match.
      const first = String(n.split(/\s+/)[0] || '').replace(/[^a-z0-9]/g, '');
      if (first.length >= 4 && hTokens.has(first)) score = 30;
    }
    if (score === 0) continue;
    // Active outranks prospect on a tie (a prospect must never capture a live
    // community's correspondence).
    const adj = score - (String(c.management_status || '') === 'prospect' ? 5 : 0);
...
async function resolveEntities(ex, email, sb) {
  ...
  // NEVER resolve a homeowner's identity from an INTERNAL sender. When a staffer
  // forwards a homeowner's email (mbravo@bedrocktx.com → the shared inbox), the
  // forwarder's address is not the homeowner — resolve from the body/thread
  // instead. Scar (Ed 2026-07-27): a homeowner contact ("Jerry, Jr. Keller")
  // wrongly held mbravo@bedrocktx.com, so EVERY one of Martha's forwards resolved
  // to Jerry / 4919 Grand Chateau at 0.95 "sender email on file" — 10 emails
  // (incl. the Still Creek ACC Form) attached to the wrong house. The data error
  // gets cleaned, but this guard makes a staff email on a contact unable to
  // hijack resolution ever again.
  const senderIsInternal = /@bedrocktx\.com$/i.test(from);
  if (from && !senderIsInternal && /^[^@\s,()]+@[^@\s,()]+\.[^@\s,()]+$/.test(from)) {
    const byId = new Map();
