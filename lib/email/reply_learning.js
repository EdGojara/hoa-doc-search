...
async function personaReadiness(supabase, { ownerEmail, window = 1000 } = {}) {
  ...
  try {
    let q = supabase.from('email_reply_edits')
      .select('persona, classification, was_edited, edit_ratio, created_at')
      .order('created_at', { ascending: false })
      .limit(window);
    if (ownerEmail) q = q.ilike('edited_by', ownerEmail);
    ...
    const subRate = (arr) => (arr.length ? arr.filter(isSub).length / arr.length : 0);
    ...
    for (const [persona, rows] of Object.entries(byPersona)) {
      ...
      const recentRate = subRate(recent);
      const ready = n >= READY_MIN_N && recentRate <= READY_MAX_SUBSTANTIVE_RATE;

      // Trend: is the recent window cleaner than the window BEFORE it? So Ed can
      // watch the curve bend (improving) or catch a teammate slipping. Needs a
      // full prior window to compare, else 'new'.
      const prior = rows.slice(RECENT_WINDOW, RECENT_WINDOW * 2);
      let trend = 'new', trend_delta = 0;
      if (prior.length >= Math.min(RECENT_WINDOW, 8)) {
        // delta > 0 means fewer substantive edits now than before = improving.
        trend_delta = Number((subRate(prior) - recentRate).toFixed(3));
        trend = trend_delta > 0.05 ? 'improving' : trend_delta < -0.05 ? 'slipping' : 'flat';
      }

      // By classification: which KINDS of mail this teammate is already clean on,
      // so Ed can release the easy classes first and keep judgment calls gated.
      const byCls = {};
      for (const r of rows) { const c = r.classification || 'other'; (byCls[c] = byCls[c] || []).push(r); }
      const by_classification = Object.entries(byCls)
        .map(([classification, cr]) => ({
          classification,
          n: cr.length,
          clean_rate: Number((1 - subRate(cr)).toFixed(3)),
          ready: cr.length >= READY_MIN_N && subRate(cr) <= READY_MAX_SUBSTANTIVE_RATE,
        }))
        .sort((a, b) => b.n - a.n);
