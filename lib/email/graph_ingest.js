...
async function ingestMailbox(mailbox, opts = {}) {
  ...
  async function handleMessage(gm) {
    ...
    // No auto-draft for the General inbox (solicitations/junk) or spam/internal —
    // Ed drafts those on demand. Real mail (homeowner, ACC, DRV, vendor) drafts.
    if (!light && email.direction === 'inbound' && !['spam', 'internal', 'other'].includes(ex.classification)) {
      try {
    ...
    const row = {
      ...
      extracted: { requested_action: ex.requested_action, community_hint: ex.community_hint, person_names: ex.person_names, addresses: ex.addresses, amounts: ex.amounts, ticket_ref: ex.ticket_ref, vendor_name: ex.vendor_name, backfill: !!light, draft, drv: drvContext },
    ...
    return { r: 'kept', linked: isLinked, invoices, acc, drv };
  }
  ...
  while (url && stats.scanned < max) {
    ...
    for (let i = 0; i < toProcess.length; i += CONCURRENCY) {
      ...
      for (const o of outs) {
        if (o && o.r === 'kept') { stats.kept += 1; if (o.linked) stats.linked += 1; if (o.invoices) stats.invoices_loaded = (stats.invoices_loaded || 0) + o.invoices; if (o.acc) stats.acc_apps = (stats.acc_apps || 0) + o.acc; if (o.drv) stats.drv_replies = (stats.drv_replies || 0) + o.drv; }
