      ...
      try {
        ...
        if (!already || !already.length) {
          ...
          for (const pdf of pdfs) {
            const out = await autoIntake({ buffer: pdf.buffer, filename: pdf.filename, intakeMethod: 'email', sourceRef: srcRef, communityId: res.community_id || null, vendorIdHint: res.vendor_id || null, achHintText: `${email.subject || ''} ${email.body_full || email.body_preview || ''}`, staffNote: email.body_full || email.body_preview || '', staffSenderEmail: email.sender_email || '' });
