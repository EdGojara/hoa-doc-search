    ...
    if (!light && !handoff.wants && email.direction === 'inbound' && !['spam', 'other', 'vendor_financial', 'vendor_general'].includes(ex.classification)) {
      try {
        ...
        if (!draft) {
          ...
          const d = await draftReply({ email, classification: ex.classification, contactId: res.contact_id, propertyId: res.property_id, communityId: res.community_id, contactName: draftContactName, communityName: nm.communityName, force: ex.classification === 'internal' });
