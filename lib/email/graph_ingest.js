  ...
  async function handleMessage(gm) {
    ...
    if (!light && !handoff.wants && email.direction === 'inbound'
      ...
      try {
        ...
        const d = await draftPaigeReply({ email, supabase });
    ...
    // file. (Ed 2026-07-20.)
    let followUp = { is_follow_up: false }, chasedInvoice = null;
    if (!light && email.direction === 'inbound' && ['vendor_financial', 'vendor_general'].includes(ex.classification)) {
      try {
    ...
    const row = {
      ...
      triage_status: ex.is_spam ? 'spam' : (handoff.wants ? 'needs_review' : (res.confidence === 'high' ? 'linked' : (res.candidates.length ? 'needs_review' : 'new'))),
