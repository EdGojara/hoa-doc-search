...
async function ingestMailbox(mailbox, opts = {}) {
  ...
  async function handleMessage(gm) {
    ...
    return { r: 'kept', linked: isLinked, invoices };
  }
  ...
  while (url && stats.scanned < max) {
    ...
    for (let i = 0; i < toProcess.length; i += CONCURRENCY) {
      ...
      for (const o of outs) {
        if (o && o.r === 'kept') { stats.kept += 1; if (o.linked) stats.linked += 1; if (o.invoices) stats.invoices_loaded = (stats.invoices_loaded || 0) + o.invoices; }
