    ...
    if (!light && email.direction === 'inbound' && email.has_attachments && email.graph_id
      ...
      try {
        ...
        if (!already || !already.length) {
          ...
          // Filed itself to Payables (awaiting approval) — clear it from Emma's
          // review queue and leave a trail. Approval in Payables is the human gate
          // (Emma is low-risk: a wrong code is caught before a dollar moves), so a
          // bill that auto-filed shouldn't also sit in the inbox as a daily chore.
          // The PDF is retained on the payable (source_storage_path) AND archived
          // to email_attachments below, so it's on file for our records either way.
          // (Ed 2026-08-01.)
          if (filedIds.length && insId) {
            try { await supabase.from('email_messages').update({ triage_status: 'handled', extracted: { ...row.extracted, auto_filed: { invoice_ids: filedIds, at: new Date().toISOString() } } }).eq('id', insId); } catch (_) {}
          }
        }
