...
function mapGraphMessage(m, mailbox) {
  ...
  // Direction: outbound if it was sent by us (a bedrocktx.com address / info@).
  const fromAddr = (from.address || '').toLowerCase();
  const direction = /@bedrocktx\.com$/.test(fromAddr) ? 'outbound' : 'inbound';
  return {
    mailbox,
    graph_id: m.id || null,
  ...
  while (url && stats.scanned < max) {
    ...
    for (const gm of (j.value || [])) {
      ...
      const row = {
        ...
        extracted: { requested_action: ex.requested_action, community_hint: ex.community_hint, person_names: ex.person_names, addresses: ex.addresses, amounts: ex.amounts, ticket_ref: ex.ticket_ref, vendor_name: ex.vendor_name, backfill: !!light },
