...
async function queueDraft({
  ...
  disposition = 'needs_review', confidence = null, dispositionReason = null,
}) {
  ...
  const row = {
    ...
    disposition, confidence, disposition_reason: dispositionReason,
    status: 'draft',
  };
  // Queue with the exception-router verdict; if migration 395 has not been
  // applied yet, retry without it rather than dropping the draft entirely.
  let ins = await supabase.from('outbound_email_drafts').insert(row).select('id').single();
  if (ins.error && String(ins.error.code) === '42703' && /disposition|confidence/i.test(ins.error.message || '')) {
    const { disposition: _d, confidence: _c, disposition_reason: _r, ...base } = row;
    ins = await supabase.from('outbound_email_drafts').insert(base).select('id').single();
  }
