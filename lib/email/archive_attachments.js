...
async function archiveInboundAttachments(email) {
  // NOTE: do NOT gate on has_attachments. Inline images (a homeowner's photo
  // pasted into the body) leave Graph's hasAttachments flag FALSE, yet they are
  // real fileAttachments with contentBytes — and they are exactly the evidence
  // photos we must capture (the 5943 Baldwin Elm dog photo was inline and was
  // silently dropped). The /attachments fetch returns [] cheaply when there are
  // truly none, so callers decide when to try; this guard only needs the ids.
  if (!email || !email.id || !email.graph_id || !email.mailbox) {
  ...
  let atts = [];
  // all:true — the archive is the association's RECORD of what was sent, so it
  // must keep EVERY file type (Word contracts, Excel budgets, zips), not just
  // PDFs and images. The old default dropped every docx/xlsx: a Canyon Gate
  // contract and 5 community budgets arrived with only signature logos archived
  // (Ed 2026-09-01).
  try { atts = await fetchAllAttachmentBuffers(email.mailbox, email.graph_id, { all: true }); }
