...
function personaForMessage(m) {
  ...
  // Functional inbox addresses route by FUNCTION, straight to the owning
  // specialist — an address dedicated to violations or ACC is unambiguous, so it
  // outranks content classification and the front-office default. (Ed 2026-09-07:
  // "make them go to the right place, not all Miranda — acc should go to Annie.")
  // A homeowner's violation REPORT to info@ still lands with Claire, who hands it
  // to Miranda; this is only for mail addressed to the function's own inbox.
  const _local = mailbox.split('@')[0];
  const FUNCTIONAL_INBOX = { violations: 'miranda', acc: 'annie', arc: 'annie', builders: 'annie', accounting: 'kat', ap: 'emma', payables: 'emma', resale: 'reese', estoppel: 'reese', board: 'paige', jobs: 'vivian', careers: 'vivian', hiring: 'vivian' };
  if (FUNCTIONAL_INBOX[_local]) return FUNCTIONAL_INBOX[_local];
  ...
  // ADDRESSED-TO-A-PERSON WINS over content classification. A homeowner who wrote
  // straight to claire@ stays Claire's, even if the content looks like an ACC or
  // other specialist matter — she owns it and hands off, keeping the thread in the
  // inbox the person actually wrote to. Ed 2026-09-07: Noreen emailed Claire a tree
  // question; it classified as acc_request and was pulled into Annie's queue, but it
  // was addressed to Claire and should have stayed there. Obvious automated noise to
  // her box still filters to general (the AppRiver quarantine scar, 2026-07-13).
  if (mailbox === String(graphSend.CLAIRE_MAILBOX || '').toLowerCase()) {
  ...
  // Mail sent to a specialist's OWN mailbox is theirs, full stop. Amanda, Kat,
  // Paige and Reese were missing here while Miranda, Annie and Emma had it, so
  // their own inboxes fell through to Claire and they only ever received a
  // message when its CONTENT happened to match a keyword further down.
  //
  // Ed 2026-08-20: Martha emailed Amanda directly asking for help, Amanda drafted
  // a good reply, and it was filed under Claire — so Amanda's queue showed one
  // email, a mailing-address update caught by the roster keyword, and the thing
  // she had actually answered was not in her queue at all. The reply existed and
  // was invisible to the person who needed to send it, which is indistinguishable
  ...
  // Darby (legal/collections) and Phoebe (community engagement) were missing here,
  // so mail sent straight to darby@ or phoebe@ fell through to Claire and never
  // showed in their own inbox — the same invisibility bug as Amanda's above. Now
  // that both mailboxes are pulled, their own mail is theirs. (Ed 2026-09-07.)
  if (mailbox === String(graphSend.DARBY_MAILBOX || '').toLowerCase()) return 'darby';
  if (mailbox === String(graphSend.PHOEBE_MAILBOX || '').toLowerCase()) return 'phoebe';
  if (mailbox === String(graphSend.VIVIAN_MAILBOX || '').toLowerCase()) return 'vivian';
  if (mailbox === String(graphSend.MAGGIE_MAILBOX || '').toLowerCase()) return 'maggie';
