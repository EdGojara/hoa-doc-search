...
function signatureHtml(communityName) {
  const logoImg = LOGO_B64 ? `<img src="cid:bedrocklogo" width="46" height="46" alt="Bedrock" style="display:block;border:0;border-radius:6px;">` : '';
  return `
  <table cellpadding="0" cellspacing="0" role="presentation" style="margin-top:18px;border-top:2px solid ${GOLD};padding-top:12px;font-family:Arial,Helvetica,sans-serif;">
    <tr>
      ${logoImg ? `<td valign="top" style="padding-right:12px;">${logoImg}</td>` : ''}
      <td valign="top" style="font-size:13px;line-height:1.5;color:${NAVY};">
        <strong style="color:${NAVY};">Claire</strong> <span style="color:${MUTED};">· AI assistant</span><br>
        ...
        <div style="margin-top:8px;font-size:11px;color:${MUTED};max-width:420px;">I'm Bedrock's AI assistant, so I can help fast. Want a person instead? Just reply and I'll pass you to the team.</div>
      </td>
    </tr>
  </table>`;
}
