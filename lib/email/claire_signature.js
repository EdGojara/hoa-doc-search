...
function signatureHtml(communityName) {
  // Native asset is 300x89 (was 600x178; shrunk 2026-08-01 so a reply-strip can't blow it up) (a wide horizontal lockup). Render at that aspect
  ...
  <table cellpadding="0" cellspacing="0" role="presentation" style="margin-top:18px;border-top:2px solid ${GOLD};padding-top:12px;font-family:Arial,Helvetica,sans-serif;">
    <tr>
      <td style="font-size:13px;line-height:1.5;color:${NAVY};">
        <strong style="color:${NAVY};">Claire Bennett</strong><br>
...
// The signature block + its inline logo attachment, for callers that build their
// own HTML body (e.g. the internal review forward) and just need to append
// Claire's sign-off. Returns { html, attachment | null }.
function claireSignatureParts(communityName) {
  const attachment = LOGO_B64 ? {
    '@odata.type': '#microsoft.graph.fileAttachment',
    name: 'bedrock-logo.png', contentType: 'image/png', contentBytes: LOGO_B64,
    contentId: 'bedrocklogo', isInline: true,
  } : null;
  return { html: signatureHtml(communityName), attachment };
}

module.exports = { buildClaireEmail, claireSignatureParts };
