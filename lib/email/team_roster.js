...
//
// DERIVED from the one canonical roster (lib/team/roster.js). It used to be a
// fourth hand-kept list, and it had gone stale in the worst possible place:
// this array is what teamRosterBlock() injects into EVERY outbound persona's
// prompt as "these people work here with you," and it was missing Kat Reed,
// Amanda Albright, Reese Calloway and Paige Chandler. Emma reading a vendor's
// "I already spoke with Kat" had no idea Kat was her colleague — the precise
// failure this whole file was written to prevent, against half the team.
//
// Tessa is deliberately excluded: she is Ed's private EA, and there is no
// reason a vendor-facing prompt should know she exists.
const _roster = require('../team/roster');
const AI_TEAM = _roster.people()
  .filter((m) => !m.owner_only)
  .map((m) => ({
    full_name: m.name,
    email: `${m.self_mailbox || 'info@'}bedrocktx.com`,
    role: `${m.title.toLowerCase()} — ${m.domain} (AI)`,
  }));
