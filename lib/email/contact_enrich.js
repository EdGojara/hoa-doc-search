...
// NEVER capture a system/relay/staff address onto a homeowner. Scar (Ed
// 2026-07-27): this enricher wrote the CALL-BACK FORM's relay address
// (email@bedrocktx.com) and staff-forward addresses (mbravo@bedrocktx.com,
// mkessler@bedrocktx.com) onto homeowner contacts. Then every future email from
// that relay matched "sender email on file" and resolved to that homeowner —
// James's Waterview report landed on Billie in Lakes of Pine Forest. A
// self-reinforcing black hole. The address the homeowner writes FROM is theirs;
// an internal relay or no-reply is never a homeowner's identity.
...
async function enrichContactFromEmail(sb, contactId, opts = {}) {
  ...
  try {
    ...
    // Email — never a system/relay/staff address (see SYSTEM_ADDR above).
    const email = String(opts.email || '').trim().toLowerCase();
    if (email && EMAIL_RE.test(email) && !SYSTEM_ADDR.test(email) && !hasEmail(email)) {
...
module.exports = { enrichContactFromEmail, normalizePhone, SYSTEM_ADDR };
