...
//                      ar_former_owners: trusted_property_id -> properties.id (explicit, approved);
//                      property_address (if given) must equal that property's street_address.
//                      Which tenure a former balance belongs to is NOT decided here.
//   AP rows            trusted_vendor_id -> vendors.id (explicit, approved; vendor_name is display only);
//                      gl_account -> chart_of_accounts.account_number
...
const NATURAL_KEYS = {
  ...
  ap_open: (r) => [r.trusted_vendor_id, r.invoice_number ? 'INV:' + r.invoice_number : 'SRC:' + r.source_row].join('|'),
...
function mapInputs(inputs, ref) {
  ...
  const propsById = index(ref.properties, (p) => p.id);
  const vendorsById = index(ref.vendors, (v) => v.id);
  ...
  for (const [kind, f] of Object.entries(inputs.files)) {
    ...
    mapped[kind] = f.rows.map((r) => {
      ...
      if (kind === 'ar_former_owners') {
        const p = one(propsById, r.trusted_property_id);
        if (!p.hit) ex(kind, r, 'PROPERTY_ID_NOT_FOUND', `trusted_property_id ${r.trusted_property_id} is not a property of this community`);
        else {
      ...
      if (kind === 'ap_open') {
        const v = one(vendorsById, r.trusted_vendor_id);
        if (v.hit) m.vendor_id = v.hit.id; else ex(kind, r, 'VENDOR_ID_NOT_FOUND', `trusted_vendor_id ${r.trusted_vendor_id} is not a Trusted vendor`);
