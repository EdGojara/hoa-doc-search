...
// Former-owner rows carry an explicit, approved Trusted property id: the identity
// key. property_address is validation/context only.
const FORMER_COLUMNS = [
  col('trusted_property_id', 'text', 'Approved Trusted property id (UUID) this former-owner balance relates to. Identity key for this file.'),
  ...AR_COLUMNS.map((c) => (c.name === 'property_address' ? opt('property_address', 'text', 'Validation/context only: if supplied it must equal the street address of trusted_property_id.') : c)),
];
...
const FILES = {
  ...
  ar_credits: {
    ...
    rule: 'amount != 0 (signed as supplied)',
    rowCheck: (r) => (r.amount !== 0 ? null : 'amount must be non-zero'),
  },
  ar_former_owners: {
    title: 'Former-owner balances', filename: 'ar_former_owners.csv', columns: FORMER_COLUMNS,
  ...
  ap_open: {
    ...
    columns: [
      col('trusted_vendor_id', 'text', 'Approved Trusted vendor id (UUID). Identity key for the vendor.'),
      col('vendor_name', 'text', 'Vendor name as in the source report. Source/display only; not used to identify the vendor.'),
      opt('vantaca_vendor_id', 'text', 'Vantaca vendor id if supplied.'),
      opt('invoice_number', 'text', 'Invoice number as supplied. May be blank ONLY when the source report shows none; then source_row is required and identifies the row. Never manufactured.'),
    ...
    rule: 'amount_open > 0 and amount_open <= original_amount; when invoice_number is blank, source_row is required',
    rowCheck: (r) => (!(r.amount_open > 0 && r.amount_open <= r.original_amount) ? 'amount_open must be > 0 and <= original_amount' : (!r.invoice_number && !r.source_row) ? 'source_row is required when invoice_number is blank' : null),
  },
  gl_trial_balance: {
