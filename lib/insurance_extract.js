...
// The Anthropic document API caps at 100 PDF pages. A policy PDF is a short
// DECLARATIONS section (the coverage facts an RFP needs) followed by dozens of
// pages of standard forms and endorsements we do NOT need. Full policies
// routinely blow past 100 pages — Waterview's Property was 155, GL 111, Cyber
// 101 — which 400'd and got SILENTLY DROPPED (the loop caught the error and
// moved on), which is exactly why Property and Cyber went missing from the RFP.
// Cap to the first N pages so the declarations always extract. (Ed 2026-09-03)
const MAX_EXTRACT_PAGES = 30;

async function capPdfPages(buffer, maxPages = MAX_EXTRACT_PAGES) {
  const { PDFDocument } = require('pdf-lib');
  let src;
  try { src = await PDFDocument.load(buffer, { ignoreEncryption: true }); }
  catch (_) { return buffer; } // unparseable at all — let the API try the original
  ...
  try {
    ...
    // Oversized AND untrimmable (some carriers — e.g. a Chubb BOP — ship a
    // malformed page tree pdf-lib can't copy). Do NOT send the >100pg original
    // and get a cryptic 400 that vanishes the line. Fail loud + actionable; the
    // caller records it in _sources and the completeness guard then blocks the
    // RFP and names the missing line.
    const err = new Error(`policy PDF has ${src.getPageCount()} pages (over the 100-page extraction limit) and could not be trimmed automatically — upload just the declarations pages for this coverage line.`);
    err.code = 'PDF_UNTRIMMABLE_OVERSIZE';
    throw err;
  }
}
...

{
  "entity": {
    ...
    "year_built_or_established": "<the physical construction year of the common-area structure OR the year the association was established. NEVER a policy effective, issue, or expiration date. If not clearly shown as a build/established year, null>",
...
async function extractOne(anthropic, buffer) {
  const safe = await capPdfPages(buffer); // never let a >100pg policy 400 + vanish
  const r = await anthropic.messages.create({
    model: EXTRACTION_MODEL, max_tokens: 8000,
    messages: [{ role: 'user', content: [
      { type: 'document', source: { type: 'base64', media_type: 'application/pdf', data: safe.toString('base64') } },
...
module.exports = { extractInsuranceProgram, extractOne, capPdfPages, SCHEMA_PROMPT, EXTRACTION_MODEL, MAX_EXTRACT_PAGES };
