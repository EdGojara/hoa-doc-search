...
const { ocrPdfWithClaude } = require('./ocr_pdf');
...
// text came from Claude vision OCR rather than embedded text in the PDF, so
...
async function extractFullTextFromFile(buffer, filename) {
  ...
  if (ext === 'pdf') {
    ...
    try {
      const ocrText = await ocrPdfWithClaude(buffer, filename);
