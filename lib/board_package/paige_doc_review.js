...
async function draftPaigeDocReview({ email, attachments = [], supabase, community, senderFirstName = null }) {
  ...
  // Do one better: when something is actually wrong, build the corrected
  // version rather than describing it. Ed 2026-08-21: "if paige can just do one
  // better have paige send it to martha."
  //
  // Only when there is a reason. Handing Martha a replacement for a document
  // that is correct on every checkable point is both wrong and the fastest way
  // to get an assistant ignored.
  // Named for what it is. `attachments` is already the parameter holding what
  // MARTHA sent; shadowing it here would make "attachments" mean two opposite
  // things twenty lines apart.
  let replacement = [];
  if (blocking.length || checks.length) {
    try {
      const { buildCallForNominations } = require('../nominations/paige_nominations');
      if (typeof buildCallForNominations === 'function') {
        replacement = await buildCallForNominations({ supabase, community, cycle, truth });
      }
    } catch (e) { console.warn('[paige_doc_review] could not build a replacement:', e.message); }
  }
  ...
  if (!blocking.length && !checks.length) {
    ...
    // Only claim it because it exists — see the note above the schedule.
    body += `I've attached a corrected version built from the cycle: ${v.seats_open} seat${v.seats_open === 1 ? '' : 's'}, ${v.term_years}-year term, the dates above, and the nomination link on it. Use it or fix yours, whichever you prefer.${sign}`;
  } else {
  ...
  return {
    ...
    review_hint: `Paige reviewed ${target.filename}: ${blocking.length} blocking · ${checks.length} to check`
      + (replacement.length ? ' · corrected version attached' : ''),
  };
}
