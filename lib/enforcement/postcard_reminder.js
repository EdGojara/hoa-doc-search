...
async function renderPostcardReminderPdf(ctx) {
  // Regulatory guard — postcard reminders are covenant enforcement,
  // NOT debt collection. Warns on bankruptcy/at_legal but does NOT
  // block. The §362 stay doesn't reach covenant enforcement.
  if (ctx.supabase && ctx.property?.id) {
    const { assertCanSendCommunication } = require('./regulatory_guard');
    const warn = await assertCanSendCommunication(ctx.supabase, ctx.property.id, 'postcard_reminder');
    if (warn) {
      console.warn(`[postcard_reminder] regulatory warning for property ${ctx.property.id}: ${warn.warning}`);
    }
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    installDemoWatermark(doc, { community: c });
    const chunks = [];
