...
// ----------------------------------------------------------------------------
// Recipient address block for a #10 window-envelope fold. The last line MUST be
// "City, ST ZIP" on ONE line (USPS standard). The previous renderer split on
// every ", " which tore "Richmond, TX 77407" onto two lines.
//   - multi-line mailing_address: trust its line breaks as-is.
//   - flat "Street, City, ST ZIP": split on commas, then KEEP the final two
//     tokens (city + "ST ZIP") joined as one line.
//   - no mailing_address: build from the property's city/state/zip.
// ----------------------------------------------------------------------------
function formatMailingLines(o, p) {
  const raw = (o && o.mailing_address ? String(o.mailing_address) : '').trim();
  if (raw && /\n/.test(raw)) {
  ...
  if (raw) {
    ...
    if (parts.length >= 3) {
      // [...street, city, "ST ZIP"] → street line(s) + "City, ST ZIP"
      return [...parts.slice(0, -2), parts.slice(-2).join(', ')];
    }
    return parts; // 1–2 tokens: leave as-is
  }
  const streetLine = `${(p && p.street_address) || ''}${p && p.unit ? ' #' + p.unit : ''}`.trim();
  const cityStateZip = `${(p && p.city) || ''}, ${(p && p.state) || 'TX'} ${(p && p.zip) || ''}`
...
module.exports = { renderViolationLetterPdf, renderViolationLetterBundlePdf, formatMailingLines };
