...
async function renderViolationLetterBundlePdf(ctx) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // -----------------------------------------------------------------------
    // HEADER — Association lockup
    // -----------------------------------------------------------------------
    // Bedrock cornerstone mark (right corner, subtle agent identity)
    const csW = 24, csH = 30;
    ...
    doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(13.5).text(hoaName, PAGE.margin, y);
    doc.fillColor(MUTED).font('Helvetica').fontSize(9.5)
       .text(`c/o ${BRAND.service.legal}`, PAGE.margin, y + 16)
       .text(BRAND.service.address, PAGE.margin, y + 28)
       .text(BRAND.service.addressCityStateZip + '  ·  ' + BRAND.service.phone, PAGE.margin, y + 40);
    ...
    if (ctx.wide_photo_buffer) {
      try {
        ...
        y += 14;
        doc.image(ctx.wide_photo_buffer, PAGE.margin, y, { fit: [contentWidth, 180], align: 'left' });
        y = doc.y + 6;
    ...
    for (let i = 0; i < violations.length; i++) {
      ...
      if (v.close_up_photo_buffer) {
        try {
          ...
          doc.image(v.close_up_photo_buffer, PAGE.margin, y, { fit: [contentWidth, 180], align: 'left' });
          y = doc.y + 4;
    ...
    // -----------------------------------------------------------------------
    // FEE NOTICE (admin fee for the mailing) + § 209.006(b)(1) disclosure
    // -----------------------------------------------------------------------
    const totalFineAmount = isFine
      ? violations.reduce((sum, v) => sum + (Number(v.fine_amount) || 0), 0)
      : 0;
    if (feeCents > 0 || totalFineAmount > 0) {
      if (y > PAGE.height - 140) { doc.addPage(); y = PAGE.margin; }
      doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(10.5)
         .text(isFine ? 'Fine and fee summary' : 'Administrative fee', PAGE.margin, y);
      y = doc.y + 4;
      if (feeCents > 0) {
        doc.fillColor(INK).font('Helvetica').fontSize(10.5)
           .text(`A $${feeDollars.toFixed(2)} administrative fee covers the cost of ${isCertified ? 'certified mailing and processing' : 'this notice mailing and processing'}. The fee is one-time for this notice.${isCertified ? ' This disclosure is included to comply with Tex. Prop. Code §209.006(b)(1).' : ''}`,
                 PAGE.margin, y, { width: contentWidth, lineGap: 2 });
        y = doc.y + 6;
      }
      if (totalFineAmount > 0) {
        doc.fillColor(INK).font('Helvetica').fontSize(10.5)
           .text(`Total fine${isMulti ? 's' : ''} assessed: $${totalFineAmount.toFixed(2)}. The Association's full enforcement and fine policy is available upon request.`,
                 PAGE.margin, y, { width: contentWidth, lineGap: 2 });
        y = doc.y + 6;
      }

      // Payment routing
      if (paymentUrl || payToAddress) {
        doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(10).text('Payment options', PAGE.margin, y);
        y = doc.y + 2;
        if (paymentUrl) {
          doc.fillColor(INK).font('Helvetica').fontSize(10.5)
             .text(`Online: ${paymentUrl} (payment portal for this Association)`, PAGE.margin + 14, y);
          y = doc.y + 1;
        }
        if (payToAddress) {
          doc.text(`By mail: check payable to ${payToName}, ${payToAddress}`, PAGE.margin + 14, y, { width: contentWidth - 14, lineGap: 1.5 });
          y = doc.y + 1;
        }
        y += 6;
      }
    ...
    doc.fillColor(MUTED).font('Helvetica-Oblique').fontSize(8.5)
       .text(`This community is professionally managed by ${BRAND.service.legal}.`,
             PAGE.margin, footerY - 2, { width: contentWidth, align: 'center' });
    doc.fillColor(MUTED).font('Helvetica').fontSize(8)
       .text(`${BRAND.service.addressInline}  ·  ${BRAND.service.phone}  ·  ${BRAND.service.email}  ·  ${BRAND.service.website}`,
             PAGE.margin, footerY + 8, { width: contentWidth, align: 'center' });
