...
async function renderViolationLetterBundlePdf(ctx) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // HOA name + address block. Reserve right-side gutter for the bigger
    // Bedrock mark so the text wraps don't collide with it.
    const headerTextWidth = csX - nameX - 14;
    doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(14).text(hoaName, nameX, y + 2, { width: headerTextWidth });
    doc.fillColor(MUTED).font('Helvetica').fontSize(9.5)
       .text(`c/o ${BRAND.service.legal}`, nameX, y + 22, { width: headerTextWidth })
       .text(BRAND.service.address, nameX, y + 34, { width: headerTextWidth })
       .text(BRAND.service.addressCityStateZip + '  ·  ' + BRAND.service.phone, nameX, y + 46, { width: headerTextWidth });
