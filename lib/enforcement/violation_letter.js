...
async function renderViolationLetterBundlePdf(ctx) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isFirstCo) {
      ...
      // Courtesy 2 — same preamble structure but notes the prior notice
      // and the cure paragraph below carries a hard date (Ed's addition;
      // Vantaca itself uses soft language even on Second Notice).
      opening = `The Association is responsible for administering and enforcing the community's governing documents for the benefit of all owners and to help preserve the appearance and value of the community. A follow-up inspection on ${inspectionDateStr} found that the condition${isMulti ? 's' : ''} previously noted at your property ${isMulti ? 'remain' : 'remains'} out of compliance with the Declaration of Covenants, Conditions and Restrictions and/or Rules and Regulations for your Association. Specifically, the following ${isMulti ? 'continue' : 'continues'} to be out of compliance:`;
