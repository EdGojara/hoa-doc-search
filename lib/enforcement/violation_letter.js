...
async function renderViolationLetterBundlePdf(ctx) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // -----------------------------------------------------------------------
    // AUTHORITY STATEMENT — courtesy 1 reads warmer when this lives at the
    // BOTTOM (just above the footer) as a small disclosure. Courtesy 2 and
    // certified stages keep it up here next to the opening so legal posture
    // is unambiguous from the first read.
    // -----------------------------------------------------------------------
    ...
    // Skipped on courtesy 1 — defensive documentation matters at §209 + fine
    // stages, not the soft first ask. Saves a page of vertical real estate.
    // -----------------------------------------------------------------------
    if (ctx.wide_photo_buffer && !isFirstCo) {
      try {
    ...
    for (let i = 0; i < violations.length; i++) {
      ...
      if (isFirstCo) {
        ...
        // Format: "Per [Article X, Section Y] of the Association's Declaration,
        // [quote sentence]." Falls back to just the reference when no quote.
        if (v.governing_doc && v.governing_doc.reference) {
          const refLine = v.governing_doc.reference.replace(/^the\s+/i, ''); // strip leading "the"
          const lead = `Per ${refLine.charAt(0).toLowerCase() + refLine.slice(1)},`;
    ...
    doc.fillColor(INK).font('Helvetica').fontSize(11);
    // "Sincerely" reads warmer for courtesy 1 than "Respectfully" — matches
    // the existing Bedrock courtesy voice.
    doc.text(isFirstCo ? 'Sincerely,' : 'Respectfully,', PAGE.margin, y);
    y += 30;
    ...
    // Courtesy 1: authority statement lives at the bottom of the body, just
    // above the page footer — keeps the warm tone in the body and the
