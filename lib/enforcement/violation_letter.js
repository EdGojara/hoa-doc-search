...
async function renderViolationLetterBundlePdf(ctx) {
  ...
  // Title matches the warmer Vantaca convention for courtesy stages.
  // Certified + fine stages keep formal statutory titles.
  const title = isFine
    ? 'NOTICE OF FINE ASSESSMENT'
    : isCertified
    ? 'FORMAL NOTICE OF COVENANT VIOLATION'
    : isSecond
    ? 'Second Notice — Covenant Violation'
    : 'Courtesy Notice';
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isFirstCo) {
      // Soft ask — no hard date on courtesy 1. Mirrors the existing Bedrock
      // courtesy voice: "We're sure that this was just an oversight..."
      doc.fillColor(INK).font('Helvetica').fontSize(11)
         .text(`We're sure that this was just an oversight, and the Board respectfully requests that you promptly correct ${isMulti ? 'the items above' : 'the matter'} upon receipt of this notice. If you do not feel that you are in non-compliance of the above-cited provisions and this letter has been sent in error, please contact our office at ${BRAND.service.email} or ${BRAND.service.phone} so we can resolve this with you.`,
               PAGE.margin, y, { width: contentWidth, lineGap: 2.5 });
      y = doc.y + 10;
      doc.fillColor(INK).font('Helvetica').fontSize(11)
         .text(`Please remember that this letter is being sent as a courtesy. We appreciate your cooperation in correcting this matter as quickly as possible. Thank you for doing your part to keep the community a beautiful place to live.`,
               PAGE.margin, y, { width: contentWidth, lineGap: 2.5 });
      y = doc.y + 10;
    } else if (isSecond) {
      // Courtesy 2 — narrative cure paragraph with a HARD DATE inlined
      // (Ed's addition; Vantaca itself keeps Second Notice soft). Closes
      // with the §209 escalation warning so the homeowner understands the
      // next step is certified mail + fines.
      doc.fillColor(INK).font('Helvetica').fontSize(11)
         .text(`The above-cited violation${isMulti ? 's need' : ' needs'} to be cured in order to bring your lot into compliance with the Declaration of Covenants, Conditions and Restrictions. Please correct the matter on or before ${fmtLongDate(cureBy)}.`,
               PAGE.margin, y, { width: contentWidth, lineGap: 2.5 });
      y = doc.y + 10;
      doc.fillColor(INK).font('Helvetica').fontSize(11)
         .text(`If the matter remains uncured after that date, our next correspondence will be a certified notice under Texas Property Code §209, which preserves the Association's right to assess fines and recover attorney fees and costs. We would much rather resolve this with you — if there are any extenuating circumstances, please contact our office at ${BRAND.service.email} or ${BRAND.service.phone}.`,
               PAGE.margin, y, { width: contentWidth, lineGap: 2.5 });
      y = doc.y + 10;
      doc.fillColor(INK).font('Helvetica').fontSize(11)
         .text(`If you believe this letter was sent in error and you are in compliance with the above-cited provisions, please log into your owner portal at ${BRAND.service.paymentUrl ? BRAND.service.paymentUrl : 'home.bedrocktx.com'} and submit a Non-compliance comment on the My Items page.`,
               PAGE.margin, y, { width: contentWidth, lineGap: 2.5 });
      y = doc.y + 10;
      doc.fillColor(INK).font('Helvetica').fontSize(11)
    ...
    doc.fillColor(INK).font('Helvetica').fontSize(11);
    // "Sincerely" reads warmer for courtesy stages than "Respectfully" —
    // matches the existing Bedrock courtesy voice.
    doc.text((isFirstCo || isSecond) ? 'Sincerely,' : 'Respectfully,', PAGE.margin, y);
    y += 30;
