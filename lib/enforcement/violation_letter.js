...
const { formatMailingLines } = require('../mail/address_block');
...
// Photo timestamp — date + time + Central timezone, formatted for visible
// printing on the letter as legal evidence of when the photo was captured.
// Examples:
//   "Mon, Jun 1, 2026 at 10:43 AM Central"
//   "Sat, May 30, 2026 at 4:17 PM Central"
// Falls back to empty string if no date provided.
function fmtPhotoTimestamp(d) {
  if (!d) return '';
  const date = d instanceof Date ? d : new Date(d);
  if (isNaN(date.getTime())) return '';
  // Format in America/Chicago (the canonical Bedrock operations timezone)
  // so the timestamp shown to homeowners matches Central wall-clock time
...
// Build the letter salutation from an owner's full_name. Owner records come in
// many shapes: "Jose Alvarez", "Chadi & Eva Abou-Mourad", "JOSE M ALVAREZ",
// "Lennar Homes LLC", "The Werdenberg Living Trust", "Hurdoiu Enterprise LLC
// Series 2", or messy comma imports. Rules, in order:
//   1. Entities (LLC/Trust/LP/Inc/…) — address by the entity name, never a
//      personal honorific. (This is the "Dear Mr. 2" bug: the old code split on
//      spaces and took "2" from "…Series 2" as a surname.)
//   2. Couples ("X & Y") — "Dear X and Y,".
//   3. Comma format — reorder a clean "Last, First"; otherwise just clean it.
//   4. Individuals — the full name, title-cased.
// We do NOT prefix Mr./Mrs.: the system stores no gender, so any honorific is a
// guess that misgenders people on a legal notice. Full name is professional and
// safe. Empty/garbage → "Dear Property Owner,".
function buildSalutation(ownerName) {
  if (!ownerName || !String(ownerName).trim()) return 'Dear Property Owner,';
  const raw = String(ownerName).trim().replace(/\s+/g, ' ');

  const ENTITY = /\b(llc|l\.l\.c|inc|incorporated|corp|corporation|co\.|company|ltd|limited|lp|llp|trust|series|enterprises?|properties|holdings|partners|partnership|ventures?|group|management|mgmt|investments?|capital|realty|associates|bank|n\.?a|foundation|estate)\b/i;
  if (ENTITY.test(raw)) return `Dear ${raw},`;

  // Title-case a personal name: "jose alvarez"/"JOSE ALVAREZ" → "Jose Alvarez".
  // Capitalizes the first letter of each word (also after - and ').
  const tc = (s) => s.toLowerCase().replace(/(^|[\s'-])([a-z])/g, (m, sep, ch) => sep + ch.toUpperCase());

  // Couples — "X & Y" / "X and Y". Title-case each side, join with lowercase "and".
  if (/&|\s+and\s+/i.test(raw)) {
    const people = raw.split(/\s*&\s*|\s+and\s+/i).map((p) => p.trim()).filter(Boolean);
    if (people.length) return `Dear ${people.map(tc).join(' and ')},`;
  }

  // Comma format from imports.
  if (raw.includes(',')) {
    const parts = raw.split(',').map((p) => p.trim()).filter(Boolean);
    // Clean "Last, First" (exactly two parts, no suffix junk) → "First Last".
    if (parts.length === 2 && !/\b(jr|sr|ii|iii|iv|v)\b\.?/i.test(parts[0])) {
      return `Dear ${tc(parts[1] + ' ' + parts[0])},`;
    }
    return `Dear ${tc(parts.join(' '))},`;
  }

  return `Dear ${tc(raw)},`;
}

// Recipient address block (City, ST ZIP on one line) now lives in the shared
// lib/mail/address_block.js so every mailed artifact formats addresses
// identically. formatMailingLines is imported at the top + re-exported below.
 ...
 */
// Compress an embedded photo. Inspection photos arrive at full camera
// resolution (~1MB+ each); embedded uncompressed they bloat a batch PDF past
// 70MB, which is slow to build and won't reliably download (Ed 2026-06-26: a
// 49-letter LOPF batch hit 71MB and the print download timed out). The photo
// displays ~470px wide on the page, so 1000px @ q0.75 is retina-sharp and lands
// ~80-220KB. Uses node-canvas (the one image lib available here). ALWAYS falls
// back to the original buffer on any failure — a compression hiccup must never
// drop the evidence photo from a legal notice.
async function _compressLetterPhoto(buffer, maxW = 1000, quality = 0.75) {
  if (!buffer || !Buffer.isBuffer(buffer) || buffer.length < 200 * 1024) return buffer;
  try {
    ...
    const out = cv.toBuffer('image/jpeg', { quality });
    return (out && out.length < buffer.length) ? out : buffer; // never grow the file
  } catch (e) {
    console.warn('[violation_letter] photo compress failed, using original:', e.message);
    return buffer;
  }
}

async function renderViolationLetterBundlePdf(ctx) {
  // REGULATORY GUARD — DRV violation letters are covenant enforcement,
  // NOT debt collection. §362 stay does not bar them. So the guard for
  // this type returns a warning (not a throw) when the property is in
  // bankruptcy/at_legal — we log the warning, let render proceed.
  // Hard throws only happen if the property is in a state that
  // unambiguously prohibits the type (none today for violation_letter).
  if (ctx.supabase && ctx.property?.id) {
    const { assertCanSendCommunication } = require('./regulatory_guard');
    const warn = await assertCanSendCommunication(ctx.supabase, ctx.property.id, 'violation_letter');
    if (warn) {
      console.warn(`[violation_letter] regulatory warning for property ${ctx.property.id}: ${warn.warning}`);
    }
  }
  ...
  // Compress embedded photos before they hit the page (see _compressLetterPhoto).
  if (ctx.wide_photo_buffer) ctx.wide_photo_buffer = await _compressLetterPhoto(ctx.wide_photo_buffer);
  for (const _v of violations) {
    if (_v && _v.close_up_photo_buffer) _v.close_up_photo_buffer = await _compressLetterPhoto(_v.close_up_photo_buffer);
  }
  const opts = ctx.options || {};
  const letterDate = ctx.letter_date ? new Date(ctx.letter_date) : new Date();
  // Operator cure-days override (migration 247) wins over the per-community
  // stage default. Cure still runs from letterDate (= postmark at mail time).
  const cureDays = (ctx.cure_days_override && Number(ctx.cure_days_override) > 0)
    ? Number(ctx.cure_days_override)
    : deriveCureDays(stage, c);
  ...
  return new Promise((resolve, reject) => {
    ...
    // Demo watermark — no-op for production communities. Fires for Drama
    // Creek (via is_demo flag OR hardcoded UUID allowlist).
    const { installDemoWatermark } = require('../pdf/demo_watermark');
    installDemoWatermark(doc, { community: ctx?.community || null });
    const chunks = [];
    ...
    // Bedrock cornerstone mark (right, agent identity at proper visual weight).
    ...
    // refresh). Lives at public/brand-assets/bedrock-mark-email-2x.png —
    // same single source of truth as every other branded surface. If the
    // file is missing (shouldn't happen in production) we silently skip.
    //
    // Sized 2026-06-07: the asset is a horizontal lockup (cornerstone +
    // 'BEDROCK' wordmark) at 1200x356 — aspect 3.37:1. Earlier 48x60
    // portrait box squeezed it to ~48x14 effective. Bumped to 160x48
    // matching the asset's natural ratio — Bedrock now has real
    // visual presence next to the HOA name. Community logo (64x64 box,
    // left side) stays larger by area so Association-as-principal
    // visual hierarchy is preserved.
    const csW = 160, csH = 48;
    const csX = PAGE.width - PAGE.margin - csW;
    const csY = y + 8;  // vertically center against the HOA name + address block
    try {
    ...
    // fits a 64×64 box on the left of the masthead.
    const logoBoxW = 64, logoBoxH = 64;
    ...
    // HOA name only on the left. Ed 2026-06-10: the c/o + Bedrock address +
    // phone block sat directly under the HOA name, but on long association
    // names that wrap to 2 lines the second line collided into the c/o text,
    // making the masthead look like a typo soup. The Bedrock mark + wordmark
    // on the right is sufficient agent identity; full mailing address +
    // phone live in the footer where postal regulators expect them. Result:
    // HOA name = principal, Bedrock mark = agent, no overlap risk.
    const headerTextWidth = csX - nameX - 14;
    doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(14).text(hoaName, nameX, y + 12, { width: headerTextWidth });

    doc.moveTo(PAGE.margin, y + 66).lineTo(PAGE.width - PAGE.margin, y + 66)
    ...
    y += 14;
    const mailingLines = formatMailingLines(o, p);
    ...
    if (isFirstCo) {
      ...
      // Only claim "documented prior notices" when there actually ARE prior
      // notices on record. A certified §209 letter is sometimes sent as a FIRST
      // notice (Ed 2026-07-31) — asserting prior notices we don't have is a false
      // statement in a legal notice. Gate on the same prior_notices signal the
      // hearing-history section uses below.
      const _hasPriorNotices = violations.some((v) => Array.isArray(v.prior_notices) && v.prior_notices.length > 0);
      if (_hasPriorNotices) {
        opening = isMulti
        ...
        opening = isMulti
          ? `This is a formal notice under Texas Property Code §209 regarding ${violations.length} ongoing covenant violations at the above property. Each violation is detailed below.`
          : `This is a formal notice under Texas Property Code §209 regarding an ongoing covenant violation at the above property.`;
      }
    } else if (isFine) {
      opening = isMulti
    ...
    if (ctx.wide_photo_buffer && !isFirstCo && !isSecond) {
      try {
        ...
        y = imgY + 180 + 6;
        // Use the first violation's observation timestamp as the wide-shot
        // capture time (wide and close-up are typically captured seconds
        // apart on the same inspection walk).
        const wideTimestamp = (violations[0] && violations[0].observation_captured_at)
          ? fmtPhotoTimestamp(violations[0].observation_captured_at)
          : '';
        // No emoji — PDFKit's default Helvetica doesn't include Unicode
        // emoji glyphs and the camera character was rendering as garbage
        // bytes that corrupted the surrounding text. Plain bold "Captured:"
        // reads cleaner on a legal document anyway.
        doc.fillColor(INK).font('Helvetica-Bold').fontSize(10)
           .text('Captured: ', PAGE.margin, y, { continued: true })
           .font('Helvetica').fontSize(10)
           .text(wideTimestamp || inspectionDateStr, { continued: false });
        y = doc.y + 2;
        doc.fillColor(MUTED).font('Helvetica').fontSize(8.5)
           .text(`Wide view of ${propRefLine}`, PAGE.margin, y);
        y = doc.y + 14;
      } catch (e) {
    ...
    for (let i = 0; i < violations.length; i++) {
      ...
      if (isFirstCo || isSecond) {
        ...
        // number; we compose the "of the [document]" wrapper here.
        if (v.governing_doc && v.governing_doc.reference) {
          ...
          // Generic-but-defensible fallback when no specific citation was
          // resolved (governing_doc lookup returned null because the
          // substrate didn't surface a clean subject-matched section).
          // The violation block should always END with a sentence that
          // ties the observation to the Declaration — even without a
          // specific Article/Section. Better than leaving the block
          // hanging after the description; matches the defensive posture
          // used in the Authority Statement at the bottom of the letter.
          doc.fillColor(INK).font('Helvetica').fontSize(11)
             .text(`This condition may be inconsistent with the maintenance and use restrictions of the Declaration of Covenants, Conditions and Restrictions for ${hoaName}, which require owners to maintain their Lot, including all landscaping and improvements thereon, in compliance with the community's covenant standards.`,
                   PAGE.margin, y, { width: contentWidth, lineGap: 2.5 });
          y = doc.y + 8;
        }
        ...
        if (v.close_up_photo_buffer) {
          try {
            ...
            if (v.observation_captured_at) {
              // Visible legal timestamp: date + time + Central timezone in
              // dark bold text so the printed letter clearly establishes
              // when the photo was captured.
              doc.fillColor(INK).font('Helvetica-Bold').fontSize(10)
                 .text('Captured: ', PAGE.margin, y, { continued: true })
                 .font('Helvetica').fontSize(10)
                 .text(fmtPhotoTimestamp(v.observation_captured_at), { continued: false });
              y = doc.y + 14;
            }
          } catch (e) {
      ...
      if (v.governing_doc && (v.governing_doc.reference || v.governing_doc.section_title)) {
        ...
        // Generic-but-defensible fallback when no specific citation was
        // resolved. Always anchor the violation in the Declaration —
        // never leave the structured block hanging after the description.
        doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(9.5)
           .text('Governing document reference', PAGE.margin, y);
        y = doc.y + 2;
        doc.fillColor(INK).font('Helvetica').fontSize(10)
           .text(`Declaration of Covenants, Conditions and Restrictions for ${hoaName} — maintenance and use restrictions governing owner obligations.`,
                 PAGE.margin, y, { width: contentWidth, lineGap: 1.5 });
        y = doc.y + 6;
      }
      ...
      if (v.close_up_photo_buffer) {
        try {
          ...
          y = imgY + 180 + 6;
          if (v.observation_captured_at) {
            // Visible legal timestamp under each close-up: date + time +
            // Central timezone, dark bold, so the printed letter clearly
            // establishes when the photo evidence was captured. No emoji
            // — PDFKit's default Helvetica doesn't include Unicode emoji
            // glyphs.
            doc.fillColor(INK).font('Helvetica-Bold').fontSize(10)
               .text('Captured: ', PAGE.margin, y, { continued: true })
               .font('Helvetica').fontSize(10)
    ...
    if (isFirstCo) {
      ...
      if (isSecond) {
        ...
        // Per Ed 2026-06-07: make the predicate-certification role of THIS
        // letter explicit. The certified §209 notice IS the prerequisite
        // written-notice + opportunity-to-cure that authorizes any future
        // fine assessment. Stating it inline closes the loop for the
        // homeowner (and any reviewing attorney) — "if you don't cure
        // by X, fines may be assessed BECAUSE this notice was given."
        doc.fillColor(INK).font('Helvetica').fontSize(10.5)
           .text(`If the violation${isMulti ? 's are' : ' is'} cured before this date, no fine may be assessed under Tex. Prop. Code §209.006(e). If not cured by this date, this notice serves as the certified §209.006 written notice and opportunity-to-cure that authorizes the Association to assess fines and pursue other enforcement remedies under the governing documents and applicable law, including recovery of attorney fees and costs where permitted.`,
                 PAGE.margin, y, { width: contentWidth, lineGap: 2 });
        y = doc.y + 6;
...
module.exports = { renderViolationLetterPdf, renderViolationLetterBundlePdf, formatMailingLines, buildSalutation };
