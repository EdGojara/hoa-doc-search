...
function buildAuthorityStatement(stage, hoaName) {
  let txt =
    `Authority: This notice is issued by ${BRAND.service.legal}, managing agent for ${hoaName}, ` +
    `acting on behalf of and at the direction of the Board of Directors pursuant to the Association's ` +
    `Declaration of Covenants, Conditions, and Restrictions (CC&Rs), which grants the Board the authority ` +
    `to enforce the community's architectural and covenant standards.`;
...
async function renderViolationLetterBundlePdf(ctx) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // -----------------------------------------------------------------------
    const authorityText = buildAuthorityStatement(stage, hoaName);
