...
//                          is less specific.
function buildAuthorityStatement(stage, hoaName, enforcementCitation, substantiveCitation, isMulti) {
  ...
  // CLAIM 2 — Substantive obligation. Use the specific citation only on
  // single-violation letters; bundles get the generic formulation because
  // multiple substantive provisions are in play (each already cited in the
  // body of the letter alongside its respective violation block).
  let claim2;
  if (sub && !isMulti) {
    claim2 = `Pursuant to ${sub} of the Declaration of Covenants, Conditions and Restrictions, owners are required to maintain their lot in compliance with the community's standards.`;
  } else {
    claim2 = `Pursuant to the provisions of the Declaration of Covenants, Conditions and Restrictions governing owner obligations, owners are required to maintain their lot in compliance with the community's standards.`;
  }

  // CLAIM 3 — Enforcement authority. Specific citation when known; generic
  // defensible fallback otherwise.
  const claim3 = enf
    ? `Pursuant to ${enf} of the Declaration, the Association is authorized to enforce compliance with these provisions by any lawful means available.`
    : `Pursuant to the provisions of the Declaration governing the Association's enforcement powers and remedies, the Association is authorized to enforce compliance with these requirements by any lawful means available.`;
...
async function renderViolationLetterBundlePdf(ctx) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // -----------------------------------------------------------------------
    // Substantive citation is the first violation's governing-doc reference
    // (e.g., "Article 7, Section 7.3" for a maintenance violation). Only
    // passed for single-violation letters — bundles use the generic
    // "owner obligations" formulation since multiple substantive provisions
    // are in play (each cited in its own block above).
