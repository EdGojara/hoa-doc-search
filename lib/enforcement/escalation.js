...
function decideEscalation(input) {
  ...
  // Exclude same-occurrence siblings BEFORE counting. A violation observed on
  // the SAME calendar day as the one being opened is the SAME occurrence — two
  // findings from one drive, or a deduped duplicate photo — NOT a prior repeat.
  // Counting it over-escalates a first occurrence straight to Courtesy 2 (the
  // Erika Helms lawn case, 2026-06-26: a same-drive duplicate bumped a first
  // occurrence up a level). We key on the occurrence DAY (the engine doesn't
  // receive an inspection id) and only the CURRENT day — historical priors,
  // including Vantaca rows that share an artificial import date, are untouched.
  // Every fresh open sets opened_at = now, so the default (today) is correct;
  // recompute callers can pass occurrence_date = the violation's opened_at.
  const _occDay = (input.occurrence_date ? new Date(input.occurrence_date) : new Date())
    .toISOString().slice(0, 10);
  const TERMINAL_STAGES = ['cured', 'closed', 'voided'];
  const effectivePriors = priorViolations.filter((v) => {
    // DEFENSIVE BACKSTOP (2026-06-29 certified-with-no-reason scar): a prior
    // that was voided/cured/closed has CLOSED its chain and must never count
    // toward §209 escalation — a new violation after it is a fresh first
    // notice. Callers should pre-filter at the DB, but several escalation
    // paths historically did not, so guard here too: drop any prior with
    // resolved_at set OR a terminal current_stage. (A voided/cured row keeps
    // its courtesy/certified stage and only sets resolved_at, so check both.)
    if (v.resolved_at) return false;
    if (TERMINAL_STAGES.includes(v.current_stage)) return false;
    if (!v.opened_at) return true;
    return new Date(v.opened_at).toISOString().slice(0, 10) !== _occDay;
  });

  const priorWeightedSum = effectivePriors.reduce((acc, v) => acc + weightFor(v), 0);
  const priorCount = effectivePriors.length;  // raw count for rationale display
  // Did any prior reach certified or fine stages? Signals chronic. Weight-aware.
  const everCertified = effectivePriors.some(
  ...
  const lowTrustPriors = effectivePriors.filter((v) => v.source && v.source !== 'trustEd_native').length;
