...
function decideEscalation(input) {
  ...
  // confidence_weight (default 1.0 for trustEd-native, 0.5 for vantaca_import,
  // 0.3 for predecessor_import, 0 for superseded). This prevents low-trust
  // legacy data from auto-escalating to certified mail.
  //
  ...
  //
  // Examples:
  //   1 verified trustEd prior (1.0)              → courtesy_2  ✓ (matches old logic)
  //   2 vantaca-import unreviewed (0.5+0.5=1.0)   → courtesy_2  ✓ (conservative)
  //   3 vantaca-import unreviewed (0.5*3=1.5)     → courtesy_2  ✓ (still doesn't trigger certified)
  //   1 verified + 2 vantaca (1.0+0.5+0.5=2.0)    → certified  ✓
  //   2 verified trustEd (1.0+1.0=2.0)            → certified  ✓
  //
  ...
  if (priorityWeight === 'elevated') {
    if (priorWeightedSum < 1.0) {
      return {
        ...
        cure_days: 21,
        requires_hearing: false,
        rationale: `Effectively first violation in 12 months for this category (weighted: ${wSumLabel}). Elevated priority → 21-day cure.${lowTrustNote}`,
      };
    }
  ...
  if (priorWeightedSum < 1.0) {
    return {
      ...
      cure_days: 30,
      requires_hearing: false,
      rationale: `Effectively first violation of this category in 12 months (weighted: ${wSumLabel}). Standard 30-day courtesy notice via regular mail.${lowTrustNote}`,
    };
  }
  if (priorWeightedSum < 2.0) {
    return {
      ...
      cure_days: 30,
      requires_hearing: false,
      rationale: `${priorCount} prior${priorCount === 1 ? '' : 's'} in 12 months (weighted: ${wSumLabel}). Courtesy 2 via regular mail with 30-day cure.${lowTrustNote}`,
    };
  }
