...
async function lookupGoverningDoc({
  ...
  try {
    ...
    //
    // WHY ONLY 3 (tested 2026-07-02): widening to 6 after the governing-doc
    // re-OCR made auto-lookup return MORE citations but WRONG ones on this §209
    // legal surface — it cited the Association's self-help REMEDY (Art VII
    // §7.12 "enter and cut the weeds") instead of the owner's OBLIGATION
    // (§3.12), and emitted garbled section refs ("Section (1)"). A confidently
    // wrong section on a §209 notice is worse than the safe generic-authority
    // fallback. Precise per-category citations come from the counsel-verified
    // manual override (community_enforcement_priorities), which wins before this
    // auto-lookup ever runs. Auto-lookup stays conservative on purpose.
