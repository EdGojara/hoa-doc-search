...
async function lookupGoverningDoc({
  ...
  try {
    ...
    // Prefer the highest-similarity chunk that ALSO (a) yields a parseable
    // Article+Section reference AND (b) isn't OCR-damaged. If no chunk
    // satisfies BOTH, return null and let the letter fall back to the
    // generic CC&R reference in the Authority statement — better than
    // printing "Section 2 of the governing documents" with garbage text.
    let chosen = null;
    let parsed = null;
    for (const ch of chunks) {
      if (Number(ch.similarity) < minSimilarity) continue;
      if (_looksOcrDamaged(ch.text)) continue;
      const ref = _extractReference(ch.text) || _extractReference(ch.section_heading);
      if (ref && ref.article && ref.section) {
        chosen = ch;
        parsed = ref;
        break;
      }
    ...
    const referenceLabel = `${parsed.full} of the ${_docKindLabel(chosen.document_title)}`;
