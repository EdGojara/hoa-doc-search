...
const DEAD = new Set(['voided', 'withdrawn', 'reclassified', 'duplicate', 'wrong_property', 'superseded']);
...
async function partitionMailable(supabase, letters) {
  ...
  for (let i = 0; i < vids.length; i += 300) {
    // NOTE: violations has NO `status` column — the terminal state lives across
    // current_stage ('voided' etc.), resolved_via ('voided'/'withdrawn'/'cured'/
    // 'fine'), and quality_status ('superseded'). Selecting a non-existent column
    // fails the WHOLE PostgREST query, which threw here and 500'd every
    // confirm-mailed run ("column violations.status does not exist", Ed
    // 2026-09-18). Read the columns that actually exist and derive the guard's
    // status from them.
    const { data, error } = await supabase
      .from('violations').select('id, current_stage, resolved_via, quality_status')
      .in('id', vids.slice(i, i + 300));
    if (error) throw error; // fail loud — never guess "no violation" on a query error
    for (const v of data || []) {
      stageBy.set(v.id, v.current_stage);
      // The guard's "status" = whichever terminal/quality marker is set.
      statusBy.set(v.id, v.resolved_via || v.quality_status || '');
    }
  }
