...
const PDF_CHUNK_PAGES = 40;   // safe under the 100-page Claude PDF cap with headroom
...
async function _extractChunkViaModel(chunkBuffer, chunkLabel) {
  ...
  try {
    const stream = _anthropic.messages.stream({
      ...
      max_tokens: 16000,  // dropped from 32k — 40-page chunks rarely need more
      messages: [{
        role: 'user',
        content: [
  ...
  try {
    ...
    const truncatedHint = completion.stop_reason === 'max_tokens'
      ? ' (output truncated — chunk had more rows than 32k tokens could express)'
      : '';
    return {
      rows: [],
