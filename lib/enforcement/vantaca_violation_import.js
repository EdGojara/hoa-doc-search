...
function parseVantacaViolations(buffer, filename, opts = {}) {
  ...
  // Grouped "Violation Report - Detail" export (Ed 2026-06-18): the report
  // groups violations under section headers ("Closed (Total Count = 901)",
  // "First Notice (Total Count = 118)", ...). The SECTION HEADER is the
  // authoritative CURRENT status — far more reliable than parsing a status
  // history string (and the thing the PDF AI path kept getting wrong). Detect
  // by the presence of "(Total Count = N)" header rows.
  const looksLikeGroupedReport = aoa.some((r) =>
    r && r[0] && /\(Total Count\s*=\s*\d+\)/.test(String(r[0])));
  if (looksLikeGroupedReport) {
    return _parseVantacaGroupedReport(aoa);
  }
...
// ----------------------------------------------------------------------------
// Parse the grouped "Violation Report - Detail" export (Excel/CSV). Each
// violation is a small multi-row block under a status section header:
//
//   First Notice (Total Count = 118)        <- section header = CURRENT status
//   XN | Account | Homeowner | Address | Hearing Date | Details   <- col labels
//   $68642 | 10110678 | Nam H Nguyen | 5327 Jay Thrush Drive | ... | Running a Business  <- identity row (col9 = category)
//   (blank)
//   First Notice - 01/06/2026 - Pennie      <- status row (col9 = status + date)
//
// We carry the current section status forward, emit one violation per identity
// row (stage from the section header), and take the event date from the
// following status row. This is deterministic — no model, no history-string
// guessing — so escalated cases land at their real stage with the right date.
// ----------------------------------------------------------------------------
function _parseVantacaGroupedReport(aoa) {
  const rows = [];
  const headerRe = /^(.+?)\s*\(Total Count\s*=\s*\d+\)/;
  const dateRe = /\s-\s(\d{1,2}\/\d{1,2}\/\d{2,4})\s-\s/;
  ...
  const flush = () => {
    if (!pending) return;
    const canonical = _normalizeStage(pending.group_status);
    const isTerminal = canonical === 'cured' || canonical === 'voided';
    rows.push({
      vantaca_account_id: pending.vantaca_account_id || null,
      street_address: pending.street_address || null,
      category_label: pending.category_label || null,
      opened_at: pending.opened_at,
      // Open stages carry the stage; terminal rows record the outcome + date.
      stage: isTerminal ? canonical : canonical,
      resolved_at: isTerminal ? pending.opened_at : null,
      resolved_via: isTerminal ? (canonical === 'voided' ? 'voided' : 'cured') : null,
      fine_amount: null,
      // Keep the section status label for traceability + the coverage panel.
      notes: pending.group_status || null,
      _source_row: pending.source_row,
    });
    pending = null;
  };
  ...
  for (let i = 0; i < aoa.length; i++) {
    const r = aoa[i] || [];
    const c0 = clean(r[0]);
    const hm = c0.match(headerRe);
    if (hm) { flush(); group = hm[1].trim(); continue; }
    if (c0 === 'XN' || clean(r[2]) === 'Account') continue;   // column-label row
    const account = clean(r[2]);
    const address = clean(r[5]);
    const col9 = clean(r[9]);
    const isIdentity = account && address && /\d/.test(account);
    if (isIdentity) {
      flush();
      pending = {
        vantaca_account_id: account,
        street_address: address,
        category_label: col9 || null,
        group_status: group,
        opened_at: null,
        source_row: i + 1,
      };
    } else if (pending && col9 && !pending.opened_at) {
      const dm = col9.match(dateRe);
      if (dm) {
        const [mm, dd, yy] = dm[1].split('/');
        const y = yy.length === 2 ? '20' + yy : yy;
        pending.opened_at = `${y}-${mm.padStart(2, '0')}-${dd.padStart(2, '0')}`;
      }
    }
  }
  flush();

  // Drop rows we couldn't anchor to a date — unusable, same rule as the other
  // parsers. (Observed: 0 on the Waterview export.)
  const usable = rows.filter((r) => r.opened_at && (r.vantaca_account_id || r.street_address));
  return {
    rows: usable,
    mapping: {
      street_address:     { header: 'Address (grouped report)',  columnIndex: 5 },
      vantaca_account_id: { header: 'Account (grouped report)',  columnIndex: 2 },
      category_label:     { header: 'Details (grouped report)',  columnIndex: 9 },
      opened_at:          { header: 'Status date (grouped report)', columnIndex: 9 },
      _source: 'vantaca_grouped_report',
    },
    headers: ['(grouped Violation Report - Detail — status taken from section headers)'],
    errors: usable.length === 0
      ? ['Grouped report detected but no violation rows parsed — verify this is the Violation Report Detail export.']
      : [],
  };
}
