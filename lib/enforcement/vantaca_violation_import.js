...
async function parseVantacaViolationsPdf(buffer, filename) {
  ...
  let completion;
  try {
    completion = await _anthropic.messages.create({
    ...
    return {
      rows: [], mapping: { source: 'pdf_extraction' }, headers: [],
      errors: [`Claude PDF extraction failed: ${err.message}`],
      raw_extracted: null,
      duration_ms: Date.now() - t0,
    };
  }
  ...
  // Per CLAUDE.md: log raw model output
  console.log('[vantaca_violation_import_pdf] Claude returned (first 800 chars):', text.slice(0, 800));
  ...
  try {
    ...
    const hint = stopReason === 'max_tokens'
      ? ' Model hit max_tokens; output truncated. Split the report into smaller PDFs and upload separately.'
      : '';
    return {
      rows: [], mapping: { source: 'pdf_extraction' }, headers: [],
      errors: [`PDF extraction returned malformed JSON.${hint} Parse error: ${err.message}`],
      raw_extracted: text.slice(0, 2000),
      duration_ms: Date.now() - t0,
    };
  }
  ...
  const rows = rawRows.map((r, i) => {
    // Use the same normalizer pipeline as the CSV path so downstream behavior
    // is identical. Stage normalization is the key step — Vantaca's stage
    // labels are wildly inconsistent ('1st Notice', 'Courtesy 1', 'C1', etc.)
  ...
  return {
    rows,
    mapping: { source: 'pdf_extraction', model: 'claude-sonnet-4-5' },
    headers: [],
    errors: rows.length === 0 ? ['No usable violation rows extracted from PDF.'] : [],
