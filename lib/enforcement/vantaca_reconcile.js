...
/**
 * Staleness closure (Ed 2026-06-18): a first/second notice with no recent
 * activity is almost certainly resolved — Vantaca just never closed it — so
 * carry it into the new period as CLOSED, not as active enforcement. A
 * certified §209 letter (or anything beyond it on the ladder) is NEVER
 * auto-closed: that's a formal legal step we close explicitly, governed by the
 * 180-day cert clock.
 *
 * "Recent" = the last two months present in the data. For a report whose
 * latest activity is in May, the cutoff is April 1 — anything older with an
 * open courtesy stage is recorded as cured.
 *
 * @param {Array} rows  parsed/resolved rows with { stage, opened_at, ... }
 * @param {Object} [opts]
 * @param {string} [opts.cutoff]  override the computed cutoff (YYYY-MM-DD).
 * @returns {{ rows, stale_closed, cutoff }}
 */
function markStaleCourtesyClosed(rows, opts = {}) {
  const list = Array.isArray(rows) ? rows : [];
  let cutoff = _dayMs(opts.cutoff);
  if (cutoff == null) {
    let maxMs = null;
    for (const r of list) { const d = _dayMs(r.opened_at); if (d != null && (maxMs == null || d > maxMs)) maxMs = d; }
    if (maxMs == null) return { rows: list, stale_closed: 0, cutoff: null };
    const md = new Date(maxMs);
    // First day of (latest month − 1): the last two months stay "recent".
    cutoff = Date.UTC(md.getUTCFullYear(), md.getUTCMonth() - 1, 1);
  }
  const cutoffISO = _fmt(cutoff);
  // Only pre-certified open stages are subject to staleness. certified_209 and
  // everything above it (fine_assessed, hearing_notice, …) are exempt.
  const STALE_STAGES = new Set(['courtesy_1', 'courtesy_2']);
  let count = 0;
  const out = list.map((r) => {
    if (!STALE_STAGES.has(r.stage)) return r;
    const d = _dayMs(r.opened_at);
    if (d == null || d >= cutoff) return r;
    count += 1;
    return {
      ...r,
      stage: 'cured',
      resolved_at: r.opened_at,
      resolved_via: 'cured',
      _stale_closed: true,
      notes: `${r.notes ? r.notes + ' ' : ''}[auto-closed: open ${r.stage} with no activity since ${r.opened_at} (before ${cutoffISO})]`,
    };
  });
  return { rows: out, stale_closed: count, cutoff: cutoffISO };
}
