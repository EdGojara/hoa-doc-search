...
async function buildReportData({ supabase, communityId, asOfDate }) {
  ...
  const { data: community, error: cErr } = await supabase
    .from('communities')
    .select('id, name, slug, mailing_address_city, mailing_address_state, mailing_address_zip')
    .eq('id', communityId)
    .maybeSingle();
  if (cErr || !community) {
  ...
  const rows = violations.map((v) => {
    ...
    const stageAsOf = letter ? letter.stage_at_send : 'courtesy_1';
    const stageEnteredAt = letter ? letter.sent_at : v.opened_at;
