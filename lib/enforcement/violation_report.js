...
async function buildReportData({ supabase, communityId, asOfDate, scope = 'active' }) {
  ...
  while (true) {
    const { data, error } = await supabase
      ...
      .select(`
        ...
        opened_at, last_continued_at, resolved_at, resolved_via, primary_category_id,
  ...
  if (!violations.length) {
    return { community, asOfDate, scope, rows: [], totals: emptyTotals(), generatedAt: new Date().toISOString() };
  }
  ...
  const rows = violations.map((v) => {
    ...
    return {
      ...
      last_continued_at: v.last_continued_at || null,
      days_open: daysOpen,
  ...
  // ---- Drive-scope filter (Ed 2026-07-09) --------------------------------
  // The month-end report reflects the CURRENT month's enforcement drive, not
  // every row that was never explicitly closed. Enforcement here is drive-
  // based: an inspector works the community each month. A courtesy notice
  // opened in a PRIOR month with no activity this month is almost always cured
  // — the cure just never got recorded (Vantaca rarely writes a close, and
  // last_continued_at is seldom set). Counting those as "open" inflates the
  // number with presumed-cured items.
  //
  // Keep a violation when EITHER:
  //   (a) it sits at certified §209 or beyond (fine / hearing / legal / lien) —
  //       deliberate, tracked escalations that persist until formally resolved,
  //       independent of the monthly drive; OR
  //   (b) it was observed or acted on during the report month (opened, entered
  //       its current stage, or was re-observed within [1st-of-month, asOf]).
  // Everything else is treated as presumed cured and excluded. scope='all'
  // bypasses the filter to show the full never-closed list.
  const monthStart = String(asOfDate).slice(0, 8) + '01';
  const ESCALATED_STAGES = new Set(['certified_209', 'fine_assessed', 'hearing_notice', 'legal_referral', 'lien_filed']);
  const inReportMonth = (d) => {
    const x = d ? String(d).slice(0, 10) : '';
    return x >= monthStart && x <= asOfDate;
  };
  let excludedPresumedCured = 0;
  const scopedRows = (scope === 'all')
    ? rows.slice()
    : rows.filter((r) => {
      const keep = ESCALATED_STAGES.has(r.stage_as_of)
        || inReportMonth(r.opened_at)
        || inReportMonth(r.stage_started_at)
        || inReportMonth(r.last_continued_at);
      if (!keep) excludedPresumedCured += 1;
      return keep;
    });
  rows.length = 0;
  ...
  totals.excluded_presumed_cured = excludedPresumedCured;
  for (const r of rows) {
    totals.by_stage[r.stage_as_of] = (totals.by_stage[r.stage_as_of] || 0) + 1;
    totals.by_category[r.category_label] = (totals.by_category[r.category_label] || 0) + 1;
  }

  return { community, asOfDate, scope, rows, totals, generatedAt: new Date().toISOString() };
}

function emptyTotals() {
  return {
    total_open: 0,
    excluded_presumed_cured: 0,
    by_stage: Object.fromEntries(STAGE_ORDER.map((s) => [s, 0])),
    by_category: {},
  };
}

// A bare 'YYYY-MM-DD' has no time/zone; `new Date('2026-06-29')` parses it as
// UTC midnight, which formats one day earlier in Central. Anchor bare dates at
// noon so the calendar day is preserved regardless of server/display timezone.
// Full ISO timestamps (with a time component) are left as-is.
function _asDate(s) {
  const str = String(s);
  return new Date(/^\d{4}-\d{2}-\d{2}$/.test(str) ? `${str}T12:00:00` : str);
}
...
function renderReportHtml({ community, asOfDate, rows, totals, generatedAt, scope = 'active' }) {
  ...
  // By-category summary — counts of open violations per category as of the
  // report date, sorted most-common first. This is the "violations by category
  // as of a date" cut boards ask for; the street sections below are the detail.
  const categoryRows = Object.entries(totals.by_category)
    .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
  const totalsByCategoryHtml = categoryRows.length ? `
    <section class="vr-catsummary">
      <h3 class="vr-section-title">${scope === 'all' ? 'Open' : 'Active'} violations by category</h3>
      <table class="vr-cat-tbl">
        <thead>
          <tr><th class="vr-col-cat">Category</th><th class="vr-num">Open as of ${esc(fmtDateUS(asOfDate))}</th></tr>
        </thead>
        <tbody>
          ${categoryRows.map(([cat, n]) => `
            <tr><td>${esc(cat)}</td><td class="vr-num">${n}</td></tr>`).join('')}
          <tr class="vr-cat-total"><td><strong>Total open</strong></td><td class="vr-num"><strong>${totals.total_open}</strong></td></tr>
        </tbody>
      </table>
    </section>` : '';
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .vr-catsummary { margin: 0 0 26px 0; page-break-inside: avoid; }
  .vr-section-title {
    font-family: 'Playfair Display', serif;
    font-size: 15px; font-weight: 600; color: var(--navy-deep);
    margin: 0 0 10px 0; padding-bottom: 6px; border-bottom: 1px solid var(--rule);
  }
  .vr-cat-tbl { width: 100%; border-collapse: collapse; max-width: 4.6in; }
  .vr-cat-tbl th {
    text-align: left; font-size: 10.5px; text-transform: uppercase; letter-spacing: 0.6px;
    color: var(--ink-faint); border-bottom: 1.5px solid var(--navy); padding: 4px 8px;
  }
  .vr-cat-tbl td { padding: 5px 8px; border-bottom: 1px solid var(--rule); }
  .vr-cat-tbl .vr-cat-total td { border-top: 1.5px solid var(--navy); border-bottom: none; background: var(--tint); }
  .vr-street {
    margin: 0 0 18px 0;
...
<div class="vr-page">
  <header class="vr-head">
    ...
    <h1 class="vr-title">${esc(community.name)} — ${scope === 'all' ? 'Open Violations' : 'Active Violations'}</h1>
    <div class="vr-sub">Status as of <strong>${esc(fmtDateLong(asOfDate))}</strong></div>
    ${scope === 'all'
      ? '<div class="vr-scope">Full list — every violation not formally closed, including older courtesy notices.</div>'
      : `<div class="vr-scope">Scope: this month's enforcement drive plus all certified &sect;209 and legal cases.${totals.excluded_presumed_cured > 0 ? ` <strong>${totals.excluded_presumed_cured}</strong> prior-month courtesy notice${totals.excluded_presumed_cured === 1 ? '' : 's'} with no activity this month ${totals.excluded_presumed_cured === 1 ? 'is' : 'are'} not shown (presumed cured).` : ''}</div>`}
  ...
  ${rows.length === 0 ? `
    <div class="vr-empty">
      <h2 style="font-family:'Playfair Display',serif; color:var(--navy-deep); margin:0 0 8px 0;">${scope === 'all' ? 'No open violations as of this date' : 'No active violations this month'}</h2>
      <p>${scope === 'all'
        ? `No violation was open in this community on ${esc(fmtDateLong(asOfDate))}.`
        : `No violation was on this month's drive or at certified &sect;209 / legal as of ${esc(fmtDateLong(asOfDate))}.${totals.excluded_presumed_cured > 0 ? ` (${totals.excluded_presumed_cured} prior-month courtesy notice${totals.excluded_presumed_cured === 1 ? '' : 's'} not shown — presumed cured.)` : ''}`}</p>
    </div>
  ` : `
    <section class="vr-summary">
      <div class="vr-stat vr-stat-headline">
        <div class="vr-stat-n">${totals.total_open}</div>
        <div class="vr-stat-l">${scope === 'all' ? 'Open violations' : 'Active violations'}</div>
      </div>
      ${totalsByStageHtml}
    </section>

    ${totalsByCategoryHtml}
    ${streetSections}
  `}
...
async function buildReport({ supabase, communityId, asOfDate, scope = 'active' }) {
  const data = await buildReportData({ supabase, communityId, asOfDate, scope });
