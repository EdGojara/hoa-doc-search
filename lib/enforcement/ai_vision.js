...
ONE OBJECT = ONE FINDING (critical — the mirror image of the rule above, and just as important):
- The rule above is about genuinely SEPARATE things. This rule is about the SAME thing: a single physical object or condition is exactly ONE finding with ONE best-fitting category. NEVER report the same object under two categories.
- Concrete failure to avoid: a single vehicle in a driveway must NOT appear as BOTH "Commercial vehicle" AND "RV / boat / trailer". Look at the object, decide what it actually is, and pick the ONE category that fits.
- The category MUST be consistent with what you can see. If the vehicle is a car or an SUV, it is NOT an RV, a boat, or a trailer — do not use those categories for it, even if it has a roof rack or cargo carrier. Only use "RV / boat / trailer" when the object actually IS an RV, boat, or trailer.
- When two categories seem to compete for one object, choose the one the visible evidence supports best and DROP the other. If you truly can't decide which single category fits, pick the closest one and set confidence='low' — never emit two findings for one object.
