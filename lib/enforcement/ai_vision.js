...
async function categorizePhoto(input) {
  ...
  try {
    ...
    const findings = rawFindings
      ...
      .map(f => ({
        // Ed 2026-07-31: real "grass in the expansion joints" cases are rare —
        // almost always it's just an overgrown lawn. Hard-default that category
        // to Lawn height for AI purposes so it can never surface on its own.
        category_slug:      (f.category_slug === 'grass_in_the_expansion_joints') ? 'lawn_height' : (f.category_slug || null),
