...
ONE PROBLEM = ONE FINDING (critical — count DISTINCT PHYSICAL PROBLEMS, not category labels):
- The rule above is about genuinely SEPARATE problems. This rule is about the SAME problem: a single physical object OR condition is exactly ONE finding with ONE best-fitting category. NEVER report the same problem under two categories.
- Before adding a finding, ask: is this a GENUINELY DIFFERENT physical problem, or the same problem a second category could also describe? If it is the same problem, emit it ONCE and drop the weaker category.
- Vehicles: a single vehicle must NOT appear as BOTH "Commercial vehicle" AND "RV / boat / trailer". Decide what the vehicle actually is and pick ONE. The category must match what you see — a car or SUV is NOT an RV/boat/trailer, even with a roof rack or cargo carrier.
- Lawn/landscaping is where over-splitting happens most — be strict: ONE lawn problem is ONE finding. A bare or dead lawn is a SINGLE finding (choose the single best of Sod Yard / Lawn dead patches — not several overlapping lawn categories, and NOT tree/shrub categories when there are no trees or shrubs). Grass growing over the driveway or curb edge is a SINGLE finding (Mow and Edge OR Grass in the expansion joints, whichever fits best — never both). Do not list Mow and Edge, Weeds, Lawn height, and Lawn dead patches all for the same patch of lawn; pick the one that best describes what is actually wrong.
- NEVER invent a category the photo does not support (e.g. "Prune Trees" or "Dead shrubs" when there are no trees or shrubs in view). Only report what you can actually see.
- When two categories compete for one problem, choose the one the visible evidence supports best and DROP the other. If you truly can't decide, pick the closest and set confidence='low' — never emit two findings for one problem.
- Sanity check before returning: would a reasonable inspector standing in front of this house say these are N SEPARATE problems? If two of your findings are really the same problem described two ways, merge them into the single best one.
