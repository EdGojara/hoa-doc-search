...
- Return AT MOST ONE finding — the single MOST BLATANT, clearly out-of-compliance violation in the photo.
- "Most blatant" = the most obvious rule violation a reasonable inspector would actually cite — NOT the biggest, most centered, or most eye-catching OBJECT. A large SUV, the house, or the mailbox often dominates the frame while the real violation is the overgrown grass at the curb, weeds, trash cans, peeling paint, or a broken fence. Staff frame the shot to capture a specific problem but nearby objects (cars, the garage) end up in view — look PAST the prominent object to the clear violation. Pick the VIOLATION, not the object.
- If the photo shows more than one issue, choose the ONE that is the most blatant / most defensible violation and report only that. Do NOT list several findings for one photo — a separate photo documents any other violation.
...
LOOK AT THE LAWN AND THE HOUSE FIRST — VEHICLES ARE ALMOST NEVER THE VIOLATION:
- These inspections are overwhelmingly about LAWN / TURF / LANDSCAPING and HOUSE / EXTERIOR maintenance. Before anything else, examine (1) the LAWN — height, tall/overgrown or shaggy grass, weeds, dead or bare patches, edging along the curb strip and bed edges; and (2) the HOUSE and yard — trash/recycling cans left at the curb, peeling or chipped paint, damaged/leaning fence or structures, debris. The real violation is almost always one of THESE.
- A vehicle is the LEAST likely violation and is almost never why the photo was taken. A car, truck, SUV, or van sitting in a driveway is COMPLIANT. If your first instinct is a vehicle category, stop and re-examine the lawn and the house — the actual violation is nearly always there. Only choose a vehicle category as a genuine last resort, and only with the unmistakable evidence described below.
- Pick the ONE category that best fits what you actually see. The category MUST match the photo:
    · A personal SUV, pickup truck, van, minivan, or sedan is a NORMAL passenger vehicle and is NEVER a Commercial vehicle, RV, boat, or trailer — not even with a roof rack, cargo carrier, or a toolbox in the bed. Only pick a Commercial vehicle when there is UNMISTAKABLE commercial evidence you can see: company logos/lettering on the vehicle, ladder or pipe racks loaded with work equipment, a box/utility truck, or commercial/DOT markings. If you are not certain it is commercial, it is a personal vehicle and NOT a violation — and a parked passenger vehicle is almost never the reason the photo was taken, so keep looking for the real violation (usually the lawn/turf).
    ...
    · Grass or weeds growing up through the expansion joints, seams, or cracks of the driveway, sidewalk, or curb is treated as an overgrown-lawn issue — classify it as "Lawn height" (lawn_height) and word the description around the lawn/grass needing to be mowed and edged. Do NOT lead the description with "expansion joints," and do NOT use the standalone "Grass in the expansion joints" category — real cases of that are rare, so it defaults to Lawn height. Tall, shaggy, or overgrown grass anywhere across the lawn, yard, or curb strip is likewise "Lawn height" (lawn_height) or "Mow and Edge" (mow_and_edge). Never let grass in the joints or cracks be the pick when the lawn simply needs mowing or edging.
    · We do NOT enforce fence staining, weathering, fading, or discoloration — do NOT use the "Fence Staining" category. A fence is only a violation when boards, pickets, or sections are missing, broken, rotted, leaning, or otherwise damaged → classify that as "Fence damage" (fence_damage) and describe the physical damage (e.g. missing or broken pickets, a leaning section), not the color or finish. A fence that is merely weathered or faded but structurally intact is NOT a violation.
    ...
    · An "inoperable / junk vehicle" violation requires that you SEE the vehicle is inoperable or abandoned — flat or missing tires, up on blocks or jacks, expired or missing license plates, shattered/broken windows, major body damage or heavy rust, or a car clearly under a tarp in long-term storage. A normal-looking, intact car parked in a driveway is NOT an inoperable vehicle just because it is sitting there — from a single photo you cannot tell whether it runs, so do NOT assume it is inoperable. Never pick an inoperable/junk-vehicle category unless the disrepair is plainly visible in the photo. When in doubt, it is a normal vehicle and NOT a violation — look to the lawn and the house instead.
...
- CATEGORY AND DESCRIPTION MUST MATCH. The description must describe the SAME condition as the category you chose. Never pair a category with a sentence about a different issue (e.g. a "Driveway repair" category with a sentence about grass, or a "Lawn height" category with a sentence about a fence). First decide the single blatant violation; then set BOTH the category and the description to that one thing. If the violation is a cracked driveway, category = driveway repair AND the sentence describes the driveway surface; if it is overgrown grass, category = a lawn category AND the sentence describes the grass.
...
async function categorizePhoto(input) {
  ...
  try {
    ...
    const findings = rawFindings
      ...
      .map(f => ({
        // Ed 2026-07-31: hard category defaults for categories we don't enforce
        // as their own thing, so they can never surface on their own:
        //  - grass in the expansion joints → almost always just an overgrown
        //    lawn → Lawn height.
        //  - fence staining (weathering/fading) → we don't enforce it; what's
        //    actually captured is missing/broken pickets → Fence damage.
        category_slug:      ({ grass_in_the_expansion_joints: 'lawn_height', fence_staining: 'fence_damage' }[f.category_slug]) || f.category_slug || null,
