...
async function findOrContinueViolation(input) {
  ...
  // A confirmed category alias (Vantaca "Trash Cans/Recycling Containers" ==
  // trustEd "trash_visible") means an open case under ANY sibling label is the
  // SAME real-world violation. Keying the lookup on the raw categoryId missed
  // those, so a re-observation under a sibling label opened a DUPLICATE case
  // and fired a courtesy letter on a property already at certified §209 for the
  // same issue (Ed 2026-07-01, 7610 Wolf Creek). Expand to the whole alias
  // group so the re-observation continues the existing case instead.
  const groupIds = await expandCategoryToAliases(categoryId);
  let lookup = supabase
    .from('violations')
    .select('id, current_stage, continuation_count, current_stage_started_at, cure_period_ends_at')
    .eq('property_id', propertyId)
    .not('current_stage', 'in', `(${OPEN_VIOLATION_STAGES_EXCLUDED.map((s) => `"${s}"`).join(',')})`);
  lookup = (groupIds && groupIds.length > 1)
    ? lookup.in('primary_category_id', groupIds)
  ...
  // Continue the FURTHEST-ADVANCED open case; ties broken by newest (openRows
  // is already opened_at-desc, and Array.sort is stable).
  const existing = (openRows || []).slice().sort(
...
module.exports = { findOrContinueViolation, isRecurrence, detectRecurrence, RECURRENCE_LOOKBACK_DAYS };
