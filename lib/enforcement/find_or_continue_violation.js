 ...
 * @property {'continuation'|'own_case'|'new'} type
 *   'own_case' = the only open case at this property+category is the one THIS
 *   observation opened (auto-drafted, awaiting confirm). Draft its first notice;
 *   do not log a continuation and do not open a duplicate.
 * @property {string|null} violation_id   set if type='continuation' or 'own_case'
...
// `excludeObservationId` — drop any open case that was OPENED FROM this
//   observation. A re-observation can never "continue" the very case that
//   observation itself opened; treating its own case as a prior open case logs a
//   self-continuation and drafts no letter (the 2026-09-14 self-continuation
//   bug: 137 rows, 24 confirmed violations left with NO notice — the "confirmed,
//   then it disappears with no way to send a letter" symptom). See the caller.
// `onlyObservationId` — the inverse: return ONLY the case opened from this
//   observation (used to tell "the obs's own auto-opened case" apart from a
//   genuinely new case).
async function findOpenCaseForCategory({ propertyId, categoryId, groupIds, excludeObservationId = null, onlyObservationId = null }) {
  const ids = groupIds || await expandCategoryToAliases(categoryId);
  let lookup = supabase
    .from('violations')
    .select('id, current_stage, continuation_count, current_stage_started_at, cure_period_ends_at, primary_category_id, opened_from_observation_id')
    .eq('property_id', propertyId)
  ...
  let rows = openRows || [];
  if (excludeObservationId) rows = rows.filter((r) => r.opened_from_observation_id !== excludeObservationId);
  if (onlyObservationId) rows = rows.filter((r) => r.opened_from_observation_id === onlyObservationId);
...
async function findOrContinueViolation(input) {
  ...
  // 1. Look up the open violation at this property+category — ALIAS-AWARE, but
  //    EXCLUDING any case opened from THIS observation. A re-observation cannot
  //    continue the case its own observation opened; that self-continuation is
  //    what stranded 24 confirmed violations with no notice (2026-09-14).
  const groupIds = await expandCategoryToAliases(categoryId);
  const existing = await findOpenCaseForCategory({ propertyId, categoryId, groupIds, excludeObservationId: observationId });

  // 1b. No OTHER open case. Is the only open case the one THIS observation
  //     already opened (the analyze/auto-draft step opens the violation and
  //     leaves the observation pending for a human to confirm)? If so this is
  //     NOT a continuation and NOT a fresh duplicate — it is the observation's
  //     own case awaiting its first notice. Tell the caller so it drafts that
  //     notice instead of logging a self-continuation.
  if (!existing && observationId) {
    const ownCase = await findOpenCaseForCategory({ propertyId, categoryId, groupIds, onlyObservationId: observationId });
    if (ownCase) {
      // Has this own case ALREADY had its first courtesy notice mailed? During
      // the Eaglewood onboarding drives (June/July 2026) analyze auto-opened the
      // violation AND auto-mailed its courtesy_1, but the observation was left
      // 'pending'. Without this check, confirming it re-drafts a DUPLICATE first
      // notice — the bulk-confirm preview showed "472 new courtesy" when 205 of
      // those own cases had already been mailed. (Ed 2026-09-17.) A stage past
      // courtesy_1, or any sent/mailed courtesy_1 letter, means already noticed.
      let firstNoticeMailed = !!(ownCase.current_stage && ownCase.current_stage !== 'courtesy_1');
      if (!firstNoticeMailed) {
        const { data: c1 } = await supabase.from('interactions')
          .select('status, mailed_at')
          .eq('violation_id', ownCase.id)
          .eq('type', 'letter_courtesy_1');
        firstNoticeMailed = (c1 || []).some((l) => l.status === 'sent' || l.mailed_at);
      }
      return {
        type: 'own_case',
        violation_id: ownCase.id,
        first_notice_mailed: firstNoticeMailed,
        continuation_id: null,
        continuation_count_after: null,
        existing_stage: ownCase.current_stage,
        existing_stage_started_at: ownCase.current_stage_started_at || null,
      };
    }
  }
