...
async function postDueRecognition({ supabase, communityId = null, throughMonth }) {
  ...
  for (const sch of schedules || []) {
    ...
    for (let k = 0; k < sch.term_months; k++) {
      ...
      const isDaily = sch.recognition_basis === 'daily';
      const monthTotal = isDaily
        ? dailyMonthCents(sch, k)
        : (isLast ? (total - monthly * (sch.term_months - 1)) : monthly);
      // A daily month can be $0 (entirely outside the coverage window) — skip
      // without posting an empty entry; it never marks the schedule recognized.
      ...
      // Recognize on the FIRST of the month: an HOA assessment is due and earned
      // at the start of the period, so the income shows on the statement any day
      // of the month — not hidden until a month-end-dated entry posts.
      const posting = `${month.slice(0, 7)}-01`;
