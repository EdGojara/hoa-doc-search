...
async function draftKatReply({ email, supabase }) {
  ...
  // Only produce the reconciliation report on genuine accounting INTENT — asking
  // where the books stand, to reconcile, close, tie out, etc. Merely NAMING a
  // community is not that: Celina's "the correct total to draft is $6,182.20"
  // mentions Waterview but is a specific correction, and the old `|| community`
  // condition fired a full books-status dump that ignored what she wrote (Ed
  // 2026-09-09). No accounting intent -> hand it to the general drafter, which
  // reads the message and answers the actual ask.
  if (!isAccounting) return { draftable: false };
