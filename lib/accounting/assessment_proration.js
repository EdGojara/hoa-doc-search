...
async function postAssessmentCharge({ community_id, property_id, effective_date, amount_cents, description }) {
  ...
  // Native-AR path — homeowner_transactions. Identity = the property's CURRENT
  // tenure; running balance = that account's committed ledger (ledger_identity.js).
  const ident = await currentLedgerIdentity(supabase, property_id);
  const trusted = ident.trusted_account_number;
  const vantaca = ident.vantaca_account_id;
  const currentBal = await committedAccountBalance(supabase, community_id, vantaca);
  ...
  const { data: txn, error: tErr } = await supabase.from('homeowner_transactions').insert({
    ...
    trusted_account_number: trusted, vantaca_account_id: vantaca, contact_id: ident.contact_id,
