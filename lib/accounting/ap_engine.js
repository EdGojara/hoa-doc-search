...
async function createInvoice(opts) {
  const {
    ...
    reimbursement_source,                   // real store on the receipt, when this payable is a board-member reimbursement (vendor kind=reimbursement)
  } = opts;
  ...
  // Pre-cutover invoice: never auto-posted (it may already be in the converted
  // books); it is saved and parked for review (lib/ap/cutover_review.js).
  const { preCutoverHold } = require('../ap/cutover_review');
  const preCut = await preCutoverHold(supabase, community_id, invoice_date);
  if (allCoded && !preCut.hold) {
    const jeLines = [];
    ...
    // Reconcile the debits to the invoice TOTAL before posting.
    //
    // Scar (Ed 2026-08-10): the extractor itemizes only the SUBTOTAL as lines and
    // parks sales tax (and any non-itemized fee) at the header (tax_cents). Debits
    // therefore summed to the subtotal while the AP credit was the full total, so
    // every invoice carrying sales tax threw `debits_X_must_equal_credits_Y` in
    // postJournalEntry and 500'd the upload — High Tech Lawn #34006 ($473.19 tax)
    // and Canyon Gate/GreenScape #72404 ($469.83) both died here. Sales tax paid
    // to a vendor on a service is part of that service's expense for the
    // association (not a recoverable input tax), so fold the unallocated
    // remainder into the largest expense account. This guarantees the JE balances
    // for ANY invoice shape (tax, freight, rounding) without depending on a
    // dedicated tax account existing in every community's chart. tax_cents is
    // still stored separately on ap_invoices for reporting.
    let _debitTotal = 0;
    for (const v of debitMap.values()) _debitTotal += v;
    const _recon = Number(total_cents) - _debitTotal;
    if (_recon !== 0 && debitMap.size > 0) {
      let _biggestAcct = null, _biggestAmt = -Infinity;
      for (const [acct, amt] of debitMap.entries()) { if (amt > _biggestAmt) { _biggestAmt = amt; _biggestAcct = acct; } }
      debitMap.set(_biggestAcct, debitMap.get(_biggestAcct) + _recon);
      console.log(`[ap_engine] reconciled JE debits to total: folded ${_recon}c (tax/fees) into account ${_biggestAcct} for invoice ${vendor_invoice_number || '?'}`);
    }
  ...
  const { data: invoice, error: invErr } = await supabase
    ...
    .insert({
      ...
      ...(preCut.hold ? {
        cutover_review: 'PENDING', needs_review: true,
        cutover_review_notes: `Invoice dated ${String(invoice_date).slice(0, 10)} is before the GL cutover ${preCut.cutover}: not posted. Review: already in the converted books, or post at cutover.`,
      } : {}),
      // Fold the MUD convenience-fee memo into the record notes (same as Emma's
      // intake path), so a $1-higher total is explained wherever the bill is
      // read — no separate line item. (Ed 2026-08-27)
      notes: [notes, _convenienceFeeCents > 0 ? `Added $${(_convenienceFeeCents / 100).toFixed(2)} MUD convenience fee (vendor setting) — folded into the bill total, no separate line item.` : null].filter(Boolean).join(' ') || null,
      // Only reference the column when it's actually a reimbursement, so a
      // normal invoice insert never names a column that migration 386 may not
      // have added yet (the deploy-before-migration gap). Reimbursements only
      // happen through the new endpoint, which runs after 386.
...
async function attachSourceAndRecode(opts) {
  ...
  if (allCoded && !totalMismatch) {
    try {
      ...
      // Pre-cutover invoice: posts only once reviewed NOT_IN_CONVERTED_BOOKS, and
      // then at the cutover date; otherwise it stays parked for review.
      const { preCutoverHold, markPendingReview } = require('../ap/cutover_review');
      const preCut = await preCutoverHold(supabase, community_id, inv.invoice_date);
      if (preCut.hold && inv.cutover_review !== 'NOT_IN_CONVERTED_BOOKS') {
        if (!inv.cutover_review || inv.cutover_review === 'PENDING') await markPendingReview(supabase, inv.id, preCut.cutover, inv.invoice_date);
        throw Object.assign(new Error('pre_cutover_invoice_awaiting_review'), { code: 'pre_cutover_review' });
      }
      const glDate = preCut.hold ? preCut.cutover : String(inv.invoice_date).slice(0, 10);
      const je = await postJournalEntry({
        community_id,
        posting_date: glDate,
        description: `AP invoice ${inv.vendor_invoice_number || ''} — ${(inv.vendor_name || 'vendor')}${preCut.hold ? ` (invoice dated ${String(inv.invoice_date).slice(0, 10)}; reviewed not in converted books; posted at GL cutover)` : ''}`.trim(),
        source_module: 'manual',
      ...
      if (e.code === 'pre_cutover_review') {
        postWarning = 'pre_cutover_awaiting_review';   // dated before GL cutover: a reviewer decides
      } else {
...
async function recordPayment(opts) {
  ...
  // Early cross-period prepayment (Ed 2026-08-31): a bill paid in an earlier
  // month than its invoice date must be booked as a PREPAID asset, not run
  // straight through AP (which would leave a negative payable in the pay month
  // and put the expense in a different month). Detect it here so BOTH the
  // mark-paid and /payments paths get it, then hand off to the prepaid engine.
  {
    const { data: invRows, error: invErr } = await supabase
      .from('ap_invoices').select('*').in('id', applications.map((a) => a.invoice_id));
    if (invErr) throw invErr;
    const invById = new Map((invRows || []).map((i) => [i.id, i]));
    const { isEarlyCrossPeriod, payInvoiceEarlyAsPrepaid } = require('../ap/early_prepay');
    const early = applications.filter((a) => {
      const inv = invById.get(a.invoice_id);
      return inv && inv.invoice_date && isEarlyCrossPeriod(payment_date, inv.invoice_date);
    });
    if (early.length) {
      // Keep it unambiguous: an early-dated bill is paid on its own, never
      // bundled into a multi-invoice check that also pays same-period bills.
      if (applications.length > 1) throw Object.assign(new Error('early_dated_invoice_must_be_paid_alone'), { code: 'invalid_input' });
      const inv = invById.get(applications[0].invoice_id);
      return await payInvoiceEarlyAsPrepaid(supabase, {
        invoice: inv, paymentDate: payment_date, amountCents: applications[0].applied_cents,
        method: payment_method, checkNumber: check_number, bankAccountId: bank_account_id, postedByUserId: posted_by_user_id,
      });
    }
  }
  ...
  const je = await postJournalEntry({
    ...
    posted_by_user_id,
    ap_posting: true,   // a payment dated before GL cutover is refused, never moved (bank rec)
    lines: [
