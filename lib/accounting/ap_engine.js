...
async function createInvoice(opts) {
  ...
  // Find the AP account (20100 by default)
  const apAccount = await findAccountByNumber(community_id, '20100');
  if (!apAccount) throw Object.assign(new Error('ap_account_20100_not_found'), { code: 'invalid_state' });
...
// approveInvoice
// ---------------------------------------------------------------------------
async function approveInvoice({ invoice_id, user_id, user_name, notes }) {
...
async function recordPayment(opts) {
  ...
  if (!cashAccountId) {
    const acct = await findAccountByNumber(community_id, '10100');  // default Operating Cash
    cashAccountId = acct?.id;
  }
