...
async function createInvoice(opts) {
  ...
  if (allCoded) {
    ...
    // Before the GL cutover the period is converted + certified: post at cutover,
    // keep the invoice's own date on the invoice (lib/accounting/cutover.js).
    const { getGlCutoverDate, apInvoicePostingDate } = require('./cutover');
    const glDate = apInvoicePostingDate(invoice_date, await getGlCutoverDate(supabase, community_id));
    const je = await postJournalEntry({
      community_id,
      posting_date: glDate,
      description: `AP invoice ${vendor_invoice_number || ''} — ${(opts.vendor_name || 'vendor')}${glDate !== String(invoice_date).slice(0, 10) ? ` (invoice dated ${String(invoice_date).slice(0, 10)}; posted at GL cutover ${glDate})` : ''}`.trim(),
...
async function attachSourceAndRecode(opts) {
  ...
  if (allCoded && !totalMismatch) {
    try {
      ...
      const { getGlCutoverDate, apInvoicePostingDate } = require('./cutover');
      const glDate = apInvoicePostingDate(inv.invoice_date, await getGlCutoverDate(supabase, community_id));
      const je = await postJournalEntry({
        community_id,
        posting_date: glDate,
        description: `AP invoice ${inv.vendor_invoice_number || ''} — ${(inv.vendor_name || 'vendor')}${glDate !== String(inv.invoice_date).slice(0, 10) ? ` (invoice dated ${String(inv.invoice_date).slice(0, 10)}; posted at GL cutover ${glDate})` : ''}`.trim(),
        source_module: 'manual',
