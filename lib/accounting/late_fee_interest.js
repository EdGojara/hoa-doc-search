...
async function runLateFeesAndInterest({ supabase, communityId, runMonth, dryRun = true }) {
  ...
  for (const pid of propIds) {
    const fee = (total[pid] > 0 && lateFee > 0) ? lateFee : 0;
  ...
  const plan = { run_month: runMonth, post_date: postDate, accounts_charged: items.length, late_fee_each: lateFee, late_fee_total_cents: feeTotal, interest_apr_pct: Number(policy.interest_apr_pct), interest_total_cents: intTotal, grand_total_cents: grand };
  ...
  const fy = Number(runMonth.slice(0, 4)), pn = Number(runMonth.slice(5, 7));
