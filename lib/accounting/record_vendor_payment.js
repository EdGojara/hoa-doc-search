...
async function recordVendorPaymentToGL({ communityId, amountCents, glAccountId, vendorId, vendorName, description, postingDate, sourceRef, notes, allowDuplicate = false }) {
  ...
  try {
    const je = await postJournalEntry({
      ...
      ap_posting: true,   // vendor payment dated before GL cutover is refused (converted period)
      notes: notes || null,
      lines: [
    ...
    if (e.code === 'before_gl_cutover') return { error: 'before_gl_cutover', message: e.message };
    // Lost the race to a concurrent post of the SAME source document — the
    // partial unique index (uq_je_email_source_ref, migration 342) rejected the
    // second insert. Return the entry that won so a double-fire reads as "already
    // recorded", never a 500 and never a second posting. Belt to the
    // check-then-act idempotency braces above. (Ed 2026-08-01.)
    if ((e.code === '23505' || /duplicate key|unique constraint/i.test(String(e.message || ''))) && sourceRef) {
      const { data: existing } = await supabase.from('journal_entries')
        .select('id').eq('community_id', communityId).eq('source_reference', sourceRef).limit(1);
      if (existing && existing.length) return { ok: true, je_id: existing[0].id, gl_account_id: acctId, duplicate: true };
    }
...
// Pull the single postable dollar amount (in cents) — the one figure that left
// (or will leave) the operating account — from classifier-extracted amount
// strings. Returns null only when the amount is GENUINELY ambiguous, so a dead
// button never appears where a real amount exists.
//
// The base+fee+total case is NOT ambiguous. Utility/water-district auto-pay
// confirmations (Inframark/Starnik) itemize the charge, a small processing fee,
// and the total — e.g. ["$407.25","$1.00","$408.25"]. The old "exactly one
// distinct amount" rule read that as three amounts and gave up, so every one of
// those payments sat un-postable in Emma's inbox (Ed 2026-07-20). When the
// largest amount equals the sum of the others (within a cent of rounding), that
// largest IS the total that hit the bank — post it. Everything posts as
// needs_review and is dedup-guarded, so resolving the total is safe.
function singleAmountCents(amounts) {
  ...
  // base(s) + fee + total: the max equals the sum of the rest → the max is the total.
  const sorted = [...cents].sort((a, b) => a - b);
  const total = sorted[sorted.length - 1];
  const sumRest = sorted.slice(0, -1).reduce((s, x) => s + x, 0);
  if (Math.abs(sumRest - total) <= 1) return total;
  return null; // truly ambiguous (e.g. two unrelated bills, no stated total) — still blocks
}
