...
const { forecastMonthsFromSchedules } = require('./recognition_schedule');

const METHODS = ['remaining_budget', 'run_rate', 'prior_year_pattern', 'recurring', 'manual', 'components', 'assessment_recognition', 'recognition_schedule'];
const COMPONENT_KINDS = ['project', 'contract', 'known_invoice', 'recurring', 'adjustment', 'recognition'];
...
function buildForecastLine(p) {
  ...
  switch (p.method || 'remaining_budget') {
    ...
    case 'recognition_schedule': {
      // Phase 3C: the P&L timing of a billed/paid amount comes from its active
      // recognition schedule(s), never from billing, payment or invoice dates.
      const schs = s.schedules || [];
      if (!schs.length) throw err('recognition_schedule_method_needs_schedules');
      schs.forEach((x, n) => {
        if (x.status !== 'active' && x.status !== 'fully_recognized') throw err(`schedule_${n + 1}_is_not_active`);
        if (!Array.isArray(x.periods) || !x.periods.length) throw err(`schedule_${n + 1}_has_no_periods`);
      });
      const r = forecastMonthsFromSchedules({ schedules: schs, fiscal_year: s.fiscal_year, as_of_month: asOf });
      rem.forEach((i) => { remaining[i] = r.months[i]; });
      basis = r.basis; confidence = r.basis === 'documented' ? 'high' : 'medium';
      explanation = `Remaining months follow ${schs.length === 1 ? 'the recognition schedule' : schs.length + ' recognition schedules'} (${schs.map((x) => x.description).join('; ')}).`;
      if (r.catch_up_cents && rem.length) explanation += ` Includes $${(r.catch_up_cents / 100).toFixed(2)} scheduled for ${r.missing_months.map((m) => MONTHS[Number(m.slice(5, 7)) - 1]).join(', ')} that has not been posted yet, carried in ${MONTHS[rem[0]]}.`;
      break;
    }
    case 'components': {
      // A line = base (recurring / remaining-budget residual / adjustment) + any
      // number of recognition-schedule components + other components. A
      // recognition component's months come from its schedule, never typed in.
      const comps = (p.components || []).map((c, n) => {
        if (c.kind !== 'recognition') return c;
        const sch = c.schedule;
        if (!sch || !Array.isArray(sch.periods)) throw err(`component_${n + 1}_recognition_needs_its_schedule`);
        if (sch.status !== 'active' && sch.status !== 'fully_recognized') throw err(`component_${n + 1}_schedule_is_not_active`);
        const r = forecastMonthsFromSchedules({ schedules: [sch], fiscal_year: s.fiscal_year != null ? s.fiscal_year : c.fiscal_year, as_of_month: asOf });
        return { ...c, months: r.months, basis: sch.schedule_basis === 'documented' ? 'documented' : 'calculated', catch_up_cents: r.catch_up_cents,
          refs: { ...(c.refs || {}), recognition_schedule_id: sch.id || (c.refs && c.refs.recognition_schedule_id) || null } };
      });
 ...
 */
function deriveDrivers({ account_type, as_of_month, cutover_month, budget_components = [], forecast_components = [], actual_months, recognition, recognition_schedules = null, fiscal_year = null }) {
  ...
  // Phase 3C: due schedule months with no effective posting (evidence = schedule + months).
  if (Array.isArray(recognition_schedules) && recognition_schedules.length) {
    const r = forecastMonthsFromSchedules({ schedules: recognition_schedules, fiscal_year, as_of_month: asOf });
    if (r.catch_up_cents) {
      drivers.push({
        type: 'recognition_not_posted', classification: 'known_timing',
        ytd_amount: favorableVariance(account_type, r.catch_up_cents, 0), full_year_amount: 0,
        missing_months: r.missing_months.map((m) => MONTHS[Number(m.slice(5, 7)) - 1]), expected_reversal_month: 'when the due recognition entries are posted',
        evidence: { recognition_schedule_ids: recognition_schedules.map((x) => x.id || null), months_without_recognition: r.missing_months },
      });
    }
  }
  return drivers;
...
/**
 * Forecast method precedence (deterministic):
 *   1. manual override with a reason (a person decided, and said why)
 *   2. active recognition schedule(s) for the account -> 'components':
 *        base + recognition component(s) + other components.
 *      One schedule never becomes the whole account (5600 may hold the Harned
 *      policy plus other policies). Only when someone has explicitly marked the
 *      schedules as covering the ENTIRE account is the whole-line
 *      'recognition_schedule' method used.
 *   3. other components (projects / contracts / known invoices)
 *   4. remaining approved budget (default)
 * Run-rate is never chosen automatically.
 */
function chooseForecastMethod({ manual_reason = null, schedules = [], has_components = false, schedules_cover_account = false }) {
  if (String(manual_reason || '').trim()) return { method: 'manual', reason: 'manual override with a documented reason' };
  const live = schedules.filter((x) => x.status === 'active' || x.status === 'fully_recognized');
  if (live.length && schedules_cover_account === true) return { method: 'recognition_schedule', reason: 'recognition schedule(s) explicitly cover the whole account' };
  if (live.length) return { method: 'components', reason: 'base + recognition schedule component(s) + other components' };
  if (has_components) return { method: 'components', reason: 'documented components' };
  return { method: 'remaining_budget', reason: 'default: remaining approved budget' };
}
...
// Variance policy: every threshold is explicit and configurable; nothing is assumed.
//   { materiality_cents?, materiality_pct?, board_decision_overrun_cents?, board_decision_overrun_pct?, source? }
// With no policy, only evidence-driven actions fire; otherwise the neutral 'review variance'.
function _threshold(cents, pct, base) {
  const a = cents != null ? Math.abs(int(cents)) : null;
  const b = pct != null ? Math.abs(int(base)) * Number(pct) / 100 : null;
  if (a == null && b == null) return null;
  return Math.max(a || 0, b || 0);
}
function normalizePolicy(policy) {
  if (!policy || typeof policy !== 'object') return null;
  const keys = ['materiality_cents', 'materiality_pct', 'board_decision_overrun_cents', 'board_decision_overrun_pct'];
  return keys.some((k) => policy[k] != null) ? policy : null;
}

// Deterministic recommended action (rule table; overridable by a person).
function recommendAction({ classification, drivers = [], forecast_variance, annual_budget, policy = null }) {
  ...
  if (drivers.some((d) => d.type === 'recognition_not_posted')) return 'post the due recognition entries (accounting)';
  ...
  const timingOnly = unexplained === 0 && Math.abs(int(classification.ytd.known_timing)) > 0;
  const p = normalizePolicy(policy);
  if (!p) {
    if (timingOnly) return 'no action; timing only';
    return unexplained === 0 ? 'no action' : 'review variance';
  }
  const board = _threshold(p.board_decision_overrun_cents, p.board_decision_overrun_pct, annual_budget);
  if (board != null && forecast_variance != null && forecast_variance < 0 && Math.abs(forecast_variance) > board) return 'board decision required (forecast overrun beyond configured policy)';
  const mat = _threshold(p.materiality_cents, p.materiality_pct, annual_budget);
  if (mat == null) return unexplained === 0 ? (timingOnly ? 'no action; timing only' : 'no action') : 'review variance';
  if (unexplained >= mat) return 'review invoices / postings';
  if (Math.abs(int(classification.ytd.known_timing)) > 0) return 'no action; timing only';
  return 'no action';
}

/**
 * Amanda-ready management facts for one line (numbers + evidence; no prose).
 * material is null when no materiality policy is configured.
 */
function managementFacts({ account, category = null, subcategory = null, account_type, line, drivers = [], manual_action = null, policy = null }) {
  ...
  const p = normalizePolicy(policy);
  const mat = p ? _threshold(p.materiality_cents, p.materiality_pct, line.annual_budget) : null;
  const material = mat == null ? null : (Math.abs(ytdVar) >= mat || Math.abs(fcVar) >= mat);
  return {
    ...
    variance_policy: p ? { ...p } : null,
    recommended_action: manual_action || recommendAction({ classification, drivers, forecast_variance: fcVar, annual_budget: line.annual_budget, policy: p }),
    recommended_action_source: manual_action ? 'manual' : 'rule',
  };
}

module.exports = { chooseForecastMethod, METHODS, COMPONENT_KINDS, BASES, MONTHS, buildForecastLine, checkComponentsExplainRemaining, deriveDrivers, classifyVariance, recommendAction, managementFacts };
