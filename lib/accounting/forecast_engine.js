 ...
 *   2. an active DOCUMENTED recognition schedule for the account
 *   3. an active calculated recognition schedule
 *   4. components (projects / contracts / known invoices)
 *   5. remaining approved budget (default)
 * Run-rate is never chosen automatically.
 */
function chooseForecastMethod({ manual_reason = null, schedules = [], has_components = false }) {
  ...
  if (live.some((x) => x.schedule_basis === 'documented')) return { method: 'recognition_schedule', reason: 'active documented recognition schedule' };
  if (live.length) return { method: 'recognition_schedule', reason: 'active recognition schedule (calculated)' };
