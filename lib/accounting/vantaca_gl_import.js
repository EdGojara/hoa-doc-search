...
function parseTrialBalance(path) {
  ...
  // Column layout varies across Vantaca export templates (Canyon Gate's takeover
  // export shifts Debit/Credit/Ending right vs the LOPF/Waterview C3-conversion
  // export). Detect columns from the header labels; fall back to the original
  // positions if a label isn't found. ([[project_portfolio_gl_migration]] —
  // "file shape varies" extraction-surface discipline.)
  const findCol = (re) => {
    for (let i = 0; i < Math.min(rows.length, 40); i++) {
      const r = rows[i] || [];
      for (let j = 0; j < r.length; j++) if (re.test(String(r[j] || '').trim())) return j;
    }
    return null;
  };
  const begLabel = findCol(/^Beginning Balance$/i);
  const C = {
    beg: begLabel != null ? begLabel + 1 : 7,   // value sits one column right of the label
    debit: findCol(/^Debit$/i) ?? 8,
    credit: findCol(/^Credit$/i) ?? 10,
    ending: findCol(/^Ending Balance$/i) ?? 11,
    date: findCol(/^Date$/i) ?? 1,
    desc: findCol(/^Description$/i) ?? 3,
    ledger: findCol(/^Ledger ID$/i) ?? 2,
    type: findCol(/^Type$/i) ?? 12,
  };
  ...
  for (const r of rows) {
    ...
    if (hm) {
      const number = hm[1];
      // Vantaca splits the trial balance into fund sections (Operating / Reserve
      // / Savings), and an account that touches more than one fund (fund-balance
      // 3050, reserve interest 4120, interfund lines) appears once PER section.
      // Sum the sections into a single account total — never let the last
      // section overwrite the earlier ones, or the account's true balance is
      // silently understated (LOPF: 3050 read as -3,653.71 instead of
      // -486,571.74; 4120 read as $0 instead of -$2,544.49).
      if (byAcct[number]) {
        cur = byAcct[number];
        cur.beginning_cents += cents(num(r[C.beg]));
        cur.debit_cents += cents(num(r[C.debit]));
        cur.credit_cents += cents(num(r[C.credit]));
        cur.ending_cents += cents(num(r[C.ending]));
        continue;
      }
      const cls = classify(number);
      cur = {
        number, name: hm[2].trim(), ...cls,
        beginning_cents: cents(num(r[C.beg])), debit_cents: cents(num(r[C.debit])),
        credit_cents: cents(num(r[C.credit])), ending_cents: cents(num(r[C.ending])),
      };
      accounts.push(cur);
      byAcct[number] = cur;
      continue;
    }
    const dm = String(r[C.date] || '').trim().match(/^(\d{2})\/(\d{2})\/(\d{4})/);
    if (!cur || !dm) continue;
    const debit = cents(num(r[C.debit])), credit = cents(num(r[C.credit]));
    if (debit === 0 && credit === 0) continue;
    const iso = `${dm[3]}-${dm[1]}-${dm[2]}`;
    (byDay[iso] = byDay[iso] || []).push({
      accountNumber: cur.number, debit_cents: debit, credit_cents: credit,
      description: String(r[C.desc] || '').trim(), type: String(r[C.type] || '').trim(),
      ledgerId: String(r[C.ledger] || '').trim() || null,
    });
  }
