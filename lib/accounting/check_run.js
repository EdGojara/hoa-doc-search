...
function formatVendorAddress(v) {
  ...
  // Drop a trailing country token — geocoded addresses append ", USA", which
  // pushes the ZIP off the end and defeats the parse below. (Ed 2026-09-04 —
  // Superior LawnCare "PO Box 15217, Houston, TX, 77220, USA" printed the city
  // on the street line of a Quail Ridge check.)
  raw = raw.replace(/,?\s*(?:u\.?\s?s\.?\s?a\.?|united states(?:\s+of\s+america)?)\.?$/i, '').trim();
  // Pull STATE + ZIP off the END (comma OR space delimiters — "City, TX 77220",
  // "City TX 77220", and "City, TX, 77220" all occur). Whatever remains is the
  // street + city.
  const tail = raw.match(/^(.*?)[\s,]+(TX|Texas|[A-Za-z]{2})\.?[\s,]+(\d{5}(?:-\d{4})?)$/i);
  if (tail) {
    const head = tail[1].replace(/[\s,]+$/, '').trim();     // street + city
    const st = /^texas$/i.test(tail[2]) ? 'TX' : tail[2].toUpperCase();
    const zip = tail[3];
    let city, street;
    if (head.includes(',')) {
      // A comma marks the street/city boundary — the city is the last segment,
      // so a multi-word city ("Sugar Land") stays intact. (Ed 2026-09-04.)
      const hp = head.split(',').map((t) => t.trim()).filter(Boolean);
      city = hp.pop() || '';
      street = hp.join(', ');
    } else {
      // No comma ("8810 Madie Drive Houston") — take the last word as the city.
      const words = head.split(/\s+/).filter(Boolean);
      city = words.length > 1 ? words.pop() : '';
      street = words.join(' ');
      if (!city) { city = head; street = ''; }
    }
    return [street, [city, `${st} ${zip}`].filter(Boolean).join(', ')].filter(Boolean);
  }
  // No state/ZIP tail found — comma-split, keeping everything except the final
  // City + ST/ZIP on one street line (never one comma-part per line).
...
async function createCheckRun({ community_id, bank_account_id, payment_date, invoice_ids, memo, user }) {
  ...
  const { data: invs } = await supabase.from('ap_invoices')
    .select('id, community_id, vendor_id, vendor_invoice_number, invoice_date, total_cents, amount_paid_cents, status, notes, is_ach_autopay, vendors(id, name, payee_name, address, remit_address_line1, remit_address_line2, remit_city, remit_state, remit_zip)')
    .in('id', invoice_ids);
  ...
  // NO MAILING ADDRESS = NO CHECK. A printed check must have somewhere to go; a
  // check to a vendor with no address on file is unmailable and posts a payment
  // against an invoice for money that can't leave. Emma flags it instead of
  // cutting it. (Ed 2026-09-08: RABKA Pest Control at Canyon Gate had no address
  // and a check was cut anyway.) Refuse the WHOLE run before any check number is
  // reserved or any payment posted, and name the vendor(s) so it's actionable.
  const missingAddress = [];
  for (const [, invoices] of byVendor) {
    const v = invoices[0].vendors || {};
    // formatVendorAddress returns an ARRAY of address lines — empty means no
    // mailable address (an empty array is truthy, so test length, not truthiness).
    const addrLines = formatVendorAddress(v) || [];
    if (!addrLines.length) missingAddress.push(v.payee_name || v.name || 'Unknown vendor');
  }
  if (missingAddress.length) {
    throw Object.assign(
      new Error(`No address on file for ${missingAddress.join(', ')} — add a remit/mailing address before cutting a check.`),
      { code: 'vendor_no_address', vendors: missingAddress },
    );
  }
...
async function updateBankCheckConfig(bank_account_id, patch) {
  ...
  // MICR line: true = stock is pre-encoded (bank printed the MICR, so DON'T print
  // one); false = blank stock (trustEd prints the full MICR). Editable from the
  // Print Checks setup form so it's fixable where staff actually are, not only on
  // the separate Bank Setup screen. (Ed 2026-09-04 — Quail Ridge printed blank.)
  if (patch.pre_encoded != null) baPatch.check_stock_micr_pre_encoded = patch.pre_encoded !== false;
...
module.exports = { listPayableInvoices, createCheckRun, getRunForRender, getChecksForReprint, voidCheck, getBankCheckConfig, updateBankCheckConfig, glAccountExists, listCommunityCashAccounts, formatVendorAddress };
