...
async function listPayableInvoices({ community_id }) {
  ...
  const { data, error } = await supabase.from('ap_invoices')
    .select('id, vendor_id, vendor_invoice_number, invoice_date, due_date, total_cents, amount_paid_cents, status, notes, vendors(id, name, payee_name, remit_address_line1, remit_address_line2, remit_city, remit_state, remit_zip)')
...
async function getBankCheckConfig(bank_account_id, { forRender = false } = {}) {
  const { data: ba } = await supabase.from('bank_accounts')
    .select('id, community_id, account_nickname, bank_name, account_last4, bank_id, next_check_number, check_stock_format, dual_sig_threshold_cents, signature_image_path, signature_image_path_secondary, account_number_encrypted, communities(name, legal_name)')
