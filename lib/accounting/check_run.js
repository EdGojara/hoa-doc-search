...
function formatVendorAddress(v) {
  ...
  if (v.remit_city || v.remit_state || v.remit_zip) {
    const csz = [[v.remit_city, v.remit_state].filter(Boolean).join(', '), v.remit_zip].filter(Boolean).join(' ');
    return [v.remit_address_line1, v.remit_address_line2, csz].map((x) => (x || '').trim()).filter(Boolean);
  }
  ...
  if (m) {
    ...
    const streetLines = m[1].replace(/[,\s]+$/, '').split(',').map((t) => t.trim()).filter(Boolean);
    return [...streetLines, `${m[2].trim()}, ${st} ${m[4]}`];
  }
  try { const { addressLinesFromString } = require('../mail/address_block'); return addressLinesFromString(raw); }
  catch (_) { return [raw]; }
}
