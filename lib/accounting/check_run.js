...
async function getChecksForReprint({ community_id, check_register_ids }) {
  ...
  for (const r of regs) {
    ...
    else if (_printedDate(r) !== today) locked.push(`#${r.check_number} printed ${_printedDate(r)}`);
  }
  if (locked.length) {
...
module.exports = { listPayableInvoices, createCheckRun, getRunForRender, getChecksForReprint, voidCheck, getBankCheckConfig, updateBankCheckConfig };
