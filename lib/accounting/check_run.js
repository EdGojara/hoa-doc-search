...
async function listPayableInvoices({ community_id }) {
  ...
  return (data || []).filter((i) => balanceOf(i) > 0 && i.is_ach_autopay !== true).map((i) => ({
    ...
    total_cents: Number(i.total_cents), balance_cents: balanceOf(i), description: payableNoteFor(i),
    ...
    if (pay) {
      ...
      for (const invoiceId of invoiceIds) {
        const { data: inv, error: invErr } = await supabase.from('ap_invoices')
          .select('id, total_cents, amount_paid_cents, status').eq('id', invoiceId).maybeSingle();
        ...
        // Never resurrect a bill the operator voided outright.
        //
        // 'cancel' means the payment should not have gone out — most often
        // because the invoice itself is wrong. Returning that bill to
        // 'approved' would drop it straight back into the ready-to-pay queue
        // with its box ticked, one click from re-cutting the very check that
        // was just voided. So a cancel parks it on hold: still owed on the
        // books, but not payable until someone fixes the amount. (Ed 2026-08-18,
        // check #1008 Texas Access Works — "incorrect amount was invoiced".)
        const newStatus = inv.status === 'voided' ? 'voided'
          : disposition === 'cancel' ? 'on_hold'
          : newPaid <= 0 ? 'approved'
...
async function getBankCheckConfig(bank_account_id, { forRender = false } = {}) {
  const { data: ba } = await supabase.from('bank_accounts')
    .select('id, community_id, management_company_id, account_nickname, bank_name, account_last4, bank_id, next_check_number, check_stock_format, dual_sig_threshold_cents, signature_image_path, signature_image_path_secondary, account_number_encrypted, gl_account_number, communities(name, legal_name)')
