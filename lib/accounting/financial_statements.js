...
async function rollingIncomeStatement({ community_id, end_date, months = 12 }) {
  ...
  for (const ln of lines) {
    ...
    if (!row) { row = { account_number: a.account_number, account_name: a.account_name, by_month: {}, total_cents: 0 }; map.set(a.account_number, row); }
  ...
  return {
    ...
    revenue: [...rev.values()].sort(byNum),
    expense: [...exp.values()].sort(byNum),
    monthly,
    zero_months: monthly.filter((m) => m.revenue_cents === 0 && m.expense_cents === 0).map((m) => m.month),
  };
}

module.exports = { balanceSheet, incomeStatement, equityStatement, budgetVsActual, rollingIncomeStatement };
