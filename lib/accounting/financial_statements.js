...
async function fetchLinesForCommunity({ community_id, from_date, to_date }) {
  const pageSize = 1000;
  // Only the columns the statements actually sum + posting_date (so a caller can
  // derive a sub-period in JS). Dropping property_id/vendor_id/bank_account_id
  // shrinks the payload materially at 5k+ lines.
  const SELECT = `account_id, fund_id, debit_cents, credit_cents, journal_entries!inner ( community_id, posting_date, status, void_reversal_je_id )`;
  const buildBase = () => {
    let q = supabase
  ...
  // Count once, then fetch all pages IN PARALLEL (was: sequential 1000-row pages,
  // ~6 round-trips for LOPF = the "very slow" income statement). ORDER BY id makes
  // the ranges deterministic (no drift/dupes) — required for safe pagination.
  let countQ = supabase
    .from('journal_entry_lines')
    .select('id, journal_entries!inner ( community_id, posting_date, status )', { count: 'exact', head: true })
    .eq('journal_entries.community_id', community_id)
    .in('journal_entries.status', COUNTED_JE_STATUSES);
  if (from_date) countQ = countQ.gte('journal_entries.posting_date', from_date);
  if (to_date) countQ = countQ.lte('journal_entries.posting_date', to_date);
  const { count, error: cErr } = await countQ;
  if (cErr) throw cErr;
  const total = Math.min(count || 0, 200000); // hard safety cap
  if (total === 0) return [];
  ...
  for (let batch = 0; batch < pages; batch += CONCURRENCY) {
    ...
    for (let p = batch; p < Math.min(batch + CONCURRENCY, pages); p++) {
      ...
      reqs.push(buildBase().order('id', { ascending: true }).range(start, start + pageSize - 1));
    }
    const results = await Promise.all(reqs);
    for (const { data, error } of results) { if (error) throw error; if (data) all.push(...data); }
  }
  // A 'voided' original counts only alongside its reversal (je_status.js).
  return all.filter((l) => countsInGl(l.journal_entries));
}
...
async function incomeStatement({ community_id, period_start, period_end, fund_id }) {
  ...
  // YTD lines are a superset of the period lines (period_start..period_end is
  // inside yearStart..period_end), so fetch YTD once and derive the period slice
  // in JS instead of a second full DB pull.
  const [coa, ytdLines, fundsMeta] = await Promise.all([
    fetchCoA(community_id),
    fetchLinesForCommunity({ community_id, from_date: yearStart, to_date: period_end }),
    fetchFunds(community_id),
  ]);
  const periodLines = ytdLines.filter((l) => l.journal_entries && l.journal_entries.posting_date >= period_start);
  const accountById = new Map(coa.map((a) => [a.id, a]));
  const accountFundById = new Map(coa.map((a) => [a.id, a.fund_id]));
  const fundMetaOf = (fundId, acct) => fundsMeta.get(fundId)
    || { fund_code: acct?.account_funds?.fund_code || null, fund_name: acct?.account_funds?.fund_name || null };
...
async function budgetVsActual({ community_id, period_end, fund_id }) {
  ...
  // Fetch budget (a failed read must not look like "no budget").
  ...
  // Fund-aware (Budget Phase 0): budget and actuals meet on (account, fund). A
  // budget line's fund is its own fund_id, else its account's home fund; the
  // same rule sumLinesPerAccountFund applies to journal lines.
  let budgetLines = [];
  if (budget) {
    const { data: lines, error: lErr } = await supabase
      .from('budget_line_items')
      .select('account_id, fund_id, annual_amount_cents, monthly_amounts_cents')
  ...
  // YTD is a superset of the month — fetch once, derive MTD in JS (was two full pulls).
  const [coa, ytdLines] = await Promise.all([
    fetchCoA(community_id),
    fetchLinesForCommunity({ community_id, from_date: yearStart, to_date: period_end }),
  ]);
  const mtdLines = ytdLines.filter((l) => l.journal_entries && l.journal_entries.posting_date >= periodStartMonth);
  const accountFundById = new Map(coa.map((a) => [a.id, a.fund_id || null]));
  const fundsById = await fetchFunds(community_id);
  const mtdBalances = sumLinesPerAccountFund(mtdLines, accountFundById);
  const ytdBalances = sumLinesPerAccountFund(ytdLines, accountFundById);
  const budgetByAcctFund = new Map(); // account_id -> Map(fundKey -> line)
  for (const l of budgetLines) {
    const fk = l.fund_id || accountFundById.get(l.account_id) || '__nofund__';
    let m = budgetByAcctFund.get(l.account_id);
    if (!m) { m = new Map(); budgetByAcctFund.set(l.account_id, m); }
    m.set(fk, l);
  }
  ...
  for (const a of coa) {
    if (a.is_summary) continue;
    if (!['revenue', 'expense'].includes(a.account_type)) continue;
    // Every fund this account has a budget or activity in: one row per (account, fund).
    const fundKeys = new Set([
      ...((budgetByAcctFund.get(a.id) || new Map()).keys()),
      ...((ytdBalances.get(a.id) || new Map()).keys()),
    ]);
    for (const fk of fundKeys) {
    const rowFundId = fk === '__nofund__' ? null : fk;
    if (fund_id && rowFundId !== fund_id) continue;
    const bdg = (budgetByAcctFund.get(a.id) || new Map()).get(fk) || null;
    ...
    const mtdSums = (mtdBalances.get(a.id) || new Map()).get(fk) || { debit: 0, credit: 0 };
    const ytdSums = (ytdBalances.get(a.id) || new Map()).get(fk) || { debit: 0, credit: 0 };
    ...
    const fundInfo = rowFundId ? fundsById.get(rowFundId) : null;
    rows.push({
      account_id: a.id,
      account_number: a.account_number,
      account_name: a.account_name,
      ...
      group: _plGroup(a.account_type, a.account_name),
      fund_id: rowFundId,
      fund_code: (fundInfo && fundInfo.fund_code) || (rowFundId === a.fund_id ? a.account_funds?.fund_code || null : null),
  ...
  // Sort: revenue first, then expenses, then by account number (then fund)
  rows.sort((a, b) => {
    if (a.account_type !== b.account_type) return a.account_type === 'revenue' ? -1 : 1;
    return a.account_number.localeCompare(b.account_number) || String(a.fund_code || '').localeCompare(String(b.fund_code || ''));
  });
...
const _AMT_KEYS = ['mtd_actual_cents', 'mtd_budget_cents', 'mtd_variance_cents', 'ytd_actual_cents', 'ytd_budget_cents', 'ytd_variance_cents', 'annual_budget_cents'];

// Budget vs Actual amount fields, in screen order. Grouped views subtotal all of them.
const BVA_AMOUNT_KEYS = ['mtd_budget_cents', 'mtd_actual_cents', 'mtd_variance_cents', 'ytd_budget_cents', 'ytd_actual_cents', 'ytd_variance_cents', 'annual_budget_cents'];

// Budget vs Actual plus the reporting-category grouping (Phase 1). The flat
// rows are exactly budgetVsActual's; `grouped` is presentation only: one tree
// across all funds, and one per fund for fund-sectioned statements.
async function budgetVsActualGrouped({ community_id, period_end, fund_id, _mapping }) {
  const { loadReportMapping, groupStatementRows } = require('./report_categories');
  const [bva, mapping] = await Promise.all([
    budgetVsActual({ community_id, period_end, fund_id }),
    _mapping || loadReportMapping(supabase, community_id),
  ]);
  const by_fund = {};
  for (const fc of [...new Set(bva.rows.map((r) => r.fund_code || '—'))]) {
    by_fund[fc] = groupStatementRows(bva.rows.filter((r) => (r.fund_code || '—') === fc), mapping, BVA_AMOUNT_KEYS);
  }
  return { ...bva, grouped: { has_mapping: mapping.has_mapping, all: groupStatementRows(bva.rows, mapping, BVA_AMOUNT_KEYS), by_fund } };
}

// Mapped groups in the printed statement's {group, rows, totals} shape: one per
// top category (rows = every account under it), subgroups per subcategory, and
// a visible Unmapped group when any account has no category.
function _mappedGroups(sectionTree) {
  const out = sectionTree.categories.map((c) => ({
    group: c.label,
    rows: [...c.rows, ...c.subcategories.flatMap((s) => s.rows)],
    direct_rows: c.rows,
    subgroups: c.subcategories.map((s) => ({ group: s.label, rows: s.rows, totals: _pick(s.totals) })),
    totals: _pick(c.totals),
  }));
  if (sectionTree.unmapped.rows.length) {
    out.push({ group: sectionTree.unmapped.label, unmapped: true, rows: sectionTree.unmapped.rows, direct_rows: sectionTree.unmapped.rows, subgroups: [], totals: _pick(sectionTree.unmapped.totals) });
  }
  return out;
}
function _pick(t) { return Object.fromEntries(_AMT_KEYS.map((k) => [k, Number(t[k] || 0)])); }
...
async function perFundIncomeStatement({ community_id, period_end, _mapping }) {
  ...
  const { loadReportMapping, groupStatementRows } = require('./report_categories');
  // _mapping: tests inject a mapping before migration 463 exists.
  const mapping = _mapping || await loadReportMapping(supabase, community_id);
  const fundsOut = fundCodes.map((fc) => {
    const { revenue, expense } = byFund.get(fc);
    if (mapping.has_mapping) {
      const tree = groupStatementRows([...revenue, ...expense], mapping, _AMT_KEYS);
      const revTotals = _pick(tree.totals.revenue), expTotals = _pick(tree.totals.expense), net = _pick(tree.totals.net);
      return {
        fund_code: fc, fund_name: fundName[fc] || fc, grouping: 'report_categories',
        revenue_groups: _mappedGroups(tree.sections[0]),
        expense_groups: _mappedGroups(tree.sections[1]),
...
module.exports = {
  balanceSheet, incomeStatement, equityStatement, budgetVsActual, budgetVsActualGrouped, BVA_AMOUNT_KEYS, rollingIncomeStatement, perFundIncomeStatement,
