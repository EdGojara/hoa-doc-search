...
async function postJournalEntry(opts) {
  ...
  // ---- Hard guard: no native AP posting before the GL cutover (Ed 2026-09-24) ----
  // The period before gl_cutover_date is converted and certified. AP invoice
  // paths move their posting date to the cutover date before calling here; any
  // AP posting that still arrives dated before cutover is refused, loudly.
  if (ap_posting || source_module === 'ap_invoice' || source_module === 'payment_intake') {
    const { getGlCutoverDate, assertNotBeforeCutover } = require('./cutover');
    assertNotBeforeCutover(posting_date, await getGlCutoverDate(supabase, community_id), `AP posting (${source_module})`);
  }
  ...
  // ---- Interfund auto-bridge (Ed 2026-09-04) ----
  // Keep every FUND self-balancing. When an entry crosses funds (expense/revenue
  // in a non-Operating fund, cash/AP in Operating, because Reserve/Adopt-a-School
  // have no bank account), append Due To / Due From lines so each fund's own
  // trial balance ties. Runs here so ALL posting paths get it. A bridged entry
  // already nets to zero per fund, so reversals/edits add nothing further.
  let finalLines = lines;
  try {
    const { resolveInterfund, computeBridge } = require('./interfund');
    const fundOf = (ln) => ln.fund_id || (byId.get(ln.account_id) || {}).fund_id || null;
    const resolved = await resolveInterfund(supabase, community_id);
    const bridge = computeBridge(lines, fundOf, resolved);
    if (bridge.length) {
      finalLines = lines.concat(bridge);
      const bd = bridge.reduce((s, l) => s + Number(l.debit_cents || 0), 0);
      const bc = bridge.reduce((s, l) => s + Number(l.credit_cents || 0), 0);
      // Bridge is balanced by construction, but assert before we trust it.
      if (bd !== bc) throw new Error(`interfund bridge unbalanced ${bd} vs ${bc}`);
      totalDebits += bd;
      totalCredits += bc;
    }
  } catch (e) {
    // Never fail a posting because bridging hiccuped — post as-is (today's
    // behavior) and surface it loudly for follow-up.
    console.warn('[posting] interfund bridge skipped:', e.message);
    finalLines = lines;
  }
  ...
  // ---- Insert lines (original + any interfund bridge) ----
  const lineRows = finalLines.map((ln, i) => ({
