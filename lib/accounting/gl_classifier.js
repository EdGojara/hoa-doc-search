...
async function matchByLineHistory({ communityId, vendorId, vendorName, description, byId, excludeInvoiceId = null }) {
  ...
  // Need a real pattern AND a clear plurality — not a split — to code CONFIDENTLY.
  if (best.n < 2 || best.n / matched < 0.6) {
    // A SINGLE, unambiguous, strong-similarity prior coding isn't a pattern yet,
    // but it IS a real prior human decision on this KIND of line — and it must not
    // be silently overridden by the vendor's majority account downstream. Surface
    // it as a low-confidence suggestion (needs_review) so the FIRST correction on a
    // distinctive line starts influencing the next bill instead of being ignored
    // until it repeats. It becomes the confident default at the 2nd agreeing line.
    // (Ed 2026-08-10 — High Tech Lawn's bi-weekly "Lakes" line was coded to 5205,
    // but the vendor-majority silently re-coded the next one to 5200.)
    if (best.n === 1 && byAcct.size === 1 && matched === 1 && best.simSum >= 0.75 && acct && acct.account_type === 'expense') {
      return { acct, confidence: 'low',
        reason: `1 prior ${vendorName || 'vendor'} line like "${description}" was coded to ${acct.account_number} ${acct.account_name} — confirm. It becomes the default once a second agrees.` };
    }
    return null;
  }
...
async function suggestClassification({ communityId, vendorId, vendorName, description, isPaymentLeg, totalCents = null, excludeJournalEntryId = null, excludeInvoiceId = null, descriptionIsLineItem = false }) {
  ...
  let weakLineHint = null;
  {
    ...
    // A CONFIDENT line pattern (a repeated coding of this KIND of line) beats
    // vendor history and returns now. A WEAK one — a single prior line at 'low'
    // — must NOT short-circuit the vendor-history branch below: if the vendor has
    // a strong overall pattern (e.g. 13/13 to one account) that's the better
    // signal, and letting a lone prior line suppress it leaves a clearly-codeable
    // bill uncoded. Merge, don't short-circuit (the Swim Houston scar). Hold the
    // weak hint as a fallback. (Ed 2026-08-12: Prepared Publications #10996 — one
    // prior line at 'low' hid the 13-of-13 pattern; bill sat uncoded as no_match.)
    if (lh && lh.confidence !== 'low') return withFit(shape(lh.acct, lh.confidence, lh.reason));
    if (lh) weakLineHint = withFit(shape(lh.acct, lh.confidence, lh.reason));
  }
  ...
  // A single prior human decision on THIS kind of line (weak, but real) still
  // outranks the lexical description guess below.
  if (weakLineHint) return weakLineHint;
