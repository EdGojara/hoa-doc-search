...
const PAGE_CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  .r { text-align: right; }
  /* MICR E-13B — install the font at public/fonts; monospace is only a fallback. */
  @font-face { font-family: 'MICR'; src: local('MICR Encoding'), local('GnuMICR'), local('MICRE13B'); }
...
function renderChecksHTML(checks, bankConfig) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>${PAGE_CSS}</style></head><body>${pages}</body></html>`;
}
...
module.exports = { amountToWords, dollarsToWords, formatMicr, renderChecksHTML, renderChecksPDF };
