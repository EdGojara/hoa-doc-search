...
// The bundled MICR font (public/fonts/micr.ttf, family "MICR" — the same font
// Vantaca uses) maps the E-13B symbols to LETTERS, not the Unicode symbols:
//   C = Transit (⑆, brackets the routing)   A = On-Us (⑈, brackets serial/account)
//   B = Amount (⑇)                           D = Dash (⑉)
// (verify: scripts render + compare to a real Vantaca check; final proof is a
// live test deposit at NewFirst.)
const MICR_TRANSIT = 'C';
const MICR_ONUS = 'A';
// Field widths are a BANK property. NewFirst's spec (routing 113104796) is a
// 7-digit on-us field and a 6-digit auxiliary on-us (serial) field, so a 6-digit
// account is right-justified and zero-filled to 7 (787385 -> 0787385), and a
// 4-digit check number to 6 (1000 -> 001000). The MICR reader reads a fixed
// field width; a short, unpadded number lands in the wrong columns and the check
// bounces. Widths come from bankConfig so a different bank isn't stuck with
// NewFirst's; the defaults match NewFirst since that's the only bank on file.
// (Ed 2026-07-16, from the NewFirst MICR spec sheet.)
function formatMicr({ routing, account, checkNumber, onusDigits = 7, serialDigits = 6 }) {
  const r = String(routing || '').replace(/\D/g, '');
  const a = String(account || '').replace(/\D/g, '');
  const c = String(checkNumber || '').replace(/\D/g, '');
  // Left-zero-pad to the field width; never truncate a number longer than the
  // field (that would be a real data problem to surface, not silently trim).
  const pad = (s, w) => (w && s.length < w ? s.padStart(w, '0') : s);
  const aPad = pad(a, onusDigits);
  const cPad = pad(c, serialDigits);
  // [aux on-us: serial/check#]  [transit: routing]  [on-us: account]
  return `${MICR_ONUS}${cPad}${MICR_ONUS} ${MICR_TRANSIT}${r}${MICR_TRANSIT} ${aPad}${MICR_ONUS}`;
}
...
function renderOneCheck(check, bankConfig) {
  ...
  const micr = formatMicr({ routing: bankConfig.routing, account: bankConfig.account_number, checkNumber: check.check_number, onusDigits: bankConfig.micr_onus_digits, serialDigits: bankConfig.micr_serial_digits });
