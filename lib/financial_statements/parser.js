...
async function generateBalanceSheetFindings(data, community) {
  ...
  const system =
    ...
    `- GOOD: "Operating cash + investments of $1.37M, net of AP ($88k) and accrued liabilities ($6k), is $1.28M unrestricted (conservative position, excludes pending A/R collections). At current monthly outflows this covers operations through approximately Q1 2027, assuming next year's budget is consistent with this year."\n\n` +
    `- BAD: "Reserve well-funded at $940k in Edward Jones investment account."\n` +
    `- GOOD: "Reserve of $940k is held in a single Edward Jones investment account — exact allocation between cash, money-market, fixed income, and equities is not visible from the balance sheet. Treasurer should pull the most recent EJ statement and confirm allocation aligns with the board's reserve-fund investment policy. Equity exposure on HOA reserves carries asymmetric risk: a downturn funds a replacement project that must happen on its scheduled date regardless."\n\n` +
    ...
    `- AI-puffery generics: "healthy," "well-funded," "solid safety buffer," "in line with norms," "consistent with industry standards," "strong position" — these are content-free fluff. SAY THE NUMBER AND WHAT THE TREASURER SHOULD DO ABOUT IT.\n\n` +
