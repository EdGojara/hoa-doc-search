...
const REMEDY_COPY = {
  ...
  tree: {
    re: 'Notice of Violation and Intent to Enter Property to Abate a Hazardous Tree',
    intro_action: 'to remove or remediate the hazardous tree and abate the condition',
    violation_desc: 'Failure to keep the Lot in good condition, including a dead or hazardous tree that is not being maintained in a safe, healthful condition as required by the Declaration.',
    services: 'Hazardous Tree Removal and Abatement',
    reserve_sentence: 'and nothing in this notice limits the Association from taking such additional action as may be authorized by the Declaration or applicable law if the condition materially deteriorates or presents an immediate threat to persons or property.',
  },
};
function remedyCopy(mode) { return REMEDY_COPY[mode] || REMEDY_COPY.lawn; }

// Collapse a free-text field to clean flowing prose. Staff-pasted descriptions
// arrive with carriage returns, blank lines, and quote markers; pdfkit renders a
// bare \r as a stray glyph ("Đ") and a \n as a hard break, so an "observed
// condition" copied from a document came out broken (Ed 2026-07-31, 19723 Canyon
// Gate). Strip control chars, drop a leading blockquote/quote marker on a line,
// and collapse all whitespace to single spaces. Also trims a trailing cure/
// self-help sentence pasted into the condition — the letter states the 10-day
// cure and self-help authority in its own paragraphs; repeating it inside the
// observed condition is duplicative.
function cleanConditionText(str) {
  let s = String(str == null ? '' : str)
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g, ' ')  // control chars (keep tab/newline)
    .replace(/\r/g, ' ');
  // Drop a pasted cure/self-help clause — the letter states this in its own paragraphs.
  s = s.replace(/(?:^|\s)(?:The\s+(?:homeowner|owner)\s+is\s+required\s+to\s+(?:remove|remedy|cure)|Failure\s+to\s+(?:act|comply|cure))[\s\S]*$/i, ' ');
  // Drop a bare blockquote/quote marker.
  s = s.replace(/(^|\s)>+(\s|$)/g, ' ');
...
// ----------------------------------------------------------------------------
// Evidence-photo compression. Inspection photos arrive at full camera
// resolution (~1MB+); embedded raw they bloat the PDF and slow the print
// download. Mirrors the §209 renderer's _compressLetterPhoto — node-canvas,
// and ALWAYS falls back to the original buffer on any failure so a compression
// hiccup never drops the evidence photo from a legal notice.
// ----------------------------------------------------------------------------
async function _compressPhoto(buffer, maxW = 1000, quality = 0.75) {
  ...
  try {
    ...
    return (out && out.length < buffer.length) ? out : buffer;
  } catch (e) {
    console.warn('[force-mow] photo compress failed, using original:', e.message);
    return buffer;
  }
}

// Format a capture timestamp for the photo caption (date + time, Central).
function fmtPhotoCaption(iso) {
  if (!iso) return '';
  try {
...
// Main renderer
// ----------------------------------------------------------------------------
// Optional evidence photo: data.photo_buffer (close-up of the condition) +
// data.photo_captured_at (ISO, for the caption). Absent → no photo section, so
// carryover cases with no inspection photo still render cleanly.
async function renderForceMowLetterPdf(data) {
  ...
  // Compress the evidence photo before it hits the page (see _compressPhoto).
  const photoBuf = await _compressPhoto(data.photo_buffer);
  ...
  return new Promise((resolve, reject) => {
    ...
    // Declaration recording citation removed per Ed 2026-07-09 — the doc number /
    // county could be wrong or superseded, so it's not printed on the notice.
    ...
    doc.font('Helvetica').text(cleanConditionText(data.observed_condition));
    doc.moveDown(0.8);

    // ------------------------------------------------------------------
    // 6b. Evidence photo (optional) — the inspection photo of the condition.
    // Placed here so the reader sees the documented condition right under the
    // description. Manual y-advance because pdfkit's doc.y doesn't move past
    // image() calls. Absent buffer → section skipped entirely.
    // ------------------------------------------------------------------
    if (photoBuf) {
      try {
        const contentWidth = PAGE.width - PAGE.margin * 2;
        const IMG_H = 210;
        if (doc.y > PAGE.height - (IMG_H + 70)) doc.addPage();
        doc.font('Helvetica-Bold').fontSize(9.5).fillColor(MUTED)
          .text('PROPERTY — AS DOCUMENTED ON SITE', PAGE.margin, doc.y, { characterSpacing: 0.8 });
        const imgY = doc.y + 4;
        doc.image(photoBuf, PAGE.margin, imgY, { fit: [contentWidth, IMG_H], align: 'left' });
        doc.y = imgY + IMG_H + 6;
        const cap = fmtPhotoCaption(data.photo_captured_at);
        doc.font('Helvetica-Bold').fontSize(9.5).fillColor(INK)
          .text('Captured: ', PAGE.margin, doc.y, { continued: true })
          .font('Helvetica').text(cap || fmtLongDate(data.observation_date), { continued: false });
        doc.moveDown(0.6);
        doc.x = PAGE.margin;
      } catch (e) {
        console.warn('[force-mow] evidence photo embed failed:', e.message);
        doc.x = PAGE.margin;
      }
    }
    ...
    doc.font('Helvetica').text(
      `This letter also constitutes formal written notice that, if the Violation is not cured within the ten (10) day period stated above, the Association intends to exercise the rights available to it under ${data.declaration_section_full}, including entering the Property and causing the Violation to be cured or the condition to be remedied. Any costs properly incurred by the Association in doing so will be charged to the Property as an Assessment in accordance with the Declaration and applicable law, `,
    ...
    // Contractor / Services / Date of Service / Cost block removed per Ed
    // 2026-07-09 — the contractor and cost aren't known when the notice is sent.
    // Defensive doc.x reset (keeps following paragraphs full-width).
    doc.x = PAGE.margin;
    doc.moveDown(0.2);
    ...
    doc.text('We appreciate your cooperation and your prompt attention to this matter.', { align: 'justify' });
    doc.moveDown(0.8);

    // 9. §209 hearing-rights (30-day written request) paragraph — REMOVED for
    // self-help notices per Ed 2026-07-09: prior counsel did not include the
    // 30-day §209.007 hearing language on the self-help cleanup / force-mow
    // letter. The 10-day cure is a self-help entry timeline, a separate clock.
    // (include_hearing_rights is still accepted for schema compatibility but no
    // longer rendered here.) The standalone SCRA notice below still prints.
    ...
    for (let i = pageRange.start; i < pageRange.start + pageRange.count; i++) {
      doc.switchToPage(i);
      // Writing footer text below the bottom margin makes PDFKit auto-add a
      // blank page (which stranded the footers on phantom pages 3-4 and doubled
      // the page count — over-billing copies + blank pages in a mailed legal
      // letter). Zero the bottom margin + lineBreak:false so the footer draws
      // in place without triggering a new page.
      doc.page.margins.bottom = 0;
      doc.font('Helvetica').fontSize(7).fillColor(MUTED)
        .text(
          `This community is professionally managed by Bedrock Association Management, LLC · Page ${i + 1 - pageRange.start} of ${pageRange.count}`,
          PAGE.margin,
