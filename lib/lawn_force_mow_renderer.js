...
function renderForceMowLetterPdf(data) {
  ...
  return new Promise((resolve, reject) => {
    ...
    refLine('Re:', 'Notice of Violation and Intent to Enter Property to Maintain the Yard');
    ...
    doc.font('Helvetica').text(`${data.declaration_section_full} — Failure to keep the Lot in good condition, including the failure to mow the lawn.`);
    doc.moveDown(0.5);
    ...
    doc.font('Helvetica-Bold').text(
      'and the Association reserves the right to continue to provide such self-help maintenance on a regular schedule without further notice if this violation continues.',
      { continued: false },
    );
    doc.moveDown(1);
    ...
    doc.font('Helvetica').fillColor(INK).text('Mowing and Yard Maintenance', PAGE.margin + 100, startY + 24);
