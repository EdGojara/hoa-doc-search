...
function validateAnalysis({ analysis, segments, mappings = [], roster = [], gaps = [], execRanges = [] }) {
  ...
  // Completeness: an open-session line where someone MAKES a motion ("I move",
  // "so moved") that no listed motion cites may be a motion Paige missed.
  const citedByMotion = new Set(motions.flatMap((m) => (m.support || []).map((x) => x.idx)));
  const possible_missed_motions = segments
    .filter((s) => s.scope === 'open' && moveRe.test(s.text) && ![s.idx - 1, s.idx, s.idx + 1].some((i) => citedByMotion.has(i)))
    .map((s) => ({ text: s.text, status: 'NEEDS_REVIEW', review_reasons: [{ code: 'possible_motion_not_listed', message: `a motion may have been made here that is not listed: "${s.text.slice(0, 80)}"` }],
      time_ref: { ref: `u${s.idx}`, audio_ms: s.start_ms, meeting_ms: s.meeting_start_ms, label: hms(s.start_ms) }, speaker_label: labelOf(s.speaker), withheld: false }));

  const all = [attendance, call_to_order, adjournment, ...motions, ...possible_missed_motions, ...decisions, ...action_items, ...follow_ups, ...next_agenda_items, ...discussion_topics];
  return {
    summary, attendance, call_to_order, adjournment, motions, possible_missed_motions, decisions, action_items, follow_ups, next_agenda_items, discussion_topics, speaker_suggestions, uncertainties,
