...
function buildDraftMinutes(checked, ctx) {
  ...
  for (const x of checked.possible_missed_motions || []) L.push(`- [NEEDS REVIEW: possible motion at ${x.time_ref.label} in the recording not captured above: "${x.text.slice(0, 120)}"]`);
  if ((checked.possible_missed_motions || []).length) review += checked.possible_missed_motions.length;
  L.push('');
