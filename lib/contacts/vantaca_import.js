    ...
    if (row.owner_name) {
      ...
      if (!currentPrimary) {
        ...
        diff.ownership_changes.push({
          ...
          // The buyer's new account rides on the proposal; the lot's account
          // changes only when the transfer is approved (mig 459).
          vantaca_account_id: row.account_id && row.account_id !== existingProp.vantaca_account_id ? row.account_id : null,
        });
