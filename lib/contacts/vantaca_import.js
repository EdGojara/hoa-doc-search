...
async function computeDiff(supabase, communityId, normalizedRows) {
  ...
  // ---------------------------------------------------------------------------
  // In-batch dedup: Vantaca exports often include 2+ rows per property when
  // a unit has both an owner-of-record and a separate resident/renter, or
  // when ownership is vested in multiple parties. We collapse duplicate
  // address keys within the upload so we don't propose 134 "new properties"
  // for a 101-home community. The first row for an address is canonical;
  // subsequent rows become hints (renter info, secondary owner, etc.) on
  // the same property.
  // ---------------------------------------------------------------------------
  const seenInBatch = new Map();      // addrKey -> first row that owned it
  const deduped = [];
  for (const row of normalizedRows) {
    const addrKey = _addressKey(row.street_address, row.unit);
    if (!seenInBatch.has(addrKey)) {
      seenInBatch.set(addrKey, row);
      deduped.push(row);
      continue;
    }
    // Duplicate within the batch — merge useful fields into the canonical row
    // and capture the duplicate for the preview so staff can see what we collapsed.
    const canonical = seenInBatch.get(addrKey);
    // If canonical row lacked owner info but this dup has it, promote.
    ...
    // If canonical lacked resident info but this dup has it, promote.
