...
async function computeDiff(supabase, communityId, normalizedRows) {
  ...
  for (const row of normalizedRows) {
    ...
    // Promote owner / resident info from the dup. The canonical row's
    // owner_name might be a Vantaca PLACEHOLDER ("Current Resident",
    // "Resident", "Owner of Record", "Tenant") that's used when the
    // owner doesn't live at the property. The mailing-address row has
    // the actual owner name — prefer that over the placeholder so the
    // letter goes addressed correctly + contact dedup doesn't collapse
    // every non-occupant property into one shared "Current Resident" record.
    const _isPlaceholderName = (n) =>
      n && /^(current resident|resident|owner of record|tenant|occupant|homeowner)$/i.test(String(n).trim());
    if (row.owner_name && (!canonical.owner_name || _isPlaceholderName(canonical.owner_name))) {
